...
(function($, window, document, undefined) {
    ...
    var pluginName = "powerformFrontPayPal",
        defaults = {
            type: 'paypal',
            paymentEl: null,
    ...
    function PowerformFrontPayPal(element, options) {
    ...
    $.extend(PowerformFrontPayPal.prototype, {
        init: function() {
        ...
        render_paypal_button: function() {
            var $form = this.$el,
                ...
                $target_message = $form.find('.powerform-response-message'),
                paypalActions,
                error_msg = PowerformFront.cform.gateway.error,
                requireSsl = this.settings.paymentRequireSsl,
            ...
            paypal.Buttons({
                ...
                onClick: function() {
                    if (!$form.valid() && paypalData.amount <= 0) {
                        $target_message.removeClass('powerform-accessible').addClass('powerform-error').html('').removeAttr('aria-hidden');
                        $target_message.html('<label class="powerform-label--error"><span>' + generalMessage.payment_require_amount_error + '</span></label>');
                        self.focus_to_element($target_message);
                    } else if (requireSsl && 'https:' !== location.protocol) {
                        $target_message.removeClass('powerform-accessible').addClass('powerform-error').html('').removeAttr('aria-hidden');
                        $target_message.html('<label class="powerform-label--error"><span>' + generalMessage.payment_require_ssl_error + '</span></label>');
                        self.focus_to_element($target_message);
                    }
                ...
                createOrder: function(data, actions) {
                    $form.addClass('powerform-partial-disabled');

                    var nonce = $form.find('input[name="powerform_nonce"]').val(),
                        form_id = self.getPayPalData('form_id');

                    return fetch(PowerformFront.ajaxUrl + '?action=powerform_pp_create_order', {
                        method: 'POST',
                        mode: "same-origin",
                        ...
                        $form.find('.powerform-paypal-input').val(orderId);
                ...
                onError: function() {
                    ...
                    $target_message.removeClass('powerform-accessible').addClass('powerform-error').html('').removeAttr('aria-hidden');
                    $target_message.html('<label class="powerform-label--error"><span>' + error_msg + '</span></label>');
                    self.focus_to_element($target_message);
                },
            }).render($form.find('.powerform-button-paypal')[0]);
        },
    ...
    $.fn[pluginName] = function(options) {
        return this.each(function() {
            if (!$.data(this, pluginName)) {
                $.data(this, pluginName, new PowerformFrontPayPal(this, options));
            }
        });
    };
