...
(function($, window, document, undefined) {
    ...
    var pluginName = "powerformFrontCondition",
        defaults = {
            fields: {},
            relations: {}
        };
    ...
    function PowerformFrontCondition(element, options, calendar) {
    ...
    $.extend(PowerformFrontCondition.prototype, {
        init: function() {
            var self = this,
                form = this.$el;
            this.add_missing_relations();

            this.$el.find(".powerform-field input, .powerform-field select, .powerform-field textarea, .powerform-field-signature").change(function(e) {
                var $element = $(this),
                    element_id = $element.closest('.powerform-col').attr('id');
            ...
            this.$el.find('.powerform-button.powerform-button-back, .powerform-button.powerform-button-next').click(function(e) {
                form.find('.powerform-field input, .powerform-field select, .powerform-field textarea').change();
            });
            // Simulate change
            this.$el.find('.powerform-field input, .powerform-field select, .powerform-field textarea').change();
            this.init_events();
        },
            ...
            relations.forEach(function(relation) {
                ...
                if ((rule === "all" && matches === conditions.length) || (rule === "any" && matches > 0)) {
                    ...
                    if ($element instanceof jQuery) {
                        var pagination = $element.closest('.powerform-pagination');
                    }
        ...
        init_events: function() {
            var self = this;
            this.$el.on('powerform.front.condition.restart', function(e) {
                self.on_restart(e);
            });
        },
        ...
        get_date_field_value: function(element_id) {
            ...
            if ($element instanceof jQuery) {
                ...
                if ($element.hasClass('powerform-col')) {
                    fake_field = true;
                }
            }
        ...
        field_is_signature: function($element) {
            ...
            $element.each(function() {
                if ($(this).find('.powerform-field-signature').length > 0) {
                    is_signature = true;
                    //break
                    return false;
                }
        ...
        field_is_datepicker: function($element) {
            var is_date = false;
            $element.each(function() {
                if ($(this).hasClass('powerform-datepicker')) {
                    is_date = true;
                    //break
                    return false;
                }
        ...
        // used in powerformFrontCalculate
        ...
        is_hidden: function(element_id) {
            var $element_id = this.get_form_field(element_id),
                $column_field = $element_id.closest('.powerform-col'),
                $row_field = $column_field.closest('.powerform-row');

            if ($row_field.hasClass("powerform-hidden-option")) {
                return true;
            }

            if ($row_field.hasClass("powerform-hidden")) {
                return false;
            }
        ...
        date_is_grater: function(date1, date2) {
            return powerformDateUtil.compare(date1, date2) === 1;
        },
        ...
        date_is_equal: function(date1, date2) {
            return powerformDateUtil.compare(date1, date2) === 0;
        },

        date_is_n_days_before_current_date: function(date1, n) {
            n = parseInt(n);
            var current_date = this.get_current_date();
            var diff = powerformDateUtil.diffInDays(date1, current_date);
            if (isNaN(diff)) {
                return false;
            }
            if (n === 0) {
        ...
        date_is_n_days_after_current_date: function(date1, n) {
            n = parseInt(n);
            var current_date = this.get_current_date();
            var diff = powerformDateUtil.diffInDays(current_date, date1);
            if (isNaN(diff)) {
                return false;
            }
            if (n === 0) {
        ...
        toggle_field: function(element_id, action, type) {
            var $element_id = this.get_form_field(element_id),
                $column_field = $element_id.closest('.powerform-col'),
                $hidden_upload = $column_field.find('.powerform-input-file-required'),
                $hidden_signature = $column_field.find('[id ^=ctlSignature][id $=_data]'),
                $hidden_wp_editor = $column_field.find('.powerform-wp-editor-required'),
                $row_field = $column_field.closest('.powerform-row'),
                $pagination_next_field = this.$el.find('.powerform-pagination-footer').find('.powerform-button-next'),
                $paypal_field = this.$el.find('.powerform-pagination-footer').find('#powerform-paypal-submit');
            if ('submit' === element_id) {
                var $pagination_field = this.$el.find('.powerform-pagination-footer').find('.powerform-button-submit')
            } else {
                var $pagination_field = this.$el.find('.powerform-pagination-footer').find('#powerform-paypal-submit');
            }

            // Handle show action
            if (action === "show") {
                if (type === "valid") {
                    $row_field.removeClass('powerform-hidden');
                    $column_field.removeClass('powerform-hidden');
                    $pagination_field.removeClass('powerform-hidden');
                    $pagination_next_field.removeClass('powerform-hidden');
                    ...
                    if ($paypal_field.length > 0) {
                        $paypal_field.removeClass('powerform-hidden');
                    }
                } else {
                    $column_field.addClass('powerform-hidden');
                    $pagination_field.addClass('powerform-hidden');
                    if ($hidden_upload.length > 0) {
                    ...
                    if ($row_field.find('> .powerform-col:not(.powerform-hidden)').length === 0) {
                        $row_field.addClass('powerform-hidden');
                    }
                }
            }
            ...
            this.$el.trigger('powerform:field:condition:toggled');
        },
    ...
    $.fn[pluginName] = function(options, calendar) {
        return this.each(function() {
            if (!$.data(this, pluginName)) {
                $.data(this, pluginName, new PowerformFrontCondition(this, options, calendar));
            }
        });
    };
