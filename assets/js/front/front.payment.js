...
(function($, window, document, undefined) {
    ...
    var pluginName = "powerformFrontPayment",
        defaults = {
            type: 'stripe',
            paymentEl: null,
    ...
    function PowerformFrontPayment(element, options) {
    ...
    $.extend(PowerformFrontPayment.prototype, {
        init: function() {
            ...
            $(this.element).on('payment.before.submit.powerform', function(e, formData, callback) {
            ...
            this.$el.find(
                'input.powerform-input, .powerform-select, .powerform-checkbox, .powerform-radio, .powerform-select2'
            ).each(function() {
        ...
        getForm: function(e) {
            ...
            if (!$form.hasClass('powerform-custom-form')) {
                $form = $form.closets('form.powerform-custom-form');
            }
        ...
        updateAmount: function(e, formData) {
            ...
            updateFormData.append('action', 'powerform_update_payment_amount');
            ...
            $.ajax({
                ...
                beforeSend: function() {
                    if (typeof self.settings.has_loader !== "undefined" && self.settings.has_loader) {
                        // Disable form fields
                        self._form.addClass('powerform-fields-disabled');

                        var $target_message = self._form.find('.powerform-response-message');
                ...
                success: function(data) {
                    if (data.success === true) {
                        ...
                        if (typeof data.data !== 'undefined' && typeof data.data.paymentid !== 'undefined') {
                            self.$el.find('#powerform-stripe-paymentid').val(data.data.paymentid);
                        ...
                        var $captcha_field = self._form.find('.powerform-g-recaptcha');
        ...
        show_error: function(message) {
            var $target_message = this._form.find('.powerform-response-message');
            ...
            $target_message.removeAttr("aria-hidden")
                ...
                .addClass('powerform-error powerform-show');
        ...
        enable_form: function() {
            if (typeof this.settings.has_loader !== "undefined" && this.settings.has_loader) {
                ...
                // Enable form fields
                this._form.removeClass('powerform-fields-disabled');
            ...
            if (typeof powerformFrontCondition !== 'undefined') {
                ...
                errors.forEach(function(value) {
                    var element_id = Object.keys(value),
                        ...
                        element = powerformFrontCondition.get_form_field(element_id);
                    if (element.length) {
                        if (i === 0) {
                        ...
                        if ($(element).hasClass('powerform-input-time')) {
                            var $time_field_holder = $(element).closest('.powerform-field:not(.powerform-field--inner)'),
                                $time_error_holder = $time_field_holder.children('.powerform-error-message');

                            if ($time_error_holder.length === 0) {
                                $time_field_holder.append('<span class="powerform-error-message" aria-hidden="true"></span>');
                                $time_error_holder = $time_field_holder.children('.powerform-error-message');
                            }
                        ...
                        var $error_holder = $field_holder.find('.powerform-error-message');

                        if ($error_holder.length === 0) {
                            $field_holder.append('<span class="powerform-error-message" aria-hidden="true"></span>');
                            $error_holder = $field_holder.find('.powerform-error-message');
                        }
        ...
        validateCard: function() {
            ...
            this._cardElement.on('change', function(event) {
                if (self.$el.find('.powerform-stripe-element').hasClass('StripeElement--empty')) {
                    self.$el.find('.powerform-stripe-element').closest('.powerform-field').removeClass('powerform-is_filled');
                } else {
                    self.$el.find('.powerform-stripe-element').closest('.powerform-field').addClass('powerform-is_filled');
                }

                if (self.$el.find('.powerform-stripe-element').hasClass('StripeElement--invalid')) {
                    self.$el.find('.powerform-stripe-element').closest('.powerform-field').addClass('powerform-has_error');
                }
            });

            this._cardElement.on('focus', function(event) {
                self.$el.find('.powerform-stripe-element').closest('.powerform-field').addClass('powerform-is_active');
            });

            this._cardElement.on('blur', function(event) {
                self.$el.find('.powerform-stripe-element').closest('.powerform-field').removeClass('powerform-is_active');
        ...
        hideCardError: function() {
            var $field_holder = this.$el.find('.powerform-card-message');
            var $error_holder = $field_holder.find('.powerform-error-message');
            ...
            $field_holder.closest('.powerform-field').removeClass('powerform-has_error');
            $error_holder.html('');
        },
        ...
        showCardError: function(message, focus) {
            ...
            $field_holder.closest('.powerform-field').addClass('powerform-has_error');
            $field_holder.closest('.powerform-field').addClass('powerform-is_filled');
            $error_holder.html(message);

            if (focus) {
                this.focus_to_element($field_holder.closest('.powerform-field'));
            }
        },
        ...
        // taken from powerformFrontCondition
    ...
    $.fn[pluginName] = function(options) {
        return this.each(function() {
            if (!$.data(this, pluginName)) {
                $.data(this, pluginName, new PowerformFrontPayment(this, options));
            }
        });
    };
