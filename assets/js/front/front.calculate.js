...
(function($, window, document, undefined) {
    ...
    var pluginName = "powerformFrontCalculate",
        defaults = {
            powerformFields: [],
            maxExpand: 5,
            generalMessages: {},
    ...
    function PowerformFrontCalculate(element, options) {
    ...
    $.extend(PowerformFrontCalculate.prototype, {
        init: function() {
            var self = this;

            // find calculation fields
            var calculationInputs = this.$el.find('input.powerform-calculation');
            ...
            if (calculationInputs.length > 0) {
                ...
                calculationInputs.each(function() {
                    ...
                    if ($(this).data('isHidden')) {
                        $(this).closest('.powerform-col').addClass('powerform-hidden powerform-hidden-option');
                        var rowField = $(this).closest('.powerform-row');
                        if (rowField.find('> .powerform-col:not(.powerform-hidden)').length === 0) {
                            rowField.addClass('powerform-hidden powerform-hidden-option');
                        }
                    }
                });
                ...
                this.$el.on('powerform:field:condition:toggled', function(e) {
        ...
        maybeExpandCalculationFieldOnFormula: function(formula) {
            ...
            var joinedFieldTypes = this.settings.powerformFields.join('|');
            ...
            if (this.triggerInputs.length > 0) {
                ...
                for (var j = 0; j < this.triggerInputs.length; j++) {
                    ...
                    if (cFields.indexOf(inputId) < 0) {
                        $input.on('change.powerformFrontCalculate, blur', function() {
        ...
        recalculate: function(calcField) {
            ...
            var res = 0;
            var calc = new window.powerformCalculator(formula);
        ...
        is_hidden: function(element_id) {
            ...
            if ($row_field.hasClass("powerform-hidden-option") || $column_field.hasClass("powerform-hidden-option")) {
                return false;
            }

            if ($row_field.hasClass("powerform-hidden") || $column_field.hasClass("powerform-hidden")) {
                return true;
            }
    ...
    $.fn[pluginName] = function(options) {
        return this.each(function() {
            if (!$.data(this, pluginName)) {
                $.data(this, pluginName, new PowerformFrontCalculate(this, options));
            }
        });
    };
