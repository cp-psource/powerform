...
(function($, window, document, undefined) {
    ...
    $.extend(PowerformFrontSubmit.prototype, {
        ...
        handle_submit_custom_form: function() {
            ...
            var success_available = self.$el.find('.powerform-response-message').find('.powerform-label--success').not(':hidden');
            if (success_available.length) {
                self.focus_to_element(self.$el.find('.powerform-response-message'), true);
            }
            $('.def-ajaxloader').hide();
            var isSent = false;
            ...
            $('body').on('submit.frontSubmit', this.settings.powerform_selector, function(e) {
                var $this = $(this),
                    ...
                    $target_message = $this.find('.powerform-response-message'),
                    $captcha_field = $this.find('.powerform-g-recaptcha');

                if (self.settings.inline_validation && self.$el.find('.powerform-uploaded-files').length > 0) {
                    var file_error = self.$el.find('.powerform-uploaded-files li.powerform-has_error');
                ...
                if (submitEvent.originalEvent !== undefined) {
                    var submitBtn = $(this).find('.powerform-button-submit').first();
                    if (submitBtn.length === 0 || $(submitBtn).closest('.powerform-col').hasClass('powerform-hidden')) {
                        return false;
                    }
                }

                if (self.$el.data('powerformFrontPayment')) {
                ...
                var submitCallback = function() {
                    ...
                    if (self.$el.hasClass('powerform_ajax')) {
                        $target_message.html('');
                        self.$el.find('.powerform-button-submit').addClass('powerform-button-onload');
                        ...
                        if (typeof self.settings.has_loader !== "undefined" && self.settings.has_loader) {
                            ...
                            if ('login' !== form_type) {
                                form.addClass('powerform-fields-disabled');
                            }
                            ...
                            $target_message.removeAttr("aria-hidden")
                                .prop("tabindex", "-1")
                                .removeClass('powerform-success powerform-error')
                                .addClass('powerform-loading powerform-show');
                        }
                        ...
                        $.ajax({
                            ...
                            beforeSend: function() {
                                ...
                                $this.trigger('before:powerform:form:submit', formData);
                            },
                            success: function(data) {
                                if (!data && 'undefined' !== typeof data) {
                                    $this.find('button').removeAttr('disabled');
                                    $target_message.addClass('powerform-error')
                                        .html('<p>' + window.PowerformFront.cform.error + '</p>');
                                    self.focus_to_element($target_message, 'powerform-error');

                                    return false;
                                }

                                // Hide validation errors
                                $this.find('.powerform-error-message').not('.powerform-uploaded-files .powerform-error-message').remove();
                                $this.find('.powerform-field').removeClass('powerform-has_error');

                                $this.find('button').removeAttr('disabled');
                                $target_message.html('').removeClass('powerform-accessible powerform-error powerform-success');
                                if (self.settings.hasLeads && 'undefined' !== typeof data.data.entry_id) {
                                    ...
                                    $('#powerform-quiz-leads-' + self.settings.quiz_id + ' .powerform-lead-form-skip').hide();
                                    if ('beginning' === self.settings.form_placement) {
                                        $('#powerform-module-' + self.settings.quiz_id).css({
                                            'height': '',
                                            'opacity': '',
                                            'overflow': '',
                                            'visibility': '',
                                    ...
                                    $('#powerform-module-' + self.settings.quiz_id + ' input[name=entry_id]').val(data.data.entry_id);
                                    if ('end' === self.settings.form_placement) {
                                        $('#powerform-module-' + self.settings.quiz_id).submit();
                                    }
                                ...
                                if (typeof data.data.authentication !== 'undefined' &&
                                    ...
                                    var moduleId = self.$el.attr('id'),
                                        ...
                                        authField = $('#' + authId),
                                        authInput = $('#' + authId + '-input');
                                    authField.find('.powerform-authentication-notice').removeClass('error');
                                    ...
                                    if ('invalid' === data.data.authentication) {
                                        authField.find('.powerform-authentication-notice').addClass('error');
                                        authField.find('.powerform-authentication-notice').html('<p>' + data.data.message + '</p>');
                                    }

                                    return false;

                                }
                                var $label_class = data.success ? 'powerform-success' : 'powerform-error';

                                if (typeof data.message !== "undefined") {
                                    $target_message.removeAttr("aria-hidden")
                                        .prop("tabindex", "-1")
                                        .addClass($label_class + ' powerform-show');
                                    self.focus_to_element($target_message, $label_class === 'powerform-success');
                                    $target_message.html('<p>' + data.message + '</p>');

                                    if (!data.data.success && data.data.errors.length) {
                                        var errors_html = '<ul class="powerform-screen-reader-only">';
                                ...
                                if (data.success === true) {
                                    ...
                                    if ($this[0]) {
                                        ...
                                        self.$el.trigger('powerform:field:condition:toggled');

                                        // reset signatures
                                        $this.find('.powerform-field-signature img').trigger('click');
                                        ...
                                        // Reset upload field
                                        $this.find(".powerform-button-delete").hide();
                                        $this.find('.powerform-file-upload input').val("");
                                        $this.find('.powerform-file-upload > span').html(window.PowerformFront.cform.no_file_chosen);
                                        $this.find('ul.powerform-uploaded-files').html('');
                                        self.$el.find('ul.powerform-uploaded-files').html('');
                                        self.$el.find('.powerform-multifile-hidden').val('');
                                        //self.$el.find( '.powerform-input-file' ).val('');

                                        // Reset selects
                                        if ($this.find('.powerform-select').length > 0) {
                                            $this.find('.powerform-select').each(function(index, value) {
                                        ...
                                        if ($this.find('.multiselect-default-values').length > 0 && '' !== $this.find('.multiselect-default-values').val()) {
                                            ...
                                            $this.find('.powerform-multiselect input[type="checkbox"]').each(function(i, val) {
                                                if (-1 !== $.inArray($(val).val(), defaultValuesArr)) {
                                                    $(val).prop('checked', true);
                                                    $(val).closest('label').addClass('powerform-is_checked');
                                                } else {
                                                    $(val).prop('checked', false);
                                                    $(val).closest('label').removeClass('powerform-is_checked');
                                                }
                                            });
                                        }
                                    ...
                                    if (typeof data.data.behav !== "undefined" && data.data.behav === 'behaviour-hide') {
                                        self.$el.find('.powerform-row').hide();
                                        self.$el.find('.powerform-pagination-steps').hide();
                                        self.$el.find('.powerform-pagination-footer').hide();
                                        self.$el.find('.powerform-pagination-steps, .powerform-pagination-progress').hide();
                                    }
                                }
                            },
                            error: function(err) {
                            ...
                            complete: function(xhr, status) {
                                self.$el.find('.powerform-button-submit').removeClass('powerform-button-onload');

                                $this.trigger('powerform:form:submit:complete', formData);
                            }
                        }).always(function() {
                            if (typeof self.settings.has_loader !== "undefined" && self.settings.has_loader) {
                                // Enable form fields
                                form.removeClass('powerform-fields-disabled powerform-partial-disabled');

                                $target_message.removeClass('powerform-loading');
                            }

                            $this.trigger('after:powerform:form:submit', formData);
                        });
                    } else {
                        if (typeof self.settings.has_loader !== "undefined" && self.settings.has_loader) {
                            // Disable form fields
                ...
                var paymentIsHidden = self.$el.find('div[data-is-payment="true"]')
                    .closest('.powerform-row').hasClass('powerform-hidden');
                if (self.$el.data('powerformFrontPayment') && !paymentIsHidden) {
                    self.$el.trigger('payment.before.submit.powerform', [formData, function() {
        ...
        handle_submit_quiz: function(data) {
            ...
            $('body').on('submit.frontSubmit', this.settings.powerform_selector, function(e) {
                ...
                var form = $(this),
                    ajaxData = [],
                    formData = new FormData(this),
                    answer = form.find('.powerform-answer'),
                    button = self.$el.find('.powerform-button'),
                ...
                if (hasLeads) {
                    ...
                    if ('end' === placement && entry_id === '') {
                        ...
                        self.$el.find('.powerform-quiz--result').addClass('powerform-hidden');
                        $('#powerform-quiz-leads-' + quiz_id + ' .powerform-lead-form-skip').show();
                ...
                if (self.settings.has_quiz_loader) {
                    answer.each(function() {
                        var answer = $(this),
                            ...
                            status = answer.find('.powerform-answer--status'),
                            loader = '<i class="powerform-icon-loader powerform-loading"></i>';
                ...
                $.ajax({
                    ...
                    beforeSend: function() {
                        ...
                        form.trigger('before:powerform:quiz:submit', [ajaxData, formData]);
                    },
                    success: function(data) {

                        if (data.success) {

                            self.$el.find('.powerform-quiz--result').removeClass('powerform-hidden');
                            ...
                            if (data.data.type === 'nowrong') {
                                ...
                                if (self.$el.find('.powerform-quiz--result').length > 0) {
                                ...
                                Object.keys(data.data.result).forEach(function(key) {
                                    ...
                                    var responseClass,
                                        ...
                                        result = parent.find('.powerform-question--result'),
                                        submit = parent.find('.powerform-submit-rightaway'),
                                        answer = parent.find('[id|="' + data.data.result[key].answer + '"]').closest('.powerform-answer'),
                                        answers = parent.find('.powerform-answer input');

                                    // Check if selected answer is right or wrong.
                                    if (data.data.result[key].isCorrect) {
                                        responseClass = 'powerform-is_correct';
                                        responseIcon = '<i class="powerform-icon-check"></i>';
                                    } else {
                                        responseClass = 'powerform-is_incorrect';
                                        responseIcon = '<i class="powerform-icon-cancel"></i>';
                                    }

                                    // Show question result.
                                    result.text(data.data.result[key].message);
                                    result.addClass('powerform-show');
                                    ...
                                    if (0 === answer.find('.powerform-answer--status').html().length) {
                                        answer.find('.powerform-answer--status').html(responseIcon);
                                    } else {

                                        if (0 !== answer.find('.powerform-answer--status .powerform-icon-loader').length) {
                                            answer.find('.powerform-answer--status').html(responseIcon);
                                        }
                                    }
                            ...
                            form.trigger('powerform:quiz:submit:success', [ajaxData, formData]);

                        } else {
                            self.$el.find('button').removeAttr('disabled');

                            form.trigger('powerform:quiz:submit:failed', [ajaxData, formData]);
                        }
                    }
                }).always(function() {
                    form.trigger('after:powerform:quiz:submit', [ajaxData, formData]);
                });
                return false;
            });

            $('body').on('click', '#powerform-quiz-leads-' + quiz_id + ' .powerform-lead-form-skip', function(e) {
                $('#powerform-module-' + leads_id).css({
                    'height': 0,
                    'opacity': 0,
                    'overflow': 'hidden',
            ...
            $('body').on('click', '.powerform-result--retake', function(e) {
                var pageId = self.$el.find('input[name="page_id"]').val();
                var ajaxData = {
                    action: 'powerform_reload_quiz',
                    pageId: pageId,
                    nonce: self.$el.find('input[name="powerform_nonce"]').val()
                };

                e.preventDefault();

                $.post(window.PowerformFront.ajaxUrl, ajaxData, function(response) {
        ...
        handle_submit_poll: function() {
            ...
            var success_available = self.$el.find('.powerform-response-message').not(':hidden');

            if (success_available.length) {

                self.focus_to_element(
                    self.$el.find('.powerform-response-message'),
                    true,
                    true
                );
            }
            ...
            $('body').on('submit.frontSubmit', this.settings.powerform_selector, function(e) {
                ...
                var $response = self.$el.find('.powerform-response-message'),
                    $options = self.$el.find('fieldset'),
                    $submit = self.$el.find('.powerform-button');
                ...
                function response_clean() {
                    ...
                    // Remove all classes
                    $response.removeClass('powerform-show');
                    $response.removeClass('powerform-error');
                    $response.removeClass('powerform-success');
                    ...
                    $options.removeClass('powerform-has_error');
                ...
                function response_message(message, custom_class) {
                    ...
                    $response.addClass('powerform-' + custom_class);
                    $response.addClass('powerform-show');
                    ...
                    if ('error' === custom_class) {
                        ...
                        if (!$options.find('input[type="radio"]').is(':checked')) {
                            $options.addClass('powerform-has_error');
                        }
                    }
                }
                ...
                if (self.$el.hasClass('powerform_ajax')) {
                    ...
                    $.ajax({
                        ...
                        beforeSend: function() {
                            ...
                            $submit.addClass('powerform-onload');

                            // Trigger "submit" action
                            $this.trigger('before:powerform:poll:submit', [ajaxData, formData]);
                        ...
                        success: function(data) {
                            ...
                            $submit.removeClass('powerform-onload');
                            ...
                            if (false === data.success) {
                                ...
                                // Failed response
                                $this.trigger('powerform:poll:submit:failed', [ajaxData, formData]);
                            ...
                            if (true === data.success) {
                                ...
                                if (typeof data.data.url !== 'undefined') {
                                    ...
                                    if (typeof data.data.chart_data !== 'undefined' && data.data.chart_data.length > 1) {
                                        ...
                                        if ('link_on' === data.data.results_behav) {
                                            ...
                                            if ($this.find('.powerform-note').length) {
                                                $this.find('.powerform-note').remove();
                                                $this.find('.powerform-poll-footer').append(data.data.results_link);
                                            }
                                        }
                                ...
                                // Success response
                                $this.trigger('powerform:poll:submit:success', [ajaxData, formData]);
                        ...
                        $this.trigger('after:powerform:poll:submit', [ajaxData, formData]);
        ...
        render_poll_chart: function(chart_data, back_button, powerformSubmit, poll_form, chart_extras) {
            var pollId = powerformSubmit.$el.attr('id') + '-' + powerformSubmit.$el.data('powerformRender'),
                chartId = 'powerform-chart-poll-' + pollId,
                pollBody = powerformSubmit.$el.find('.powerform-poll-body'),
                pollFooter = powerformSubmit.$el.find('.powerform-poll-footer');

            function chart_clean() {

                var canvas = powerformSubmit.$el.find('.powerform-chart-wrapper'),
                    wrapper = powerformSubmit.$el.find('.powerform-chart');
            ...
            function chart_create() {
                var canvas = $('<canvas id="' + chartId + '" class="powerform-chart" role="img" aria-hidden="true"></canvas>');

                pollBody.append(canvas);
            }

            function chart_show() {
                var canvas = powerformSubmit.$el.find('.powerform-chart'),
                    wrapper = powerformSubmit.$el.find('.powerform-chart-wrapper');

                if (wrapper.length) {

                    // Show canvas
                    canvas.addClass('powerform-show');

                    // Show wrapper
                    wrapper.addClass('powerform-show');
            ...
            function hide_answers() {
                var answers = pollBody.find('.powerform-field');
            ...
            function back_to_poll() {
                ...
                var button = powerformSubmit.$el.find('.powerform-button');

                button.click(function(e) {

                    if (powerformSubmit.$el.hasClass('powerform_ajax')) {
                        powerformSubmit.$el.html(poll_form);
                    } else {
                        location.reload();
                    }
            ...
            FUI.pollChart(
                '#' + chartId,
                chart_data,
                powerformSubmit.settings.chart_design,
                chart_extras
            );
        ...
        show_messages: function(errors) {
            var self = this,
                powerformFrontCondition = self.$el.data('powerformFrontCondition');
            if (typeof powerformFrontCondition !== 'undefined') {

                // clear all validation message before show new one
                this.$el.find('.powerform-error-message').remove();
                var i = 0;

                errors.forEach(function(value) {

                    var elementId = Object.keys(value),
                        getElement = powerformFrontCondition.get_form_field(elementId);

                    var holder = $(getElement),
                        holderField = holder.closest('.powerform-field'),
                        holderDate = holder.closest('.powerform-date-input'),
                        holderTime = holder.closest('.powerform-timepicker'),
                        holderError = '',
                        getColumn = false,
                        getError = false,
                        getDesc = false;
                    ...
                    var errorMessage = Object.values(value),
                        errorMarkup = '<span class="powerform-error-message" aria-hidden="true"></span>';

                    if (getElement.length) {

                        // Focus on first error
                        if (i === 0) {
                            self.$el.trigger('powerform.front.pagination.focus.input', [getElement]);
                        ...
                        if (holderDate.length > 0) {
                            ...
                            var getError = holderField.find('.powerform-error-message'),
                                getDesc = holderField.find('.powerform-description');
