...
(function ($, window, document, undefined) {
	...
	var pluginName = "powerformFrontMergeTags",
	    defaults   = {
		    powerformFields: [],
	    };

	// The actual plugin constructor
	function powerformFrontMergeTags(element, options) {
	...
	$.extend(powerformFrontMergeTags.prototype, {
		init: function () {
			var self = this;
			var fields = this.$el.find('.powerform-merge-tags');
		...
		attachEvents: function () {
			...
			this.$el.find(
				'input.powerform-input, .powerform-select, .powerform-checkbox, .powerform-radio, .powerform-input-file, .powerform-select2'
			).each(function () {
		...
		is_calculation: function (element_id) {
			...
			if ( $element.hasClass("powerform-calculation") ) {
				return true;
			}
		...
		get_field_value: function (element_id) {
			...
			if ( this.is_calculation( element_id ) ) {
				...
				if ( ! $row_field.hasClass("powerform-hidden-option") && this.is_hidden( element_id ) ) {
					return '';
				}
			}
		...
		is_hidden: function (element_id) {
			...
			if ( $row_field.hasClass("powerform-hidden-option") || $row_field.hasClass("powerform-hidden") ) {
				return true;
			}

			if( $column_field.hasClass("powerform-hidden") ) {
				return true;
			}
	...
	$.fn[pluginName] = function (options) {
		return this.each(function () {
			if (!$.data(this, pluginName)) {
				$.data(this, pluginName, new powerformFrontMergeTags(this, options));
			}
		});
	};
