...
(function($, window, document, undefined) {
    ...
    var pluginName = "powerformFrontMultiFile",
        defaults = {};

    // The actual plugin constructor
    function PowerformFrontMultiFile(element, options) {
    ...
    $.extend(PowerformFrontMultiFile.prototype, {
        init: function() {
            var self = this,
                fileList = [],
                ajax_request = [];
            ...
            this.uploader.on("dragover dragenter", function(a) {
                $(this).addClass("powerform-dragover");
            });
            this.uploader.on("dragleave dragend drop", function(a) {
                $(this).removeClass("powerform-dragover");
            });
            this.uploader.find(".powerform-upload-file--powerform-field-" + this.element).on("click", function(e) {
                self.form.find('.powerform-field-' + self.element + '-' + self.form_id).click();
            });

            this.uploader.on("drop", function(e) {
                document.querySelector('.powerform-field-' + self.element + '-' + self.form_id).files = e.originalEvent.dataTransfer.files;
                self.form.find('.powerform-field-' + self.element + '-' + self.form_id).change();
            });
            ...
            this.uploader.find('.powerform-multi-upload-message, .powerform-multi-upload-message p, .powerform-multi-upload-message .powerform-icon-upload').on("click", function(e) {
                if (e.target === e.currentTarget) {
                    self.form.find('.powerform-field-' + self.element + '-' + self.form_id).click();
                }
            });

            this.form.on("powerform:form:submit:success", function(e) {
                fileList = [];
            });
            this.form.find('.powerform-field-' + self.element + '-' + self.form_id).on("change", function(e) {
                var $this = $(this),
                    param = this.files,
                    uploadParam = [];
                $this.closest('.powerform-field').removeClass('powerform-has_error');
        ...
        handleChangeCallback: function(param, $this, ajax_request) {
            var self = this,
                ...
                nonce = this.form.find('input[name="powerform_nonce"]').val(),
                method = $this.data('method');
            uploadData.append("action", "powerform_multiple_file_upload");
            ...
            $.each(param, function(i, item) {
                ...
                if ('ajax' === method) {
                    ...
                    ajax_request.push($.ajax({
                        ...
                        processData: false,
                        beforeSend: function() {
                            self.form.find('.powerform-button-submit').attr('disabled', true);
                            self.$el.trigger('before:powerform:multiple:upload', uploadData);
                        },
                        success: function(data) {
                            ...
                            if (true === data.success && true === data.data.success && 'undefined' !== typeof data.data) {
                                ...
                                self.$el.trigger('success:powerform:multiple:upload', uploadData);
                            } else {
                                self.upload_fail_response(unique_id, data.data.message);
                                if ('undefined' !== typeof data.data.error_type && 'limit' === data.data.error_type) {
                                    self.form.find('#' + unique_id).addClass('powerform-upload-limit_error');
                                }
                                self.$el.trigger('fail:powerform:multiple:upload', uploadData);
                            }
                        },
                        complete: function(xhr, status) {
                            ajax_inc++;
                            if (param.length === ajax_inc) {
                                self.form.find('.powerform-button-submit').attr('disabled', false);
                            }
                            self.$el.trigger('complete:powerform:multiple:upload', uploadData);
                        },
                        error: function(err) {
                            self.upload_fail_response(unique_id, window.PowerformFront.cform.process_error);
                        }
                    }))
                } else {
                    var has_error = true,
                        error_messsage = window.PowerformFront.cform.process_error;
        ...
        upload_fail_response: function(unique_id, message) {
            this.form.find('#' + unique_id).addClass('powerform-has_error');
            this.form.find('#' + unique_id).find('.powerform-uploaded-file--size [class*="powerform-icon-"]')
                .addClass('powerform-icon-warning')
                .removeClass('powerform-icon-loader')
                .removeClass('powerform-loading');
            this.form.find('#' + unique_id + ' .progress-percentage').html('0% of ');
            this.form.find('#' + unique_id + ' .powerform-uploaded-file--content')
                .after('<div class="powerform-error-message">' + message + '</div>');
        },
        ...
        upload_success_response: function(unique_id) {
            ...
            this.form.find('#' + unique_id + ' .powerform-uploaded-file--size [class*="powerform-icon-"]').remove();
        ...
        progress_bar: function(file, method) {
            var self = this,
                ...
                wrapper = this.uploader.closest('.powerform-field').find('.powerform-uploaded-files'),
                markup = '';
             ...
             */
            var preview = '<div class="powerform-uploaded-file--preview" aria-hidden="true">' +
                '<span class="powerform-icon-file" aria-hidden="true"></span>' +
                '</div>';

            if (isImage(filename)) {
                preview = '<div class="powerform-uploaded-file--image" aria-hidden="true">' +
                    '<div class="powerform-img-preview" role="image"></div>' +
                    '</div>';
            }
            ...
            var name = '<p class="powerform-uploaded-file--title">' + filename + '</p>';
            ...
            var size = '<p class="powerform-uploaded-file--size">' +
                '<span class="powerform-icon-loader powerform-loading" aria-hidden="true"></span>' +
            ...
            var trash = '<button type="button" class="powerform-uploaded-file--delete powerform-button-delete" data-method="' + method + '" data-element="' + self.element + '" data-value="' + uniqueId + '">' +
                '<span class="powerform-icon-close" aria-hidden="true"></span>' +
                '<span class="powerform-screen-reader-only">Delete uploaded file</span>' +
                '</button>';
            ...
            markup += '<li id="' + uniqueId + '" class="powerform-uploaded-file">';
            markup += '<div class="powerform-uploaded-file--content">';
            markup += preview;
            markup += '<div class="powerform-uploaded-file--text">';
            markup += name;
            markup += size;
            markup += '</div>';
            markup += trash;
             ...
             * Has Files Class.
             *
             * Add "powerform-has-files" class to wrapper.
             */

            if (!wrapper.hasClass('.powerform-has-files')) {
                wrapper.addClass('powerform-has-files');
            }
        ...
        progress_image_preview: function(image, uniqueId) {
            if (image) {
                ...
                reader.onload = function(e) {
                    $('#' + uniqueId + ' .powerform-img-preview').css('background-image', 'url(' + e.target.result + ')');
                };
        ...
        get_uplaoded_files: function() {
            var uploaded_value = this.form.find('.powerform-multifile-hidden'),
                files;
        ...
        set_upload_file: function(element, files) {
            var upload_file = this.get_uplaoded_files(),
                uploaded_value = this.form.find('.powerform-multifile-hidden');
        ...
        delete_files: function(fileList, ajax_request) {
            var self = this;
            $(document).on("click", ".powerform-uploaded-file--delete", function(e) {
                e.preventDefault();
                ...
                if ('undefined' !== typeof file_id && 'undefined' !== typeof element_id && 'undefined' !== typeof method) {
                    ...
                    if (uploaded_arr && 'ajax' === method) {
                        ...
                        var uploaded_value = self.form.find('.powerform-multifile-hidden');
                ...
                var fileInput = self.form.find('.powerform-field-' + self.element + '-' + self.form_id);
                if ('undefined' !== typeof fileInput.data('limit')) {
                    ...
                    $.each(liList, function(index) {
                        if (fileInput.data('limit') > index && $(this).hasClass('powerform-upload-limit_error')) {
                            var fileID = $(this).attr('id'),
                                fileIndex = self.get_uploaded_file_id(element_id, fileID);
                            $(this).removeClass('powerform-has_error');
                            $(this).find('.powerform-error-message, .powerform-icon-warning, .progress-percentage').remove();
        ...
        remove_object: function(index, fileList, element_id) {
            var upload_input = document.querySelector('.powerform-field-' + element_id + '-' + this.form_id);
    ...
    $.fn[pluginName] = function(options) {
        return this.each(function() {
            if (!$.data(this, pluginName)) {
                $.data(this, pluginName, new PowerformFrontMultiFile(this, options));
            }
        });
    };
