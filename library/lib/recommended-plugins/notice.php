<?php
/**
 * PSOURCE - Recommended Plugins Notice
 *
 * @author  PSOURCE (https://n3rds.work)
 * @license GPLv2
 */

if ( ! class_exists( 'PSOURCE_Recommended_Plugins_Notice_Registered_Plugin' ) ) {
	/**
	 * Class PSOURCE_Recommended_Plugins_Notice_Registered_Plugin
	 *
	 * Hold registered plugin as object
	 */
	class PSOURCE_Recommended_Plugins_Notice_Registered_Plugin { //phpcs:ignore
		...
		/**
		 * PSOURCE_Recommended_Plugins_Notice_Registered_Plugin constructor.
		 *
		...
		public function get_selector() {
			 ...
			 * @return array
			 */
			$selector = apply_filters( "psource-recommended-plugin-{$this->basename}-notice-selector", $selector, $active_screen );
		...
		public function is_time_to_display_notice() {
			 ...
			 * @return string
			 */
			$seconds_after_registered = apply_filters( "psource-recommended-plugins-notice-display-seconds-after-registered", $seconds_after_registered, $active_screen );
			 ...
			 * @return string
			 */
			$seconds_after_registered = apply_filters( "psource-recommended-plugin-{$this->basename}-notice-display-seconds-after-registered", $seconds_after_registered, $active_screen );
		...
		public function get_pre_text_notice() {
			$pre_text_notice = sprintf( __( 'Enjoying %s? Try out a few of our other popular free plugins...', 'psource_recommended_plugins_notice' ), $this->name );
			 ...
			 * @return string
			 */
			$pre_text_notice = apply_filters( "psource-recommended-plugin-{$this->basename}-pre-text-notice", $pre_text_notice );
...
if ( ! class_exists( 'PSOURCE_Recommended_Plugins_Notice' ) ) {

	/**
	 * Class PSOURCE_Recommended_Plugins_Notice
	 *
	 * @internal
	 */
	class PSOURCE_Recommended_Plugins_Notice {//phpcs:ignore

		/**
		 * @var PSOURCE_Recommended_Plugins_Notice
		 */
		 ...
		 * Pointer name
		 */
		const POINTER_NAME = 'psource_recommended_plugins';

		/**
		 * Registered plugins
		 */
		const OPTION_NAME = 'psource_recommended_plugins_registered';

		/**
		 * Collection of registered plugins to use this notice
		 *
		 * @var PSOURCE_Recommended_Plugins_Notice_Registered_Plugin[]
		 */
		...
		/**
		 * PSOURCE_Recommended_Plugins_Notice constructor.
		 */
		public function __construct() {
			...
			if ( is_admin() ) {
				...
				add_action( 'psource-recommended-plugins-register-notice', array( $this, 'register' ), 10, 4 );
		...
		private function init_recommended_plugins() {
			$recommended_plugins = array(
				array(
					...
					'desc'         => __( 'Resize, optimize and compress all of your images to the max.', 'psource_recommended_plugins_notice' ),
				...
				array(
					...
					'desc'         => __( 'Add powerful caching and optimize your assets.', 'psource_recommended_plugins_notice' ),
				...
				array(
					...
					'desc'         => __( 'Secure and protect your site from malicious hackers and bots.', 'psource_recommended_plugins_notice' ),
				...
				array(
					'name'         => 'SmartCrawl SEO',
					'desc'         => __( 'Configure your markup for optimal page and social ranking.', 'psource_recommended_plugins_notice' ),
				...
				array(
					'name'         => 'Powerform Forms, Polls & Quizzes',
					'desc'         => __( 'Create dynamic forms easily and quickly with our form builder.', 'psource_recommended_plugins_notice' ),
					'image'        => trailingslashit( plugin_dir_url( __FILE__ ) ) . '/assets/images/plugins-ppowerform.png',
					'free_slug'    => 'powerform/powerform.php',
					'pro_slug'     => '',
					'install_link' => 'https://wordpress.org/plugins/powerform/',
				),
				array(
					'name'         => 'Hustle Marketing',
					'desc'         => __( 'Generate leads with pop-ups, slide-ins and email opt-ins.', 'psource_recommended_plugins_notice' ),
			...
			$recommended_plugins = apply_filters( 'psource-all-recommended-plugins', $recommended_plugins );
		...
		protected function get_recommended_plugins_for_notice( $min = 2, $max = 2 ) {
			 ...
			 * @return array
			 */
			$recommended_plugins_for_notice = apply_filters( 'psource-recommended-plugins', $recommended_plugins_for_notice, $recommended_plugins, $min, $max );
		...
		protected function is_plugin_installed( $plugin_data ) {
			 ...
			 * @return bool
			 */
			$is_installed = apply_filters( 'psource-recommended-plugin-is-installed', $is_installed, $plugin_data, $installed_plugins );
		...
		public function display() {
			 ...
			 */
			do_action( 'psource-recommended-plugins-before-display' );
			 ...
			 * @return bool
			 */
			$is_displayable = apply_filters( 'psource-recommended-plugins-is-displayable', $is_displayable );
			if ( ! $is_displayable ) {
				return;
			}
			 ...
			 * @return bool
			 */
			$active_registered_plugin = apply_filters( 'psource-recommended-plugin-active-registered', $active_registered_plugin );

			if ( ! $active_registered_plugin instanceof PSOURCE_Recommended_Plugins_Notice_Registered_Plugin ) {
				return;
			}
			...
			wp_register_style( 'psource-recommended-plugins-css', trailingslashit( plugin_dir_url( __FILE__ ) ) . 'assets/css/notice.css', array(), self::VERSION );
			wp_enqueue_style( 'psource-recommended-plugins-css' );

			wp_register_script( 'psource-recommended-plugins-js', trailingslashit( plugin_dir_url( __FILE__ ) ) . 'assets/js/notice.js', array( 'jquery' ), self::VERSION, true );
			wp_enqueue_script( 'psource-recommended-plugins-js' );

			$dismissed_text = __( 'Dismiss', 'psource_recommended_plugins_notice' );
			 ...
			 * @return string
			 */
			$dismissed_text = apply_filters( 'psource-recommended-plugins-dismissed-text', $dismissed_text );
			...
			?>
			<div class="psource-recommended-plugins" style="display: none"
			     data-selector-el="<?php echo esc_attr( $data_selector_el ); ?>"
			     data-selector-fn="<?php echo esc_attr( $data_selector_fn ); ?>">
				<p class="psource-notice-status"><?php echo $active_registered_plugin->get_pre_text_notice();// wpcs xss ok. allow html here ?></p>
				<div class="psource-recommended-plugin-blocks">
				<?php foreach ( $recommended_plugins as $recommended_plugin ) : ?>
					<div class="psource-recommended-plugin-block">
						<a href="<?php echo esc_attr( $recommended_plugin['install_link'] ); ?>" target="_blank">
							<div class="psource-recommended-plugin-block-image">
								<img src="<?php echo esc_url( $recommended_plugin['image'] ); ?>" alt="<?php echo esc_attr( $recommended_plugin['name'] ); ?>"/>
							</div>
							<div class="psource-recommended-plugin-block-detail">
								<h3 class="psource-plugin-name"><?php echo esc_html( $recommended_plugin['name'] ); ?></h3>
								<p class="psource-plugin-description"><?php echo esc_html( $recommended_plugin['desc'] ); ?></p>
							</div>
						</a>
					</div>
				<?php endforeach; ?>
				...
				<div class="psource-recommended-plugins-dismiss">
					<a 	class="dismiss"
						href="#"
					  	aria-label="Dismiss"
			 ...
			 */
			do_action( 'psource-recommended-plugins-after-display' );
		}
		...
		public function is_displayable() {
			 ...
			 * @return bool
			 */
			$dismissed = apply_filters( 'psource-recommended-plugins-dismissed', $dismissed, $dismissed_wp_pointers );
			 ...
			 * @return string
			 */
			$capability_to_check = apply_filters( 'psource-recommended-plugins-capability', $capability_to_check );
			...
			/**
			 * CHECK #3 is_psource_member
			 *
			 * This guy is a pro!!
			 * - psource dash activated and or membership type = full
			 */
			if ( function_exists( 'is_psource_member' ) ) {
				if ( is_psource_member() ) {
					return false;
				}
			}
		...
		public function register( $plugin_basename, $plugin_name, $screens = array(), $selector = array() ) {
			...
			if ( ! isset( $this->registered_plugins[ $plugin_basename ] ) ) {
				$registered_plugin_object                     = new PSOURCE_Recommended_Plugins_Notice_Registered_Plugin( $plugin_basename );
		...
		public function parse_saved_registered_plugins() {
			 ...
			 */
			do_action( 'psource-recommended-plugins-before-parse-saved-registered-plugins' );
		...
		/**
		 * @return PSOURCE_Recommended_Plugins_Notice
		 */
	...
	$GLOBALS['PSOURCE_Recommended_Plugins_Notice'] = PSOURCE_Recommended_Plugins_Notice::get_instance();
}
