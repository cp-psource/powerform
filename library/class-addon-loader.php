...
require_once powerform_plugin_dir() . 'library/addon/class-addon-exception.php';
require_once powerform_plugin_dir() . 'library/addon/class-addon-container.php';
require_once powerform_plugin_dir() . 'library/addon/contracts/interface-addon.php';
require_once powerform_plugin_dir() . 'library/addon/class-addon-abstract.php';
require_once powerform_plugin_dir() . 'library/addon/class-addon-form-settings-abstract.php';
require_once powerform_plugin_dir() . 'library/addon/class-addon-form-hooks-abstract.php';
require_once powerform_plugin_dir() . 'library/addon/class-addon-poll-settings-abstract.php';
require_once powerform_plugin_dir() . 'library/addon/class-addon-poll-hooks-abstract.php';
require_once powerform_plugin_dir() . 'library/addon/class-addon-quiz-settings-abstract.php';
require_once powerform_plugin_dir() . 'library/addon/class-addon-quiz-hooks-abstract.php';
require_once powerform_plugin_dir() . 'library/addon/admin/class-ajax.php';

/**
 * Class Powerform_Addon_Loader
...
class Powerform_Addon_Loader {
	 ...
	 * @var string
	 */
	private static $_active_addons_option = 'powerform_activated_addons';
	...
	/**
	 * Flag when options of `powerform_activated_addons` not exist on database
	 * When its true, Loader will assume, user is first time updated to 1.1
	 * And will try to activate @see Powerform_Addon_Loader::$pre_activated_addons
	 *
	 * @since 1.1
	 * @var bool
	 */
	...
	/**
	 * Flag when options of `powerform_activated_addons` exist but empty on database
	...
	/**
	 * Powerform_Addon_Loader constructor.
	 *
	 * @since 1.1
	 */
	public function __construct() {
		$this->addons = new Powerform_Addon_Container();
		...
		Powerform_Addon_Admin_Ajax::get_instance();
	}
	 ...
	 * @since 1.1
	 *
	 * @param Powerform_Addon_Abstract|string $class_name instance of Addon or its classname
	 *
	 * @return bool
	 */
	public function register( $class_name ) {
		try {
			 ...
			 * when validation fail or requirement not set @see Powerform_Addon_Loader::register()
			 *
			 * @since 1.1
			 *
			 * @param Powerform_Addon_Abstract|string $class_name instance of Addon or its class name
			 */
			do_action( 'powerform_before_addon_registered', $class_name );

			if ( $class_name instanceof Powerform_Addon_Abstract ) {
			 ...
			 * Return must be instance of @see Powerform_Addon_Abstract.
			 * It will be then validated by @see Powerform_Addon_Loader::validate_addon_instance()
			 *
			 * @since 1.1
			 *
			 * @param Powerform_Addon_Abstract $addon_class       Current Addon class instance
			 * @param array                     $registered_addons Current registered addons
			 */
			$addon_class = apply_filters( 'powerform_addon_instance', $addon_class, $registered_addons );
			 ...
			 * @since 1.1
			 *
			 * @param Powerform_Addon_Abstract $addon_class Current addon that successfully registered
			 */
			do_action( 'powerform_after_addon_registered', $addon_class );

			return true;
		} catch ( Powerform_Addon_Exception $e ) {
			powerform_addon_maybe_log( __METHOD__, $class_name, $e->getMessage() );
	...
	private function validate_addon_class( $class_name ) {
		if ( ! class_exists( $class_name ) ) {
			throw new Powerform_Addon_Exception( 'Addon with ' . $class_name . ' does not exist' );
		}

		if ( ! is_callable( array( $class_name, 'get_instance' ) ) ) {
			throw new Powerform_Addon_Exception( 'Addon with ' . $class_name . ' does not have get_instance method' );
		}
	 ...
	 * @since 1.1
	 *
	 * @param Powerform_Addon_Abstract $instance
	 *
	 * @return Powerform_Addon_Abstract
	 * @throws Powerform_Addon_Exception
	 */
	private function validate_addon_instance( Powerform_Addon_Abstract $instance ) {
		/** @var Powerform_Addon_Abstract $addon_class */
		$addon_class = $instance;
		$class_name  = get_class( $instance );

		if ( ! $addon_class instanceof Powerform_Addon_Abstract ) {
			throw new Powerform_Addon_Exception( 'Addon with ' . $class_name . ' is not instanceof Powerform_Addon_Abstract' );
		}
		$slug    = $addon_class->get_slug();
		$version = $addon_class->get_version();

		if ( empty( $slug ) ) {
			throw new Powerform_Addon_Exception( 'Addon with ' . $class_name . ' does not have slug' );
		}

		// FIFO
		if ( isset( $this->addons[ $slug ] ) ) {
			throw new Powerform_Addon_Exception( 'Addon with slug ' . $slug . ' already exist' );
		}
		if ( empty( $version ) ) {
			throw new Powerform_Addon_Exception( 'Addon with slug ' . $slug . ' does not have valid version' );
		}
		...
		if ( $this->addon_is_active( $slug ) ) {
			try {
				...
				powerform_addon_maybe_log( $addon_class->get_slug(), 'failed to trigger version_changed', $e->getMessage() );
			}
		}
	 ...
	 *
	 * @since 1.1
	 * @return Powerform_Addon_Container
	 */
	...
	public function deactivate_addon( $slug ) {
		 ...
		 * @since 1.1
		 *
		 * @param Powerform_Addon_Abstract $addon Current deactivated addon
		 */
		do_action( 'powerform_after_addon_deactivated', $addon );
	...
	public function force_remove_activated_addons( $slug ) {
		 ...
		 * @param Powerform_Addon_Abstract|null Addon instance or null when addon instance unavailable
		 */
		do_action( 'powerform_after_activated_addons_removed', $slug, $addon );
	}
	...
	public function activate_addon( $slug ) {
		 ...
		 * @param Powerform_Addon_Abstract|null $addon addon instance or null, when its not unavailable
		 */
		do_action( 'powerform_before_addon_activated', $slug, $addon );
		 ...
		 * @since 1.1
		 *
		 * @param Powerform_Addon_Abstract $addon Current activated addon
		 */
		do_action( 'powerform_after_addon_activated', $addon );
	...
	public function get_last_error_message() {
		 ...
		 */
		$last_error_message = apply_filters( 'powerform_addon_loader_last_error_message', $last_error_message );
	...
	public function get_default_messages() {
		 ...
		 */
		$default_addon_error_messages = apply_filters( 'powerform_addon_loader_default_messages', $default_addon_error_messages );
