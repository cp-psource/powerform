...
class Powerform_PayPal_Express extends Powerform_Payment_Gateway {
	...
	public function init_settings() {
		...
		if ( empty( $this->sandbox_id ) && defined( 'POWERFORM_PAYPAL_SANDBOX_ID' ) ) {
			$this->sandbox_id = POWERFORM_PAYPAL_SANDBOX_ID;
		}

		if ( empty( $this->sandbox_secret ) && defined( 'POWERFORM_PAYPAL_SANDBOX_SECRET' ) ) {
			$this->sandbox_secret = POWERFORM_PAYPAL_SANDBOX_SECRET;
		}

		add_filter( 'script_loader_src', array( $this, 'powerform_remove_ver_paypal' ), 9999 );
	}
	...
	public static function store_settings( $settings ) {
		update_option( 'powerform_paypal_configuration', $settings );
	}
	...
	public static function is_available() {
		...
		$loaded          = powerform_payment_lib_paypal_version_loaded();
	...
	public function validate_id( $mode, $id, $secret, $error = self::INVALID_SANDBOX_SECRET_EXCEPTION ) {
		...
		try {
			...
			if ( ! isset( $result->access_token ) || empty( $result->access_token ) ) {
				throw new Powerform_Gateway_Exception( __( 'Failed to configure PayPal payment', Powerform::DOMAIN ) );
			}
		} catch ( Exception $e ) {
			powerform_maybe_log( __METHOD__, $e->getMessage() );
			throw new Powerform_Gateway_Exception(
				__( 'Some error has occurred while connecting to your PayPal account. Please resolve the following errors and try to connect again.', Powerform::DOMAIN ),
				$error,
				$e
			);
		}
	...
	public function api_request( $request, $mode, $args = array(), $method = 'GET' ) {
		...
		$query_args = array(
			...
			'headers'   => $headers,
			'body'		=> $body,
			'sslverify' => apply_filters( 'powerform_paypal_request_sslverify', false ),
			'timeout'   => apply_filters( 'powerform_paypal_request_timeout', 30 ),
		);
	 ...
	 * @return string
	 */
	public function powerform_remove_ver_paypal( $src ) {
