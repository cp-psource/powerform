...
/**
 * Class Powerform_CForm_General_Data_Protection
 *
 * General Data Protection Applied for Custom Form
 *
 * @since 1.0.6
 */
class Powerform_CForm_General_Data_Protection extends Powerform_General_Data_Protection {
	...
	public function __construct() {
		parent::__construct( __( 'Powerform-Formulare', Powerform::DOMAIN ) );
	...
	public function get_privacy_message() {
		...
		$content = apply_filters( 'powerform_custom_form_privacy_policy_content', $content );
	...
	public static function form_submissions_exporter( $email_address, $page ) {
		$entry_ids = Powerform_Form_Entry_Model::get_custom_form_entry_ids_by_email( $email_address );
		...
		if ( ! empty( $entry_ids ) && is_array( $entry_ids ) ) {
			foreach ( $entry_ids as $entry_id ) {
				...
				if ( ! isset( self::$custom_form_model_instances[ $entry_model->form_id ] ) ) {
					$model                                                      = Powerform_Custom_Form_Model::model()->load( $entry_model->form_id );
				...
				if ( is_object( $model ) ) {
					...
					foreach ( $mappers as $mapper ) {
						...
						if ( isset( $mapper['property'] ) ) {
							...
							if ( ! isset( $mapper['sub_metas'] ) ) {
								$data[] = array(
									'name'  => $mapper['label'],
									'value' => Powerform_Form_Entry_Model::meta_value_to_string( $mapper['type'], $meta_value ),
								);
							} else {
								// sub_metas available
								foreach ( $mapper['sub_metas'] as $sub_meta ) {
									...
									if ( isset( $meta_value[ $sub_key ] ) && ! empty( $meta_value[ $sub_key ] ) ) {
										...
										$data[]     = array(
											'name'  => $sub_meta['label'],
											'value' => Powerform_Form_Entry_Model::meta_value_to_string( $field_type, $value ),
										);
					...
					$data [] = array(
						'name'  => __( 'Eintrags-ID', Powerform::DOMAIN ),
						'value' => '#' . $entry_model->entry_id,
					);
					$data [] = array(
						'name'  => __( 'Abgabetermin', Powerform::DOMAIN ),
					...
					foreach ( $entry_model->meta_data as $key => $meta_datum ) {
						...
						$data [] = array(
							'name'  => $key,
							'value' => Powerform_Form_Entry_Model::meta_value_to_string( '', $meta_datum_encoded, false ),
						);
		 ...
		 */
		$data_to_export = apply_filters( 'powerform_general_data_custom_form_submissions_export_data', $data_to_export, $email_address, $entry_ids );
	...
	public static function get_custom_form_export_mappers( $model ) {
		...
		$mappers = array(
			array(
				...
				'label'    => __( 'Eintrags-ID', Powerform::DOMAIN ),
				'type'     => 'entry_id',
			),
			array(
	...
	public static function form_submissions_eraser( $email_address, $page ) {
		...
		foreach ( $entry_ids as $entry_id ) {
			...
			if ( ! empty( $entry_model->form_id ) ) {
				$custom_form = Powerform_Custom_Form_Model::model()->load( $entry_model->form_id );
			...
			$remove_form_submission = apply_filters( 'powerform_general_data_erase_form_submission', $form_submission_erasure_enabled, $entry_model );
	...
	public function personal_data_cleanup() {
		$overridden_forms_privacy = get_option( 'powerform_form_privacy_settings', array() );
		...
		foreach ( $overridden_forms_privacy as $form_id => $retentions ) {
			...
			$entry_ids = Powerform_Form_Entry_Model::get_older_entry_ids_of_form_id( $form_id, $retain_time );
		...
		$entry_ids = Powerform_Form_Entry_Model::get_older_entry_ids( 'custom-forms', $retain_time );
	...
	public function cleanup_ip_address() {
		$retain_number = get_option( 'powerform_retain_ip_interval_number', 0 );
		$retain_unit   = get_option( 'powerform_retain_ip_interval_unit', 'days' );
	 ...
	 */
	private function anonymize_entry_model( Powerform_Form_Entry_Model $entry_model ) {
		if ( isset( $entry_model->meta_data['_powerform_user_ip'] ) ) {
			$meta_id    = $entry_model->meta_data['_powerform_user_ip']['id'];
			$meta_value = $entry_model->meta_data['_powerform_user_ip']['value'];
			...
			if ( $anon_value !== $meta_value ) {
				$entry_model->update_meta( $meta_id, '_powerform_user_ip', $anon_value );
			}
