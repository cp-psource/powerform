 ...
 * @since 1.11
 */
class Powerform_CForm_User_Signups {
	...
	public function __construct( $signup ) {
		...
		$this->entry    = new Powerform_Form_Entry_Model( $this->meta['entry_id'] );
		$this->form     = Powerform_Custom_Form_Model::model()->load( $this->meta['form_id'] );
	...
	public static function get( $key ) {
		...
		if ( empty( $signup ) ) {
			return new WP_Error( 'invalid_key', __( 'Ungültiger Aktivierungsschlüssel.' ) );
		}
		if ( $signup->active ) {
			return new WP_Error( 'already_active', __( 'Der Benutzer ist bereits aktiv.' ), $signup );
		}

		return new Powerform_CForm_User_Signups( $signup );
	}
	...
	public static function prep_signups_functionality() {
		...
		add_filter( 'wpmu_signup_user_notification_email', array( 'Powerform_CForm_User_Signups', 'modify_signup_user_notification_message' ), 10, 4 );
		add_filter( 'wpmu_signup_blog_notification_email', array( 'Powerform_CForm_User_Signups', 'modify_signup_blog_notification_message' ), 10, 7 );

		//Disable activation email for manual activation method
		add_filter( 'wpmu_signup_user_notification', array( 'Powerform_CForm_User_Signups', 'maybe_suppress_signup_user_notification' ), 10, 3 );
		add_filter( 'wpmu_signup_blog_notification', array( 'Powerform_CForm_User_Signups', 'maybe_suppress_signup_blog_notification' ), 10, 6 );

		add_filter( 'wpmu_signup_user_notification', array( 'Powerform_CForm_User_Signups', 'add_site_name_filter' ) );
		add_filter( 'wpmu_signup_user_notification_subject', array( 'Powerform_CForm_User_Signups', 'remove_site_name_filter' ) );
	}
	 ...
	 *
	 * @return array|Powerform_CForm_User_Signups|WP_Error
	 */
	public static function activate_signup( $key, $is_user_signon ) {
		...
		if ( $user_id ) {
			...
			return new WP_Error( 'user_already_exists', __( 'Dieser Benutzername ist bereits aktiviert.', Powerform::DOMAIN ), $signup );
		}

		if ( email_exists( $signup->user_data['user_email'] ) ) {
			//Email already exists
			return new WP_Error( 'email_already_exists', __( 'Entschuldigung, diese E-Mail Adresse wird bereits verwendet!', Powerform::DOMAIN ), $signup );
		}

		if ( powerform_is_main_site() ) {
			remove_action( 'powerform_cform_user_registered', array( 'Powerform_CForm_Front_User_Registration', 'create_site' ) );
		}

		$password = Powerform_CForm_Front_User_Registration::openssl_decrypt( $signup->user_data['user_pass'] );
		...
		$signup->submitted_data = $powerform_user_registration->change_submitted_data( $signup->submitted_data, $password );

		$user_id = $powerform_user_registration->create_user( $user_data, $signup->form, $signup->entry, $signup->submitted_data, $is_user_signon );
		if ( ! $user_id ) {
			return new WP_Error( 'create_user', __( 'Benutzer konnte nicht erstellt werden', Powerform::DOMAIN ), $signup );
		}

		$signup->set_as_activated();

		do_action( 'powerform_activate_user', $user_id, $signup->meta );

		//Create site only on main site and if option for that is enabled
		if ( powerform_is_main_site() ) {
			$option_create_site = powerform_get_property( $signup->settings, 'site-registration' );
			if ( isset( $option_create_site ) && 'enable' === $option_create_site ) {
				$powerform_user_registration->create_site( $user_id, $signup->form, $signup->entry, $password, $signup->submitted_data );
			}
		}
	...
	public static function delete_signup( $key ) {
		...
		do_action( 'powerform_cform_userregistration_delete_signup', $signup );
