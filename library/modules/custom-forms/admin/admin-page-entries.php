...
/**
 * Class Powerform_CForm_View_Page
 *
 * @since 1.0
 */
class Powerform_CForm_View_Page extends Powerform_Admin_Page {

	/**
	 * Current model
	 *
	 * @var Powerform_Custom_Form_Model|bool
	 */
	...
	public function before_render() {
		...
		if ( isset( $_REQUEST['form_id'] ) ) { // WPCS: CSRF OK
			 ...
			 * @param Powerform_Custom_Form_Model $custom_form_model Current Form Model
			 * @param array                        $visible_fields    Visible fields on page
			 * @param int                          $pagenum           current page number
			 */
			do_action(
				'powerform_custom_form_admin_page_entries',
				$form_id,
				$custom_form_model,
				$visible_fields,
				$pagenum
	...
	public function process_request() {
		...
		if ( ! wp_verify_nonce( $nonce, 'powerformCustomFormEntries' ) ) {
			return;
		}
	 ...
	 *
	 * @return string
	 * TO-DO: replace Powerform_CForm_View_Page::render_entry() by render_entry() on other files
	 */
	...
	public function paginate() {
		...
		powerform_list_pagination( $count, 'entries' );
	}

	/**
	 * Mimic from export
	 *
	 * @see Powerform_Export::get_custom_form_export_mappers()
	 ...
	 * @return array
	 */
	private function attach_addon_on_render_entry( Powerform_Form_Entry_Model $entry_model ) {
	...
	public function get_connected_addons() {
		if ( is_null( self::$connected_addons ) ) {
			...
			$connected_addons = powerform_get_addons_instance_connected_with_form( $this->form_id );
		...
		if ( empty( self::$registered_addons ) ) {
			...
			foreach ( $registered_addons as $registered_addon ) {
				try {
					...
					powerform_addon_maybe_log( $registered_addon->get_slug(), 'failed to get_addon_form_hooks', $e->getMessage() );
				}
			}
		}
