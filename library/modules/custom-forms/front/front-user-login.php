...
add_action( 'powerform_cform_render_fields', array( 'Powerform_CForm_Front_User_Login', 'render_fields' ), 11, 2 );
 ...
 * @since 1.11
 */
class Powerform_CForm_Front_User_Login extends Powerform_User {
	 ...
	 * @return array
	 */
	public function process_login( $custom_form, $submitted_data, Powerform_Form_Entry_Model $entry, $field_data_array ) {
		...
		$username = apply_filters( 'powerform_custom_form_login_username_before_signon', $username, $custom_form, $submitted_data, $entry );
		...
		$password              = apply_filters( 'powerform_custom_form_login_password_before_signon', $password, $custom_form, $submitted_data, $entry );
		...
		$remember = apply_filters( 'powerform_custom_form_login_remember_before_signon', $remember, $custom_form, $entry );
		...
		if ( is_object( $custom_form ) ) {
			...
			foreach ( $fields as $field ) {
				...
				if ( 'checkbox' === $field_type ) {
					$last_id = Powerform_Field::get_property( 'element_id', $field_array );
	...
	public static function render_fields( $wrappers, $id ) {
		$custom_form = Powerform_Custom_Form_Model::model()->load( $id );
		...
		if ( isset( $custom_form->settings['form-type'] )
			...
			if ( isset( $custom_form->settings['remember-me-label'] ) && ! empty( $custom_form->settings['remember-me-label'] ) ) {
				...
				$label = __( 'Behalte mich in Erinnerung', Powerform::DOMAIN );
			}
