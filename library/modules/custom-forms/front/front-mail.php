 ...
 * @since 1.0
 */
class Powerform_CForm_Front_Mail extends Powerform_Mail {
	...
	public function init( $post_vars ) {
		 ...
		 */
		$this->message_vars = apply_filters( 'powerform_custom_form_message_vars', $message_vars, $embed_id, $post_vars );
	}
	...
	public function process_mail( $custom_form, $data, Powerform_Form_Entry_Model $entry ) {
		 ...
		 * @param Powerform_Custom_Form_Model $custom_form - the form
		 * @param Powerform_Form_Entry_Model  $entry       - saved entry @since 1.0.3
		 *
		 *
		 * @return array $data
		 */
		$data = apply_filters( 'powerform_custom_form_mail_data', $data, $custom_form, $entry );
		...
		if ( $this->send_admin_mail( $setting ) || $this->send_user_mail( $setting ) ) {
			...
			if ( $this->send_admin_mail( $setting ) ) {
				...
				$recipients = apply_filters_deprecated(
					'powerform_custom_form_mail_admin_recipients',
					array( $recipients, $custom_form, $data, $entry, $this ),
					'1.6.2',
					'powerform_get_admin_email_recipients'
				);
				...
				if ( ! empty( $recipients ) ) {
					 ...
					 */
					$subject = apply_filters( 'powerform_custom_form_mail_admin_subject', $subject, $custom_form, $data, $entry, $this );
					 ...
					 * @param Powerform_CForm_Front_Mail  $this        mail class
					 */
					$bcc_addresses = apply_filters( 'powerform_custom_form_mail_admin_bcc_addresses', $bcc_addresses, $custom_form, $data, $entry, $this );
					 ...
					 * @param Powerform_CForm_Front_Mail  $this        mail class
					 */
					$content_type = apply_filters( 'powerform_custom_form_mail_admin_content_type', $content_type, $custom_form, $data, $entry, $this );
					 ...
					 * @param Powerform_CForm_Front_Mail  $this        mail class
					 */
					$headers = apply_filters( 'powerform_custom_form_mail_admin_headers', $headers, $custom_form, $data, $entry, $this );
		 ...
		 * @param array $data - current data
		 */
		do_action( 'powerform_custom_form_mail_after_send_mail', $this, $custom_form, $data );
	}
	...
	public function get_user_email( $data, $custom_form ) {
		...
		return apply_filters( 'powerform_get_user_email', $email, $data, $custom_form, $data_email, $this );
	}
	...
	public function get_user_email_recipients( $data, $custom_form, $entry ) {
		...
		if ( ! isset( $setting['user-email-recipients'] ) || ! is_array( $setting['user-email-recipients'] ) || empty( $setting['user-email-recipients'] ) ) {
			...
			foreach ( $setting_recipients as $key => $setting_recipient ) {
				$setting_recipient = powerform_replace_form_data( $setting_recipient, $data );
				$setting_recipient = powerform_replace_variables( $setting_recipient, $custom_form->id, $data['current_url'] );
				$setting_recipient = powerform_replace_custom_form_data( $setting_recipient, $custom_form, $data, $entry, $this->skip_custom_form_data['user'] );
		...
		$email = apply_filters_deprecated( 'powerform_get_admin_email_recipients', array( $email, $setting, $custom_form, $entry ), '1.6.2', 'powerform_get_user_email_recipients' );

		return apply_filters( 'powerform_get_user_email_recipients', $email, $setting, $data, $custom_form, $entry );
	}
}