 ...
 * @since 1.11
 */
class Powerform_CForm_Front_User_Registration extends Powerform_User {
	...
	public function __construct() {
		...
		$this->mail_sender = new Powerform_CForm_Front_Mail();

		if ( powerform_is_main_site() ) {
			add_action( 'powerform_cform_user_registration_validation', array( $this, 'multisite_validation' ), 10, 4 );
			add_action( 'powerform_cform_user_registered', array( $this, 'create_site' ), 10, 5 );
		}
		add_filter( 'powerform_custom_registration_form_errors', array( $this, 'submit_errors' ), 11, 3 );
		//Change text of thankyou-message for other activation methods: 'email' && 'manual'
		add_filter( 'powerform_custom_form_thankyou_message', array( $this, 'change_thankyou_message' ), 11, 3 );
		//Change value of a field that is not saved in DB
		add_filter( 'powerform_custom_form_after_render_value', array( $this, 'change_field_value' ), 11, 4 );
	}
	...
	public function handle_user_activation( $user_data, $custom_form, $entry, $submitted_data ) {
		...
		require_once __DIR__ . '/../user/class-powerform-cform-user-signups.php';

		Powerform_CForm_User_Signups::prep_signups_functionality();
		...
		$option_create_site = powerform_get_property( $settings, 'site-registration' );
		if ( is_multisite()
		...
		Powerform_CForm_User_Signups::add_signup_meta( $entry, 'activation_method', $settings['activation-method'] );
		Powerform_CForm_User_Signups::add_signup_meta( $entry, 'activation_key', $activation_key );
	...
	public function create_user( $new_user_data, $custom_form, $entry, $submitted_data, $is_user_signon = false ) {
		$new_user_data = apply_filters( 'powerform_custom_form_user_registration_before_insert', $new_user_data, $custom_form, $entry );

		$user_id = wp_insert_user( $new_user_data );
		if ( is_wp_error( $user_id ) ) {

			return __( 'Du konnteyt Dich nicht registrieren. Bitte kontaktiere uns, wenn Du weiterhin Probleme hast.', Powerform::DOMAIN );
		}
		...
		if ( isset( $settings['activation-email'] ) && 'default' === $settings['activation-email'] ) {
			$this->powerform_new_user_notification( $user_id, $new_user_data['user_pass'] );
		} else {
			//Send notification to admin
			$this->powerform_new_user_notification( $user_id, '' );
		}

		do_action( 'powerform_cform_user_registered', $user_id, $custom_form, $entry, $new_user_data['user_pass'], $submitted_data );
	...
	public function pending_activation_exists( $key, $value ) {
		...
		if ( Powerform_CForm_User_Signups::table_exists( $table_name ) && in_array( $key, array( 'user_login', 'user_email' ) ) ) {// phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict
			if ( 'user_login' === $key ) {
	...
	public function validate( $custom_form, $submitted_data, $field_data_array, $is_approve = false ) {
		...
		if ( ! is_multisite() && $this->pending_activation_exists( 'user_login', $username ) ) {
			...
			return __( 'Dieser Benutzername ist derzeit reserviert, kann jedoch in einigen Tagen verfügbar sein', Powerform::DOMAIN );
		}
		...
		if ( ! is_multisite() && $this->pending_activation_exists( 'user_email', $user_email ) ) {
			...
			return __( 'Diese E-Mail-Adresse wurde bereits verwendet. Bitte überprüfe Deinen Posteingang auf eine Aktivierungs-E-Mail. Es wird in ein paar Tagen verfügbar sein, wenn Du nichts tust.', Powerform::DOMAIN );
		}

		//Multisite validation
		$validate = apply_filters( 'powerform_cform_user_registration_validation', $validate, $custom_form, $submitted_data, $is_approve );
		if ( ! $validate['result'] ) {
	 ...
	 * @return array|mixed
	 */
	public function process_registration( $custom_form, $submitted_data, Powerform_Form_Entry_Model $entry ) {
		$settings      = $custom_form->settings;
	...
	private function validate_email( $email ) {
		...
		if ( $email ) {
			if ( ! is_email( $email ) ) {
				$data['result']  = false;
				$data['message'] = __( 'Diese E-Mail-Adresse ist ungültig.', Powerform::DOMAIN );
			...
			if ( email_exists( $email ) ) {
				$data['result']  = false;
				$data['message'] = __( 'Diese Email-Adresse ist bereits registriert.', Powerform::DOMAIN );

				return $data;
			}
		} else {
			$data['result']  = false;
			$data['message'] = __( 'Die E-Mail-Adresse darf nicht leer sein.', Powerform::DOMAIN );
	...
	private function validate_username( $username ) {
		...
		if ( $username ) {
			...
			if ( ! validate_username( $username ) ) {
				$data['result']  = false;
				$data['message'] = __( 'Dieser Benutzername ist ungültig, da unzulässige Zeichen verwendet werden. Bitte gib einen gültigen Benutzername ein.', Powerform::DOMAIN );
			...
			if ( username_exists( $username ) ) {
				$data['result']  = false;
				$data['message'] = __( 'Dieser Benutzername ist bereits registriert.', Powerform::DOMAIN );

				return $data;
			}
		} else {
			$data['result']  = false;
			$data['message'] = __( 'Der Benutzername darf nicht leer sein.', Powerform::DOMAIN );
	...
	public function multisite_validation( $validate, $custom_form, $submitted_data, $is_approve ) {
		...
		$option_create_site = powerform_get_property( $setting, 'site-registration' );
	...
	public function create_site( $user_id, $custom_form, $entry, $password, $submitted_data ) {
		 ...
		 */
		$site_meta = apply_filters( 'powerform_cform_user_registration_new_site_meta', array( 'public' => 1 ), $custom_form, $entry, $user_id );
		...
		$site_role = powerform_get_property( $setting, 'site-registration-role-field' );
		if ( $site_role ) {
			$user = new WP_User( $user_id, null, $blog_id );
			$user->set_role( $site_role );
		}

		$root_role = powerform_get_property( $setting, 'registration-role-field' );
		...
		do_action( 'powerform_cform_site_created', $blog_id, $user_id, $entry, $custom_form, $password );
	...
	private function replace_site_data( $setting, $submitted_data ) {
		$blog_address = '';
		$address      = powerform_get_property( $setting, 'site-registration-name-field' );
		...
		$blog_title = powerform_get_property( $setting, 'site-registration-title-field' );
	...
	public function add_user_meta( $user_id, $setting, $submitted_data ) {
		...
		foreach ( $custom_meta as $meta_key => $meta_value ) {
			...
			if ( strpos( $meta_value, '{' ) !== false ) {
				$meta_value = powerform_replace_form_data( $meta_value, $submitted_data );
				$meta_value = powerform_replace_variables( $meta_value, $setting['form_id'] );
			}
	 ...
	 */
	public function powerform_new_user_notification( $user_id, $plaintext_pass = '', $notify = '' ) {
		...
		$message  = sprintf( __( 'Registrierung neuer Benutzer auf Deiner Webseite %s:', Powerform::DOMAIN ), $blogname ) . "\r\n\r\n";
		$message .= sprintf( __( 'Nutzername: %s', Powerform::DOMAIN ), $user->user_login ) . "\r\n\r\n";
		$message .= sprintf( __( 'Email: %s', Powerform::DOMAIN ), $user->user_email ) . "\r\n";

		$result = @wp_mail( get_option( 'admin_email' ), sprintf( __( '[%s] Neue Benutzerregistrierung', Powerform::DOMAIN ), $blogname ), $message );
		...
		if ( empty( $plaintext_pass ) ) {
			$message .= __( 'Um Dein Passwort festzulegen, besuche die folgende Adresse:', Powerform::DOMAIN ) . "\r\n\r\n";
			$message .= '<' . $this->get_set_password_url( $user ) . ">\r\n\r\n";
		} else {
			$message .= sprintf( __( 'Passwort: %s', Powerform::DOMAIN ), $plaintext_pass ) . "\r\n\r\n";
		}

		$message .= wp_login_url() . "\r\n";

		$result = wp_mail( $user->user_email, sprintf( __( '[%s] Dein Benutzername und Passwort', Powerform::DOMAIN ), $blogname ), $message );
	 ...
	 * @param Powerform_Custom_Form_Model $custom_form
	 *
	 * @return Powerform_Custom_Form_Model
	 */
