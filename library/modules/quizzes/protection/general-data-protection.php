...
/**
 * Class Powerform_Quiz_General_Data_Protection
 *
 * General Data Protection Applied for Quiz
 *
 * @since 1.6.2
 */
class Powerform_Quiz_General_Data_Protection extends Powerform_General_Data_Protection {

	public function __construct() {
		parent::__construct( __( 'Powerform Tests', Powerform::DOMAIN ) );
	}
	...
	public function get_privacy_message() {
		...
		$content = apply_filters( 'powerform_quiz_privacy_policy_content', $content );
	...
	public function personal_data_cleanup() {
		...
		$global_retain_number = get_option( 'powerform_retain_quiz_submissions_interval_number', 0 );
		$global_retain_unit   = get_option( 'powerform_retain_quiz_submissions_interval_unit', 'days' );
		 ...
		 * @return string
		 */
		$quiz_status = apply_filters( 'powerform_quiz_general_data_protection_cleanup_quiz_status', $quiz_status );

		/**
		 * Get all quizzes
		 */
		$quizzes = Powerform_Quiz_Form_Model::model()->get_all_models( $quiz_status );

		/** @var Powerform_Quiz_Form_Model[] $models */
	...
	private function delete_older_entries( $quiz_id, $retain_time ) {
		$entry_ids = Powerform_Form_Entry_Model::get_older_entry_ids_of_form_id( $quiz_id, $retain_time );
