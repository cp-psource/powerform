 ...
 */
class Powerform_QForm_Front extends Powerform_Render_Form {
	...
	protected $ajax_load_action = 'powerform_load_quiz';
	...
	public function init() {
		add_shortcode( 'powerform_quiz', array( $this, 'render_shortcode' ) );
	}
...
//			}
	...
	public function message_required() {
		return esc_html__( "Formular-ID-Attribut ist erforderlich!", Powerform::DOMAIN );
	}
	...
	public function get_form_design() {
		...
		if ( 'clean' !== $quiz_theme ) {
			$form_design .= ' powerform-design--' . $quiz_theme;
		}
	...
	public function get_quiz_theme() {
		$quiz_theme = 'default';
		$settings   = $this->get_form_settings();
		if ( isset( $settings['powerform-quiz-theme'] ) && ! empty( $settings['powerform-quiz-theme'] ) ) {
		 ...
		 */
		$quiz_theme = apply_filters( 'powerform_quiz_theme', $quiz_theme, $quiz_id, $settings );
	...
	public function get_google_fonts() {
		 ...
		 * @param array  $settings  quiz settings
		 */
		$fonts = apply_filters( 'powerform_quiz_google_fonts', $fonts, $quiz_id, $quiz_type, $settings );
	...
	public function ajax_display( $id, $is_preview = false, $data = false, $hide = true, $last_submit_data = array(), $extra = array() ) {
		...
		foreach ( $google_fonts as $font_name ) {
			if ( ! empty( $font_name ) ) {
				$response['styles'][ 'powerform-font-' . sanitize_title( $font_name ) ] =
