...
class Powerform_Calculator {
	...
	/**
	 * @var Powerform_Calculator_Parser_Tokenizer
	 */
	protected $tokenizer;
	...
	/**
	 * @var Powerform_Calculator_Parser
	 */
	protected $parser;
	...
	public function __construct( $term ) {
		...
		$this->symbol_loader = new Powerform_Calculator_Symbol_Loader();
	...
	public function setup_tokenizer() {
		$term      = $this->term;
		$tokenizer = new Powerform_Calculator_Parser_Tokenizer( $term );
		 ...
		 * @since 1.7
		 *
		 * @param Powerform_Calculator_Parser_Tokenizer $tokenizer
		 * @param string                                 $term
		 *
		 * @return Powerform_Calculator_Parser_Tokenizer
		 */
		$tokenizer = apply_filters( 'powerform_calculator_tokenizer', $tokenizer, $term );
	...
	public function setup_parser() {
		...
		$parser        = new Powerform_Calculator_Parser( $symbol_loader );
		 ...
		 * @since 1.7
		 *
		 * @param Powerform_Calculator_Parser        $parser
		 * @param Powerform_Calculator_Symbol_Loader $symbol_loader
		 * @param string                              $term
		 *
		 * @return Powerform_Calculator_Parser
		 */
		$parser = apply_filters( 'powerform_calculator_tokenizer', $parser, $symbol_loader, $term );
	...
	public function prepare_term() {
		 ...
		 * @return string
		 */
		$term = apply_filters( 'powerform_calculator_prepare_term', $term );

		$this->term = $term;
	}

	/**
	 * @return Powerform_Calculator_Parser_Node_Container|boolean
	...
	public function parse() {
		try {
			...
			if ( count( $tokens ) === 0 ) {
				throw new Powerform_Calculator_Exception( 'Error: Empty token of calculator term.' );
			}

			$root_node = $this->parser->parse( $tokens );
			if ( $root_node->is_empty() ) {
				throw new Powerform_Calculator_Exception( 'Error: Empty nodes of calculator tokens.' );
			}
	 ...
	 * calculable.)
	 *
	 * @param Powerform_Calculator_Parser_Node_Abstract $node
	 *
	 * @return float|int
	 * @throws Powerform_Calculator_Exception
	 */
	protected function calculate_node( $node ) {
		...
		} elseif ( $node instanceof Powerform_Calculator_Parser_Node_Function ) {
			/** @var Powerform_Calculator_Parser_Node_Function $node */

			return $this->calculate_function_node( $node );
		} elseif ( $node instanceof Powerform_Calculator_Parser_Node_Container ) {
			/** @var Powerform_Calculator_Parser_Node_Container $node */

			return $this->calculate_container_node( $node );
		} else {
			throw new Powerform_Calculator_Exception( 'Error: Cannot calculate node of unknown type "' . get_class( $node ) . '"' );// @codeCoverageIgnore
		}
	}
	 ...
	 *
	 * @param Powerform_Calculator_Parser_Node_Container $container_node
	 *
	 * @return float|int
	 * @throws Powerform_Calculator_Exception
	 */
	protected function calculate_container_node( $container_node ) {
		if ( $container_node instanceof Powerform_Calculator_Parser_Node_Function ) {
			throw new Powerform_Calculator_Exception( 'Error: Expected container node but got a function node' ); // @codeCoverageIgnore
		}
		...
		foreach ( $ordered_operator_nodes as $index => $operator_node ) {
			...
			/** @var Powerform_Calculator_Symbol_Operator_Abstract $symbol */
		...
		if ( count( $nodes ) === 0 ) {
			throw new Powerform_Calculator_Exception( 'Error: Missing calculable subterm. Are there empty brackets?' );
		}

		if ( count( $nodes ) > 1 ) {
			throw new Powerform_Calculator_Exception( 'Error: Missing operators between parts of the term.' );
		}
	 ...
	 *
	 * @param Powerform_Calculator_Parser_Node_Function $function_node
	 *
	 * @return int|float
	 * @throws Powerform_Calculator_Exception
	 */
	protected function calculate_function_node( $function_node ) {
		...
		foreach ( $nodes as $node ) {
			if ( $node instanceof Powerform_Calculator_Parser_Node_Symbol ) {
				...
				if ( $node->get_symbol() instanceof Powerform_Calculator_Symbol_Separator ) {
					$container_node       = new Powerform_Calculator_Parser_Node_Container( $argument_child_nodes );
		...
		/** @var Powerform_Calculator_Symbol_Function_Abstract $symbol */
	 ...
	 *
	 * @param Powerform_Calculator_Parser_Node_Symbol $symbol_node
	 *
	 * @return int|float
	 * @throws Powerform_Calculator_Exception
	 */
	protected function calculate_symbol_node( $symbol_node ) {
		...
		if ( $symbol instanceof Powerform_Calculator_Symbol_Number ) {
			...
			$number = 0 + $number;
		} elseif ( $symbol instanceof Powerform_Calculator_Symbol_Constant_Abstract ) {
			/** @var Powerform_Calculator_Symbol_Constant_Abstract $symbol */

			$number = $symbol->get_value();
		} else {
			throw new Powerform_Calculator_Exception( 'Error: Found symbol of unexpected type "' . get_class( $symbol ) . '", expected number or constant' );
		}
	...
	protected function detect_calculation_order( $nodes ) {
		...
		foreach ( $nodes as $index => $node ) {
			if ( $node instanceof Powerform_Calculator_Parser_Node_Symbol ) {
				if ( $node->get_symbol() instanceof Powerform_Calculator_Symbol_Operator_Abstract ) {
	 ...
	 *
	 * @param Powerform_Calculator_Parser_Node_Symbol $node_one
	 * @param Powerform_Calculator_Parser_Node_Symbol $node_two
	 *
	 * @return int
	 */
	private function sort_operator_precedence( $node_one, $node_two ) {
		...
		/** @var Powerform_Calculator_Symbol_Operator_Abstract $symbol_one */
		...
		/** @var Powerform_Calculator_Symbol_Operator_Abstract $symbol_two */
