...
/**
 * Class Powerform_Calculator_Tokenizer
 *
 */
class Powerform_Calculator_Parser_Tokenizer {
	...
	/**
	 * @return Powerform_Calculator_Parser_Token[]
	...
	/**
	 * @return Powerform_Calculator_Parser_Token|null
	 * @throws Powerform_Calculator_Exception
	 */
	protected function read_token() {
		...
		if ( $this->is_letter( $char ) ) {
			...
			$type  = Powerform_Calculator_Parser_Token::TYPE_WORD;
		} elseif ( $this->is_digit( $char ) || $this->is_period( $char ) ) {
			$value = $this->read_number();
			$type  = Powerform_Calculator_Parser_Token::TYPE_NUMBER;
		} else {
			$value = $this->read_char();
			$type  = Powerform_Calculator_Parser_Token::TYPE_CHAR;
		}

		$token = new Powerform_Calculator_Parser_Token( $type, $value, $this->get_current_position() );
		...
		while ( null !== $char ) {
			if ( $this->is_period( $char ) || $this->is_digit( $char ) ) {
				if ( $this->is_period( $char ) ) {
					if ( $found_period ) {
						throw new Powerform_Calculator_Exception( 'Error: A number cannot have more than one period' );
					}
