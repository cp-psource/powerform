...
/**
 * Class Powerform_Addon_Poll_Hooks_Abstract
 ...
 * @since 1.6.1
 */
abstract class Powerform_Addon_Poll_Hooks_Abstract {
	 ...
	 *
	 * @since 1.6.1
	 * @var Powerform_Addon_Poll_Settings_Abstract|null
	 *
	 */
	...
	/**
	 * Powerform_Addon_Poll_Hooks_Abstract constructor.
	 *
	...
	public function on_before_render_poll_fields() {
		...
		/**
		 * Fires before poll fields rendered by powerform
		 *
		 ...
		 * @param Powerform_Addon_Poll_Settings_Abstract|null $poll_settings_instance of Addon Poll Settings
		 */
		do_action(
			'powerform_addon_' . $addon_slug . '_on_before_render_poll_fields',
			$poll_id,
	...
	public function on_after_render_poll_fields() {
		 ...
		 * @param Powerform_Addon_Form_Settings_Abstract|null $poll_settings_instance of Addon Poll Settings
		 */
		do_action(
			'powerform_addon_' . $addon_slug . '_on_after_render_poll_fields',
			$poll_id,
			$poll_settings_instance
		);
	}
	...
	public function on_after_render_poll() {
		 ...
		 */
		do_action(
			'powerform_addon_' . $addon_slug . '_on_after_render_poll',
			$poll_id,
			$poll_settings_instance
		);
	}
	 ...
	 * and display error message to user @see Powerform_Addon_Poll_Hooks_Abstract::get_submit_poll_error_message()
	 *
	 * @since 1.6.1
	 *
	...
	public function on_poll_submit( $submitted_data ) {
		 ...
		 * @param int                                          $poll_id                current Poll ID
		 * @param Powerform_Addon_Poll_Settings_Abstract|null $poll_settings_instance Addon Poll Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_' . $addon_slug . '_poll_submitted_data',
			$submitted_data,
			$poll_id,
			$poll_settings_instance
		);
		...
		$is_success = apply_filters(
			'powerform_addon_' . $addon_slug . '_on_poll_submit_result',
			$is_success,
			$poll_id,
			$submitted_data,
			$poll_settings_instance
	...
	public function add_entry_fields( $submitted_data, $current_entry_fields = array() ) {
		 ...
		 * @param array                                        $submitted_data
		 * @param int                                          $poll_id                current Poll ID
		 * @param Powerform_Addon_Poll_Settings_Abstract|null $poll_settings_instance Addon Form Settings instance
		 */
		$poll_entry_fields = apply_filters(
			'powerform_addon_' . $addon_slug . '_poll_entry_fields',
			$poll_entry_fields,
			$submitted_data,
			$poll_id,
			$poll_settings_instance
		...
		$entry_fields = apply_filters(
			'powerform_addon_poll_' . $addon_slug . '_entry_fields',
			$entry_fields,
			$poll_id,
			$submitted_data,
			$poll_settings_instance,
	...
	public function after_entry_saved( Powerform_Form_Entry_Model $entry_model ) {
		 ...
		 */
		do_action(
			'powerform_addon_poll_' . $addon_slug . '_after_entry_saved',
			$poll_id,
			$entry_model,
			$poll_settings_instance
		);
...
//	}
	 ...
	 *
	 * This TITLE_ID will be referenced on @see Powerform_Addon_Poll_Hooks_Abstract::on_export_render_entry_row()
	 *
	 * @example
	 * {
	 *         TITLE_ID_1 => 'TITLE 1',
	...
	public function on_export_render_title_row() {
		...
		$export_headers = apply_filters(
			'powerform_addon_poll_' . $addon_slug . '_export_headers',
			$export_headers,
			$poll_id,
			$poll_settings_instance
		);
	 ...
	 *
	 * Use TITLE_ID from @see Powerform_Addon_Poll_Hooks_Abstract::on_export_render_title_row()
	 *
	 * @example
	 * {
	...
	public function on_export_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		 ...
		 */
		$addon_meta_data = apply_filters(
			'powerform_addon_poll_' . $addon_slug . '_metadata',
			$addon_meta_data,
			$poll_id,
			$entry_model,
			$poll_settings_instance
		...
		$export_columns = apply_filters(
			'powerform_addon_poll_' . $addon_slug . '_export_columns',
			$export_columns,
			$poll_id,
			$entry_model,
			$addon_meta_data,
	...
	public function get_submit_poll_error_message() {
		...
		$error_message = apply_filters(
			'powerform_addon_' . $addon_slug . '_submit_poll_error_message',
			$error_message,
			$poll_id,
			$poll_settings_instance
		);
	...
	public function on_before_delete_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		 ...
		 */
		do_action(
			'powerform_addon_poll_' . $addon_slug . '_on_before_delete_submission',
			$poll_id,
			$entry_model,
			$addon_meta_data,
			$poll_settings_instance
