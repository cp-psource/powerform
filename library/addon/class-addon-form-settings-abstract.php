...
/**
 * Class Powerform_Addon_Form_Settings_Abstract
 ...
 * @since 1.1
 */
abstract class Powerform_Addon_Form_Settings_Abstract {
	...
	/**
	 * Powerform_Addon_Form_Settings_Abstract constructor.
	 *
	 * @since 1.1
	 *
	...
	final public function get_form_settings_meta_key() {
		return 'powerform_addon_' . $this->addon->get_slug() . '_form_settings';
	}
	...
	final public function get_form_settings_values() {
		 ...
		 */
		$values = apply_filters( 'powerform_addon_' . $addon_slug . '_get_form_settings_values', $values, $form_id );
	...
	final public function save_form_settings_values( $values ) {
		 ...
		 * @param int   $form_id current form id
		 */
		$values = apply_filters( 'powerform_addon_' . $addon_slug . '_save_form_settings_values', $values, $form_id );
	...
	final public function save_force_form_disconnect_reason() {
		if ( $this->is_force_form_disconnected ) {
			update_post_meta(
				$this->form_id,
				'powerform_addon_' . $this->addon->get_slug() . '_form_disconnect',
				array(
					'disconnect'         => true,
	...
	final public function remove_saved_force_form_disconnect_reason() {
		delete_post_meta( $this->form_id, 'powerform_addon_' . $this->addon->get_slug() . '_form_disconnect' );
	}
	...
	public function to_exportable_data() {
		 ...
		 * @param int   $form_id
		 */
		$exportable_data = apply_filters( "powerform_addon_{$addon_slug}_form_settings_to_exportable_data", $exportable_data, $form_id );
	...
	public function import_data( $import_data ) {
		...
		$import_data = apply_filters( "powerform_addon_{$addon_slug}_form_settings_import_data", $import_data, $form_id );
		 ...
		 * @param array                                        $exportable_data
		 * @param int                                          $form_id
		 * @param Powerform_Addon_Form_Settings_Abstract|null $form_settings_instance
		 */
		do_action( "powerform_addon_{$addon_slug}_on_import_form_settings_data", $form_id, $import_data );

		try {
			// pre-basic-validation
			if ( empty( $import_data ) ) {
				throw new Powerform_Addon_Exception( 'import_data_empty' );
			}

			if ( ! isset( $import_data['form_settings'] ) ) {
				throw new Powerform_Addon_Exception( 'import_data_no_form_settings' );
			}

			if ( empty( $import_data['form_settings'] ) ) {
				throw new Powerform_Addon_Exception( 'import_data_form_settings_empty' );
			}

			if ( ! isset( $import_data['version'] ) ) {
				throw new Powerform_Addon_Exception( 'import_data_no_version' );
			}
			$this->save_form_settings_values( $import_data['form_settings'] );

		} catch ( Powerform_Addon_Exception $e ) {
			powerform_addon_maybe_log( $e->getMessage() );
			//do nothing
		}
