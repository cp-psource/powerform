...
/**
 * Class Powerform_Addon_Poll_Settings_Abstract
 ...
 * @since 1.6.1
 */
abstract class Powerform_Addon_Poll_Settings_Abstract {
	...
	/**
	 * Powerform_Addon_Poll_Settings_Abstract constructor.
	 *
	 * @since 1.6.1
	 *
	...
	final public function get_poll_settings_meta_key() {
		return 'powerform_addon_' . $this->addon->get_slug() . '_poll_settings';
	}
	...
	final public function get_poll_settings_values() {
		 ...
		 */
		$values = apply_filters( 'powerform_addon_' . $addon_slug . '_get_poll_settings_values', $values, $poll_id );
	...
	final public function save_poll_settings_values( $values ) {
		 ...
		 * @param int   $poll_id current poll id
		 */
		$values = apply_filters( 'powerform_addon_' . $addon_slug . '_save_poll_settings_values', $values, $poll_id );
	...
	final public function save_force_poll_disconnect_reason() {
		if ( $this->is_force_poll_disconnected ) {
			update_post_meta(
				$this->poll_id,
				'powerform_addon_' . $this->addon->get_slug() . '_poll_disconnect',
				array(
					'disconnect'        => true,
	...
	final public function remove_saved_force_poll_disconnect_reason() {
		delete_post_meta( $this->poll_id, 'powerform_addon_' . $this->addon->get_slug() . '_poll_disconnect' );
	}
	...
	public function poll_settings_wizards() {
		...
		$steps = array(
			...
			array(
				...
				/**
				 * Before Powerform call the `callback`, Powerform will attempt to run `is_completed` from the previous step
	...
	public function to_exportable_data() {
		 ...
		 * @param int   $poll_id
		 */
		$exportable_data = apply_filters( "powerform_addon_{$addon_slug}_poll_settings_to_exportable_data", $exportable_data, $poll_id );
	...
	public function import_data( $import_data ) {
		...
		$import_data = apply_filters( "powerform_addon_{$addon_slug}_poll_settings_import_data", $import_data, $poll_id );
		 ...
		 * @param int   $poll_id
		 */
		do_action( "powerform_addon_{$addon_slug}_on_import_poll_settings_data", $poll_id, $import_data );
		...
		try {
			...
			if ( ! isset( $import_data['poll_settings'] ) ) {
				throw new Powerform_Addon_Exception( 'import_data_no_poll_settings' );
			}

			if ( empty( $import_data['poll_settings'] ) ) {
				throw new Powerform_Addon_Exception( 'import_data_poll_settings_empty' );
			}
