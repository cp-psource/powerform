...
/**
 * Class Powerform_Addon_Quiz_Settings_Abstract
 ...
 * @since 1.6.2
 */
abstract class Powerform_Addon_Quiz_Settings_Abstract {
	 ...
	 *
	 * @since 1.6.2
	 * @var Powerform_Quiz_Form_Model|null
	 */
	protected $quiz = null;
	...
	final public function get_quiz_settings_meta_key() {
		return 'powerform_addon_' . $this->addon->get_slug() . '_quiz_settings';
	}
	...
	final public function get_quiz_settings_values() {
		 ...
		 */
		$values = apply_filters( 'powerform_addon_' . $addon_slug . '_get_quiz_settings_values', $values, $quiz_id );
	...
	final public function save_quiz_settings_values( $values ) {
		 ...
		 * @param int   $quiz_id current quiz id
		 */
		$values = apply_filters( 'powerform_addon_' . $addon_slug . '_save_quiz_settings_values', $values, $quiz_id );
	...
	final public function save_force_quiz_disconnect_reason() {
		if ( $this->is_force_quiz_disconnected ) {
			update_post_meta(
				$this->quiz_id,
				'powerform_addon_' . $this->addon->get_slug() . '_quiz_disconnect',
				array(
					'disconnect'        => true,
	...
	final public function remove_saved_force_quiz_disconnect_reason() {
		delete_post_meta( $this->quiz_id, 'powerform_addon_' . $this->addon->get_slug() . '_quiz_disconnect' );
	}
	...
	public function to_exportable_data() {
		 ...
		 * @param int   $quiz_id
		 */
		$exportable_data = apply_filters( "powerform_addon_{$addon_slug}_quiz_settings_to_exportable_data", $exportable_data, $quiz_id );
	...
	public function import_data( $import_data ) {
		...
		$import_data = apply_filters( "powerform_addon_{$addon_slug}_quiz_settings_import_data", $import_data, $quiz_id );
		 ...
		 * @param int   $quiz_id
		 */
		do_action( "powerform_addon_{$addon_slug}_on_import_quiz_settings_data", $quiz_id, $import_data );
		...
		try {
			...
			if ( ! isset( $import_data['quiz_settings'] ) ) {
				throw new Powerform_Addon_Exception( 'import_data_no_quiz_settings' );
			}

			if ( empty( $import_data['quiz_settings'] ) ) {
				throw new Powerform_Addon_Exception( 'import_data_quiz_settings_empty' );
			}
