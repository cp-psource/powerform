 ...
 * Extend this class to create new powerform addon / integrations
 ...
 * - Properly Written Sample Usage on @see Powerform_Addon_Simple
 *
 * @since 1.1
 */
abstract class Powerform_Addon_Abstract implements Powerform_Addon_Interface {

	/**
	 * Slug will be used as identifier throughout powerform
	 ...
	 * And if user updated the addon, it will try to call @see Powerform_Addon_Abstract::version_changed()
	 *
	 * @since 1.1
	 * @var string
	 */
	...
	/**
	 * Minimum version of Powerform, that the addon will work correctly
	 *
	 * @since 1.1
	 * @var string
	 */
	protected $_min_powerform_version;
	 ...
	 *          Default is empty, which will be replaced by powerform default messages
	 *
	 */
	/**
	 ...
	 *
	 * @since  1.1
	 * @var Powerform_Addon_Form_Settings_Abstract[]|array
	 */
	 ...
	 *
	 * @since  1.1
	 * @var Powerform_Addon_Form_Hooks_Abstract[]|array
	 */
	 ...
	 *
	 * @since  1.6.1
	 * @var Powerform_Addon_Poll_Settings_Abstract[]|array
	 */
	 ...
	 *
	 * @since  1.6.1
	 * @var Powerform_Addon_Poll_Hooks_Abstract[]|array
	 */
	 ...
	 *
	 * @since  1.6.1
	 * @var Powerform_Addon_Quiz_Settings_Abstract[]|array
	 */
	 ...
	 *
	 * @since  1.6.1
	 * @var Powerform_Addon_Quiz_Hooks_Abstract[]|array
	 */
	 ...
	 * Get this addon slug
	 *
	 * @see    Powerform_Addon_Abstract::$_slug
	 *
	 * its behave like `IDENTIFIER`, used for :
	 * - easly calling this instance with @see powerform_get_addon(`slug`)
	...
	/**
	 * Get this addon requirement of installed powerform version
	 *
	 * @since  1.1
	 * @return string
	 */
	final public function get_min_powerform_version() {
		return $this->_min_powerform_version;
	}
	...
	final public function get_settings_options_name() {
		...
		$addon                 = $this;
		$settings_options_name = 'powerform_addon_' . $this->get_slug() . '_settings';
		 ...
		 * @param Powerform_Addon_Abstract $addon Addon instance
		 */
		$settings_options_name = apply_filters( 'powerform_addon_' . $addon_slug . '_settings_options_name', $settings_options_name, $addon );
	...
	final public function get_version_options_name() {
		$addon_slug           = $this->get_slug();
		$addon                = $this;
		$version_options_name = 'powerform_addon_' . $this->get_slug() . '_version';
		 ...
		 * @param Powerform_Addon_Abstract $addon Addon instance
		 */
		$version_options_name = apply_filters( 'powerform_addon_' . $addon_slug . '_version_options_name', $version_options_name, $addon );
	...
	final public function to_array() {
		 ...
		 * @param Powerform_Addon_Abstract $addon    Addon Instance
		 */
		$to_array = apply_filters( 'powerform_addon_' . $addon_slug . '_to_array', $to_array, $addon );
	...
	final public function to_array_with_form( $form_id ) {
		 ...
		 * @param Powerform_Addon_Abstract $addon              Addon instance
		 */
		$to_array_with_form = apply_filters( 'powerform_addon_' . $addon_slug . '_to_array_with_form', $to_array_with_form, $form_id, $addon );
	 ...
	 *
	 * @see    powerform_get_pro_addon_list()
	 * @since  1.1
	 * @return bool
	 */
	final public function is_pro() {
		if ( in_array( $this->_slug, array_keys( powerform_get_pro_addon_list() ), true ) ) {
			return true;
		}
	...
	public function check_is_activable() {
		if ( ! file_exists( $this->get_full_path() ) ) {
			powerform_addon_maybe_log( __METHOD__, $this->get_slug(), $this->get_full_path(), 'NOT Exist' );

			return false;
		}

		// Check supported powerform version
		if ( empty( $this->_min_powerform_version ) ) {
			powerform_addon_maybe_log( __METHOD__, $this->get_slug(), 'empty _min_powerform_version' );
	 ...
	 * return false on failure, powerform will stop deactivate process
	 *
	 * @since   1.1
	 * @return bool
	 */
	 ...
	 * return true when succes
	 * return false on failure, powerform will stop activation process
	 *
	 * @since   1.1
	 * @return bool
	 */
	 ...
	 * - `callback` : when application requesting wizard, Powerform will do `call_user_func` on this value, with these arguments
	...
	public function settings_wizards() {
		...
		$steps = array(
			...
			array(
				 ...
				 *
				 * This callback should return an array @see Powerform_Addon_Abstract::sample_setting_first_step()
				 *
				 * @see Powerform_Addon_Abstract::sample_setting_first_step()
				 *
				 */
				'callback'     => array( $this, 'sample_setting_first_step' ),
				/**
				 * Before Powerform call the `calback`, Powerform will attempt to run `is_completed` from the previous step
				 * In this case, `is_completed` will be called when Powerform trying to display Settings Wizard for Second Step / step `1`
				 ...
				 *
				 * @see Powerform_Addon_Abstract::sample_setting_first_step_is_completed()
				 */
	 ...
	 * This function will process @see Powerform_Addon_Abstract::settings_wizards()
	 * Please keep in mind this function will only be called when @see Powerform_Addon_Abstract::is_settings_available() return `true`
	 * Which is doing check on @see Powerform_Addon_Abstract::settings_wizards() requirements is passed
	 *
	 * @since 1.1
	 *
	...
	final public function get_form_settings_wizard( $submitted_data, $form_id, $current_step = 0, $step = 0 ) {
		 ...
		 * @param Powerform_Addon_Abstract                    $addon                  Addon Instance
		 * @param Powerform_Addon_Form_Settings_Abstract|null $form_settings_instance Addon Form settings instancce, or null if unavailable
		 */
		$form_settings_wizard = apply_filters(
			'powerform_addon_' . $addon_slug . '_form_settings_wizard',
	...
	private function get_form_settings_steps( $form_id ) {
		...
		$form_settings_instance = $this->get_addon_form_settings( $form_id );
		if ( ! is_null( $form_settings_instance ) && $form_settings_instance instanceof Powerform_Addon_Form_Settings_Abstract ) {
		 ...
		 *
		 * More detail : @see Powerform_Addon_Form_Settings_Abstract::form_settings_wizards()
		 *
		 * @since 1.1
		 *
		 * @param array                                   $form_settings_steps
		 * @param int                                     $form_id current form id
		 * @param Powerform_Addon_Form_Settings_Abstract $addon   Addon instance
		 * @param Powerform_Addon_Form_Settings_Abstract|null Form settings of addon if available, or null otherwise
		 */
		$form_settings_steps = apply_filters( 'powerform_addon_' . $addon_slug . '_form_settings_steps', $form_settings_steps, $form_id, $addon, $form_settings_instance );
	...
	private function get_form_settings_multi_ids( $form_id ) {
		...
		$multi_ids              = array();
		$form_settings_instance = $this->get_addon_form_settings( $form_id );
		if ( $this->is_allow_multi_on_form() && ! is_null( $form_settings_instance ) && $form_settings_instance instanceof Powerform_Addon_Form_Settings_Abstract ) {
		 ...
		 * @param Powerform_Addon_Form_Settings_Abstract $addon                  Addon Instance
		 * @param Powerform_Addon_Form_Settings_Abstract $form_settings_instance Addon Form Settings Instance
		 */
		$multi_ids = apply_filters( 'powerform_addon_' . $addon_slug . '_form_settings_multi_ids', $multi_ids, $addon, $form_settings_instance );
	...
	private function get_wizard( $steps, $submitted_data, $module_id, $step = 0 ) {
		...
		$wizard['powerform_addon_has_next_step'] = ( ( $step + 1 ) >= $total_steps ? false : true );
		$wizard['powerform_addon_has_prev_step'] = ( $step > 0 ? true : false );
		 ...
		 * @param Powerform_Addon_Abstract $addon          current addon instance
		 ...
		 */
		$wizard = apply_filters( 'powerform_addon_' . $addon_slug . '_wizard', $wizard, $addon, $steps, $submitted_data, $module_id, $step );
	...
	protected function get_empty_wizard( $notice ) {
		 ...
		 */
		$empty_wizard_html = apply_filters( 'powerform_addon_empty_wizard_html', $empty_wizard_html, $notice );
	...
	final public function get_settings_values() {
		 ...
		 * @param mixed $values
		 */
		$values = apply_filters( 'powerform_addon_' . $addon_slug . '_get_settings_values', $values );
	...
	final public function save_settings_values( $values ) {
		 ...
		 * @param mixed $values
		 */
		$values = apply_filters( 'powerform_addon_' . $addon_slug . '_save_settings_values', $values );
	...
	final public function admin_hookable() {
		...
		$default_filters = array(
			'powerform_addon_' . $this->get_slug() . '_save_settings_values' => array( array( $this, 'before_save_settings_values' ), 1 ),
		);

		if ( $this->is_connected() ) {
			$default_filters[ 'powerform_addon_' . $this->get_slug() . '_save_form_settings_values' ] = array( array( $this, 'before_save_form_settings_values' ), 2 );
			$default_filters[ 'powerform_addon_' . $this->get_slug() . '_save_poll_settings_values' ] = array( array( $this, 'before_save_poll_settings_values' ), 2 );
			$default_filters[ 'powerform_addon_' . $this->get_slug() . '_save_quiz_settings_values' ] = array( array( $this, 'before_save_quiz_settings_values' ), 2 );
		}
	...
	final public function global_hookable() {
		...
		$default_filters = array(
			'powerform_addon_' . $this->get_slug() . '_get_settings_values' => array( array( $this, 'before_get_settings_values' ), 1 ),
		);

		if ( $this->is_connected() ) {
			$default_filters[ 'powerform_addon_' . $this->get_slug() . '_get_form_settings_values' ] = array( array( $this, 'before_get_form_settings_values' ), 2 );
			$default_filters[ 'powerform_addon_' . $this->get_slug() . '_get_poll_settings_values' ] = array( array( $this, 'before_get_poll_settings_values' ), 2 );
			$default_filters[ 'powerform_addon_' . $this->get_slug() . '_get_quiz_settings_values' ] = array( array( $this, 'before_get_quiz_settings_values' ), 2 );
		}
	 ...
	 * By Default this function will only check @see Powerform_Addon_Abstract::settings_wizards() as valid multi array
	 *
	 * @since 1.1
	 * @return bool
	 */
	 ...
	 *
	 * @see     Powerform_Addon_Abstract::settings_wizards()
	 * @see     Powerform_Addon_Form_Settings_Abstract::form_settings_wizards()
	...
	final public function is_active() {
		return powerform_addon_is_active( $this->get_slug() );
	}

	/**
	 * Get ClassName of addon Form Settings
	 *
	 * @see   Powerform_Addon_Form_Settings_Abstract
	 *
	 * @since 1.1
	 * @return null|string
	 */
	final public function get_form_settings_class_name() {
		 ...
		 * it will be validated by `class_exists` and must be instanceof @see Powerform_Addon_Form_Settings_Abstract
		 *
		 * @since 1.1
		 *
		 * @param string $form_settings_class_name
		 */
		$form_settings_class_name = apply_filters( 'powerform_addon_' . $addon_slug . '_form_settings_class_name', $form_settings_class_name );
	 ...
	 * @param $form_id
	 *
	 * @return Powerform_Addon_Form_Settings_Abstract | null
	 */
	final public function get_addon_form_settings( $form_id ) {
		$class_name = $this->get_form_settings_class_name();
		if ( ! isset( $this->_addon_form_settings_instances[ $form_id ] ) || ! $this->_addon_form_settings_instances[ $form_id ] instanceof Powerform_Addon_Form_Settings_Abstract ) {
			if ( empty( $class_name ) ) {
				return null;
			}
			...
			try {
				...
				if ( ! $form_settings_instance instanceof Powerform_Addon_Form_Settings_Abstract ) {
					throw new Powerform_Addon_Exception( $class_name . ' is not instanceof Powerform_Addon_Form_Settings_Abstract' );
				}
				$this->_addon_form_settings_instances[ $form_id ] = $form_settings_instance;
				powerform_maybe_attach_addon_hook( $this );
			} catch ( Exception $e ) {
				powerform_addon_maybe_log( $this->get_slug(), 'Failed to instantiate its _addon_form_settings_instance', $e->getMessage(), $e->getTrace() );
	 ...
	 * @param $form_id
	 *
	 * @return Powerform_Addon_Form_Hooks_Abstract|null
	 */
	final public function get_addon_form_hooks( $form_id ) {
		if ( ! isset( $this->_addon_form_hooks_instances[ $form_id ] ) || ! $this->_addon_form_hooks_instances[ $form_id ] instanceof Powerform_Addon_Form_Hooks_Abstract ) {
			...
			try {
				...
				powerform_addon_maybe_log( $this->get_slug(), 'Failed to instantiate its _addon_form_hooks_instance', $e->getMessage() );
	 ...
	 * buttons [
	 *      submit [
	 *          action: powerform_load_mailchimp_settings
	 *          data: {
	 *              step: 2
	 *          },
	 *          markup: '<a></a>'
	 *      ],
	 *      disconnect [
	 *          action: powerform_disconnect_mailchimp,
	 *          data: [],
	 *          markup: '<a></a>'
	 *      ]
	 * }
	...
	public static function get_button_markup( $label, $classes = '', $tooltip = '' ) {
		 ...
		 *
		 * Its possible @see Powerform_Addon_Abstract::get_button_markup() overridden.
	...
	public static function get_template( $template, $params ) {
		 ...
		 */
		$html = apply_filters( 'powerform_addon_get_template', $html, $template, $params );
	 ...
	 *
	 * @see   wp-admin/admin.php?page=powerform-integrations
	 *
	 * @since 1.2
	 *
	 * @return array
	...
	public function get_integration_section_callback( $section = null ) {
		 ...
		 */
		$callback = apply_filters( 'powerform_addon_' . $addon_slug . '_integration_section_callback', $callback, $section, $integration_sections );
	...
	private function generate_form_settings_multi_id( $form_id ) {
		 ...
		 * @param Powerform_Addon_Form_Settings_Abstract $form_settings_instance Addon Form Settings Instance
		 */
		$multi_ids = apply_filters( 'powerform_addon_' . $addon_slug . '_form_settings_multi_id', $multi_id, $addon, $form_settings_instance );
	 ...
	 *
	 * @see   Powerform_Addon_Poll_Settings_Abstract
	 *
	 * @since 1.1
	 * @return null|string
	 */
	final public function get_poll_settings_class_name() {
		 ...
		 * it will be validated by `class_exists` and must be instanceof @see Powerform_Addon_Poll_Settings_Abstract
		 *
		 * @since 1.1
		 *
		 * @param string $poll_settings_class_name
		 */
		$poll_settings_class_name = apply_filters( 'powerform_addon_' . $addon_slug . '_poll_settings_class_name', $poll_settings_class_name );
	 ...
	 * @param $poll_id
	 *
	 * @return Powerform_Addon_Poll_Settings_Abstract|null
	 */
	final public function get_addon_poll_settings( $poll_id ) {
		...
		if ( ! isset( $this->_addon_poll_settings_instances[ $poll_id ] )
			...
			try {
				...
				if ( ! $poll_settings_instance instanceof Powerform_Addon_Poll_Settings_Abstract ) {
					throw new Powerform_Addon_Exception( $class_name . ' is not instanceof Powerform_Addon_Poll_Settings_Abstract' );
				}
				$this->_addon_poll_settings_instances[ $poll_id ] = $poll_settings_instance;
				powerform_maybe_attach_addon_hook( $this );
			} catch ( Exception $e ) {
				powerform_addon_maybe_log( $this->get_slug(), 'Failed to instantiate its _addon_poll_settings_instances', $e->getMessage(), $e->getTrace() );
	...
	final public function get_poll_settings_wizard( $submitted_data, $poll_id, $current_step = 0, $step = 0 ) {
		 ...
		 * @param int                                          $step                   Step requested by client, start from 0
		 * @param Powerform_Addon_Abstract                    $addon                  Addon Instance
		 * @param Powerform_Addon_Poll_Settings_Abstract|null $poll_settings_instance Addon Form settings instance, or null if unavailable
		 */
		$poll_settings_wizard = apply_filters(
			'powerform_addon_' . $addon_slug . '_poll_settings_wizard',
	...
	private function get_poll_settings_steps( $poll_id ) {
		...
		$poll_settings_instance = $this->get_addon_poll_settings( $poll_id );
		if ( ! is_null( $poll_settings_instance ) && $poll_settings_instance instanceof Powerform_Addon_Poll_Settings_Abstract ) {
		 ...
		 * Filter form settings step that will be used for building wizard
		 *
		 * More detail : @see Powerform_Addon_Poll_Settings_Abstract::poll_settings_wizards()
		 *
		 * @since 1.6.1
		 *
		 ...
		 * @param Powerform_Addon_Abstract                    $addon                  Addon instance
		 * @param Powerform_Addon_Poll_Settings_Abstract|null $poll_settings_instance Form settings of addon if available, or null otherwise
		 */
		$poll_settings_steps = apply_filters( 'powerform_addon_' . $addon_slug . '_poll_settings_steps', $poll_settings_steps, $poll_id, $addon, $poll_settings_instance );
	...
	private function get_poll_settings_multi_ids( $poll_id ) {
		...
		$multi_ids              = array();
		$poll_settings_instance = $this->get_addon_poll_settings( $poll_id );
		if ( $this->is_allow_multi_on_poll() && ! is_null( $poll_settings_instance ) && $poll_settings_instance instanceof Powerform_Addon_Poll_Settings_Abstract ) {
		 ...
		 * @param array                                   $multi_ids
		 * @param Powerform_Addon_Abstract               $addon                  Addon Instance
		 * @param Powerform_Addon_Poll_Settings_Abstract $poll_settings_instance Addon Form Settings Instance
		 */
		$multi_ids = apply_filters( 'powerform_addon_' . $addon_slug . '_poll_settings_multi_ids', $multi_ids, $addon, $poll_settings_instance );
	...
	final public function to_array_with_poll( $poll_id ) {
		 ...
		 * @param Powerform_Addon_Abstract $addon              Addon instance
		 */
		$to_array_with_poll = apply_filters( 'powerform_addon_' . $addon_slug . '_to_array_with_poll', $to_array_with_poll, $poll_id, $addon );
	 ...
	 *
	 * @see     Powerform_Addon_Abstract::settings_wizards()
	 * @see     Powerform_Addon_Poll_Settings_Abstract::poll_settings_wizards()
	 * as valid multi array
	 *
	 * @since   1.6.1
	 *
	...
	private function generate_poll_settings_multi_id( $poll_id ) {
		 ...
		 * @param string                                  $multi_id
		 * @param Powerform_Addon_Abstract               $addon                  Addon Instance
		 * @param Powerform_Addon_Poll_Settings_Abstract $poll_settings_instance Addon Poll Settings Instance
		 */
		$multi_ids = apply_filters( 'powerform_addon_' . $addon_slug . '_poll_settings_multi_id', $multi_id, $addon, $poll_settings_instance );
	 ...
	 * @param $poll_id
	 *
	 * @return Powerform_Addon_Poll_Hooks_Abstract|null
	 */
	final public function get_addon_poll_hooks( $poll_id ) {
		if ( ! isset( $this->_addon_poll_hooks_instances[ $poll_id ] ) || ! $this->_addon_poll_hooks_instances[ $poll_id ] instanceof Powerform_Addon_Poll_Hooks_Abstract ) {
			...
			try {
				...
				powerform_addon_maybe_log( $this->get_slug(), 'Failed to instantiate its _addon_poll_hooks_instances', $e->getMessage() );
	 ...
	 *
	 * @see   Powerform_Addon_Quiz_Settings_Abstract
	 *
	 * @since 1.6.2
	 * @return null|string
	 */
	final public function get_quiz_settings_class_name() {
		 ...
		 * it will be validated by `class_exists` and must be instanceof @see Powerform_Addon_Quiz_Settings_Abstract
		 *
		 * @since 1.1
		 *
		 * @param string $quiz_settings_class_name
		 */
		$quiz_settings_class_name = apply_filters( 'powerform_addon_' . $addon_slug . '_quiz_settings_class_name', $quiz_settings_class_name );
	 ...
	 * @param $quiz_id
	 *
	 * @return Powerform_Addon_Quiz_Settings_Abstract|null
	 */
	final public function get_addon_quiz_settings( $quiz_id ) {
		...
		if ( ! isset( $this->_addon_quiz_settings_instances[ $quiz_id ] )
			...
			try {
				...
				if ( ! $quiz_settings_instance instanceof Powerform_Addon_Quiz_Settings_Abstract ) {
					throw new Powerform_Addon_Exception( $class_name . ' is not instanceof Powerform_Addon_Quiz_Settings_Abstract' );
				}
				$this->_addon_quiz_settings_instances[ $quiz_id ] = $quiz_settings_instance;
				powerform_maybe_attach_addon_hook( $this );
			} catch ( Exception $e ) {
				powerform_addon_maybe_log( $this->get_slug(), 'Failed to instantiate its _addon_quiz_settings_instances', $e->getMessage(), $e->getTrace() );
	...
	final public function get_quiz_settings_wizard( $submitted_data, $quiz_id, $current_step = 0, $step = 0 ) {
		 ...
		 * @param int                                          $step                   Step requested by client, start from 0
		 * @param Powerform_Addon_Abstract                    $addon                  Addon Instance
		 * @param Powerform_Addon_Quiz_Settings_Abstract|null $quiz_settings_instance Addon Form settings instance, or null if unavailable
		 */
		$quiz_settings_wizard = apply_filters(
			'powerform_addon_' . $addon_slug . '_quiz_settings_wizard',
	...
	private function get_quiz_settings_steps( $quiz_id ) {
		...
		$quiz_settings_instance = $this->get_addon_quiz_settings( $quiz_id );
		if ( ! is_null( $quiz_settings_instance ) && $quiz_settings_instance instanceof Powerform_Addon_Quiz_Settings_Abstract ) {
		 ...
		 * Filter form settings step that will be used for building wizard
		 *
		 * More detail : @see Powerform_Addon_Quiz_Settings_Abstract::quiz_settings_wizards()
		 *
		 * @since 1.6.2
		 *
		 ...
		 * @param Powerform_Addon_Abstract                    $addon                  Addon instance
		 * @param Powerform_Addon_Quiz_Settings_Abstract|null $quiz_settings_instance Quiz settings of addon if available, or null otherwise
		 */
		$quiz_settings_steps = apply_filters( 'powerform_addon_' . $addon_slug . '_quiz_settings_steps', $quiz_settings_steps, $quiz_id, $addon, $quiz_settings_instance );
	...
	private function get_quiz_settings_multi_ids( $quiz_id ) {
		...
		$multi_ids              = array();
		$quiz_settings_instance = $this->get_addon_quiz_settings( $quiz_id );
		if ( $this->is_allow_multi_on_quiz() && ! is_null( $quiz_settings_instance ) && $quiz_settings_instance instanceof Powerform_Addon_Quiz_Settings_Abstract ) {
		 ...
		 * @param array                                   $multi_ids
		 * @param Powerform_Addon_Abstract               $addon                  Addon Instance
		 * @param Powerform_Addon_Quiz_Settings_Abstract $quiz_settings_instance Addon Quiz Settings Instance
		 */
		$multi_ids = apply_filters( 'powerform_addon_' . $addon_slug . '_quiz_settings_multi_ids', $multi_ids, $addon, $quiz_settings_instance );
	...
	final public function to_array_with_quiz( $quiz_id ) {
		 ...
		 * @param Powerform_Addon_Abstract $addon              Addon instance
		 */
		$to_array_with_quiz = apply_filters( 'powerform_addon_' . $addon_slug . '_to_array_with_quiz', $to_array_with_quiz, $quiz_id, $addon );
	 ...
	 *
	 * @see     Powerform_Addon_Abstract::settings_wizards()
	 * @see     Powerform_Addon_Quiz_Settings_Abstract::quiz_settings_wizards()
	 * as valid multi array
	 *
	 * @since   1.6.2
	 *
	...
	private function generate_quiz_settings_multi_id( $quiz_id ) {
		 ...
		 * @param Powerform_Addon_Quiz_Settings_Abstract $quiz_settings_instance Addon Quiz Settings Instance
		 */
		$multi_ids = apply_filters( 'powerform_addon_' . $addon_slug . '_quiz_settings_multi_id', $multi_id, $addon, $quiz_settings_instance );
	 ...
	 * @param $quiz_id
	 *
	 * @return Powerform_Addon_Quiz_Hooks_Abstract|null
	 */
	final public function get_addon_quiz_hooks( $quiz_id ) {
		if ( ! isset( $this->_addon_quiz_hooks_instances[ $quiz_id ] ) || ! $this->_addon_quiz_hooks_instances[ $quiz_id ] instanceof Powerform_Addon_Quiz_Hooks_Abstract ) {
			...
			try {
				...
				powerform_addon_maybe_log( $this->get_slug(), 'Failed to instantiate its _addon_quiz_hooks_instances', $e->getMessage() );
