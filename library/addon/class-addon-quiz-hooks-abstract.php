...
/**
 * Class Powerform_Addon_Quiz_Hooks_Abstract
 ...
 * @since 1.6.2
 */
abstract class Powerform_Addon_Quiz_Hooks_Abstract {
	...
	public function on_before_render_quiz_questions() {
		...
		/**
		 * Fires before quiz questions rendered by powerform
		 *
		 ...
		 */
		do_action(
			'powerform_addon_' . $addon_slug . '_on_before_render_quiz_questions',
			$quiz_id,
	...
	public function on_after_render_quiz_questions() {
		 ...
		 */
		do_action(
			'powerform_addon_' . $addon_slug . '_on_after_render_quiz_questions',
			$quiz_id,
			$quiz_settings_instance
		);
	}
	...
	public function on_after_render_quiz() {
		 ...
		 */
		do_action(
			'powerform_addon_' . $addon_slug . '_on_after_render_quiz',
			$quiz_id,
			$quiz_settings_instance
		);
	}
	 ...
	 * and display error message to user @see Powerform_Addon_Quiz_Hooks_Abstract::get_submit_quiz_error_message()
	 *
	 * @since 1.6.2
	 *
	...
	public function on_quiz_submit( $submitted_data ) {
		 ...
		 * @param int                                          $quiz_id                current Quiz ID
		 * @param Powerform_Addon_Quiz_Settings_Abstract|null $quiz_settings_instance Addon Quiz Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_' . $addon_slug . '_quiz_submitted_data',
			$submitted_data,
			$quiz_id,
			$quiz_settings_instance
		);
		...
		$is_success = apply_filters(
			'powerform_addon_' . $addon_slug . '_on_quiz_submit_result',
			$is_success,
			$quiz_id,
			$submitted_data,
			$quiz_settings_instance
	...
	public function add_entry_fields( $submitted_data, $current_entry_fields = array() ) {
		...
		$quiz_entry_fields = apply_filters(
			'powerform_addon_' . $addon_slug . '_quiz_entry_fields',
			$quiz_entry_fields,
			$submitted_data,
			$quiz_id,
			$quiz_settings_instance
		...
		$entry_fields = apply_filters(
			'powerform_addon_quiz_' . $addon_slug . '_entry_fields',
			$entry_fields,
			$quiz_id,
			$submitted_data,
			$quiz_settings_instance,
	...
	public function after_entry_saved( Powerform_Form_Entry_Model $entry_model ) {
		 ...
		 */
		do_action(
			'powerform_addon_quiz_' . $addon_slug . '_after_entry_saved',
			$quiz_id,
			$entry_model,
			$quiz_settings_instance
		);
	...
	public function on_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$addon_meta_data = apply_filters(
			'powerform_addon_quiz_' . $addon_slug . '_metadata',
			$addon_meta_data,
			$quiz_id,
			$entry_model,
			$quiz_settings_instance
		...
		$entry_items = apply_filters(
			'powerform_addon_quiz_' . $addon_slug . '_entry_items',
			$entry_items,
			$quiz_id,
			$entry_model,
			$addon_meta_data,
	 ...
	 *
	 * This TITLE_ID will be referenced on @see Powerform_Addon_Quiz_Hooks_Abstract::on_export_render_entry_row()
	 *
	 * @example
	 * {
	 *         TITLE_ID_1 => 'TITLE 1',
	...
	public function on_export_render_title_row() {
		...
		$export_headers = apply_filters(
			'powerform_addon_quiz_' . $addon_slug . '_export_headers',
			$export_headers,
			$quiz_id,
			$quiz_settings_instance
		);
	 ...
	 *
	 * Use TITLE_ID from @see Powerform_Addon_Quiz_Hooks_Abstract::on_export_render_title_row()
	 *
	 * @example
	 * {
	...
	public function on_export_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$export_columns = apply_filters(
			'powerform_addon_quiz_' . $addon_slug . '_export_columns',
			$export_columns,
			$quiz_id,
			$entry_model,
			$addon_meta_data,
	...
	public function get_submit_quiz_error_message() {
		...
		$error_message = apply_filters(
			'powerform_addon_' . $addon_slug . '_submit_quiz_error_message',
			$error_message,
			$quiz_id,
			$quiz_settings_instance
		);
	...
	public function on_before_delete_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		 ...
		 */
		do_action(
			'powerform_addon_quiz_' . $addon_slug . '_on_before_delete_submission',
			$quiz_id,
			$entry_model,
			$addon_meta_data,
			$quiz_settings_instance
