...
/**
 * Class Powerform_Stripe
 *
 * @since 1.7
 */
class Powerform_Stripe extends Powerform_Field {
	...
	/**
	 * Powerform_Stripe constructor.
	 *
	 */
	public function __construct() {
		parent::__construct();

		$this->name = __( 'Stripe', Powerform::DOMAIN );
	...
	public function defaults() {
		...
		return array(
			'field_label'         => __( 'Credit / Debit Card', Powerform::DOMAIN ),
			'mode'                => 'test',
	...
	public function markup( $field, $settings = array() ) {
		...
		$customer_email = powerform_clear_field_id( $customer_email );
		$custom_fonts   = false;
		...
		if ( 'material' === $settings['form-style'] ) {
			$classes = 'powerform-input--wrap powerform-input--stripe';

			if ( empty( $label ) ) {
				$classes .= ' powerform--no_label';
			}

			$html .= '<div class="' . $classes . '">';
		}

		$html .= sprintf( '<div id="card-element-%s" %s" class="powerform-stripe-element"></div>', $uniqid, $attributes );

		$html .= sprintf( '<input type="hidden" name="paymentid" value="%s" id="powerform-stripe-paymentid"/>', "" );

		if ( 'material' === $settings['form-style'] ) {
			$html .= '</div>';
		}

		$html .= '<span class="powerform-card-message"><span class="powerform-error-message" aria-hidden="true"></span></span>';

		$html .= self::get_description( $description );

		$html .= '</div>';

		return apply_filters( 'powerform_field_stripe_markup', $html, $attr, $field );
	}
	...
	public function generate_paymentIntent( $amount, $field, $submitted_data ) {
		...
		\Powerform\Stripe\Stripe::setApiKey( $key );

		Powerform_Gateway_Stripe::set_stripe_app_info();
		...
		$options = apply_filters( 'powerform_field_stripe_intent_options', $options, $submitted_data );

		try {
			// Create Payment Intent object
			$intent = \Powerform\Stripe\PaymentIntent::create( $options );
		} catch ( Exception $e ) {
			$response = array(
	...
	public function update_paymentIntent( $id, $amount, $submitted_data, $field, $pseudo_submitted_data ) {
		...
		if( !empty($product_description ) ) {
			$product_description = powerform_replace_form_data( $product_description, $submitted_data );
		...
		try {
			// Retrieve PI object
			$intent = \Powerform\Stripe\PaymentIntent::retrieve( $id );
		} catch ( Exception $e ) {
			$payment_intent = $this->generate_paymentIntent( $amount, $field, $submitted_data );

			$intent = \Powerform\Stripe\PaymentIntent::retrieve( $payment_intent->$id );
		}
		...
		if ( ! empty( $stored_metadata ) ) {
			foreach( (array) $stored_metadata as $key => $meta ) {
				$metadata[ $key ] = powerform_replace_form_data( '{' . $meta . '}', $submitted_data_combined );
			}
		}
		...
		if ( empty( $id ) || "" === $id ) {
			$response = array(
				'message' => __( 'Your Payment ID is empty, please reload the page and try again!', Powerform::DOMAIN ),
				'errors' => array()
			);
		...
		if ( "succeeded" === $intent->status ) {
			...
			try {
				...
				if ( 0 > $amount ) {
					throw new Exception( __( 'Payment amount should be larger than 0.', Powerform::DOMAIN ) );
				}

				// Check payment ID
				if ( empty( $id ) ) {
					throw new Exception( __( 'Your Payment ID is empty!', Powerform::DOMAIN ) );
				}
				...
				\Powerform\Stripe\PaymentIntent::update(
					$id,
					$options
				);
	...
	public function sanitize( $field, $data ) {
		...
		return apply_filters( 'powerform_field_stripe_sanitize', $data, $field, $original_data );
	}
	...
	public function process_to_entry_data( $field, $custom_form, $submitted_data, $pseudo_submitted_data, $field_data_array ) {
		...
		try {
			...
			if ( ! isset( $submitted_data['paymentid'] ) ) {
				throw new Exception( __('Stripe Payment ID does not exist.', Powerform::DOMAIN ) );
			}

			// Check payment amount
			$intent = \Powerform\Stripe\PaymentIntent::retrieve( $submitted_data['paymentid'] );

			// Makue sure Payment Intent succeeded
			if ( "succeeded" !== $intent->status ) {
				throw new Exception( __('Stripe Token not found on submitted data.', Powerform::DOMAIN ) );
			}
		 ...
		 * @return array
		 */
		$entry_data = apply_filters( 'powerform_field_stripe_process_to_entry_data', $entry_data, $field, $custom_form, $submitted_data, $field_data_array );
	...
	public static function linkify_transaction_id( $transaction_id, $meta_value ) {
		 ...
		 * @return string
		 */
		$transaction_link = apply_filters( 'powerform_field_stripe_linkify_transaction_id', $transaction_link, $transaction_id, $meta_value );
	...
	public function get_payment_amount( $field, $custom_form, $submitted_data, $pseudo_submitted_data ) {
		 ...
		 * @param array                        $pseudo_submitted_data
		 */
		$payment_amount = apply_filters( 'powerform_field_stripe_payment_amount', $payment_amount, $field, $custom_form, $submitted_data, $pseudo_submitted_data );
