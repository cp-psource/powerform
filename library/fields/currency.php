...
/**
 * Class Powerform_Currency
 *
 * @since 1.7
 */
class Powerform_Currency extends Powerform_Field {
	...
	/**
	 * Powerform_Currency constructor.
	 *
	 * @since 1.7
	 */
	public function __construct() {
		...
		$this->name = __( 'Währung', Powerform::DOMAIN );
	}
	...
	public function defaults() {
		...
		return apply_filters(
			'powerform_currency_defaults_settings',
			array(
				'calculations' => 'true',
				'limit_min'    => 1,
				'limit_max'    => 150,
				'currency'     => 'EUR',
				'field_label'  => __( 'Währung', Powerform::DOMAIN ),
	...
	public function markup( $field, $settings = array() ) {
		...
		$number_attr = array(
			...
			'id'            => $id,
			'class'         => 'powerform-input powerform-currency',
		...
		$html .= '<div class="powerform-field">';
			...
			$html .= self::create_input(
				...
				$design,
				array(
					'<div class="powerform-input-with-suffix">',
					sprintf( '<span class="powerform-suffix">%s</span></div>', $currency ),
					'',
				)
			);
		...
		return apply_filters( 'powerform_field_currency_markup', $html, $id, $required, $placeholder, $value );
	}
	...
	public function get_validation_rules() {
		...
		return apply_filters( 'powerform_field_currency_validation_rules', $rules, $id, $field );
	}
	...
	public function get_validation_messages() {
		...
		if ( $this->is_required( $field ) ) {
			...
			$required_validation_message = apply_filters(
				'powerform_field_currency_required_validation_message',
		...
		$number_validation_message = apply_filters(
			'powerform_field_currency_number_validation_message',
		...
		if ( $min ) {
			...
			$min_validation_message = apply_filters(
				'powerform_field_currency_min_validation_message',
				$custom_message && $min_validation_message ? $min_validation_message : __( 'Bitte gib einen Wert größer oder gleich {0} ein.', Powerform::DOMAIN ),
				$field
			);
		...
		if ( $max ) {
			...
			$max_validation_message = apply_filters(
				'powerform_field_currency_max_validation_message',
				$custom_message && $max_validation_message ? $max_validation_message : __( 'Bitte gib einen Wert kleiner oder gleich {0} ein.', Powerform::DOMAIN ),
				$field
			);
		...
		return apply_filters( 'powerform_field_currency_validation_message', $messages, $field );
	}
	...
	public function validate( $field, $data, $post_data = array() ) {
		...
		if ( $this->is_required( $field ) ) {
			...
			if ( empty( $data ) && '0' !== $data ) {
				...
				$this->validation_message[ $id ] = apply_filters(
					'powerform_field_currency_required_field_validation_message',
					$required_validation_message,
					$id,
					$field,
					$data,
		...
		if ( ! is_numeric( $data ) && ! empty( $data ) ) {
			$this->validation_message[ $id ] = apply_filters(
				'powerform_field_currency_numeric_validation_message',
			...
			if ( ! empty( $data ) ) {
				...
				if ( $min_len !== 0 && $data < $min ) {
					$min_validation_message          = self::get_property( 'limit_min_message', $field );
					$min_validation_message          = $custom_message && $min_validation_message ? $min_validation_message : __( 'Die Zahl sollte kleiner als %1$d und größer als %2$d sein.', Powerform::DOMAIN );
					$this->validation_message[ $id ] = sprintf(
						apply_filters(
							'powerform_field_currency_max_min_validation_message',
							/* translators: ... */
					...
					$max_validation_message          = self::get_property( 'limit_max_message', $field );
					$max_validation_message          = $custom_message && $max_validation_message ? $max_validation_message : __( 'Die Zahl sollte kleiner als %1$d und größer als %2$d sein.', Powerform::DOMAIN );
					$this->validation_message[ $id ] = sprintf(
						apply_filters(
							'powerform_field_currency_max_min_validation_message',
							/* translators: ... */
	...
	public function sanitize( $field, $data ) {
		...
		return apply_filters( 'powerform_field_currency_sanitize', $data, $field, $original_data );
	}
	...
	public function get_calculable_value( $submitted_data, $field_settings ) {
		 ...
		 */
		$calculable_value = apply_filters( 'powerform_field_currency_calculable_value', $calculable_value, $submitted_data, $field_settings );
