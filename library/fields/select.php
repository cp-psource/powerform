...
class Powerform_Select extends Powerform_Field {
	...
	public function __construct() {
		...
		$this->name = __( 'Select', Powerform::DOMAIN );
	}
	...
	public function defaults() {
		return array(
			'value_type'  => 'single',
			'field_label' => __( 'Select', Powerform::DOMAIN ),
			'options'     => array(
				array(
		...
		if ( 'multiselect' === $field_type ) {
			...
			$default          = '';
				...
				if ( isset( $is_limit ) && 'enable' === $is_limit && ! empty( $limit ) ) {
					$entries = Powerform_Form_Entry_Model::select_count_entries_by_meta_field(
						$settings['form_id'],
						$field_name,
						$value,
						$field_type
				...
				$extra_class = $selected ? ' powerform-is_checked' : '';

				$class = 'powerform-option' . $extra_class;
			...
			$options_markup = '';
			$default        = '';
			$select_class   = 'powerform-select';

			if ( 'enable' === $search_status ) {
				$select_class = 'powerform-select2';
			}
			...
			$html .= sprintf( '<select class="powerform-select--field %s" id="%s" data-required="%s" name="%s" data-default-value="%s" data-placeholder="%s">', $select_class, $id, $required, $name, $default, esc_html( $placeholder ) );
	...
	public function get_validation_messages() {
		...
		if ( $is_required ) {
			$required_message = self::get_property( 'required_message', $field, __( 'This field is required. Please select a value.', Powerform::DOMAIN ) );
	...
	public function get_calculable_value( $submitted_data, $field_settings ) {
		 ...
		 */
		$calculable_value = apply_filters( 'powerform_field_select_calculable_value', $calculable_value, $submitted_data, $field_settings );
