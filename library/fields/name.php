...
/**
 * Class Powerform_Name
 *
 * @since 1.0
 */
class Powerform_Name extends Powerform_Field {
	...
	/**
	 * Powerform_Name constructor.
	 *
	 * @since 1.0
	 */
	public function __construct() {
		parent::__construct();
		$this->name = __( 'Name', Powerform::DOMAIN );
	}
	...
	public function autofill_settings( $settings = array() ) {
		...
		//single name
		$name_providers = apply_filters( 'powerform_field_' . $this->slug . '_autofill', array(), $this->slug );

		//multi name
		$prefix_providers = apply_filters( 'powerform_field_' . $this->slug . '_prefix_autofill', array(), $this->slug . '_prefix' );
		$fname_providers  = apply_filters( 'powerform_field_' . $this->slug . '_first_name_autofill', array(), $this->slug . '_first_name' );
		$mname_providers  = apply_filters( 'powerform_field_' . $this->slug . '_middle_name_autofill', array(), $this->slug . '_middle_name' );
		$lname_providers  = apply_filters( 'powerform_field_' . $this->slug . '_last_name_autofill', array(), $this->slug . '_last_name' );

		$autofill_settings = array(
			'name'             => array(
				'values' => powerform_build_autofill_providers( $name_providers ),
			),
			'name-prefix'      => array(
				'values' => powerform_build_autofill_providers( $prefix_providers ),
			),
			'name-first-name'  => array(
				'values' => powerform_build_autofill_providers( $fname_providers ),
			),
			'name-middle-name' => array(
				'values' => powerform_build_autofill_providers( $mname_providers ),
			),
			'name-last-name'   => array(
				'values' => powerform_build_autofill_providers( $lname_providers ),
			),
		);
	...
	public function markup( $field, $settings = array() ) {
		...
		return apply_filters( 'powerform_field_name_markup', $html, $field );
	}
