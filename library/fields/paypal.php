...
/**
 * Class Powerform_PayPal
 *
 * @since 1.7
 */
class Powerform_PayPal extends Powerform_Field {
	...
	/**
	 * Powerform_PayPal constructor.
	 *
	 */
	public function __construct() {
		parent::__construct();

		$this->name = __( 'PayPal', Powerform::DOMAIN );
	}
	...
	public function markup( $field, $settings = array() ) {
		...
		$attr = array(
			'type'              => 'hidden',
			'name'              => $element_name,
			'id'                => 'powerform-' . $field_id . '-field',
			'class'             => 'powerform-paypal-input',
		...
		return apply_filters( 'powerform_field_paypal_markup', $html, $attr, $field );
	}
	...
	public function sanitize( $field, $data ) {
		...
		return apply_filters( 'powerform_field_paypal_sanitize', $data, $field, $original_data );
	}
	...
	public function process_to_entry_data( $field, $custom_form, $submitted_data, $pseudo_submitted_data, $field_data_array ) {
		...
		if ( ! isset( $order->intent ) || $order->intent !== "CAPTURE" ) {
			return array(
				'error' => __( 'Invalid payment intent', Powerform::DOMAIN )
			);
		}
		...
		if ( ! isset( $order->status ) || $order->status !== "APPROVED" ) {
			return array(
				'error' => __( 'Invalid order status', Powerform::DOMAIN )
			);
		}
		...
		if ( ! isset( $order->purchase_units[0]->amount->value ) || floatval( $order->purchase_units[0]->amount->value ) !== floatval( $charge_amount ) ) {
			return array(
				'error' => __( 'Invalid payment amount', Powerform::DOMAIN )
			);
		}
		...
		if ( ! isset( $order->purchase_units[0]->amount->currency_code ) || $order->purchase_units[0]->amount->currency_code !== $currency ) {
			return array(
				'error' => __( 'Invalid currency code', Powerform::DOMAIN )
			);
		}
		 ...
		 * @return array
		 */
		$entry_data = apply_filters( 'powerform_field_paypal_process_to_entry_data', $entry_data, $field, $custom_form, $submitted_data, $field_data_array );
	...
	public static function linkify_transaction_id( $transaction_id, $meta_value ) {
		 ...
		 * @return string
		 */
		$transaction_link = apply_filters( 'powerform_field_paypal_linkify_transaction_id', $transaction_link, $transaction_id, $meta_value );
	...
	public function get_payment_amount( $field, $custom_form, $submitted_data, $pseudo_submitted_data ) {
		 ...
		 * @param array                        $pseudo_submitted_data
		 */
		$payment_amount = apply_filters( 'powerform_field_paypal_payment_amount', $payment_amount, $field, $custom_form, $submitted_data, $pseudo_submitted_data );
