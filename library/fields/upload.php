...
class Powerform_Upload extends Powerform_Field {
	...
	public function __construct() {
		...
		$this->name = __( 'File Upload', Powerform::DOMAIN );
	}
	...
	public function defaults() {
		...
		return array(
			'field_label'  => __( 'Upload file', Powerform::DOMAIN ),
			'filetypes'    => $file_type,
			'file-type'    => 'single',
	...
	public function markup( $field, $settings = array() ) {
		...
		if ( 'multiple' === $file_type ) {
			...
			if ( 'custom' === $file_limit_type ) {
				...
				$upload_attr['data-limit-message'] = sprintf( __( 'You can upload a maximum of %d files.', Powerform::DOMAIN ), $file_limit );
			}
			...
			$upload_attr['data-size-message'] = sprintf( __( 'Maximum file size allowed is %1$s. ', Powerform::DOMAIN ), $rounded_max_size );
			if ( $custom_file_type ) {
				$upload_attr['data-filetype']         = implode( '|', array_values( $mime_types ) );
				$upload_attr['data-filetype-message'] = __( 'file extension is not allowed.', Powerform::DOMAIN );
			}
		...
		if ( 'multiple' === $file_type ) {
			$html .= sprintf( '<ul class="powerform-uploaded-files upload-container-%s"></ul>', $id );
		}
		...
		if ( $this->is_required( $field ) ) {
			...
			if ( empty( $data ) ) {
				$this->validation_message[ $id ] = apply_filters(
					...
					( ! empty( $required_message ) ? $required_message : __( 'This field is required. Please upload a file.', Powerform::DOMAIN ) ),
					$id,
					$field
				);
			}
	...
	public function get_validation_rules() {
		...
		return apply_filters( 'powerform_field_file_validation_rules', $rules, $id, $field );
	}
	...
	public function get_validation_messages() {
		...
		if ( $is_required ) {
			...
			$required_message          = apply_filters(
				'powerform_upload_field_required_validation_message',
				( ! empty( $settings_required_message ) ? $settings_required_message : __( 'This field is required. Please upload a file.', Powerform::DOMAIN ) ),
				$id,
				$field
			);
			$messages                  = $messages . '"required": "' . powerform_addcslashes( $required_message ) . '",' . "\n";
		}
		$extension_message = __( 'Error saving form. Uploaded file extension is not allowed.', Powerform::DOMAIN );
			...
			if ( isset( $file_object['name'] ) && ! empty( $file_object['name'] ) ) {
				...
				if ( false === $valid['ext'] ) {
					if ( 'multiple' === $file_type ) {
						return array(
							'success' => false,
							'message' => sprintf( __( '.%1$s file extension is not allowed.', Powerform::DOMAIN ), $ext ),
						);
					} else {
						return array(
							'success' => false,
				...
				if ( 'upload' === $upload_type && 'multiple' === $file_type ) {
					$file_path = powerform_upload_root();
				...
				if ( 0 === $file_object['size'] ) {
					return array(
						'success' => false,
						'message' => __( 'The attached file is empty and can\'t be uploaded.', Powerform::DOMAIN ),
					);
				}
				...
				if ( $file_object['size'] > $max_size ) {
					...
					return array(
						...
						'message' => sprintf( /* translators: ... */
							__( 'Maximum file size allowed is %1$s. ', Powerform::DOMAIN ), $rounded_max_size ),
					);
				}
					...
					if ( 'custom' === $file_limit_type ) {
						...
						if ( isset( $post_data['totalFiles'] ) && $post_data['totalFiles'] > $file_limit ) {
							...
							return array(
								'error_type' => 'limit',
								'success'    => false,
								'message'    => sprintf( __( 'You can upload a maximum of %d files.', Powerform::DOMAIN ), $file_limit ),
							);
						}
					}
				}
		...
		if ( ! empty( $upload_data ) ) {
			...
			foreach ( $upload_data as $upload ) {
				if ( ! empty( $upload ) ) {
					...
					$temp_path = powerform_upload_root() . '/' . $file_name;
