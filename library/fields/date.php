...
/**
 * Class Powerform_Date
 *
 * @since 1.0
 */
class Powerform_Date extends Powerform_Field {
...
//						}
		...
		if ( "picker" === $type ) {
			...
			if ( $this->is_required( $field ) ) {
				$required_validation_message = apply_filters(
					'powerform_field_date_required_validation_message',
			...
			if ( $this->is_required( $field ) ) {
				$day_validation_message = apply_filters(
					'powerform_field_date_day_validation_message',
					$required_validation_message,
					$field,
					$type,
					$date_format,
	...
	public function sanitize( $field, $data ) {
		...
		return apply_filters( 'powerform_field_date_sanitize', $data, $field, $original_data );
	}
}