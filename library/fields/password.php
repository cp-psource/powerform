...
/**
 * Class Powerform_Password
 *
 * @since 1.0
 */
class Powerform_Password extends Powerform_Field {
	...
	public function __construct() {
		...
		$this->name = __( 'Password', Powerform::DOMAIN );
	}
	...
	public function defaults() {
		return array(
			'field_label'                  => __( 'Password', Powerform::DOMAIN ),
			'placeholder'                  => __( 'Enter your password', Powerform::DOMAIN ),
			'confirm-password-label'       => __( 'Confirm Password', Powerform::DOMAIN ),
			'confirm-password-placeholder' => __( 'Confirm new password', Powerform::DOMAIN ),
			'strength'                     => 'none',
		);
	}
	...
	public function markup( $field, $settings = array() ) {
		...
		if ( ! empty( $description ) || ( ! empty( $limit ) && ! empty( $limit_type ) ) ) {
			...
			$html .= '<div class="powerform-description powerform-description-password">';
		...
		if ( $is_confirm ) {
			...
			//Field 'Confirm password' is inside the separated 'powerform row'
			$html_prev_field = '</div></div>';//because there are '.powerform-row' and '.powerform-col' classes for Password field
			$html .= apply_filters( 'powerform_prev_last_tag_before_conf_password_field_markup', $html_prev_field, $field );

			$html .= '<div class="powerform-row">';
			$cols = 12;
			$html_before_conf_password_field = sprintf( '<div class="powerform-col powerform-col-%s">', $cols );

			$html .= apply_filters( 'powerform_before_conf_password_field_markup', $html_before_conf_password_field );
			$html .= '<div class="powerform-field">';
		...
		return apply_filters( 'powerform_field_password_markup', $html, $field );
	}
	...
	public function get_validation_rules() {
		...
		if ( isset( $min_password_strength ) && '' !== $min_password_strength && 'none' !== $min_password_strength ) {
			$rules .= '"powerformPasswordStrength": true,';
		}
		$rules .= '},';
		...
		if ( $is_confirm ) {
			...
			if ( 'true' === $is_valid ) {
				$rules .= '"equalTo": "#powerform-field-' . $this->get_id( $field ) . '",' . "\n";
			}
			$rules .= '},';
		}
	...
	public function get_validation_messages() {
		...
		if ( $is_required || $has_limit ) {
			if ( $is_required ) {
				$required_error = apply_filters(
					...
					! empty( $required_message ) ? $required_message : __( 'Your password is required.', Powerform::DOMAIN ),
					$id,
					$field
				);
		...
		if ( isset( $min_password_strength ) && '' !== $min_password_strength && 'none' !== $min_password_strength ) {
			...
			$min_strength_error = apply_filters(
				'powerform_text_field_min_password_strength_validation_message',
				! empty( $strength_validation_message ) ? $strength_validation_message : __( 'Your password doesn\'t meet the minimum strength requirement. We recommend using 8 or more characters with a mix of letters, numbers & symbols.', Powerform::DOMAIN ),
				$id,
				$field
			);
			$messages .= '"powerformPasswordStrength": "' . $min_strength_error . '",' . "\n";
		}
		$messages .= '},';
		...
		if ( $is_confirm ) {
			...
			if ( $is_required ) {
				$required_error = apply_filters(
					'powerform_confirm_password_field_required_validation_message',
					! empty( $required_confirm_message ) ? $required_confirm_message : __( 'You must confirm your chosen password.', Powerform::DOMAIN ),
					$id,
					$field
				);
			...
			if( 'true' === $is_valid ) {
				...
				$not_match_error = apply_filters(
					'powerform_confirm_password_field_not_match_validation_message',
					! empty( $validation_message_not_match ) ? $validation_message_not_match : __( 'Your passwords don\'t match.', Powerform::DOMAIN ),
					$id,
					$field
				);
