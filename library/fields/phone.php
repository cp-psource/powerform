...
class Powerform_Phone extends Powerform_Field {
	...
	public function defaults() {
		return apply_filters(
			'powerform_phone_defaults_settings',
			array(
				'required'              => false,
				'limit'                 => 10,
				'limit_type'            => 'characters',
				'validation'            => 'none',
				'field_label'           => __( 'Phone', Powerform::DOMAIN ),
				'placeholder'           => __( 'E.g. +1 300 400 5000', Powerform::DOMAIN ),
			)
		);
	}
	...
	public function get_phone_formats() {
		...
		$phone_formats = array(
			'standard'      => array(
				 ...
				 */
				'regex'       => '/^(\d|\s|\(|\)|\-|\.|\+){5,20}$/',
				'instruction' => __( 'Please make sure the number has a national format.', Powerform::DOMAIN ),
			),
			'international' => array(
				 ...
				 */
				'regex'       => '/^(\+){0,1}(\d|\s|\(|\)|\-){10,20}$/',
				'instruction' => __( 'Please make sure the number has an international format.', Powerform::DOMAIN ),
			),
		);
	...
	public function markup( $field, $settings = array() ) {
		...
		$phone_attr = array(
			...
			'id'            => $id,
			'class'         => 'powerform-input powerform-field--phone',
	...
	public function get_validation_rules() {
		...
		return apply_filters( 'powerform_field_phone_validation_rules', $rules, $id, $field );
	}
	...
	public function get_validation_messages() {
		...
		if ( $this->is_required( $field ) ) {
			$required_message = self::get_property( 'required_message', $field, __( 'This field is required. Please input a phone number.', Powerform::DOMAIN ) );
			...
			$messages        .= '"required": "' . powerform_addcslashes( $required_message ) . '",' . "\n";
			$required_message = apply_filters(
			...
			$messages        .= '"trim": "' . powerform_addcslashes( $required_message ) . '",' . "\n";
		}
		...
		if ( 'none' !== $format_check ) {
			if ( 'standard' === $format_check ) {
				$validation_message = apply_filters( // phpcs:ignore
					...
					( ! empty( $validation_message ) ? $validation_message : __( 'Please input a valid phone number.', Powerform::DOMAIN ) ),
					$field,
					$format_check,
					$this
				);
				$messages          .= '"powerformPhoneNational": "' . powerform_addcslashes( $validation_message ) . '",' . "\n";
			} elseif ( 'character_limit' === $format_check ) {
				$validation_message = apply_filters(
					'powerform_field_phone_maxlength_validation_message',
					( ! empty( $validation_message ) ? $validation_message : __( 'You exceeded the allowed amount of numbers. Please check again.', Powerform::DOMAIN ) ),
					$field,
					$format_check,
					$this
				);
				$messages          .= '"maxlength": "' . powerform_addcslashes( $validation_message ) . '",' . "\n";
			} elseif ( 'international' === $format_check ) {
				$validation_message = apply_filters(
					'powerform_field_phone_internation_validation_message',
					( ! empty( $validation_message ) ? $validation_message : __( 'Please input a valid international phone number.', Powerform::DOMAIN ) ),
					$field,
					$format_check,
					$this
				);
				$messages          .= '"powerformPhoneInternational": "' . powerform_addcslashes( $validation_message ) . '",' . "\n";
			}
		}

		$phone_message = apply_filters(
			'powerform_field_phone_invalid_validation_message',
			( ! empty( $validation_message ) ? $validation_message : __( 'Please enter a valid phone number.', Powerform::DOMAIN ) ),
			$field,
			$format_check,
			$this
		);

		$messages     .= '"phone": "' . powerform_addcslashes( $phone_message ) . '",' . "\n";
						...
						$this->validation_message[ $id ] =
							apply_filters(
								...
								( ! empty( $validation_message )
									? $validation_message
									: sprintf(/* translators: ... */
										__( 'Invalid phone number. %s', Powerform::DOMAIN ),
