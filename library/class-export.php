...
class Powerform_Export {
	...
	public function listen_for_csv_export() {
		if ( ! isset( $_POST['powerform_export'] ) ) {
			return;
		}
		...
		if ( ! wp_verify_nonce( $_POST['_powerform_nonce'], 'powerform_export' ) ) {
			return;
		}
		...
		$logs = get_option( 'powerform_exporter_log', array() );
	...
	public function listen_for_saving_export_schedule() {
		...
		if ( isset( $post_data['action'] ) && 'powerform_export_entries' === $post_data['action'] ) {
			...
			if ( ! isset( $post_data['form_id'] ) || empty( $post_data['form_id'] ) ) {
				$redirect = add_query_arg(
					array(
						'err_msg' => rawurlencode( __( 'Ungültige Formular-ID.', Powerform::DOMAIN ) ),
					)
				);
			...
			if ( ! isset( $post_data['form_type'] ) || empty( $post_data['form_type'] ) ) {
				$redirect = add_query_arg(
					array(
						'err_msg' => rawurlencode( __( 'Ungültiger Formulartyp.', Powerform::DOMAIN ) ),
					)
				);
			...
			powerform_maybe_log( $data[ $key ] );

			update_option( 'powerform_entries_export_schedule', $data );
			...
			if ( ! empty( $referer ) && ! headers_sent() ) {
				...
				if ( ! empty( $referer_query ) ) {
					...
					if ( ! empty( $query_strings ) && isset( $query_strings['page'] ) && 'powerform-entries' === $query_strings['page'] ) {
	...
	public function maybe_send_export( $force = false ) {
		...
		foreach ( $export_schedules as $row ) {
			...
			if ( $is_send ) {
				...
				if ( ! $info instanceof Powerform_Export_Result || empty( $info->file_path ) ) {
					continue;
				}
				...
				if ( $if_new ) {
					...
					if ( $last_row_id >= $export_result->latest_entry_id ) {
						powerform_maybe_log(
							__METHOD__,
							sprintf(
								'Geplanter Export von E-Mails für %s ID %s wurde übersprungen, da keine neuen Beiträge eingereicht wurden last_sent_row_id: %s latest_entry_id: %s',
	...
	private function _prepare_export_data( $form_id, $type, $latest_exported_entry_id = 0 ) {
		...
		$data                     = array();
		$entries                  = array();
		$export_result            = new Powerform_Export_Result();
		...
		switch ( $type ) {
			case 'quiz':
				...
				break;
			case 'poll':
				$model = Powerform_Poll_Form_Model::model()->load( $form_id );
				if ( ! is_object( $model ) ) {
					return null;
				}
				...
				$map_entries  = Powerform_Form_Entry_Model::map_polls_entries_for_export( $form_id, $fields_array );
				$header       = array(
					__( 'Datum', Powerform::DOMAIN ),
					__( 'Antwort', Powerform::DOMAIN ),
					__( 'Extra', Powerform::DOMAIN ),
				);
				...
				foreach ( $map_entries as $map_entry ) {
					...
					$entry = new Powerform_Form_Entry_Model( $map_entry['entry_id'] );
					...
					foreach ( $mappers as $mapper ) {
						...
						if ( isset( $mapper['property'] ) ) {
							...
							if ( ! isset( $mapper['sub_metas'] ) ) {
								$data[] = Powerform_Form_Entry_Model::meta_value_to_string( $mapper['type'], $meta_value );
							} else {
								...
								foreach ( $mapper['sub_metas'] as $sub_meta ) {
									...
									if ( isset( $meta_value[ $sub_key ] ) && ! empty( $meta_value[ $sub_key ] ) ) {
										...
										$data[]     = Powerform_Form_Entry_Model::meta_value_to_string( $field_type, $value );
									} else {
										$data[] = '';
									}
								}
		...
		if ( isset( $entries[0] ) && $entries[0] instanceof Powerform_Form_Entry_Model ) {
	...
	private function _prepare_attachment( $form_id, $type, $email, $last_entry_id = 0 ) {
		...
		if ( ! $export_result instanceof Powerform_Export_Result ) {
			return false;
		}
		...
		$tmp_path = $upload_dirs['basedir'] . '/powerform/';
		...
		if ( ! $wp_filesystem->put_contents( $tmp_path, $this->csvstr( $data ), $mode ) ) {
			if ( is_wp_error( $wp_filesystem->errors ) ) {
				powerform_maybe_log( __METHOD__, $wp_filesystem->errors->get_error_message() );
			}
	 ...
	 * @since   1.0.5
	 *
	 * @param Powerform_Custom_Form_Model|Powerform_Base_Form_Model $model
	 *
	 * @return array
	 */
	private function get_custom_form_export_mappers( $model ) {
		...
		$mappers = array(
			array(
				...
				'label'    => __( 'Abgabetermin', Powerform::DOMAIN ),
		 ...
		 * @return array
		 */
		$mappers = apply_filters( 'powerform_custom_form_export_mappers', $mappers, $model->id, $model );
	 ...
	 *
	 * @see   Powerform_Addon_Form_Hooks_Abstract::on_export_render_title_row()
	 *
	 * @since 1.1
	 ...
	 *
	 * @param                               $form_id
	 * @param Powerform_Form_Entry_Model[] $entries
	 *
	 * @return array
	 */
	private function attach_form_addons_on_export_render_title_row( $form_id, $entries = array() ) {
		...
		foreach ( $registered_addons as $registered_addon ) {
			try {
				...
				powerform_addon_maybe_log( $registered_addon->get_slug(), 'failed to on_export_render_title_row', $e->getMessage() );
			}
	 ...
	 * Format used is `powerform_addon_export_title_{$addon_slug}_{$title_id_data_from_addon}`
	 *
	 * @since 1.1
	 *
	 ...
	 * @return array
	 */
	private function format_addon_additional_headers( Powerform_Addon_Abstract $addon, $addon_headers ) {
		...
		foreach ( $addon_headers as $title_id => $title ) {
			...
			$title_id = 'powerform_addon_export_title_' . $addon->get_slug() . '_' . $title_id;
	 ...
	 *
	 * @see   Powerform_Addon_Form_Hooks_Abstract::on_export_render_entry()
	 * @since 1.1
	 ...
	 * @return array
	 */
	private function attach_form_addons_on_export_render_entry_row( $form_id, Powerform_Form_Entry_Model $entry_model ) {
		...
		foreach ( $registered_addons as $registered_addon ) {
			try {
				...
				powerform_addon_maybe_log( $registered_addon->get_slug(), 'failed to on_export_render_entry', $e->getMessage() );
			}
	 ...
	 * @return array
	 */
	private function format_addon_additional_data( Powerform_Addon_Abstract $addon, $addon_data ) {
		...
		foreach ( $addon_data as $title_id => $value ) {
			$value = Powerform_Form_Entry_Model::meta_value_to_string( 'addon_' . $addon->get_slug(), $value );
	 ...
	 * @param Powerform_Form_Entry_Model[] $entries
	 *
	 * @return array|Powerform_Addon_Abstract[]
	 */
	public function get_form_registered_addons( $form_id, $entries = array() ) {
		if ( empty( self::$form_registered_addons ) ) {
			...
			foreach ( $entries as $entry ) {
				...
				$entry_addon_slugs = powerform_find_addon_slugs_from_entry_model( $entry );
				foreach ( $entry_addon_slugs as $entry_addon_slug ) {
					...
					if ( in_array( $entry_addon_slug, array_keys( $registered_addons ), true ) ) {
						...
						if ( ! in_array( $entry_addon_slug, array_keys( self::$form_registered_addons ), true ) ) {
							$addon = powerform_get_addon( $entry_addon_slug );
							if ( $addon instanceof Powerform_Addon_Abstract ) {
								try {
									...
									powerform_addon_maybe_log( $addon->get_slug(), 'failed to get_addon_form_hooks one export', $e->getMessage() );
								}
							}
						}
					}
	...
	public function get_entries_export_schedule( $schedule_key = null ) {
		$opt           = get_option( 'powerform_entries_export_schedule', array() );
		$validated_opt = $opt;
		...
		if ( $validated_opt !== $opt ) {
			update_option( 'powerform_entries_export_schedule', $validated_opt );
		}
	...
	public function get_mail_subject( $export_results ) {
		...
		$subject = sprintf( __( "Übermittlungsdaten für %s", Powerform::DOMAIN ), implode( ', ', $form_names ) );
		 ...
		 * @return string
		 */
		$subject = apply_filters( 'powerform_export_email_subject', $subject, $form_names, $export_results );
	...
	public function get_mail_content( $export_results ) {
		$submissions_link_format = admin_url( 'admin.php?page=powerform-entries&form_type=%1$s&form_id=%2$d' );
		...
		foreach ( $export_results as $export_result ) {
			...
			switch ( $export_result->form_type ) {
				case 'cform':
					...
					break;
				case 'poll':
					$form_type = 'powerform_polls';
					break;
				case 'quiz':
					$form_type = 'powerform_quizzes';
					break;
				default:
					break;
			}
		 ...
		 * @return string
		 */
		$mail_content = apply_filters( 'powerform_export_email_content', $mail_content, $form_names, $export_results );
						...
						if ( ! in_array( $entry_addon_slug, array_keys( self::$poll_registered_addons ), true ) ) {
							...
							if ( $addon instanceof Powerform_Addon_Abstract ) {
								try {
									...
									powerform_addon_maybe_log( $addon->get_slug(), 'failed to get_addon_poll_hooks on export', $e->getMessage() );
								}
							}
						}
					}
	 ...
	 *
	 * @see   Powerform_Addon_Poll_Hooks_Abstract::on_export_render_title_row()
	 *
	 * @since 1.6.1
	 *
	 * @param                               $poll_id
	...
	private function attach_poll_addons_on_export_render_title_row( $poll_id, $entries = array() ) {
		...
		foreach ( $registered_addons as $registered_addon ) {
			try {
				...
				powerform_addon_maybe_log( $registered_addon->get_slug(), 'failed to attach_poll_addons_on_export_render_title_row', $e->getMessage() );
			}
	 ...
	 *
	 * @see   Powerform_Addon_Poll_Hooks_Abstract::on_export_render_entry()
	 * @since 1.6.1
	 *
	 * @param                             $form_id
	 * @param Powerform_Form_Entry_Model $entry_model
	 ...
	 * @return array
	 */
	private function attach_poll_addons_on_export_render_entry_row( $form_id, Powerform_Form_Entry_Model $entry_model ) {
		$additional_data = array();
		//find all registered addons, so history can be shown even for deactivated addons
		...
		foreach ( $registered_addons as $registered_addon ) {
			try {
				...
				powerform_addon_maybe_log( $registered_addon->get_slug(), 'failed to attach_poll_addons_on_export_render_entry_row', $e->getMessage() );
			}
						...
						if ( ! in_array( $entry_addon_slug, array_keys( self::$quiz_registered_addons ), true ) ) {
							...
							if ( $addon instanceof Powerform_Addon_Abstract ) {
								try {
									...
									powerform_addon_maybe_log( $addon->get_slug(), 'failed to get_addon_quiz_hooks on export', $e->getMessage() );
								}
							}
						}
					}
	 ...
	 *
	 * @see   Powerform_Addon_Quiz_Hooks_Abstract::on_export_render_title_row()
	 *
	 * @since 1.6.2
	 *
	 * @param                               $quiz_id
	...
	private function attach_quiz_addons_on_export_render_title_row( $quiz_id, $entries = array() ) {
		...
		foreach ( $registered_addons as $registered_addon ) {
			try {
				...
				powerform_addon_maybe_log( $registered_addon->get_slug(), 'failed to attach_quiz_addons_on_export_render_title_row', $e->getMessage() );
			}
	 ...
	 *
	 * @see   Powerform_Addon_Quiz_Hooks_Abstract::on_export_render_entry()
	 * @since 1.6.2
	 *
	 * @param                             $form_id
	 * @param Powerform_Form_Entry_Model $entry_model
	 ...
	 * @return array
	 */
	private function attach_quiz_addons_on_export_render_entry_row( $form_id, Powerform_Form_Entry_Model $entry_model ) {
		$additional_data = array();
		//find all registered addons, so history can be shown even for deactivated addons
		...
		foreach ( $registered_addons as $registered_addon ) {
			try {
				...
				powerform_addon_maybe_log( $registered_addon->get_slug(), 'failed to attach_quiz_addons_on_export_render_entry_row', $e->getMessage() );
			}
