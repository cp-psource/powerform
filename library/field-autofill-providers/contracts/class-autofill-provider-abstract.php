...
/**
 * Class Powerform_Autofill_Provider_Abstract
 * Abstraction for Autofill Provider
 *
 * @since 1.0.5
 */
abstract class Powerform_Autofill_Provider_Abstract implements Powerform_Autofill_Provider_Interface {
	...
	/**
	 * Add hook filter to powerform fields based on @see get_attribute_to_hook
	 *
	 * @since 1.0.5
	 *
	 */
	final protected function hook_to_fields() {
		...
		foreach ( $hookable_attributes as $field_slug => $hookable_attribute ) {
			add_filter( 'powerform_field_' . $field_slug . '_autofill', array( $this, 'attribute_hook' ), 10, 2 );
		}
	}

	/**
	 * The Filter Executed when Powerform Fields initialized its @see Powerform_Field::get_autofill_setting()
	 *
	 * @since 1.0.5
	 *
	 * @param $providers
