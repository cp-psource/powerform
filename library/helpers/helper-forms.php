 ...
 * @return string
 */
function powerform_get_post_data( $property, $post_id = null, $default = '' ) {
	global $post;
...
function powerform_cforms_total( $status = '' ) {
 ...
 * @return int
 */
function powerform_polls_total( $status = '' ) {
 ...
 * @return int
 */
function powerform_quizzes_total( $status = '' ) {
 ...
 * @return bool
 */
function powerform_quiz_has_leads( $model ) {
 ...
 * @return int
 */
function powerform_total_forms( $status = '' ) {
	$modules = array(
		powerform_cforms_total( $status ),
		powerform_polls_total( $status ),
		powerform_quizzes_total( $status ),
	);
	...
	if ( $total > $per_page ) {
		...
		<ul class="sui-pagination">
			...
			<?php if ( ! $disable_first ) : ?>
				...
				?>
				<?php if ( $show_skip ) : ?>
					<li class="psource-pagination--prev">
			...
			<?php if ( ! $disable_last ) : ?>
				...
				?>
				<?php if ( $disable_next ) : ?>
					<li class="psource-pagination--next">
...
function powerform_get_latest_entry_time( $entry_type ) {
	...
	if ( $latest_entry instanceof Powerform_Form_Entry_Model ) {
		...
		$last_entry_time = sprintf( /* translators: ... */ __( '%s zuvor', Powerform::DOMAIN ), $time_diff );

		return $last_entry_time;
	} else {
		return __( 'Noch keine', Powerform::DOMAIN );
	}
}
...
function powerform_get_latest_entry_time_by_form_id( $form_id ) {
	...
	if ( $latest_entry instanceof Powerform_Form_Entry_Model ) {
		...
		return esc_html__( 'Noch keine', Powerform::DOMAIN );
	}
}
 ...
 * @return mixed|string
 */
function powerform_get_social_message( $settings, $title, $result, $data = array() ) {
    $message = __( "I got {quiz_result} on {quiz_name} quiz!", Powerform::DOMAIN );
    ...
    $message = powerform_replace_variables( $message, false, $data['current_url'], $post_id );
 ...
 * @return array
 */
function powerform_get_chart_data( Powerform_Poll_Form_Model $poll ) {
	 ...
	 * @since 1.13
	 */
	return apply_filters( 'powerform_polls_chart_datas', $chart_datas );
}
 ...
 */
function powerform_get_property( $array, $prop, $default = null ) {
 ...
 * @return bool
 */
function powerform_is_main_site() {
 ...
 * @return bool
 */
function powerform_is_subdomain_network() {
 ...
 * @return string
 */
function powerform_get_quiz_name( $id ) {
	$model = Powerform_Quiz_Form_Model::model()->load( $id );

	return ! empty( $model->settings['quiz_name'] ) ? $model->settings['quiz_name'] : '';
}

function powerform_defender_compatibility() {
	$defender_data = array(
		'is_activated'    => false,
		'is_free'         => false,
		'img_dir_url'     => '',
