 ...
 * @return bool
 */
function powerform_is_import_plugin_enabled( $plugin ) {
	 ...
	 */
	$active = apply_filters( 'powerform_is_import_export_feature_enabled', $active );
 ...
 * @return bool
 */
function powerform_get_disable_url( $plugin, $slug ) {
	if ( ! powerform_is_import_plugin_enabled( $plugin ) ) {
		return false;
	}
 ...
 * @return bool
 */
function powerform_are_import_plugins_enabled( $plugins = array() ) {

	$active = false;
	foreach ($plugins as $key => $plugin) {
		if ( true === $active ) break;

		$active = powerform_is_import_plugin_enabled( $plugin );
	}

	$active = apply_filters( 'powerform_are_import_plugins_enabled', $active );
 ...
 */
function powerform_list_thirdparty_contact_forms( $type ) {
	//get all forms
	$forms = array();
	switch ($type) {
		case 'cf7':
