 ...
 */
function powerform_get_addon( $slug ) {
	return Powerform_Addon_Loader::get_instance()->get_addon( $slug );
}
 ...
 * @return array
 */
function powerform_get_pro_addon_list() {
	$pro_addons = array(
		'mailchimp' => array(
...
function powerform_get_registered_addons_list() {
	...
	foreach ( $addon_list as $key => $addon ) {
		$addon_list[ $key ]['is_active'] = Powerform_Addon_Loader::get_instance()->addon_is_active( $key );
	}
 ...
 * @return array
 */
function powerform_get_registered_addons_grouped_by_connected() {
	$addon_list           = powerform_get_registered_addons_list();
 ...
 */
function powerform_get_addons_instance_connected_with_form( $form_id ) {
	$addons = array();

	$active_addons_slug = Powerform_Addon_Loader::get_instance()->get_activated_addons();

	foreach ( $active_addons_slug as $active_addon_slug ) {
		$addon = powerform_get_addon( $active_addon_slug );
		if ( $addon ) {
 ...
 *
 * Every addon inside this array will be formatted first by @see Powerform_Addon_Abstract::to_array_with_form()
 *
 * @since 1.1
 *
 * @param $form_id
 ...
 * @return array
 */
function powerform_get_registered_addons_grouped_by_form_connected( $form_id ) {
	$connected_addons     = array();
	$not_connected_addons = array();
 ...
 * defined in @see Powerform_Addon_Abstract::global_hookable()
 * and @see Powerform_Addon_Abstract::admin_hookable on admin mode
 *
 * @since 1.1
 *
 * @param Powerform_Addon_Abstract $addon
 */
function powerform_maybe_attach_addon_hook( Powerform_Addon_Abstract $addon ) {
 ...
 * @return bool
 */
function powerform_addon_is_active( $slug ) {
	return Powerform_Addon_Loader::get_instance()->addon_is_active( $slug );
}
 ...
 * @return array
 */
function powerform_get_allowed_field_types_for_addon() {
	 ...
	 */
	$allowed_field_types = apply_filters( 'powerform_addon_allowed_field_types', $allowed_field_types );
 ...
 * @since 1.1
 *
 * @param Powerform_Base_Form_Model $custom_form_model
 *
 * @return array
 */
function powerform_addon_format_form_fields( Powerform_Base_Form_Model $custom_form_model ) {
	$formatted_fields    = array();
	$fields              = $custom_form_model->get_fields();
	$allowed_field_types = powerform_get_allowed_field_types_for_addon();

	foreach ( $fields as $field ) {
		$ignored_fields = Powerform_Form_Entry_Model::ignored_fields();
		...
		// handle multiple
		$multi_fields = powerform_addon_flatten_mutiple_field( $field_as_array );
	 ...
	 * @param Powerform_Custom_Form_Model $custom_form_model Custom form Model
	 */
	$formatted_fields = apply_filters( 'powerform_addon_formatted_fields', $formatted_fields, $custom_form_model );
 ...
 */
function powerform_addon_flatten_mutiple_field( $field_array ) {
	...
	if ( 'name' === $field_array['type'] ) {
		...
		if ( $prefix_enabled || $first_name_enabled || $middle_name_enabled || $last_name_enabled ) {
			...
			if ( $prefix_enabled ) {
				...
				$default_label = Powerform_Form_Entry_Model::translate_suffix( 'prefix' );
			...
			if ( $first_name_enabled ) {
				...
				$default_label = Powerform_Form_Entry_Model::translate_suffix( 'first-name' );
			...
			if ( $middle_name_enabled ) {
				...
				$default_label = Powerform_Form_Entry_Model::translate_suffix( 'middle-name' );
			...
			if ( $last_name_enabled ) {
				...
				$default_label = Powerform_Form_Entry_Model::translate_suffix( 'last-name' );
		...
		if ( $title_enabled || $content_enabled || $excerpt_enabled || $category_enabled || $tags_enabled || $image_enabled ) {
			...
			if ( $title_enabled ) {
				...
				$default_label = Powerform_Form_Entry_Model::translate_suffix( 'post-title' );
			...
			if ( $content_enabled ) {
				...
				$default_label = Powerform_Form_Entry_Model::translate_suffix( 'post-content' );
			...
			if ( $excerpt_enabled ) {
				...
				$default_label = Powerform_Form_Entry_Model::translate_suffix( 'post-excerpt' );
			...
			if ( $image_enabled ) {
				...
				$default_label = Powerform_Form_Entry_Model::translate_suffix( 'post-image' );
		...
		if ( $street_enabled || $line_enabled || $city_enabled || $state_enabled || $zip_enabled || $country_enabled ) {
			...
			if ( $street_enabled ) {
				...
				$default_label = Powerform_Form_Entry_Model::translate_suffix( 'street_address' );
			...
			if ( $line_enabled ) {
				...
				$default_label = Powerform_Form_Entry_Model::translate_suffix( 'address_line' );
			...
			if ( $city_enabled ) {
				...
				$default_label = Powerform_Form_Entry_Model::translate_suffix( 'city' );
			...
			if ( $state_enabled ) {
				...
				$default_label = Powerform_Form_Entry_Model::translate_suffix( 'state' );
			...
			if ( $zip_enabled ) {
				...
				$default_label = Powerform_Form_Entry_Model::translate_suffix( 'zip' );
			...
			if ( $country_enabled ) {
				...
				$default_label = Powerform_Form_Entry_Model::translate_suffix( 'country' );
 ...
 * @return array
 */
function powerform_format_submitted_data_for_addon( $post_data, $files_data, $form_fields, $current_entry_fields = array() ) {
	...
	unset( $post_data['powerform_nonce'] );
		...
		if ( isset( $post_data[ $form_field['element_id'] ] ) ) {
			...
			if ( 'time' === $form_field['type'] ) {
				...
				if ( isset( $post_data[ $hours_element_id ] ) && isset( $post_data[ $minutes_element_id ] ) ) {
					...
					$time = Powerform_Form_Entry_Model::meta_value_to_string( $form_field['type'], $data, false );
				...
				if ( isset( $post_data[ $day_element_id ] ) && isset( $post_data[ $month_element_id ] ) && isset( $post_data[ $year_element_id ] ) ) {
					...
					$date = Powerform_Form_Entry_Model::meta_value_to_string( $form_field['type'], $data, false );
	 ...
	 */
	$formatted_post_data = apply_filters( 'powerform_addon_formatted_submitted_data', $formatted_post_data, $post_data, $files_data, $form_fields );
 ...
 * @since 1.1
 *
 * @param Powerform_Base_Form_Model $custom_form
 *
 * @return array formatted and filtered form settings
 */
function powerform_addon_format_form_settings( Powerform_Base_Form_Model $custom_form ) {
	 ...
	 * @param Powerform_Base_Form_Model $custom_form   Custom Form Model
	 */
	$form_settings = apply_filters( 'powerform_addon_formatted_form_settings', $form_settings, $custom_form );
 ...
 * @since 1.1
 *
 * @param Powerform_Addon_Abstract   $connected_addon
 * @param Powerform_Form_Entry_Model $entry_model
 *
 * @return array
 */
function powerform_find_addon_meta_data_from_entry_model( Powerform_Addon_Abstract $connected_addon, Powerform_Form_Entry_Model $entry_model ) {
	$addon_meta_data        = array();
	$addon_meta_data_prefix = 'powerform_addon_' . $connected_addon->get_slug() . '_';
	 ...
	 */
	$addon_meta_data = apply_filters( 'powerform_addon_meta_data_from_entry_model', $addon_meta_data, $connected_addon, $entry_model, $addon_meta_data_prefix );
 ...
 * @return string
 */
function powerform_addon_row_html_markup( $addon, $form_id, $show_pro_info = false, $is_active = false ) {
	ob_start();

	$single_addon_template_path = powerform_plugin_dir() . 'admin/views/integrations/addon.php';
	 ...
	 */
	$single_addon_template_path = apply_filters( 'powerform_addon_single_addon_template_path', $single_addon_template_path );
...
/**
 * Add log of powerform addon related if permitted
 *
 * To be this can be active :
 * - WP_DEBUG : true
 * - POWERFORM_ADDON_DEBUG : true
 * - powerform_addon_enable_log : true
 *
 * @see   powerform_maybe_log()
 *
 * @since 1.1
 * @since 1.3 add POWERFORM_ADDON_DEBUG as enabled flag
 */
function powerform_addon_maybe_log() {
	$enabled = ( defined( 'POWERFORM_ADDON_DEBUG' ) && POWERFORM_ADDON_DEBUG );

	/**
	 * Filter log enable for powerform addon
	 *
	 * By default it will check `WP_DEBUG`, `POWERFORM_ADDON_DEBUG`, `POWERFORM_DEBUG` must be true
	 *
	 * @since 1.1
	 *
	 * @param bool $enabled current enable status
	 */
	$enabled = apply_filters( 'powerform_addon_enable_log', $enabled );

	if ( $enabled ) {
		if ( is_callable( 'powerform_maybe_log' ) ) {
			$args  = array( '[ADDON]' );
			$fargs = func_get_args();
...
function powerform_addon_replace_custom_vars( $content, $submitted_data, Powerform_Custom_Form_Model $custom_form, $entry_meta, $allow_html = false ) {
	...
	$content = powerform_replace_custom_form_data( $content, $custom_form, $submitted_data, $entry_model );
	...
	if ( preg_match_all( $pattern, $content, $matches ) ) {
		...
		foreach ( $matches[0] as $match ) {
			...
			$value = Powerform_Form_Entry_Model::meta_value_to_string( $field_type, $meta_value, $allow_html );
 ...
 * @return string
 */
function powerform_addon_integration_section_admin_url( $slug, $section, $with_nonce = true ) {
	if ( ! class_exists( 'Powerform_Integrations_Page' ) ) {
		return '';
	}

	$admin_url  = admin_url( 'admin.php' );
	$query_args = array(
		'page'    => 'powerform-integrations',
		'slug'    => $slug,
		'section' => $section,
	);
	if ( $with_nonce ) {
		$nonce               = Powerform_Integrations_Page::get_addon_page_nonce();
 ...
 * @return Powerform_Addon_Abstract[]
 */
function powerform_get_registered_addons() {
	$addons            = array();
	$registered_addons = Powerform_Addon_Loader::get_instance()->get_addons();

	foreach ( $registered_addons as $slug => $registered_addon ) {
		$addon = powerform_get_addon( $slug );
		if ( $addon instanceof Powerform_Addon_Abstract ) {
 ...
 * @return array
 */
function powerform_find_addon_slugs_from_entry_model( Powerform_Form_Entry_Model $entry_model ) {
	$addon_slugs                   = array();
	$addon_metadata_pattern_prefix = 'powerform_addon_';
	foreach ( $entry_model->meta_data as $key => $meta_datum ) {
	 ...
	 * @param string                      $addon_meta_data_prefix default prefix of connected addon meta data key
	 */
	$addon_slugs = apply_filters( 'powerform_addon_slugs_from_entry_model', $addon_slugs, $entry_model, $addon_metadata_pattern_prefix );
 ...
 * @since 1.6.1
 *
 * @param Powerform_Base_Form_Model $poll
 *
 * @return array formatted and filtered form settings
 */
function powerform_addon_format_poll_settings( Powerform_Base_Form_Model $poll ) {
	 ...
	 * @param Powerform_Base_Form_Model $poll          Custom Form Model
	 */
	$poll_settings = apply_filters( 'powerform_addon_formatted_poll_settings', $poll_settings, $poll );
 ...
 * @return array
 */
function powerform_addon_format_poll_fields( Powerform_Base_Form_Model $poll ) {
	$formatted_fields = array();
	 ...
	 *
	 * @param array                        $formatted_fields current formatted fields
	 * @param Powerform_Custom_Form_Model $poll             Custom form Model
	 */
	$formatted_fields = apply_filters( 'powerform_addon_formatted_poll_fields', $formatted_fields, $poll );
 ...
 *
 * Every addon inside this array will be formatted first by @see Powerform_Addon_Abstract::to_array_with_poll()
 *
 * @since 1.6.1
 *
 * @param $poll_id
 ...
 * @return array
 */
function powerform_get_registered_addons_grouped_by_poll_connected( $poll_id ) {
	$connected_addons     = array();
	$not_connected_addons = array();
 ...
 * @return string
 */
function powerform_addon_poll_row_html_markup( $addon, $poll_id, $show_pro_info = false, $is_active = false ) {
	ob_start();

	$single_addon_template_path = powerform_plugin_dir() . 'admin/views/integrations/poll-addon.php';
	 ...
	 * @param string $single_addon_template_path current used path
	 */
	$single_addon_template_path = apply_filters( 'powerform_addon_single_poll_addon_template_path', $single_addon_template_path );
 ...
 * @return array
 */
function powerform_addon_format_poll_submitted_data( $post_data, $files_data ) {
	$formatted_post_data = array();
	 ...
	 * @param array $form_fields         form fields that exist on the form
	 */
	$formatted_post_data = apply_filters( 'powerform_addon_formatted_poll_submitted_data', $formatted_post_data, $post_data, $files_data );
 ...
 * @return Powerform_Addon_Abstract[]
 */
function powerform_get_addons_instance_connected_with_poll( $poll_id ) {
	$addons = array();
 ...
 * @return array formatted and filtered form settings
 */
function powerform_addon_format_quiz_settings( Powerform_Quiz_Form_Model $quiz ) {
	 ...
	 * @param Powerform_Quiz_Form_Model $quiz          Quiz Model
	 */
	$quiz_settings = apply_filters( 'powerform_addon_formatted_quiz_settings', $quiz_settings, $quiz );
 ...
 *
 * Every addon inside this array will be formatted first by @see Powerform_Addon_Abstract::to_array_with_quiz()
 *
 * @since 1.6.2
 *
 * @param $quiz_id
 ...
 * @return array
 */
function powerform_get_registered_addons_grouped_by_quiz_connected( $quiz_id ) {
	$connected_addons     = array();
	$not_connected_addons = array();
 ...
 * @return string
 */
function powerform_addon_quiz_row_html_markup( $addon, $quiz_id, $show_pro_info = false, $is_active = false ) {
	ob_start();

	$single_addon_template_path = powerform_plugin_dir() . 'admin/views/integrations/quiz-addon.php';
	 ...
	 * @param string $single_addon_template_path current used path
	 */
	$single_addon_template_path = apply_filters( 'powerform_addon_single_quiz_addon_template_path', $single_addon_template_path );
 ...
 * @return array
 */
function powerform_addon_format_quiz_submitted_data( $post_data, $files_data ) {
	$formatted_post_data = array();
	 ...
	 * @param array $form_fields         form fields that exist on the form
	 */
	$formatted_post_data = apply_filters( 'powerform_addon_formatted_quiz_submitted_data', $formatted_post_data, $post_data, $files_data );
 ...
 * @return Powerform_Addon_Abstract[]
 */
function powerform_get_addons_instance_connected_with_quiz( $quiz_id ) {
	$addons = array();
