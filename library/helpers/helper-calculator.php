 ...
 * dummy value will usually this format (1) depends @see Powerform_Field::get_dummy_calculable_value()
 *
 ...
 * @return string
 */
function powerform_calculator_maybe_dummify_fields_on_formula( $formula ) {
 ...
 * @return int
 */
function powerform_calculator_get_max_nested_formula() {
	// default : 5
	$max_nested_formula = defined( 'POWERFORM_CALC_MAX_NESTED_FORMULA' ) ? POWERFORM_CALC_MAX_NESTED_FORMULA : 5;
	 ...
	 * @return int
	 */
	$max_nested_formula = apply_filters( 'powerform_calculator_max_nested_formula', $max_nested_formula );
 ...
 * @return string
 */
function powerform_calculator_maybe_replace_fields_on_formula( $formula, $submitted_data, $custom_form, &$nested_count = 0 ) {

	// more then allowed nested formula
	if ( $nested_count > powerform_calculator_get_max_nested_formula() ) {
		return $formula;
	}
	...
	if ( preg_match_all( $pattern, $formula, $matches ) ) {
		...
		foreach ( $matches[2] as $key => $field_type ) {
			...
			if ( $field_object->is_hidden( $field_settings, $submitted_data, [], $custom_form ) ) {
				...
				// see Powerform_CForm_Front_Action::replace_hidden_field_values()
	...
	if ( $nested_exists ) {
		$nested_count ++;
		$formula = powerform_calculator_maybe_replace_fields_on_formula( $formula, $submitted_data, $custom_form, $nested_count );
	}
