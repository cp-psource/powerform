 ...
 * Author: Hoang Ngo
 */
class Powerform_Poll_Form_Model extends Powerform_Base_Form_Model {
	protected $post_type = 'powerform_polls';
	 ...
	 * @param $data
	 *
	 * @return bool|Powerform_Base_Form_Model
	 */
	 ...
	 * which is powerform poll doesnt have element_id on poll answers
	 *
	 * @since 1.0.5
	 *
	 * @param      $id
	 * @param bool $callback
	 *
	 * @return bool|Powerform_Poll_Form_Model
	 */
	 ...
	 * @since 1.0.5
	 *
	 * @param Powerform_Poll_Form_Model $model
	 *
	 * @return Powerform_Poll_Form_Model
	 */
	private function maybe_add_element_id_on_answers( Powerform_Poll_Form_Model $model ) {
	 ...
	 * @return Powerform_Poll_Form_Model
	 */
	private function resave_and_reload( Powerform_Poll_Form_Model $model ) {
		$model->save();
	...
	public function to_exportable_data() {
		...
		if ( Powerform::is_export_integrations_feature_enabled() ) {
			add_filter( 'powerform_poll_model_to_exportable_data', array( $this, 'export_integrations_data' ), 1, 1 );
		}

		$exportable_data = parent::to_exportable_data();

		// avoid filter executed on next cycle
		remove_filter( 'powerform_poll_model_to_exportable_data', array( $this, 'export_integrations_data' ), 1 );
	...
	public function export_integrations_data( $exportable_data ) {
		...
		$connected_addons = powerform_get_addons_instance_connected_with_poll( $model_id );
		 ...
		 */
		$exportable_integrations         = apply_filters( 'powerform_poll_model_export_integrations_data', $exportable_integrations, $model_id );
	...
	public static function import_integrations_data( $model, $import_data, $module ) {
		...
		/** @var Powerform_Poll_Form_Model $model */
	...
	public function is_ajax_load( $force = false ) {
		 ...
		 * @return bool
		 */
		$enabled = apply_filters( 'powerform_poll_is_ajax_load', $enabled, $global_enabled, $poll_id, $form_settings );
