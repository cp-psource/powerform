 ...
 * @since 1.0
 */
class Powerform_Entries_List_Table extends WP_List_Table {
	...
	/**
	 * Powerform_Entries_List_Table constructor.
	 *
	 * @since 1.0
	 * @param array $args
	 */
	...
	public function no_items() {
		esc_html_e( 'No entries found.', Powerform::DOMAIN );
	}
	...
	public function get_columns() {
		$columns = array(
			...
			'date' => esc_html__( 'Date added', Powerform::DOMAIN ),
		);
	...
	public function prepare_items() {
		...
		$this->total_items = Powerform_Form_Entry_Model::count_entries( $form_id );
		...
		$this->items           = Powerform_Form_Entry_Model::list_entries( $form_id, $per_page, $offset );
	...
	public function print_column_headers( $with_id = true ) {
		...
		if ( ! empty( $columns['cb'] ) ) {
			...
			$columns['cb']     = '<label class="screen-reader-text" for="cb-select-all-' . $cb_counter . '">' . __( 'Select All' ) . '</label>'
				. '<div class="psource-checkbox"><input id="cb-select-all-' . $cb_counter . '" type="checkbox" /><label for="cb-select-all-' . $cb_counter . '" class="wpdui-icon wpdui-icon-check"></label></div>';
			$cb_counter++;
		}
		...
		foreach ( $columns as $column_key => $column_display_name ) {
			...
			if ( 'cb' === $column_key ) {
				$class[] = 'psource-head-check check-column';
			}
	...
	public function display() {
		...
		?>
		<table class="psource-list-table" cellspacing="0" cellpadding="0">
			<thead class="psource-table-head">
				<tr>
					<?php $this->print_column_headers(); ?>
				</tr>
			</thead>

			<tbody class="psource-table-body">
	 ...
	 * @access public
	 *
	 * @param Powerform_Form_Entry_Model $item
	 */
	public function column_cb( $item ) {
		?>
		<div class="psource-checkbox">
	...
	public function column_date( $item ) {
		?>
		<p class="psource-cell-content"><?php echo esc_attr( $item->date_created ); ?></p>
		<?php
	}
	 ...
	 *
	 * @since 1.0
	 * @param Powerform_Form_Entry_Model $item - the current item
	...
	public function column_default( $item, $column_name ) {
		...
		if ( $data ) {
			if ( is_array( $data ) ) {
				...
				foreach ( $data as $key => $value ) {
					if ( is_array( $value ) ) {
						if ( 'file' === $key && isset( $value['file_url'] ) ) {
							...
							$file_name = "<a href='" . $value['file_url'] . "' target='_blank' rel='noreferrer' title='" . __( 'View File', Powerform::DOMAIN ) . "'>$file_name</a> ,";
							$output   .= $file_name;
						}
					} else {
						if ( ! is_int( $key ) ) {
				...
				if ( $is_product ) {
					$output = sprintf( /* translators: ... */ __( 'Total %d', Powerform::DOMAIN ), $product_cost );
				} else {
					if ( ! empty( $output ) ) {
