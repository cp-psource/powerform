...
class Powerform_Entries_List_Table extends WP_List_Table {
	...
	public function no_items() {
		esc_html_e( 'Keine Einträge gefunden.', Powerform::DOMAIN );
	}
	...
	public function get_columns() {
		$columns = array(
			...
			'date'	=> esc_html__( 'Datum hinzugefügt', Powerform::DOMAIN ),
		);
	...
	public function column_default( $item, $column_name ) {
		...
		if ( $data ) {
			if ( is_array( $data ) ) {
				...
				foreach ( $data as $key => $value ) {
					if ( is_array( $value ) ) {
						if ( 'file' === $key && isset( $value['file_url'] ) ) {
							...
							$file_name 	= "<a href='" .$value['file_url'] . "' target='_blank' rel='noreferrer' title='". __( 'Datei ansehen', Powerform::DOMAIN ) ."'>$file_name</a> ,";
							$output 	.= $file_name;
						}
					} else {
						if ( !is_int( $key ) ) {
							if ( 'postdata' === $key ) {
								...
								$output .= "<a href='" .$url . "' target='_blank' rel='noreferrer' title='". __( 'Beitrag bearbeiten', Powerform::DOMAIN ) ."'>$name</a> ,";
							} else {
								if ( is_string( $key ) ) {
				...
				if ( $is_product ) {
					$output = sprintf( __( 'Gesamt %d', Powerform::DOMAIN ), $product_cost );
				} else {
					if ( !empty( $output ) ) {
