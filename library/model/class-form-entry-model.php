 ...
 * @since 1.0
 */
class Powerform_Form_Entry_Model {
	...
	public function __construct( $entry_id = null ) {
		$this->table_name      = Powerform_Database_Tables::get_table_name( Powerform_Database_Tables::FORM_ENTRY );
		$this->table_meta_name = Powerform_Database_Tables::get_table_name( Powerform_Database_Tables::FORM_ENTRY_META );
		...
		if ( is_numeric( $entry_id ) && $entry_id > 0 ) {
			...
			if ( ! empty( $this->form_id ) ) {
				 ...
				 */
				$entry_types = apply_filters( 'powerform_addon_entry_types', $entry_types );
	...
	public function load_meta( $db = false ) {
		...
		$table_meta_name = Powerform_Database_Tables::get_table_name( Powerform_Database_Tables::FORM_ENTRY_META );
	...
	public function save( $data_created = null ) {
		...
		wp_cache_delete( $this->form_id, 'powerform_total_entries' );
		wp_cache_delete( 'all_form_types', 'powerform_total_entries' );
		wp_cache_delete( $this->entry_type . '_form_type', 'powerform_total_entries' );
	...
	public static function count_entries( $form_id, $db = false ) {
		...
		$cache_key     = 'powerform_total_entries';
	 ...
	 *
	 * @since 1.0
	 * @since 1.1 Add init addons and Add hooks `powerform_before_delete_entry`
	 *
	 * @param           $form_id
	 * @param           $entries
	 * @param bool|wpdb $db
	...
	public static function delete_by_entrys( $form_id, $entries, $db = false ) {
		...
		powerform_maybe_log( 'delete_by_entrys', $form_id, $entries );
		$entries_to_array = explode( ',', $entries );
		powerform_maybe_log( 'delete_by_entrys', $form_id, $entries_to_array );
		 ...
		 */
		do_action_ref_array( 'powerform_before_delete_entries', array( $form_id, $entries ) );
		...
		wp_cache_delete( $form_id, 'powerform_total_entries' );
		wp_cache_delete( 'all_form_types', 'powerform_total_entries' );

		$model = powerform_get_model_from_id( $form_id );
		if ( is_object( $model ) ) {
			wp_cache_delete( $model->get_post_type() . '_form_type', 'powerform_total_entries' );
		}
	}
	...
	public static function delete_by_entry( $form_id, $entry_id, $db = false ) {
		...
		$table_name      = Powerform_Database_Tables::get_table_name( Powerform_Database_Tables::FORM_ENTRY );
		$table_meta_name = Powerform_Database_Tables::get_table_name( Powerform_Database_Tables::FORM_ENTRY_META );
		$cache_key       = 'Powerform_Form_Entry_Model';

		$form_id  = (int) $form_id;
		$entry_id = (int) $entry_id;
		powerform_maybe_log( 'powerform_before_delete_entry', $form_id, $entry_id );
		/**
		 * Fires just before an entry getting deleted
		 *
		 * @since 1.1
		...
		if ( ! isset( self::$connected_addons[ $form_id ] ) ) {
			...
			foreach ( $connected_addons as $connected_addon ) {
				try {
					...
					powerform_addon_maybe_log( $connected_addon->get_slug(), 'failed to get_addon_form_hooks', $e->getMessage() );
				}
			}
		}
	 ...
	 */
	public static function attach_addons_on_before_delete_entry( $form_id, Powerform_Form_Entry_Model $entry_model ) {
		...
		foreach ( $connected_addons as $connected_addon ) {
			try {
				$form_hooks      = $connected_addon->get_addon_form_hooks( $form_id );
				$addon_meta_data = powerform_find_addon_meta_data_from_entry_model( $connected_addon, $entry_model );
	...
	public static function query_entries( $args, &$count ) {
		...
		$entries_table_name      = Powerform_Database_Tables::get_table_name( Powerform_Database_Tables::FORM_ENTRY );
		 ...
		 * @param array  $args
		 */
		$where = apply_filters( 'powerform_query_entries_where', $where, $args );
		 ...
		 * @param array  $args
		 */
		$group_by = apply_filters( 'powerform_query_entries_group_by', $group_by, $args );
		 ...
		 * @param array  $args
		 */
		$order_by = apply_filters( 'powerform_query_entries_order_by', $order_by, $args );
		 ...
		 * @param array  $args
		 */
		$limit = apply_filters( 'powerform_query_entries_limit', $limit, $args );
