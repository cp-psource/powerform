 ...
 */
abstract class Powerform_Base_Form_Model {
	const META_KEY = 'powerform_form_meta';
	/**
	 * Form ID
	 *
	 * @int
	 ...
	 *
	 * @var Powerform_Form_Field_Model[]
	 */
	public $fields = array();
	...
	/**
	 * @since 1.0
	 * @return Powerform_Form_Field_Model[]
	 */
	 ...
	 * @param $slug
	 *
	 * @return Powerform_Form_Field|null
	 */
	...
	public function get_all_paged( $current_page = 1, $per_page = null, $status = '' ) {
		if ( is_null( $per_page ) ) {
			$per_page = powerform_form_view_per_page();
		}
		$args = array(
			'post_type'      => $this->post_type,
			'post_status'    => 'any',
	...
	public function get_fields_grouped() {
		...
		foreach ( $this->fields as $field ) {
			/** @var Powerform_Form_Field_Model $field */
	 ...
	 *
	 * @since 1.4
	 * Override-able, but use of hook `powerform_{$module_type}_model_to_exportable_data` encouraged
	 * @return array
	 */
	public function to_exportable_data() {
		...
		$exportable_data = apply_filters( "powerform_{$module_type}_model_to_exportable_data", $exportable_data, $module_type, $model_id );
	 ...
	 * @param string $module
	 *
	 * @return self|Powerform_Custom_Form_Model|Powerform_Poll_Form_Model|Powerform_Quiz_Form_Model|WP_Error
	 */
	public static function create_from_import_data( $import_data, $module = __CLASS__ ) {
		try {
			...
			switch ( $type ) {
				case 'quiz':
					$post_type = 'powerform_quizzes';
					break;
				case 'poll':
					$post_type = 'powerform_polls';
					break;
				default:
			...
			/** @var Powerform_Base_Form_Model|Powerform_Poll_Form_Model|Powerform_Quiz_Form_Model|Powerform_Custom_Form_Model $model */
			...
			$code = $e->getCode();
			if ( empty( $code ) ) {
				$code = 'powerform_import_model_error';
			}
		 ...
		 * @since 1.4
		 *
		 * @param Powerform_Base_Form_Model|WP_Error $model
		 * @param array                               $import_data
		 * @param string                              $module
		 */
		$model = apply_filters( 'powerform_import_model', $model, $import_data, $module );
	...
	public function is_ajax_load( $force = false ) {
		...
		// from settings
		$settings_enabled = get_option( 'powerform_module_enable_load_ajax', false );

		// from constant
		$enabled = defined( 'POWERFORM_MODULE_ENABLE_LOAD_AJAX' ) && POWERFORM_MODULE_ENABLE_LOAD_AJAX;
		 ...
		 * @return bool
		 */
		$enabled = apply_filters( 'powerform_module_is_ajax_load', $enabled, $settings_enabled, $force );
	...
	public function is_use_donotcachepage_constant() {
		...
		// from settings
		$settings_enabled = get_option( 'powerform_module_use_donotcachepage', false );

		// from constant
		$enabled = defined( 'POWERFORM_MODULE_USE_DONOTCACHEPAGE' ) && POWERFORM_MODULE_USE_DONOTCACHEPAGE;
