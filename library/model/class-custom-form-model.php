 ...
 * Author: Hoang Ngo
 */
class Powerform_Custom_Form_Model extends Powerform_Base_Form_Model {

	protected $post_type = 'powerform_forms';
	 ...
	 * @param bool $to_array
	 *
	 * @return array|null|Powerform_Form_Field_Model
	 */
	 ...
	 * @param $data
	 *
	 * @return bool|Powerform_Custom_Form_Model
	 */
	...
	public function form_can_submit() {
		...
		return apply_filters( 'powerform_cform_form_is_submittable', $can_show, $this->id, $form_settings );
	}
	...
	public function form_is_visible() {
		...
		return apply_filters( 'powerform_cform_form_is_visible', $can_show, $this->id, $form_settings );
	}
	...
	public function to_exportable_data() {
		...
		if ( Powerform::is_export_integrations_feature_enabled() ) {
			add_filter( 'powerform_form_model_to_exportable_data', array( $this, 'export_integrations_data' ), 1, 1 );
		}

		$exportable_data = parent::to_exportable_data();

		// avoid filter executed on next cycle
		remove_filter( 'powerform_form_model_to_exportable_data', array( $this, 'export_integrations_data' ), 1 );
	...
	public function export_integrations_data( $exportable_data ) {
		...
		$connected_addons = powerform_get_addons_instance_connected_with_form( $model_id );
		 ...
		 */
		$exportable_integrations         = apply_filters( 'powerform_form_model_export_integrations_data', $exportable_integrations, $model_id );
	 ...
	 * Import Model
	 *
	 * add filter `powerform_import_model`
	 *
	 * @since 1.4
	 *
	 * @param        $import_data
	 * @param string $module
	 *
	 * @return Powerform_Base_Form_Model|WP_Error
	 */
	public static function create_from_import_data( $import_data, $module = __CLASS__ ) {
		if ( Powerform::is_import_integrations_feature_enabled() ) {
			add_filter( 'powerform_import_model', array( 'Powerform_Custom_Form_Model', 'import_integrations_data' ), 1, 3 );
		}

		$model = parent::create_from_import_data( $import_data, $module );

		// avoid filter executed on next cycle
		remove_filter( 'powerform_import_model', array( 'Powerform_Custom_Form_Model', 'import_integrations_data' ), 1 );
	...
	public function is_prevent_store() {
		 ...
		 */
		$is_prevent_store = apply_filters( 'powerform_custom_form_is_prevent_store', $is_prevent_store, $form_id, $form_settings );
	 ...
	 * @since 1.5.3
	 *
	 * @return bool|Powerform_Form_Field_Model
	 */
	public function get_captcha_field() {
		...
		$captcha_field = apply_filters( 'powerform_custom_form_get_captcha_field', $captcha_field, $form_id, $fields );
	...
	public function is_ajax_load( $force = false ) {
		 ...
		 * @return bool
		 */
		$enabled = apply_filters( 'powerform_custom_form_is_ajax_load', $enabled, $global_enabled, $form_id, $form_settings );
