...
class Powerform_API {
	 ...
	 *
	 * @return Powerform_Custom_Form_Model[]|WP_Error
	 */
	public static function get_forms( $form_ids = null, $page = 1, $per_page = 10, $status = '' ) {
		...
		if ( is_null( $form_ids ) ) {
			$temp = Powerform_Custom_Form_Model::model()->get_all_paged( $page, $per_page, $status );
	 ...
	 *
	 * @return Powerform_Custom_Form_Model|WP_Error Custom Form Model on success or WP_Error otherwise
	 */
	public static function get_form( $form_id ) {
		...
		if ( empty( $form_id ) ) {
			return new WP_Error( 'missing_id', __( 'Formular ID ist erforderlich!', Powerform::DOMAIN ) );
		}

		$model = Powerform_Custom_Form_Model::model()->load( $form_id );
		if ( ! $model instanceof Powerform_Custom_Form_Model ) {
			return new WP_Error( 'custom_form_not_found', __( 'Formular nicht gefunden!', Powerform::DOMAIN ) );
		}
	...
	public static function delete_form( $form_id ) {
		...
		if ( is_object( $model ) ) {
			...
			return new WP_Error( 'not_found', sprintf( __( 'Kann kein Formular finden mit ID: %s', Powerform::DOMAIN ), $form_id ), $form_id );
		}
	}
	 ...
	 */
	public static function add_form( $name, $wrappers = array(), $settings = array(), $status = Powerform_Custom_Form_Model::STATUS_PUBLISH ) {
		// Initialize API
		self::initialize();

		if ( empty( $name ) ) {
			return new WP_Error( 'missing_name', __( 'Formularname ist erforderlich!', Powerform::DOMAIN ) );
		}
	...
	public static function update_form( $id, $wrappers = array(), $settings = array(), $status = '' ) {
		...
		if ( false === $id ) {
			return new WP_Error( 'form_save_error', __( 'Beim Aktualisieren des Formulars ist ein Problem aufgetreten', Powerform::DOMAIN ) );
		}
	...
	public static function get_form_wrappers( $id ) {
		...
		return new WP_Error( 'missing_fields', __( "Formular hat keine Felder", Powerform::DOMAIN ) );
	}
	...
	public static function get_form_wrapper( $form_id, $id ) {
		...
		if ( empty( $id ) ) {
			return new WP_Error( 'missing_id', __( 'Wrapper ID ist erforderlich!', Powerform::DOMAIN ) );
		}
		...
		return new WP_Error( 'missing_field', __( "Wrapper existiert nicht", Powerform::DOMAIN ) );
	}
	...
	public static function move_form_wrapper( $form_id, $id, $new_position ) {
		...
		if ( false === $form_id ) {
			return new WP_Error( 'form_save_error', __( 'Beim Verschieben des Formularumschlags ist ein Problem aufgetreten', Powerform::DOMAIN ) );
		}
	...
	public static function get_form_fields_by_type( $id, $type ) {
		...
		return new WP_Error( 'missing_fields', __( "Keine Felder mit diesem Typ", Powerform::DOMAIN ) );
	}
	 ...
	 * @param int    $form_id
	 * @param string $id
	 * @param bool   $to_array whether to return as array or Powerform_Form_Field_Model
	 *
	 * @return array|Powerform_Form_Field_Model|WP_Error
	 */
	public static function get_form_field( $form_id, $id, $to_array = true ) {
		...
		if ( empty( $id ) ) {
			return new WP_Error( 'missing_id', __( 'Field ID is required!', Powerform::DOMAIN ) );
		}
	...
	public static function update_form_field( $form_id, $id, $data = array() ) {
		...
		if ( empty( $data ) ) {
			return new WP_Error( 'missing_data', __( 'Field data is required!', Powerform::DOMAIN ) );
		}
	...
	public static function add_form_field( $form_id, $type, $data = array(), $wrapper = null ) {
		...
		if ( empty( $type ) ) {
			return new WP_Error( 'missing_type', __( 'Field Type is required!', Powerform::DOMAIN ) );
		}
	...
	public static function move_form_field( $form_id, $id, $new_position, $new_wrapper_id = null ) {
		...
		if ( $is_cross_wrapper && count( $new_wrapper['fields'] ) >= self::MAX_CUSTOM_FORM_FIELDS_PER_WRAPPER ) {
			return new WP_Error( 'target_wrapper_is_full', __( 'Target Wrapper is already full.', Powerform::DOMAIN ), $new_wrapper );
		}
	 ...
	 * @param string      $status (draft,publish,any)
	 *
	 * @return Powerform_Poll_Form_Model[]|WP_Error
	 */
	public static function get_polls( $poll_ids = null, $page = 1, $per_page = 10, $status = '' ) {
		...
		if ( is_null( $poll_ids ) ) {
			$temp = Powerform_Poll_Form_Model::model()->get_all_paged( $page, $per_page, $status );
			if ( isset( $temp['models'] ) && is_array( $temp['models'] ) ) {
				return $temp['models'];
			}
	 ...
	 *
	 * @return Powerform_Poll_Form_Model|WP_Error Powerform_Poll_Form_Model on success, or WP_Error otherwise
	 */
	...
	public static function delete_poll( $poll_id ) {
		...
		if ( is_object( $model ) ) {
			...
			// Delete form entries
			Powerform_Form_Entry_Model::delete_by_form( $poll_id );
	 ...
	 */
	public static function add_poll( $name, $fields = array(), $settings = array(), $status = Powerform_Poll_Form_Model::STATUS_PUBLISH ) {
		// Initialize API
		self::initialize();
	 ...
	 * @param string     $status
	 *
	 * @return Powerform_Quiz_Form_Model[]|WP_Error
	 */
	public static function get_quizzes( $quiz_ids = null, $page = 1, $per_page = 10, $status = '' ) {
		...
		if ( is_null( $quiz_ids ) ) {
			$temp = Powerform_Quiz_Form_Model::model()->get_all_paged( $page, $per_page, $status );
			if ( isset( $temp['models'] ) && is_array( $temp['models'] ) ) {
				return $temp['models'];
			}
	 ...
	 *
	 * @return Powerform_Quiz_Form_Model|WP_Error, Quiz Object on success, or WP_Error otherwise
	 */
	...
	public static function delete_quiz( $quiz_id ) {
		...
		if ( is_object( $model ) ) {
			...
			// Delete form entries
			Powerform_Form_Entry_Model::delete_by_form( $quiz_id );
	 ...
	 * Get entries objects
	 *
	 * @uses  Powerform_Form_Entry_Model::get_entries
	 *
	 * @since 1.2
	 *
	 * @param int $form_id Form ID
	 *
	 * @return Powerform_Form_Entry_Model[]|WP_Error
	 */
	 ...
	 * Delete entry
	 *
	 * @uses  Powerform_Form_Entry_Model::delete_by_entry
	 *
	 * @since 1.2
	 *
	 * @param $form_id
	...
	public static function delete_entry( $form_id, $entry_id ) {
		...
		// Delete entry
		Powerform_Form_Entry_Model::delete_by_entry( $form_id, $entry_id );
	 ...
	 *
	 * @uses  Powerform_Form_Entry_Model::delete_by_entrys
	 *
	 * @since 1.2
	 *
	 * @param $form_id
	...
	public static function delete_entries( $form_id, $entries_ids ) {
		...
		// Delete entries
		Powerform_Form_Entry_Model::delete_by_entrys( $form_id, $entries_ids );
	 ...
	 * Count entries
	 *
	 * @uses  Powerform_Form_Entry_Model::count_entries
	 *
	 * @since 1.2
	 *
	 * @param $form_id
	 ...
	 *
	 * @see    Powerform_API::add_form_entry()
	 *
	 ...
	 * @param array $entry_meta
	 *
	 * @return bool|Powerform_Form_Entry_Model|WP_Error
	 */
