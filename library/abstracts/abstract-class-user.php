...
add_action( 'powerform_custom_form_build_fields_mappers', array( 'Powerform_User', 'remove_password_from_all_fields' ) );
add_action( 'powerform_custom_form_filter_fields', array( 'Powerform_User', 'remove_password_from_all_fields' ) );
//Change form settings
add_filter( 'powerform_builder_data_settings_before_saving', array( 'Powerform_User', 'change_form_settings' ), 11, 2 );
//Remove password field from merge tags {all_fields}, {all_non_empty_fields}
add_filter( 'powerform_custom_form_before_form_fields', array( 'Powerform_User', 'remove_password_from_all_fields' ) );

/**
 * Class Powerform_User
 *
 * @since 1.11
 */
abstract class Powerform_User {
	...
	public function __construct() {
		...
		add_filter( 'powerform_custom_form_mail_data', array( $this, 'remove_password_in_form_mail_data' ) );
	}
	...
	protected static function get_encryption_key() {
		return 'powerform_encryption_key' . wp_salt( 'nonce' );
	}
	...
	public static function openssl_encrypt( $text, $encryption_key = null, $cipher_name = 'aes-256-ctr', $mac_key = null ) {
		if ( function_exists( 'openssl_encrypt' ) ) {
			...
			if ( empty( $mac_key ) ) {
				$mac_key = 'powerform_encryption_mac' . wp_salt( 'nonce' );
			}
	...
	public static function change_form_settings( $sanitized_settings, $settings ) {
		...
		foreach ( $message_settings as $form_key ) {
			if ( isset( $settings[ $form_key ] ) && ! empty( $settings[ $form_key ] ) ) {
				 ...
				 * @since 1.14.1
				 */
				$allowed_html = apply_filters( 'powerform_change_form_allowed_html', $allowed_html );
