...
/**
 * Class Powerform_Addon_Autoload
 * Handling Autoloader
 *
 * @since 1.1
 */
class Powerform_Addon_Autoload {
	...
	/**
	 * Powerform_Addon_Autoload constructor.
	...
	public function load() {
		 ...
		 */
		$pro_addons_dir = apply_filters( 'powerform_addon_pro_addons_dir', $pro_addons_dir );

		// All of Powerform Official Addons must be registered here with fallback array
		// fallback array will be used to display pro addons on the list of addons, without files on `/pro` being available
		if ( empty( $pro_addons ) ) {
			$pro_addons = powerform_get_pro_addon_list();
		}
		...
		$pro_slugs        = Powerform_Addon_Loader::get_instance()->get_addons()->get_slugs();
