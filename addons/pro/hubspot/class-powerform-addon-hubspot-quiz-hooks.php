...
/**
 * Class Powerform_Addon_Hubspot_Quiz_Hooks
 *
 * @since 1.0 HubSpot Addon
 *
 */
class Powerform_Addon_Hubspot_Quiz_Hooks extends Powerform_Addon_Quiz_Hooks_Abstract {
	 ...
	 *
	 * @since 1.0 HubSpot Addon
	 * @var Powerform_Addon_Hubspot_Quiz_Settings | null
	 */
	protected $quiz_settings_instance;

	/**
	 * Powerform_Addon_Hubspot_Quiz_Hooks constructor.
	 *
	 * @since 1.0 HubSpot Addon
	 *
	...
	public function __construct( Powerform_Addon_Abstract $addon, $quiz_id ) {
		...
		$this->_submit_quiz_error_message = __( 'HubSpot failed to process submitted data. Please check your quiz and try again', Powerform::DOMAIN );
	}
	...
	public function add_entry_fields( $submitted_data, $form_entry_fields = array() ) {
		 ...
		 * @param Powerform_Addon_Hubspot_Quiz_Settings $quiz_settings_instance HubSpot Addon quiz Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_hubspot_quiz_submitted_data',
			$submitted_data,
			$quiz_id,
		 ...
		 * @param Powerform_Addon_Hubspot_Quiz_Settings $quiz_settings_instance HubSpot Addon Quiz Settings instance
		 */
		do_action( 'powerform_addon_hubspot_before_send_message', $quiz_id, $submitted_data, $quiz_settings_instance );
		 ...
		 */
		$data = apply_filters(
			'powerform_addon_hubspot_entry_fields',
			$entry_fields,
			$quiz_id,
			$submitted_data,
			$quiz_settings_instance
	...
	private function get_status_on_contact_sync( $connection_id, $submitted_data, $connection_settings, $form_entry_fields ) {
		...
		try {
			...
			if ( ! isset( $submitted_data[ $email_element_id ] ) || empty( $submitted_data[ $email_element_id ] ) ) {
				/* translators: ... */
				throw new Powerform_Addon_Hubspot_Exception( __( 'Email on element %1$s not found or not filled on submitted data.', Powerform::DOMAIN ) );
			}
			 ...
			 */
			$args = apply_filters(
				'powerform_addon_hubspot_create_contact_args',
				$args,
				$quiz_id,
				$connection_id,
				$submitted_data,
			...
			if ( '1' === $create_ticket ) {
				...
				$ticket_name                  = str_ireplace( '{quiz_name}', powerform_get_name_from_model( $this->quiz ), $connection_settings['ticket_name'] );
				$ticket_name                  = powerform_addon_replace_custom_vars( $ticket_name, $submitted_data, $this->lead_model, $form_entry_fields, false );
				$ticket['ticket_name']        = $ticket_name;
				$ticket_description           = str_ireplace( '{quiz_name}', powerform_get_name_from_model( $this->quiz ), $connection_settings['ticket_description'] );
				$ticket_description           = powerform_addon_replace_custom_vars( $ticket_description, $submitted_data, $this->lead_model, $form_entry_fields, false );
			...
			return array(
				...
				'description'     => __( 'Successfully send data to HubSpot', Powerform::DOMAIN ),
			...
			powerform_addon_maybe_log( __METHOD__, 'Failed to Send to HubSpot' );

			$addon_entry_fields = array(
				'is_sent'         => false,
				'description'     => $e->getMessage(),
				'connection_name' => $connection_settings['name'],
				'data_sent'       => ( ( $api instanceof Powerform_Addon_Hubspot_Wp_Api ) ? $api->get_last_data_sent() : array() ),
				'data_received'   => ( ( $api instanceof Powerform_Addon_Hubspot_Wp_Api ) ? $api->get_last_data_received() : array() ),
				'url_request'     => ( ( $api instanceof Powerform_Addon_Hubspot_Wp_Api ) ? $api->get_last_url_request() : '' ),
				'contact_id'      => null,
				'ticket_id'       => null,
			);
	 ...
	 * subentries included are defined in @see Powerform_Addon_Hubspot_Quiz_Hooks::get_additional_entry_item()
	 *
	 * @since 1.0 HubSpot Addon
	 *
	...
	public function on_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$addon_meta_data = apply_filters(
			'powerform_addon_hubspot_metadata',
			$addon_meta_data,
			$quiz_id,
			$quiz_settings_instance
		);
	 ...
	 * - Info : Text that are generated by addon when building and sending data to HubSpot @see Powerform_Addon_Hubspot_Quiz_Hooks::add_entry_fields()
	 * - Below subentries will be added if full log enabled, @see Powerform_Addon_Hubspot::is_show_full_log() @see POWERFORM_ADDON_HUBSPOT_SHOW_FULL_LOG
	...
	private function get_additional_entry_item( $addon_meta_data ) {
		...
		$additional_entry_item = array(
			'label' => __( 'HubSpot Integration', Powerform::DOMAIN ),
			'value' => '',
		);
		...
		if ( isset( $status['is_sent'] ) ) {
			...
			$sub_entries[] = array(
				'label' => __( 'Sent To HubSpot', Powerform::DOMAIN ),
				'value' => $is_sent,
			);
		}
		...
		if ( Powerform_Addon_Hubspot::is_show_full_log() ) {
			// too long to be added on entry data enable this with `define('POWERFORM_ADDON_HUBSPOT_SHOW_FULL_LOG', true)`
			...
			if ( isset( $status['data_sent'] ) ) {
				$sub_entries[] = array(
					'label' => __( 'Data sent to HubSpot', Powerform::DOMAIN ),
			...
			if ( isset( $status['data_received'] ) ) {
				$sub_entries[] = array(
					'label' => __( 'Data received from HubSpot', Powerform::DOMAIN ),
	 ...
	 * its called `HubSpot Info` which can be translated on powerform lang
	 *
	 * @since 1.0 HubSpot Addon
	 * @return array
	 */
	public function on_export_render_title_row() {
		...
		$export_headers = array(
			'info' => __( 'HubSpot Info', Powerform::DOMAIN ),
		);
		...
		$export_headers = apply_filters(
			'powerform_addon_hubspot_export_headers',
			$export_headers,
			$quiz_id,
			$quiz_settings_instance
		);
	...
	public function on_export_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		 ...
		 * @param int $quiz_id current Quiz ID
		 * @param Powerform_Quiz_Entry_Model $entry_model Form Entry Model
		 * @param array $addon_meta_data meta data saved by addon on entry fields
		 * @param Powerform_Addon_Hubspot_Quiz_Settings $quiz_settings_instance HubSpot Addon Quiz Settings instance
		 */
		$export_columns = apply_filters(
			'powerform_addon_hubspot_export_columns',
			$export_columns,
			$quiz_id,
			$entry_model,
			$addon_meta_data,
	...
	public function on_before_delete_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		 ...
		 */
		do_action(
			'powerform_addon_hubspot_on_before_delete_submission',
			$quiz_id,
			$entry_model,
			$addon_meta_data,
			$quiz_settings_instance
		);
		if ( ! Powerform_Addon_Hubspot::is_enable_delete_member() ) {
		...
		try {
			...
			$contact_to_delete = apply_filters(
				'powerform_addon_hubspot_contact_to_delete',
				$contact_to_delete,
				$quiz_id,
				$addon_meta_data,
				$quiz_settings_instance
			...
			$ticket_to_delete = apply_filters(
				'powerform_addon_hubspot_ticket_to_delete',
				$ticket_to_delete,
				$quiz_id,
				$addon_meta_data,
				$quiz_settings_instance
			...
			$wp_error
				= new WP_Error( 'powerform_addon_hubspot_delete_contact', $e->getMessage() );
