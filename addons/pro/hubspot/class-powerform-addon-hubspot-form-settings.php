...
require_once dirname( __FILE__ ) . '/class-powerform-addon-hubspot-form-settings-exception.php';

/**
 * Class Powerform_Addon_Hubspot_Form_Settings
 * Handle how form settings displayed and saved
 *
 * @since 1.0 HubSpot Addon
 */
class Powerform_Addon_Hubspot_Form_Settings extends Powerform_Addon_Form_Settings_Abstract {
	...
	/**
	 * Powerform_Addon_Hubspot_Form_Settings constructor.
	 *
	 * @since 1.0 HubSpot Addon
	 *
	 ...
	 *
	 * @return array
	 * @throws Powerform_Addon_Hubspot_Exception
	...
	public function map_fields( $submitted_data ) {
		$template = powerform_addon_hubspot_dir() . 'views/form-settings/create-contact.php';
		...
		$fields                    = array(
			...
			'firstname' => __( 'First Name', Powerform::DOMAIN ),
			'lastname'  => __( 'Last Name', Powerform::DOMAIN ),
			'jobtitle'  => __( 'Job Title', Powerform::DOMAIN ),
		);
		...
		} catch ( Powerform_Addon_Hubspot_Form_Settings_Exception $e ) {
		...
		if ( $is_submit ) {
			...
			try {
				$input_exceptions = new Powerform_Addon_Hubspot_Form_Settings_Exception();
		...
		if ( $this->map_fields_is_completed( array( 'multi_id' => $multi_id ) ) ) {
			$buttons['disconnect']['markup'] = Powerform_Addon_Abstract::get_button_markup(
				...
				esc_html__( 'Deactivate this HubSpot Integration from this Form.', Powerform::DOMAIN )
			);
		}

		$buttons['next']['markup'] = '<div class="sui-actions-right">' .
			Powerform_Addon_Abstract::get_button_markup( esc_html__( 'Continue', Powerform::DOMAIN ), 'powerform-addon-next' ) .
		                             '</div>';
	...
	public function create_ticket( $submitted_data ) {
		$template = powerform_addon_hubspot_dir() . 'views/form-settings/create-ticket.php';
		...
		try {
			...
			if ( empty( $pipeline ) ) {
				throw new Powerform_Addon_Hubspot_Exception( __( 'No pipeline found on your HubSpot account. Please create one.', Powerform::DOMAIN ) );
			}

			if ( empty( $status ) ) {
				throw new Powerform_Addon_Hubspot_Exception( __( 'No status found on your HubSpot account. Please create one.', Powerform::DOMAIN ) );
			}
		...
		if ( $this->create_ticket_is_completed( array( 'multi_id' => $multi_id ) ) ) {
			...
			$buttons['next']['markup']       = '<div class="sui-actions-right">' .
											   Powerform_Addon_Abstract::get_button_markup( esc_html__( 'Update', Powerform::DOMAIN ), 'powerform-addon-next sui-button-blue' ) .
											   '</div>';
			...
			$buttons['next']['markup'] = '<div class="sui-actions-right" id="' . $buttonID . '">' .
										 Powerform_Addon_Abstract::get_button_markup( esc_html__( 'Activate', Powerform::DOMAIN ), 'powerform-addon-next sui-button-blue' ) .
										 '</div>';
		}
