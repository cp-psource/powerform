...
<div class="integration-header">
	...
	<h3 id="dialogTitle2" class="sui-box-title"><?php echo esc_html( __( 'Create Contact', Powerform::DOMAIN ) ); ?></h3>

	<p class="sui-description" style="max-width: 400px; margin: 20px auto 0; line-height: 22px;"><?php esc_html_e( 'Let\'s start with choosing a HubSpot list and matching up your quiz lead fields with the default HubSpot contact fields to make sure weâ€™re sending data to the right place.', Powerform::DOMAIN ); ?></p>
...
<form style="display: block; margin-top: -10px;">
	...
	<div tabindex="0" role="group" class="sui-form-field<?php echo esc_attr( ! empty( $vars['list_id_error'] ) ? ' sui-form-field-error' : '' ); ?>"<?php echo empty( $vars['lists'] ) ? ' disabled="disabled"' : ''; ?>>
		...
		<label for="hubspot-list-id" id="hubspot-list-id-label" class="sui-label">
			<?php esc_html_e( 'HubSpot List (optional)', Powerform::DOMAIN ); ?>
			<span class="sui-label-note"><?php esc_html_e( 'Static list only', Powerform::DOMAIN ); ?></span>
		</label>
		...
		<select id="hubspot-list-id" class="<?php echo esc_attr( $list_selector_class ); ?>" aria-labelledby="hubspot-list-id-label" aria-describedby="hubspot-list-id-desc hubspot-list-id-error" name="list_id">
			...
			if ( ! empty( $vars['lists'] ) ) { ?>
				...
				<option value=""><?php esc_html_e( 'Select a list', Powerform::DOMAIN ); ?></option>
				...
				<option value=""><?php esc_html_e( 'No static lists found on your HubSpot account', Powerform::DOMAIN ); ?></option>
			<?php } ?>
		...
		<span id="hubspot-list-id-desc" class="sui-description"><?php esc_html_e( 'You can optionally add the contact to a static HubSpot list. Leave it empty to create a contact without adding it to a list.', Powerform::DOMAIN ); ?></span>

	</div>

	<div tabindex="0" role="group" class="sui-form-field" style="margin-bottom: 0;">

		<label for="hubspot-list-id" id="hubspot-list-id-label" class="sui-label"><?php esc_html_e( 'Field Mapping', Powerform::DOMAIN ); ?></label>
		...
		<table class="sui-table" style="margin-top: 5px; margin-bottom: 0;">
			...
			<tr>
				<th><?php esc_html_e( 'HubSpot Fields', Powerform::DOMAIN ); ?></th>
				<th><?php esc_html_e( 'Powerform Fields', Powerform::DOMAIN ); ?></th>
			</tr>
						...
						<td>
                            ...
                            <div class="sui-form-field <?php echo esc_attr( ! empty( $current_error ) ? 'sui-form-field-error' : '' ); ?>"<?php echo ( ! empty( $current_error ) ) ? ' style="padding-top: 5px;"' : ''; ?>>
                                <select class="sui-select" name="fields_map[<?php echo esc_attr( $key ); ?>]">
									...
									<?php if ( ! empty( $powerform_fields ) ) :
										foreach ( $powerform_fields as $powerform_field ) : ?>
                            ...
                            <div class="fui-select-with-delete">
                                ...
                                <div class="sui-form-field">
                                    <select class="sui-select" name="custom_field[]">
                                        <option value=""><?php esc_html_e( 'None', Powerform::DOMAIN ); ?></option>
										<?php
										if ( ! empty( $powerform_fields ) ) :
											foreach ( $powerform_fields as $powerform_field ) : ?>
                                                <option value="<?php echo esc_attr( $powerform_field['element_id'] ); ?>" <?php selected( $custom_field, $powerform_field['element_id'] ); ?>>
													<?php echo esc_html( $powerform_field['field_label'] . ' | ' . $powerform_field['element_id'] ); ?>
                                                </option>
											<?php endforeach;
										endif;
            ...
            <tr class="add-additional-field">
                <td>
                    <div class="sui-button sui-button-ghost add-hubspot-field">
						...
						<?php esc_html_e( 'Add Additional field', Powerform::DOMAIN ); ?>
                    </div>
                </td>
                <td></td>
            </tr>
