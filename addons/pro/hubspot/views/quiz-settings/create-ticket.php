...
<div class="integration-header">
	...
	<h3 id="dialogTitle2" class="sui-box-title"><?php echo esc_html( __( 'Create Ticket', Powerform::DOMAIN ) ); ?></h3>

	<p class="sui-description" style="max-width: 400px; margin: 20px auto 0; line-height: 22px;"><?php esc_html_e( 'In addition to adding a new contact to your HubSpot account, you can also create a HubSpot ticket for each quiz lead submission.', Powerform::DOMAIN ); ?></p>
...
<form style="display: block; margin-top: -10px; margin-bottom: 0;">
	...
	<div class="fui-section-toggle">
		...
		<label for="create-ticket" id="create-ticket-label" class="sui-toggle-label"><?php esc_html_e( 'Create a HubSpot ticket for each submission', Powerform::DOMAIN ); ?></label>
	...
	>
		<?php if ( empty( $vars['re-authorize'] ) && ! empty( $vars['token'] ) ) { ?>
            <div class="sui-notice sui-notice-info">
                <p style="margin-bottom: 5px;"><strong><?php esc_html_e( 'Authorize Powerform to access HubSpot tickets', Powerform::DOMAIN ); ?></strong></p>
				<p style="margin-top: 5px; margin-bottom: 10px;"><?php esc_html_e( 'Powerform requires additional permissions to create HubSpot tickets. Note that you will be taken to HubSpot website to grant Powerform access to HubSpot tickets and redirected back here.', Powerform::DOMAIN ); ?></p>
                <p style="margin-top: 10px;"><a href="<?php echo esc_attr( $vars['auth_url'] ); ?>" target="_blank" class="sui-button sui-button-primary powerform-addon-connect"><?php esc_html_e( 'Authorize', Powerform::DOMAIN ); ?></a></p>
            </div>
		<?php } else { ?>
		...
		<div class="sui-form-field">
			...
			<label for="hubspot-support-request" id="hubspot-support-request-label" class="sui-label"><?php esc_html_e( 'Pipeline', Powerform::DOMAIN ); ?></label>
		...
		<div class="sui-form-field">
			...
			<label for="hubspot-ticket-status" id="hubspot-ticket-status-label" class="sui-label"><?php esc_html_e( 'Ticket Status', Powerform::DOMAIN ); ?></label>
		...
		<div class="sui-form-field<?php echo ( ! empty( $vars['ticket_name_error'] ) ) ? ' sui-form-field-error' : ''; ?>">
			...
			<label for="ticket-name-input" id="ticket-name-input-label" class="sui-label">
				<?php esc_html_e( 'Ticket Name', Powerform::DOMAIN ); ?>
				<span class="sui-label-note"><?php esc_html_e( 'Use the "+" icon to add form fields', Powerform::DOMAIN ); ?></span>
			</label>
			...
			<div class="sui-insert-variables">
				...
				/>
                    ...
                    <select id="select-ticket-name" class="select-field">
                        <option value="{quiz_name}" data-content="{quiz_name}"><?php esc_html_e( 'Quiz Name', Powerform::DOMAIN ); ?></option>
						<?php
		...
		<div class="sui-form-field">
			...
			<label for="ticket-description" id="ticket-description-label" class="sui-label">
				<?php esc_html_e( 'Ticket Description (optional)', Powerform::DOMAIN ); ?>
				<span class="sui-label-note"><?php esc_html_e( 'Use the "+" icon to add form fields', Powerform::DOMAIN ); ?></span>
			</label>
		...
		<div class="sui-form-field">
			...
			<label for="hubspot-support-file" id="hubspot-support-file-label" class="sui-label"><?php esc_html_e( 'Supported File (optional)', Powerform::DOMAIN ); ?></label>
			...
			>
				...
				<option value=""><?php esc_html_e( 'Select a file upload field', Powerform::DOMAIN ); ?></option>
	...
	(function ($) {
		$(document).ready(function (e) {
			...
			$('#hubspot-support-request').on('change', function () {
				let value = $(this).val();
				$.ajax({
					url: '<?php echo powerform_ajax_url();// phpcs:ignore ?>',
					type: "POST",
					data: {
						action: "powerform_hubspot_support_request",
						value: value
					},
