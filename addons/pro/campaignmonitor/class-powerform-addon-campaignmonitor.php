...
require_once dirname( __FILE__ ) . '/class-powerform-addon-campaignmonitor-exception.php';
require_once dirname( __FILE__ ) . '/lib/class-powerform-addon-campaignmonitor-wp-api.php';

/**
 * Class Powerform_Addon_Campaignmonitor
 * Campaignmonitor Addon Main Class
 *
 * @since 1.0 Campaignmonitor Addon
 */
final class Powerform_Addon_Campaignmonitor extends Powerform_Addon_Abstract {
	...
	protected $_version                = POWERFORM_ADDON_CAMPAIGNMONITOR_VERSION;
	...
	protected $_form_settings = 'Powerform_Addon_Campaignmonitor_Form_Settings';
	protected $_form_hooks    = 'Powerform_Addon_Campaignmonitor_Form_Hooks';

	protected $_quiz_settings = 'Powerform_Addon_Campaignmonitor_Quiz_Settings';
	protected $_quiz_hooks    = 'Powerform_Addon_Campaignmonitor_Quiz_Hooks';

	/**
	 * Campaign Monitor API
	 *
	 * @var Powerform_Addon_Campaignmonitor_Wp_Api|null
	 */
	private static $api = null;

	/**
	 * Powerform_Addon_Campaignmonitor constructor.
	 *
	 * @since 1.0 Campaignmonitor Addon
	 */
	public function __construct() {
		...
		$this->_activation_error_message   = __( 'Sorry but we failed to activate Campaign Monitor Integration, don\'t hesitate to contact us', Powerform::DOMAIN );
		$this->_deactivation_error_message = __( 'Sorry but we failed to deactivate Campaign Monitor Integration, please try again', Powerform::DOMAIN );
		...
		$this->_icon     = powerform_addon_campaignmonitor_assets_url() . 'icons/campaignmonitor.png';
		$this->_icon_x2  = powerform_addon_campaignmonitor_assets_url() . 'icons/campaignmonitor@2x.png';
		$this->_image    = powerform_addon_campaignmonitor_assets_url() . 'img/campaignmonitor.png';
		$this->_image_x2 = powerform_addon_campaignmonitor_assets_url() . 'img/campaignmonitor@2x.png';
	}
	...
	public function is_connected() {
		try {
			// check if its active
			if ( ! $this->is_active() ) {
				throw new Powerform_Addon_Campaignmonitor_Exception( __( 'Campaign Monitor is not active', Powerform::DOMAIN ) );
			}

			// if user completed api setup
			$is_connected = $this->is_api_completed();

		} catch ( Powerform_Addon_Campaignmonitor_Exception $e ) {
			$is_connected = false;
		}
		 ...
		 */
		$is_connected = apply_filters( 'powerform_addon_campaignmonitor_is_connected', $is_connected );
	...
	public function is_form_connected( $form_id ) {
		try {
			...
			if ( ! $this->is_connected() ) {
				throw new Powerform_Addon_Campaignmonitor_Exception( __( ' Campaign Monitor is not connected', Powerform::DOMAIN ) );
			}

			$form_settings_instance = $this->get_addon_form_settings( $form_id );
			if ( ! $form_settings_instance instanceof Powerform_Addon_Campaignmonitor_Form_Settings ) {
				throw new Powerform_Addon_Campaignmonitor_Exception( __( 'Invalid Form Settings of Campaign Monitor', Powerform::DOMAIN ) );
			}

			// Mark as active when there is at least one active connection
			if ( false === $form_settings_instance->find_one_active_connection() ) {
				throw new Powerform_Addon_Campaignmonitor_Exception( __( 'No active Campaign Monitor connection found in this form', Powerform::DOMAIN ) );
			}
		 ...
		 * @param Powerform_Addon_Campaignmonitor_Form_Settings|null $form_settings_instance Instance of form settings, or null when unavailable
		 *
		 */
		$is_form_connected = apply_filters( 'powerform_addon_campaignmonitor_is_form_connected', $is_form_connected, $form_id, $form_settings_instance );
	...
	public static function is_show_full_log() {
		$show_full_log = false;
		if ( defined( 'POWERFORM_ADDON_CAMPAIGNMONITOR_SHOW_FULL_LOG' ) && POWERFORM_ADDON_CAMPAIGNMONITOR_SHOW_FULL_LOG ) {
			$show_full_log = true;
		}
		 ...
		 */
		$show_full_log = apply_filters( 'powerform_addon_campaignmonitor_show_full_log', $show_full_log );
	...
	public static function is_enable_delete_subscriber() {
		...
		if ( defined( 'POWERFORM_ADDON_CAMPAIGNMONITOR_ENABLE_DELETE_SUBSCRIBER' ) && POWERFORM_ADDON_CAMPAIGNMONITOR_ENABLE_DELETE_SUBSCRIBER ) {
		 ...
		 */
		$delete_subscriber = apply_filters( 'powerform_addon_campaignmonitor_enable_delete_subscriber', $delete_subscriber );
	...
	public function setup_api( $submitted_data, $form_id = 0 ) {
		...
		$template         = powerform_addon_campaignmonitor_dir() . 'views/settings/setup-api.php';
		$template_success = powerform_addon_campaignmonitor_dir() . 'views/settings/setup-api-success.php';
			...
			if ( ! $has_errors ) {
				...
				try {
					...
					if ( ! isset( $client_details->BasicDetails ) //phpcs:ignore WordPress.NamingConventions.ValidVariableName.NotSnakeCaseMemberVar
						...
						throw new Powerform_Addon_Campaignmonitor_Exception( __( 'Could not find client details, please try again', Powerform::DOMAIN ) );
					}
					...
					if ( ! powerform_addon_is_active( $this->_slug ) ) {
						...
						if ( ! $activated ) {
							throw new Powerform_Addon_Campaignmonitor_Exception( Powerform_Addon_Loader::get_instance()->get_last_error_message() );
						}
					}
	 ...
	 * @param null $api_key
	 *
	 * @return Powerform_Addon_Campaignmonitor_Wp_Api
	 * @throws Powerform_Addon_Campaignmonitor_Wp_Api_Exception
	 */
	public function get_api( $api_key = null ) {
		if ( is_null( self::$api ) ) {
			...
			$api       = Powerform_Addon_Campaignmonitor_Wp_Api::get_instance( $api_key );
			self::$api = $api;
		}
	 ...
	 *
	 * @return string
	 * @throws Powerform_Addon_Campaignmonitor_Exception
	 */
	public function validate_api_key( $api_key ) {
		if ( empty( $api_key ) ) {
			throw new Powerform_Addon_Campaignmonitor_Exception( __( 'Please put a valid Campaign Monitor API Key', Powerform::DOMAIN ) );
		}
	...
	public function validate_api( $api_key ) {
		...
		if ( ! isset( $system_date->SystemDate ) || empty( $system_date->SystemDate ) ) { //phpcs:ignore WordPress.NamingConventions.ValidVariableName.NotSnakeCaseMemberVar
			throw new Powerform_Addon_Campaignmonitor_Exception( __( 'Failed to validate API Key.', Powerform::DOMAIN ) );
		}
	}
	 ...
	 * @throws Powerform_Addon_Campaignmonitor_Exception
	 * @throws Powerform_Addon_Campaignmonitor_Wp_Api_Exception
	 * @throws Powerform_Addon_Campaignmonitor_Wp_Api_Not_Found_Exception
	 */
	...
	public function get_client_id() {
		 ...
		 */
		$client_id = apply_filters( 'powerform_addon_campaignmonitor_client_id', $client_id );
	...
	public function is_quiz_connected( $quiz_id ) {
		try {
			...
			if ( ! $quiz_settings_instance instanceof Powerform_Addon_Campaignmonitor_Quiz_Settings ) {
				throw new Powerform_Addon_Campaignmonitor_Exception( __( 'Invalid Quiz Settings of Campaign Monitor', Powerform::DOMAIN ) );
			}

			// Mark as active when there is at least one active connection
			if ( false === $quiz_settings_instance->find_one_active_connection() ) {
				throw new Powerform_Addon_Campaignmonitor_Exception( __( 'No active Campaign Monitor connection found in this quiz', Powerform::DOMAIN ) );
			}
		 ...
		 * @param Powerform_Addon_Campaignmonitor_Quiz_Settings|null $quiz_settings_instance Instance of quiz settings, or null when unavailable
		 *
		 */
		$is_quiz_connected = apply_filters( 'powerform_addon_campaignmonitor_is_quiz_connected', $is_quiz_connected, $quiz_id, $quiz_settings_instance );
	...
	public function is_quiz_lead_connected( $quiz_id ) {
		 ...
		 *
		 */
		$is_quiz_connected = apply_filters( 'powerform_addon_campaignmonitor_is_quiz_lead_connected', $is_quiz_connected, $quiz_id, $quiz_settings_instance );
