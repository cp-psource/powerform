...
require_once dirname( __FILE__ ) . '/powerform-addon-campaignmonitor-form-settings-exception.php';

/**
 * Class Powerform_Addon_Campaignmonitor_Form_Settings
 * Handle how form settings displayed and saved
 *
 * @since 1.0 Campaignmonitor Addon
 */
class Powerform_Addon_Campaignmonitor_Form_Settings extends Powerform_Addon_Form_Settings_Abstract {
	...
	/**
	 * Powerform_Addon_Campaignmonitor_Form_Settings constructor.
	 *
	 * @since 1.0 Campaignmonitor Addon
	 *
	...
	public function pick_name( $submitted_data ) {
		$template = powerform_addon_campaignmonitor_dir() . 'views/form-settings/pick-name.php';
		...
		if ( $is_submit ) {
			...
			try {
				...
				if ( empty( $name ) ) {
					throw new Powerform_Addon_Campaignmonitor_Exception( __( 'Please pick valid name' ) );
				}
		...
		if ( $this->pick_name_is_completed( array( 'multi_id' => $multi_id ) ) ) {
			$buttons['disconnect']['markup'] = Powerform_Addon_Abstract::get_button_markup( esc_html__( 'DISCONNECT', Powerform::DOMAIN ),
			                                                                                 ...
			                                                                                 esc_html__( 'Disconnect this Campaign Monitor Integration from this Form.', Powerform::DOMAIN )
			);
		}
	...
	public function setup_list( $submitted_data ) {
		$template = powerform_addon_campaignmonitor_dir() . 'views/form-settings/setup-list.php';
		...
		try {
			...
			if ( empty( $lists ) ) {
				throw new Powerform_Addon_Campaignmonitor_Exception( __( 'No lists found on your Campaign Monitor. Please create one.', Powerform::DOMAIN ) );
			}
		...
		if ( $is_submit ) {
			...
			try {
				...
				if ( empty( $list_id ) ) {
					throw new Powerform_Addon_Campaignmonitor_Exception( __( 'Please pick valid list' ) );
				}
	...
	public function map_fields( $submitted_data ) {
		$template = powerform_addon_campaignmonitor_dir() . 'views/form-settings/map-fields.php';
		...
		try {
			...
			if ( ! is_array( $list_custom_fields ) ) {
				throw new Powerform_Addon_Campaignmonitor_Exception( __( 'Campaign Monitor list\'s custom fields could not be found', Powerform::DOMAIN ) );
			}
		...
		if ( $is_submit ) {
			...
			try {
				if ( empty( $fields_map ) ) {
					throw new Powerform_Addon_Campaignmonitor_Exception( __( 'Please assign fields.', Powerform::DOMAIN ) );
				}

				$input_exceptions = new Powerform_Addon_Campaignmonitor_Form_Settings_Exception();
			...
			} catch ( Powerform_Addon_Campaignmonitor_Form_Settings_Exception $e ) {
	...
	public function setup_options( $submitted_data ) {
		$template = powerform_addon_campaignmonitor_dir() . 'views/form-settings/setup-options.php';
		...
		if ( $is_submit ) {
			...
			try {
				...
				if ( ! in_array( $consent_to_track, $available_consents, true ) ) {
					$input_exceptions->add_input_exception( __( 'Please pick valid Consent To Track options', Powerform::DOMAIN ), 'consent_to_track_error' );
				}
