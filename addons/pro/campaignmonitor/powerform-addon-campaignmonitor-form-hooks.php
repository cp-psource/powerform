	...
	private function get_status_on_add_subscriber( $connection_id, $submitted_data, $connection_settings, $form_settings ) {
		...
		try {
			...
			if ( ! isset( $submitted_data[ $email_element_id ] ) || empty( $submitted_data[ $email_element_id ] ) ) {
				throw new Powerform_Addon_Campaignmonitor_Exception(
					sprintf(
						__( 'Email Address on element %1$s not found or not filled on submitted data.', Powerform::DOMAIN ),
						$email_element_id
					)
				);
			}
			...
			if ( ! isset( $submitted_data[ $name_element_id ] ) || empty( $submitted_data[ $name_element_id ] ) ) {
				throw new Powerform_Addon_Campaignmonitor_Exception(
					sprintf(
						__( 'Name on element %1$s not found or not filled on submitted data.', Powerform::DOMAIN ),
						$name_element_id
					)
				);
			}
