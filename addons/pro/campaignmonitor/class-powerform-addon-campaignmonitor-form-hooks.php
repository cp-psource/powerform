...
/**
 * Class Powerform_Addon_Campaignmonitor_Form_Hooks
 *
 * @since 1.0 Campaignmonitor Addon
 *
 */
class Powerform_Addon_Campaignmonitor_Form_Hooks extends Powerform_Addon_Form_Hooks_Abstract {
	 ...
	 *
	 * @since 1.0 Campaignmonitor Addon
	 * @var Powerform_Addon_Campaignmonitor
	 */
	protected $addon;
	 ...
	 *
	 * @since 1.0 Campaignmonitor Addon
	 * @var Powerform_Addon_Campaignmonitor_Form_Settings | null
	 */
	protected $form_settings_instance;

	/**
	 * Powerform_Addon_Campaignmonitor_Form_Hooks constructor.
	 *
	 * @since 1.0 Campaignmonitor Addon
	 *
	...
	public function __construct( Powerform_Addon_Abstract $addon, $form_id ) {
		...
		$this->_submit_form_error_message = __( 'Campaign Monitor failed to process submitted data. Please check your form and try again', Powerform::DOMAIN );
	}
	...
	public function add_entry_fields( $submitted_data, $form_entry_fields = array() ) {
		 ...
		 * @param Powerform_Addon_Campaignmonitor_Form_Settings $form_settings_instance Campaign Monitor Addon Form Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_campaignmonitor_form_submitted_data',
			$submitted_data,
			$form_id,
		 ...
		 * @param Powerform_Addon_Campaignmonitor_Form_Settings $form_settings_instance Campaign Monitor Addon Form Settings instance
		 */
		do_action( 'powerform_addon_campaignmonitor_before_add_subscriber', $form_id, $submitted_data, $form_settings_instance );
		 ...
		 */
		$data = apply_filters(
			'powerform_addon_campaignmonitor_entry_fields',
			$entry_fields,
			$form_id,
			$submitted_data,
			$form_settings_instance
	...
	private function get_status_on_add_subscriber( $connection_id, $submitted_data, $connection_settings, $form_settings, $form_entry_fields = array() ) {
		...
		try {
			...
			if ( ! isset( $connection_settings['list_id'] ) ) {
				throw new Powerform_Addon_Campaignmonitor_Exception( __( 'List ID not properly setup.', Powerform::DOMAIN ) );
			}
			...
			if ( ! isset( $submitted_data[ $email_element_id ] ) || empty( $submitted_data[ $email_element_id ] ) ) {
				throw new Powerform_Addon_Campaignmonitor_Exception(/* translators: ... */
					sprintf( __( 'Email Address on element %1$s not found or not filled on submitted data.', Powerform::DOMAIN ), $email_element_id )
				);
			}
			...
			if ( self::element_is_calculation( $name_element_id ) ) {
				...
				$element_value = Powerform_Form_Entry_Model::meta_value_to_string( 'calculation', $meta_value );
				$name          = $element_value;
			} elseif ( self::element_is_stripe( $name_element_id ) ) {
				$meta_value    = self::find_meta_value_from_entry_fields( $name_element_id, $form_entry_fields );
				$element_value = Powerform_Form_Entry_Model::meta_value_to_string( 'stripe', $meta_value );
				$name          = $element_value;
			} elseif ( ! isset( $submitted_data[ $name_element_id ] ) || empty( $submitted_data[ $name_element_id ] ) ) {
				throw new Powerform_Addon_Campaignmonitor_Exception(/* translators: ... */
					sprintf( __( 'Name on element %1$s not found or not filled on submitted data.', Powerform::DOMAIN ), $name_element_id )
				);
			}
			 ...
			 */
			$args = apply_filters(
				'powerform_addon_campaignmonitor_add_subscriber_args',
				$args,
				$form_id,
				$connection_id,
				$submitted_data,
			...
			powerform_addon_maybe_log( __METHOD__, $api->get_last_data_received() );

			return array(
				'is_sent'          => true,
				'connection_name'  => $connection_settings['name'],
				'description'      => __( 'Successfully send data to Campaign Monitor', Powerform::DOMAIN ),
			...
			powerform_addon_maybe_log( __METHOD__, 'Failed to Send to Campaign Monitor' );

			return array(
				'is_sent'         => false,
				'description'     => $e->getMessage(),
				'connection_name' => $connection_settings['name'],
				'data_sent'       => ( ( $api instanceof Powerform_Addon_Campaignmonitor_Wp_Api ) ? $api->get_last_data_sent() : array() ),
				'data_received'   => ( ( $api instanceof Powerform_Addon_Campaignmonitor_Wp_Api ) ? $api->get_last_data_received() : array() ),
				'url_request'     => ( ( $api instanceof Powerform_Addon_Campaignmonitor_Wp_Api ) ? $api->get_last_url_request() : '' ),
			);
		}
	}
	 ...
	 * subentries included are defined in @see Powerform_Addon_Campaignmonitor_Form_Hooks::get_additional_entry_item()
	 *
	 * @since 1.0 Campaign Monitor Addon
	 *
	...
	public function on_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$addon_meta_data = apply_filters(
			'powerform_addon_campaignmonitor_metadata',
			$addon_meta_data,
			$form_id,
			$form_settings_instance
		);
	 ...
	 * - Info : Text that are generated by addon when building and sending data to Campaign Monitor @see Powerform_Addon_Campaignmonitor_Form_Hooks::add_entry_fields()
	 * - Below subentries will be added if full log enabled, @see Powerform_Addon_Campaignmonitor::is_show_full_log() @see POWERFORM_ADDON_CAMPAIGNMONITOR_SHOW_FULL_LOG
	...
	private function get_additional_entry_item( $addon_meta_data ) {
		...
		$additional_entry_item = array(
			'label' => __( 'Campaign Monitor Integration', Powerform::DOMAIN ),
			'value' => '',
		);
		...
		if ( isset( $status['is_sent'] ) ) {
			...
			$sub_entries[] = array(
				'label' => __( 'Sent To Campaign Monitor', Powerform::DOMAIN ),
				'value' => $is_sent,
			);
		}
		...
		if ( Powerform_Addon_Campaignmonitor::is_show_full_log() ) {
			// too long to be added on entry data enable this with `define('POWERFORM_ADDON_CAMPAIGNMONITOR_SHOW_FULL_LOG', true)`
			...
			if ( isset( $status['data_sent'] ) ) {
				$sub_entries[] = array(
					'label' => __( 'Data sent to Campaign Monitor', Powerform::DOMAIN ),
			...
			if ( isset( $status['data_received'] ) ) {
				$sub_entries[] = array(
					'label' => __( 'Data received from Campaign Monitor', Powerform::DOMAIN ),
	 ...
	 * its called `Campaign Monitor Info` which can be translated on powerform lang
	 *
	 * @since 1.0 Campaign Monitor Addon
	 * @return array
	 */
	public function on_export_render_title_row() {
		...
		$export_headers = array(
			'info' => __( 'Campaign Monitor Info', Powerform::DOMAIN ),
		);
		...
		$export_headers = apply_filters(
			'powerform_addon_campaignmonitor_export_headers',
			$export_headers,
			$form_id,
			$form_settings_instance
		);
	...
	public function on_export_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$export_columns = apply_filters(
			'powerform_addon_campaignmonitor_export_columns',
			$export_columns,
			$form_id,
			$entry_model,
			$addon_meta_data,
	...
	public function on_before_delete_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		 ...
		 */
		do_action(
			'powerform_addon_campaignmonitor_on_before_delete_submission',
			$form_id,
			$entry_model,
			$addon_meta_data,
			$form_settings_instance
		...
		if ( ! Powerform_Addon_Campaignmonitor::is_enable_delete_subscriber() ) {
		...
		try {
			...
			$subscribers_to_delete = apply_filters(
				'powerform_addon_campaignmonitor_subscribers_to_delete',
			...
			// handle all internal addon exceptions with `Powerform_Addon_Campaignmonitor_Exception`

			// use wp_error, for future usage it can be returned to page entries
			$wp_error = new WP_Error( 'powerform_addon_campaignmonitor_delete_subscriber', $e->getMessage() );
