...
require_once dirname( __FILE__ ) . '/class-powerform-addon-campaignmonitor-wp-api-exception.php';
require_once dirname( __FILE__ ) . '/class-powerform-addon-campaignmonitor-wp-api-not-found-exception.php';

/**
 * Class Powerform_Addon_Campaignmonitor_Wp_Api
 */
class Powerform_Addon_Campaignmonitor_Wp_Api {
	...
	/**
	 * Powerform_Addon_Campaignmonitor_Wp_Api constructor.
	 *
	 * @since 1.0 Campaignmonitor Addon
	 *
	 * @param $api_key
	...
	public function __construct( $api_key ) {
		//prerequisites
		if ( ! $api_key ) {
			throw new Powerform_Addon_Campaignmonitor_Wp_Api_Exception( __( 'Missing required API Key', Powerform::DOMAIN ) );
		}
	 ...
	 *
	 * @return Powerform_Addon_Campaignmonitor_Wp_Api|null
	 * @throws Powerform_Addon_Campaignmonitor_Wp_Api_Exception
	 */
	...
	public function filter_user_agent( $user_agent ) {
		$user_agent .= ' PowerformCampaignmonitor/' . POWERFORM_ADDON_CAMPAIGNMONITOR_VERSION;
		 ...
		 */
		$user_agent = apply_filters( 'powerform_addon_campaignmonitor_api_user_agent', $user_agent );
	...
	private function request( $verb, $path, $args = array() ) {
		 ...
		 */
		$url = apply_filters( 'powerform_addon_campaignmonitor_api_url', $url, $verb, $path, $args );

		$this->_last_url_request = $url;

		$encoded_auth = base64_encode( $this->_api_key . ':powerform-no_pass' ); //phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_encode
		$headers      = array(
		 ...
		 * @param array  $args argument sent to this function
		 */
		$headers = apply_filters( 'powerform_addon_campaignmonitor_api_request_headers', $headers, $verb, $path, $args );
		 ...
		 * @param string $path         requested path resource
		 */
		$args = apply_filters( 'powerform_addon_campaignmonitor_api_request_data', $request_data, $verb, $path );
		...
		if ( is_wp_error( $res ) || ! $res ) {
			...
			throw new Powerform_Addon_Campaignmonitor_Wp_Api_Exception(
				__( 'Failed to process request, make sure your Webhook URL is correct and your server has internet connection.', Powerform::DOMAIN )
			);
		}
		...
		if ( isset( $res['response']['code'] ) ) {
			...
			if ( $status_code >= 400 ) {
				...
				if ( 404 === $status_code ) {
					/* translators: ... */
					throw new Powerform_Addon_Campaignmonitor_Wp_Api_Not_Found_Exception( sprintf( __( 'Failed to processing request : %s', Powerform::DOMAIN ), $msg ) );
				}
				/* translators: ... */
				throw new Powerform_Addon_Campaignmonitor_Wp_Api_Exception( sprintf( __( 'Failed to processing request : %s', Powerform::DOMAIN ), $msg ) );
			}
		}
		 ...
		 */
		$res = apply_filters( 'powerform_addon_campaignmonitor_api_response', $response, $body, $wp_response );
