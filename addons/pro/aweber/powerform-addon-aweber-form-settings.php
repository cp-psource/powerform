...
require_once dirname( __FILE__ ) . '/powerform-addon-aweber-form-settings-exception.php';

/**
 * Class Powerform_Addon_Aweber_Form_Settings
 * Handle how form settings displayed and saved
 *
 * @since 1.0 Aweber Addon
 */
class Powerform_Addon_Aweber_Form_Settings extends Powerform_Addon_Form_Settings_Abstract {
	...
	/**
	 * Powerform_Addon_Aweber_Form_Settings constructor.
	 *
	 * @since 1.0 Aweber Addon
	 *
	...
	public function pick_name( $submitted_data ) {
		$template = powerform_addon_aweber_dir() . 'views/form-settings/pick-name.php';
		...
		if ( $this->pick_name_is_completed( array( 'multi_id' => $multi_id ) ) ) {
			$buttons['disconnect']['markup'] = Powerform_Addon_Abstract::get_button_markup( esc_html__( 'DISCONNECT', Powerform::DOMAIN ),
			                                                                                 ...
			                                                                                 esc_html__( 'Disconnect this AWeber Integration from this Form.', Powerform::DOMAIN )
			);
		}
	...
	public function setup_list( $submitted_data ) {
		$template = powerform_addon_aweber_dir() . 'views/form-settings/setup-list.php';
	...
	public function map_fields( $submitted_data ) {
		$template = powerform_addon_aweber_dir() . 'views/form-settings/map-fields.php';
		...
		if ( $is_submit ) {
			...
			try {
				...
				$input_exceptions = new Powerform_Addon_Aweber_Form_Settings_Exception();
			...
			} catch ( Powerform_Addon_Aweber_Form_Settings_Exception $e ) {
	...
	public function setup_options( $submitted_data ) {
		$template = powerform_addon_aweber_dir() . 'views/form-settings/setup-options.php';
		...
		$template_params = array(
			'multi_id'             => $multi_id,
			'error_message'        => '',
			'ad_tracking'          => $this->get_multi_id_form_settings_value( $multi_id, 'ad_tracking', 'POWERFORM {form_name}' ),
