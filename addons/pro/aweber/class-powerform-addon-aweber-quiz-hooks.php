...
/**
 * Class Powerform_Addon_Aweber_Quiz_Hooks
 *
 * @since 1.0 Aweber Addon
 *
 */
class Powerform_Addon_Aweber_Quiz_Hooks extends Powerform_Addon_Quiz_Hooks_Abstract {
	 ...
	 *
	 * @since 1.0 Aweber Addon
	 * @var Powerform_Addon_Aweber_Quiz_Settings | null
	 */
	protected $quiz_settings_instance;

	/**
	 * Powerform_Addon_Aweber_Quiz_Hooks constructor.
	 *
	 * @since 1.0 Aweber Addon
	 *
	...
	public function __construct( Powerform_Addon_Abstract $addon, $quiz_id ) {
		...
		$this->_submit_quiz_error_message = __( 'AWeber failed to process submitted data. Please check your quiz and try again', Powerform::DOMAIN );
	}
	...
	public function add_entry_fields( $submitted_data, $form_entry_fields = array() ) {
		 ...
		 * @param Powerform_Addon_Aweber_Quiz_Settings $quiz_settings_instance AWeber Addon Quiz Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_aweber_quiz_submitted_data',
			$submitted_data,
			$quiz_id,
		 ...
		 * @param Powerform_Addon_Aweber_Quiz_Settings $quiz_settings_instance AWeber Addon Quiz Settings instance
		 */
		do_action( 'powerform_addon_aweber_before_add_subscriber', $quiz_id, $submitted_data, $quiz_settings_instance );
	...
	private function get_status_on_add_subscriber( $connection_id, $submitted_data, $connection_settings, $form_entry_fields, $lead_submitted_data ) {
		...
		try {
			...
			if ( isset( $connection_settings['ad_tracking'] ) && ! empty( $connection_settings['ad_tracking'] ) ) {
				...
				$ad_tracking = str_ireplace( '{quiz_name}', powerform_get_name_from_model( $this->quiz ), $ad_tracking );
				$ad_tracking = powerform_addon_replace_custom_vars( $ad_tracking, $lead_submitted_data, $this->lead_model, $form_entry_fields, false );
				/**
	 ...
	 * subentries included are defined in @see Powerform_Addon_Aweber_Quiz_Hooks::get_additional_entry_item()
	 *
	 * @since 1.0 AWeber Addon
	 *
	 ...
	 * - Info : Text that are generated by addon when building and sending data to AWeber @see Powerform_Addon_Aweber_Quiz_Hooks::add_entry_fields()
	...
	public function on_export_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		 ...
		 * @param array $addon_meta_data
		 * @param int $quiz_id current Quiz ID
		 * @param Powerform_Addon_Aweber_Quiz_Settings $quiz_settings_instance AWeber Quiz Settings instance
		 */
		$addon_meta_data = apply_filters(
			'powerform_addon_aweber_metadata',
			$addon_meta_data,
