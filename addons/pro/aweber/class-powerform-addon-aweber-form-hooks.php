...
/**
 * Class Powerform_Addon_Aweber_Form_Hooks
 *
 * @since 1.0 Aweber Addon
 *
 */
class Powerform_Addon_Aweber_Form_Hooks extends Powerform_Addon_Form_Hooks_Abstract {
	 ...
	 *
	 * @since 1.0 Aweber Addon
	 * @var Powerform_Addon_Aweber_Form_Settings | null
	 */
	protected $form_settings_instance;

	/**
	 * Powerform_Addon_Aweber_Form_Hooks constructor.
	 *
	 * @since 1.0 Aweber Addon
	 *
	...
	public function __construct( Powerform_Addon_Abstract $addon, $form_id ) {
		...
		$this->_submit_form_error_message = __( 'AWeber failed to process submitted data. Please check your form and try again', Powerform::DOMAIN );
	}
	...
	public function add_entry_fields( $submitted_data, $form_entry_fields = array() ) {
		 ...
		 * @param Powerform_Addon_Aweber_Form_Settings $form_settings_instance AWeber Addon Form Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_aweber_form_submitted_data',
			$submitted_data,
			$form_id,
		 ...
		 * @param Powerform_Addon_Aweber_Form_Settings $form_settings_instance AWeber Addon Form Settings instance
		 */
		do_action( 'powerform_addon_aweber_before_add_subscriber', $form_id, $submitted_data, $form_settings_instance );
		 ...
		 */
		$data = apply_filters(
			'powerform_addon_aweber_entry_fields',
			$entry_fields,
			$form_id,
			$submitted_data,
			$form_settings_instance
	...
	private function get_status_on_add_subscriber( $connection_id, $submitted_data, $connection_settings, $form_entry_fields ) {
		...
		try {
			...
			if ( ! isset( $connection_settings['list_id'] ) ) {
				throw new Powerform_Addon_Aweber_Exception( __( 'List ID not properly setup.', Powerform::DOMAIN ) );
			}
			...
			if ( ! isset( $submitted_data[ $email_element_id ] ) || empty( $submitted_data[ $email_element_id ] ) ) {
				throw new Powerform_Addon_Aweber_Exception(/* translators: ... */
			 ...
			 */
			$args = apply_filters(
				'powerform_addon_aweber_find_subscriber_args',
				$args,
				$form_id,
				$connection_id,
				$submitted_data,
				...
				if ( isset( $existing_subscriber_entries[0] ) ) {
					...
					if ( isset( $existing_subscriber->id ) ) {
						...
						if ( isset( $existing_subscriber->status ) && 'unconfirmed' === $existing_subscriber->status ) {
							throw new Powerform_Addon_Aweber_Exception( __( 'Unconfirmed subscriber can\'t be modified.', Powerform::DOMAIN ) );
						}
					}
				}
			}
			...
			if ( self::element_is_calculation( $name_element_id ) ) {
				...
				$name       = Powerform_Form_Entry_Model::meta_value_to_string( 'calculation', $meta_value );
			} elseif ( self::element_is_stripe( $name_element_id ) ) {
				$meta_value = self::find_meta_value_from_entry_fields( $name_element_id, $form_entry_fields );
				$name       = Powerform_Form_Entry_Model::meta_value_to_string( 'stripe', $meta_value );
			...
			if ( isset( $connection_settings['ad_tracking'] ) && ! empty( $connection_settings['ad_tracking'] ) ) {
				...
				$ad_tracking = powerform_addon_replace_custom_vars( $ad_tracking, $submitted_data, $this->custom_form, $form_entry_fields, false );
				...
				$ad_tracking = apply_filters(
					'powerform_addon_aweber_subscriber_ad_tracking',
					$ad_tracking,
					$form_id,
					$connection_id,
					$submitted_data,
			...
			if ( isset( $connection_settings['tags'] ) && ! empty( $connection_settings['tags'] ) ) {
				 ...
				 */
				$tags = apply_filters(
					'powerform_addon_aweber_subscriber_tags',
					$tags,
					$form_id,
					$connection_id,
					$submitted_data,
			...
			$ip_address = Powerform_Geo::get_user_ip();
			...
			$ip_address = apply_filters(
				'powerform_addon_aweber_subscriber_ip_address',
				$ip_address,
				$form_id,
				$connection_id,
				$submitted_data,
			...
			if ( ! $subscriber_is_exist ) {
				 ...
				 */
				$args = apply_filters(
					'powerform_addon_aweber_add_subscriber_args',
					$args,
					$form_id,
					$connection_id,
					$submitted_data,
				 ...
				 * use hooks @see powerform_addon_aweber_update_subscriber_args, if needed
				 */
				// update if exist
				$current_tags = array();
				 ...
				 */
				$args = apply_filters(
					'powerform_addon_aweber_update_subscriber_args',
					$args,
					$form_id,
					$connection_id,
					$submitted_data,
			...
			return array(
				...
				'description'     => __( 'Successfully send data to AWeber', Powerform::DOMAIN ),
			...
			$addon_entry_fields = array(
				...
				'connection_name' => $connection_settings['name'],
				'data_sent'       => ( ( $api instanceof Powerform_Addon_Aweber_Wp_Api ) ? $api->get_last_data_sent() : array() ),
				'data_received'   => ( ( $api instanceof Powerform_Addon_Aweber_Wp_Api ) ? $api->get_last_data_received() : array() ),
				'url_request'     => ( ( $api instanceof Powerform_Addon_Aweber_Wp_Api ) ? $api->get_last_url_request() : '' ),
			);
	 ...
	 * subentries included are defined in @see Powerform_Addon_Aweber_Form_Hooks::get_additional_entry_item()
	 *
	 * @since 1.0 AWeber Addon
	 *
	...
	public function on_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$addon_meta_data = apply_filters(
			'powerform_addon_aweber_metadata',
			$addon_meta_data,
			$form_id,
			$form_settings_instance
		);
	 ...
	 * - Info : Text that are generated by addon when building and sending data to AWeber @see Powerform_Addon_Aweber_Form_Hooks::add_entry_fields()
	 * - Below subentries will be added if full log enabled, @see Powerform_Addon_Aweber::is_show_full_log() @see POWERFORM_ADDON_AWEBER_SHOW_FULL_LOG
	...
	private function get_additional_entry_item( $addon_meta_data ) {
		...
		$additional_entry_item = array(
			'label' => __( 'AWeber Integration', Powerform::DOMAIN ),
			'value' => '',
		);
		...
		if ( isset( $status['is_sent'] ) ) {
			...
			$sub_entries[] = array(
				'label' => __( 'Sent To AWeber', Powerform::DOMAIN ),
				'value' => $is_sent,
			);
		}
		...
		if ( Powerform_Addon_Aweber::is_show_full_log() ) {
			// too long to be added on entry data enable this with `define('POWERFORM_ADDON_AWEBER_SHOW_FULL_LOG', true)`
			...
			if ( isset( $status['data_sent'] ) ) {
				$sub_entries[] = array(
					'label' => __( 'Data sent to AWeber', Powerform::DOMAIN ),
			...
			if ( isset( $status['data_received'] ) ) {
				$sub_entries[] = array(
					'label' => __( 'Data received from AWeber', Powerform::DOMAIN ),
	 ...
	 * its called `AWeber Info` which can be translated on powerform lang
	 *
	 * @since 1.0 AWeber Addon
	 * @return array
	 */
	public function on_export_render_title_row() {
		...
		$export_headers = array(
			'info' => __( 'AWeber Info', Powerform::DOMAIN ),
		);
		...
		$export_headers = apply_filters(
			'powerform_addon_aweber_export_headers',
			$export_headers,
			$form_id,
			$form_settings_instance
		);
	...
	public function on_export_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		 ...
		 * @param array                                 $addon_meta_data
		 * @param int                                   $form_id                current Form ID
		 * @param Powerform_Addon_Aweber_Form_Settings $form_settings_instance AWeber Form Settings instance
		 */
		$addon_meta_data = apply_filters(
			'powerform_addon_aweber_metadata',
			$addon_meta_data,
		...
		$export_columns = apply_filters(
			'powerform_addon_aweber_export_columns',
			$export_columns,
			$form_id,
			$entry_model,
			$addon_meta_data,
