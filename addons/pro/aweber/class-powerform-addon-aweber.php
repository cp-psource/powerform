...
require_once dirname( __FILE__ ) . '/class-powerform-addon-aweber-exception.php';
require_once dirname( __FILE__ ) . '/lib/class-powerform-addon-aweber-wp-api.php';

/**
 * Class Powerform_Addon_Aweber
 * Aweber Addon Main Class
 *
 * @since 1.0 Aweber Addon
 */
final class Powerform_Addon_Aweber extends Powerform_Addon_Abstract {
	...
	protected $_version                = POWERFORM_ADDON_AWEBER_VERSION;
	...
	protected $_form_settings = 'Powerform_Addon_Aweber_Form_Settings';
	protected $_form_hooks    = 'Powerform_Addon_Aweber_Form_Hooks';

	protected $_quiz_settings = 'Powerform_Addon_Aweber_Quiz_Settings';
	protected $_quiz_hooks    = 'Powerform_Addon_Aweber_Quiz_Hooks';
	 ...
	 * API Adapter
	 *
	 * @var Powerform_Addon_Aweber_Wp_Api
	 */
	private static $api;

	/**
	 * Powerform_Addon_Aweber constructor.
	 *
	 * @since 1.0 Aweber Addon
	 */
	public function __construct() {
		...
		$this->_activation_error_message   = __( 'Sorry but we failed to activate AWeber Integration, don\'t hesitate to contact us', Powerform::DOMAIN );
		$this->_deactivation_error_message = __( 'Sorry but we failed to deactivate AWeber Integration, please try again', Powerform::DOMAIN );
		...
		$this->_icon     = powerform_addon_aweber_assets_url() . 'icons/aweber.png';
		$this->_icon_x2  = powerform_addon_aweber_assets_url() . 'icons/aweber@2x.png';
		$this->_image    = powerform_addon_aweber_assets_url() . 'img/aweber.png';
		$this->_image_x2 = powerform_addon_aweber_assets_url() . 'img/aweber@2x.png';
	}
	...
	public function is_connected() {
		try {
			// check if its active
			if ( ! $this->is_active() ) {
				throw new Powerform_Addon_Aweber_Exception( __( 'AWeber is not active', Powerform::DOMAIN ) );
			}
		 ...
		 */
		$is_connected = apply_filters( 'powerform_addon_aweber_is_connected', $is_connected );
	...
	public function is_form_connected( $form_id ) {
		try {
			...
			if ( ! $this->is_connected() ) {
				throw new Powerform_Addon_Aweber_Exception( __( ' AWeber is not connected', Powerform::DOMAIN ) );
			}

			$form_settings_instance = $this->get_addon_form_settings( $form_id );
			if ( ! $form_settings_instance instanceof Powerform_Addon_Aweber_Form_Settings ) {
				throw new Powerform_Addon_Aweber_Exception( __( 'Invalid Form Settings of AWeber', Powerform::DOMAIN ) );
			}

			// Mark as active when there is at least one active connection
			if ( false === $form_settings_instance->find_one_active_connection() ) {
				throw new Powerform_Addon_Aweber_Exception( __( 'No active AWeber connection found in this form', Powerform::DOMAIN ) );
			}
		 ...
		 * @param Powerform_Addon_Aweber_Form_Settings|null $form_settings_instance Instance of form settings, or null when unavailable
		 *
		 */
		$is_form_connected = apply_filters( 'powerform_addon_aweber_is_form_connected', $is_form_connected, $form_id, $form_settings_instance );
	...
	public static function is_show_full_log() {
		$show_full_log = false;
		if ( defined( 'POWERFORM_ADDON_AWEBER_SHOW_FULL_LOG' ) && POWERFORM_ADDON_AWEBER_SHOW_FULL_LOG ) {
			$show_full_log = true;
		}
		 ...
		 */
		$show_full_log = apply_filters( 'powerform_addon_aweber_show_full_log', $show_full_log );
	...
	public function authorize_access() {
		$template = powerform_addon_aweber_dir() . 'views/settings/authorize.php';
	...
	public function wait_authorize_access() {
		$template         = powerform_addon_aweber_dir() . 'views/settings/wait-authorize.php';
		$template_success = powerform_addon_aweber_dir() . 'views/settings/success-authorize.php';
	...
	public function authorize_page_callback( $query_args ) {
		$template        = powerform_addon_aweber_dir() . 'views/sections/authorize.php';
		$template_params = array(
			'error_message' => '',
			'is_close'      => false,
		);
		...
		if ( isset( $query_args['authorization_code'] ) && ! empty( $query_args['authorization_code'] ) ) {
			try {
				...
				if ( ! is_array( $split_codes ) || 5 !== count( $split_codes ) ) {
					new Powerform_Addon_Aweber_Exception( __( 'Invalid Authorization Code', Powerform::DOMAIN ) );
				}
				...
				if ( ! $this->is_active() ) {
					...
					if ( ! $activated ) {
						...
						throw new Powerform_Addon_Aweber_Exception( $last_message );
					}
				}
	...
	public function get_auth_url( $return_url = '' ) {
		 ...
		 */
		$auth_params = apply_filters( 'powerform_addon_aweber_authorize_params', $auth_params );
	...
	public function get_app_id() {
		...
		if ( defined( 'POWERFORM_ADDON_AWEBER_APP_ID' ) && POWERFORM_ADDON_AWEBER_APP_ID ) {
			$app_id = POWERFORM_ADDON_AWEBER_APP_ID;
		}
		 ...
		 * @param string $app_id
		 */
		$app_id = apply_filters( 'powerform_addon_aweber_app_id', $app_id );
	 ...
	 *
	 * @return Powerform_Addon_Aweber_Wp_Api
	...
	public function get_api( $api_credentials = null ) {
		if ( is_null( self::$api ) ) {
			...
			$api       = new Powerform_Addon_Aweber_Wp_Api( $_application_key, $_application_secret, $_oauth_token, $_oauth_token_secret );
			self::$api = $api;
		}
	 ...
	 *
	 * @return integer
	 * @throws Powerform_Addon_Aweber_Exception
	 * @throws Powerform_Addon_Aweber_Wp_Api_Exception
	 * @throws Powerform_Addon_Aweber_Wp_Api_Not_Found_Exception
	 */
	public function get_validated_account_id() {
		...
		if ( ! isset( $accounts->entries ) ) {
			throw new Powerform_Addon_Aweber_Exception( __( 'Failed to get AWeber account information', Powerform::DOMAIN ) );
		}
		 ...
		 * @param Powerform_Addon_Aweber_Wp_Api $api
		 */
		$account_id = apply_filters( 'powerform_addon_aweber_validated_account_id', $account_id, $accounts, $api );
	...
	public function is_quiz_connected( $quiz_id ) {
		try {
			...
			if ( ! $quiz_settings_instance instanceof Powerform_Addon_Aweber_Quiz_Settings ) {
				throw new Powerform_Addon_Aweber_Exception( __( 'Invalid Quiz Settings of AWeber', Powerform::DOMAIN ) );
			}

			// Mark as active when there is at least one active connection
			if ( false === $quiz_settings_instance->find_one_active_connection() ) {
				throw new Powerform_Addon_Aweber_Exception( __( 'No active AWeber connection found in this quiz', Powerform::DOMAIN ) );
			}
		 ...
		 * @param Powerform_Addon_Aweber_Form_Settings|null $quiz_settings_instance Instance of quiz settings, or null when unavailable
		 *
		 */
		$is_quiz_connected = apply_filters( 'powerform_addon_aweber_is_form_connected', $is_quiz_connected, $quiz_id, $quiz_settings_instance );
	...
	public function is_quiz_lead_connected( $quiz_id ) {
		 ...
		 * @param bool $is_quiz_connected
		 * @param int $quiz_id Current Quiz ID
		 * @param Powerform_Addon_Aweber_Quiz_Settings|null $quiz_settings_instance Instance of quiz settings, or null when unavailable
		 *
		 */
		$is_quiz_connected = apply_filters( 'powerform_addon_aweber_is_quiz_lead_connected', $is_quiz_connected, $quiz_id, $quiz_settings_instance );
