...
require_once dirname( __FILE__ ) . '/class-powerform-addon-aweber-oauth.php';
require_once dirname( __FILE__ ) . '/class-powerform-addon-aweber-wp-api-exception.php';
require_once dirname( __FILE__ ) . '/class-powerform-addon-aweber-wp-api-not-found-exception.php';

/**
 * Class Powerform_Addon_Aweber_Wp_Api
 */
class Powerform_Addon_Aweber_Wp_Api {
	...
	/**
	 * Powerform_Addon_Aweber_Wp_Api constructor.
	 *
	 * @since 1.0 Aweber Addon
	 *
	...
	public function __construct( $_application_key, $_application_secret, $_oauth_token, $_oauth_token_secret ) {
		...
		if ( ! $_application_key || ! $_application_secret || ! $_oauth_token || ! $_oauth_token_secret ) {
			throw new Powerform_Addon_Aweber_Wp_Api_Exception( __( 'Missing required API Credentials', Powerform::DOMAIN ) );
		}
	 ...
	 * @param $_oauth_token_secret
	 *
	 * @return Powerform_Addon_Aweber_Wp_Api|null
	 * @throws Powerform_Addon_Aweber_Wp_Api_Exception
	 */
	...
	public function filter_user_agent( $user_agent ) {
		$user_agent .= ' PowerformAweber/' . POWERFORM_ADDON_AWEBER_VERSION;
		 ...
		 */
		$user_agent = apply_filters( 'powerform_addon_aweber_api_user_agent', $user_agent );
	...
	private function request( $verb, $url, $args = array(), $headers = array() ) {
		 ...
		 */
		$url = apply_filters( 'powerform_addon_aweber_api_url', $url, $verb, $args );
		 ...
		 * @param array  $args argument sent to this function
		 */
		$headers = apply_filters( 'powerform_addon_aweber_api_request_headers', $headers, $verb, $url, $args );
		 ...
		 * @param string $url  full url with scheme
		 */
		$args = apply_filters( 'powerform_addon_aweber_api_request_data', $request_data, $verb, $url );
		 ...
		 * @param array $_args
		 */
		$_args = apply_filters( 'powerform_addon_aweber_api_remote_request_args', $_args );
		...
		if ( is_wp_error( $res ) || ! $res ) {
			throw new Powerform_Addon_Aweber_Wp_Api_Exception(
				__( 'Failed to process request, make sure your API URL is correct and your server has internet connection.', Powerform::DOMAIN )
			);
		}
		...
		if ( isset( $res['response']['code'] ) ) {
			...
			if ( $status_code >= 400 ) {
				...
				if ( 404 === $status_code ) {
					/* translators: ... */
					throw new Powerform_Addon_Aweber_Wp_Api_Not_Found_Exception( sprintf( __( 'Failed to processing request : %s', Powerform::DOMAIN ), $msg ) );
				}
				/* translators: ... */
				throw new Powerform_Addon_Aweber_Wp_Api_Exception( sprintf( __( 'Failed to processing request : %s', Powerform::DOMAIN ), $msg ) );
			}
		}
		 ...
		 */
		$res = apply_filters( 'powerform_addon_aweber_api_response', $response, $body, $wp_response );
	...
	public function get_oauth_request_data() {
		...
		$oauth_request_data = array(
			...
			'oauth_nonce'            => Powerform_Addon_Aweber_Oauth::generate_oauth_nonce( $timestamp ),
		);
		 ...
		 */
		$oauth_request_data = apply_filters( 'powerform_addon_aweber_oauth_request_data', $oauth_request_data, $timestamp );
	...
	public function get_signed_request( $method, $url, $data ) {
		...
		$base                    = Powerform_Addon_Aweber_Oauth::create_signature_base( $method, $url, $data );
		$key                     = Powerform_Addon_Aweber_Oauth::create_signature_key( $application_secret, $oauth_token_secret );
		$data['oauth_signature'] = Powerform_Addon_Aweber_Oauth::create_signature( $base, $key );
		$signed_request          = $data;
		 ...
		 */
		$signed_request = apply_filters( 'powerform_addon_aweber_oauth_signed_request', $signed_request, $method, $url, $data, $application_secret, $oauth_token_secret );
	...
	public function get_prepared_request( $method, $url, $data ) {
		 ...
		 * @param array  $data
		 */
		$prepared_request_data = apply_filters( 'powerform_addon_aweber_oauth_prepared_request', $prepared_request_data, $method, $url, $data );
	...
	public function get_access_token( $oauth_verifier, $args = array() ) {
		...
		if ( ! is_object( $access_tokens ) ) {
			throw new Powerform_Addon_Aweber_Wp_Api_Exception( __( 'Invalid access token', Powerform::DOMAIN ) );
		}
	...
	public function add_account_list_subscriber( $account_id, $list_id, $args = array() ) {
		...
		if ( empty( $args['email'] ) ) {
			throw new Powerform_Addon_Aweber_Wp_Api_Exception( __( 'Email is required on add AWeber subscriber.', Powerform::DOMAIN ) );
		}
	...
	public function update_account_list_subscriber( $account_id, $list_id, $subscriber_id, $args = array() ) {
		...
		if ( empty( $args['email'] ) ) {
			throw new Powerform_Addon_Aweber_Wp_Api_Exception( __( 'Email is required on update AWeber subscriber.', Powerform::DOMAIN ) );
		}
	...
	public function get_access_token_url() {
		 ...
		 */
		$access_token_url = apply_filters( 'powerform_addon_aweber_oauth_access_token_url', $access_token_url );
	...
	public function get_api_url( $path ) {
		 ...
		 * @param string $path
		 */
		$api_url = apply_filters( 'powerform_addon_aweber_oauth_api_url', $api_url, $api_base_url, $path );
