...
/**
 * Class Powerform_Addon_Zapier_Quiz_Hooks
 *
 * @since 1.6.2
 *
 */
class Powerform_Addon_Zapier_Quiz_Hooks extends Powerform_Addon_Quiz_Hooks_Abstract {
	 ...
	 *
	 * @since 1.6.2
	 * @var Powerform_Addon_Zapier_Quiz_Settings | null
	 */
	protected $quiz_settings_instance;

	/**
	 * Powerform_Addon_Zapier_Quiz_Hooks constructor.
	 *
	 * @since 1.6.2
	 *
	...
	public function __construct( Powerform_Addon_Abstract $addon, $quiz_id ) {
		...
		$this->_submit_quiz_error_message = __( 'Zapier failed to process submitted data. Please check your form and try again', Powerform::DOMAIN );
	}
	...
	public function add_entry_fields( $submitted_data, $current_entry_fields = array() ) {
		 ...
		 * @param Powerform_Addon_Zapier_Quiz_Settings $quiz_settings_instance Zapier Addon Quiz Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_zapier_quiz_submitted_data',
			$submitted_data,
			$quiz_id,
		 ...
		 * @param Powerform_Addon_Zapier_Quiz_Settings $quiz_settings_instance Zapier Addon Quiz Settings instance
		 */
		do_action( 'powerform_addon_zapier_quiz_before_post_to_webhook', $quiz_id, $submitted_data, $quiz_settings_instance );
		 ...
		 * @param int                                   $quiz_id                current Quiz ID
		 * @param array                                 $submitted_data
		 * @param Powerform_Addon_Zapier_Quiz_Settings $quiz_settings_instance Zapier Quiz Settings instance
		 */
		$data = apply_filters(
			'powerform_addon_zapier_quiz_entry_fields',
			$entry_fields,
			$quiz_id,
			$submitted_data,
			$quiz_settings_instance
	...
	private function get_status_on_send_data( $connection_id, $submitted_data, $connection_settings, $quiz_settings, $current_entry_fields ) {
		...
		try {
			 ...
			 */
			$args = apply_filters(
				'powerform_addon_zapier_quiz_post_to_webhook_args',
				$args,
				$quiz_id,
				$connection_id,
				$submitted_data,
	...
	public function on_export_render_title_row() {
		...
		$export_headers = apply_filters(
			'powerform_addon_zapier_quiz_export_headers',
			$export_headers,
			$quiz_id,
			$quiz_settings_instance
		);
	...
	public function on_export_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$addon_meta_data = apply_filters(
			'powerform_addon_zapier_quiz_metadata',
			$addon_meta_data,
			$quiz_id,
			$quiz_settings_instance
		);
		...
		$export_columns = apply_filters(
			'powerform_addon_zapier_quiz_export_columns',
			$export_columns,
			$quiz_id,
			$entry_model,
			$addon_meta_data,
	 ...
	 * subentries included are defined in @see Powerform_Addon_Zapier_Quiz_Hooks::get_additional_entry_item()
	 *
	 * @since 1.6.1
	 *
	 * @param Powerform_Form_Entry_Model $entry_model
	...
	public function on_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		 ...
		 */
		$addon_meta_data = apply_filters(
			'powerform_addon_quiz_zapier_metadata',
			$addon_meta_data,
			$quiz_id,
			$quiz_settings_instance
		);
	 ...
	 * - Info : Text that are generated by addon when building and sending data to Zapier @see Powerform_Addon_Zapier_Quiz_Hooks::add_entry_fields()
