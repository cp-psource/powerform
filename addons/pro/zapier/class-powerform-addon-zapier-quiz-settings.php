...
require_once dirname( __FILE__ ) . '/class-powerform-addon-zapier-quiz-settings-exception.php';

/**
 * Class Powerform_Addon_Zapier_Quiz_Settings
 * Handle how quiz settings displayed and saved
 *
 * @since 1.6.2
 */
class Powerform_Addon_Zapier_Quiz_Settings extends Powerform_Addon_Quiz_Settings_Abstract {
	...
	public function setup_webhook_url( $submitted_data ) {
		...
		$template = powerform_addon_zapier_dir() . 'views/quiz-settings/setup-webhook.php';
		...
		if ( $is_submit ) {
			...
			try {
				$input_exceptions = new Powerform_Addon_Zapier_Quiz_Settings_Exception();
			...
			} catch ( Powerform_Addon_Zapier_Quiz_Settings_Exception $e ) {
		...
		if ( $this->setup_webhook_url_is_completed( array( 'multi_id' => $multi_id ) ) ) {
			$buttons['disconnect']['markup'] = Powerform_Addon_Zapier::get_button_markup(
				...
				esc_html__( 'Deactivate Zapier from this Quiz.', Powerform::DOMAIN )
			);
		}
	 ...
	 *
	 * @param                                                 $submitted_data
	 * @param Powerform_Addon_Zapier_Quiz_Settings_Exception $current_input_exception
	 *
	 * @throws Powerform_Addon_Zapier_Quiz_Settings_Exception
	 * @throws Powerform_Addon_Zapier_Wp_Api_Not_Found_Exception
	 * @throws Powerform_Addon_Zapier_Wp_Api_Exception
	 */
	private function validate_and_send_sample( $submitted_data, Powerform_Addon_Zapier_Quiz_Settings_Exception $current_input_exception ) {
		...
		$endpoint = apply_filters(
			'powerform_addon_zapier_quiz_endpoint',
			$endpoint,
			$quiz_id,
			$connection_settings
		);
		...
		$sample_data = apply_filters(
			'powerform_addon_zapier_quiz_sample_data',
			$sample_data,
			$quiz_id,
			$submitted_data
		);
	...
	private function build_form_sample_data() {
		...
		$sample['quiz-name'] = powerform_get_name_from_model( $this->quiz );
		$answers             = array();
