...
require_once dirname( __FILE__ ) . '/powerform-addon-zapier-poll-settings-exception.php';

/**
 * Class Powerform_Addon_Zapier_Poll_Settings
 * Handle how poll settings displayed and saved
 *
 * @since 1.6.1
 */
class Powerform_Addon_Zapier_Poll_Settings extends Powerform_Addon_Poll_Settings_Abstract {
	...
	public function setup_webhook_url( $submitted_data ) {
		...
		$template = powerform_addon_zapier_dir() . 'views/poll-settings/setup-webhook.php';
		...
		if ( $is_submit ) {
			...
			try {
				$input_exceptions = new Powerform_Addon_Zapier_Poll_Settings_Exception();

				if ( empty( $name ) ) {
					$input_exceptions->add_input_exception( __( 'Please specify integration name.', Powerform::DOMAIN ), 'name_error' );
				}
				...
				$notification = array(
					'type' => 'success',
					'text' => '<strong>' . $this->addon->get_title() . '</strong> ' . __( 'Successfully connected and sent sample data to your ZAP', Powerform::DOMAIN ),
				);
				$is_close     = true;

			} catch ( Powerform_Addon_Zapier_Poll_Settings_Exception $e ) {
		...
		if ( $this->setup_webhook_url_is_completed( array( 'multi_id' => $multi_id ) ) ) {
			$buttons['disconnect']['markup'] = Powerform_Addon_Zapier::get_button_markup( esc_html__( 'DISCONNECT', Powerform::DOMAIN ),
			                                                                               ...
			                                                                               esc_html__( 'Disconnect Zapier from this Poll.', Powerform::DOMAIN )
			);
		}
	 ...
	 *
	 * @param                                                 $submitted_data
	 * @param Powerform_Addon_Zapier_Poll_Settings_Exception $current_input_exception
	 *
	 * @throws Powerform_Addon_Zapier_Poll_Settings_Exception
	 * @throws Powerform_Addon_Zapier_Wp_Api_Not_Found_Exception
	 * @throws Powerform_Addon_Zapier_Wp_Api_Exception
	 */
	private function validate_and_send_sample( $submitted_data, Powerform_Addon_Zapier_Poll_Settings_Exception $current_input_exception ) {
		$poll_id = $this->poll_id;
		if ( ! isset( $submitted_data['webhook_url'] ) ) {
			$current_input_exception->add_input_exception( __( 'Please put a valid Webhook URL.', Powerform::DOMAIN ), 'webhook_url_error' );
		...
		if ( stripos( $submitted_data['webhook_url'], 'silent' ) !== false ) {
			$current_input_exception->add_input_exception( __( 'Please disable Silent Mode on Webhook URL.', Powerform::DOMAIN ), 'webhook_url_error' );
			throw $current_input_exception;
		}
		...
		$endpoint = apply_filters(
			'powerform_addon_zapier_poll_endpoint',
			$endpoint,
			$poll_id,
			$connection_settings
		);
		...
		$sample_data = apply_filters(
			'powerform_addon_zapier_poll_sample_data',
			$sample_data,
			$poll_id,
			$submitted_data
		);
	...
	private function build_form_sample_data() {
		...
		$sample['poll-name']  = powerform_get_name_from_model( $this->poll );
		$sample['vote']       = 'Vote';
