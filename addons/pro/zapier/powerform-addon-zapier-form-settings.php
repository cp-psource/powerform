...
require_once dirname( __FILE__ ) . '/powerform-addon-zapier-form-settings-exception.php';

/**
 * Class Powerform_Addon_Zapier_Form_Settings
 * Handle how form settings displayed and saved
 *
 * @since 1.0 Zapier Addon
 */
class Powerform_Addon_Zapier_Form_Settings extends Powerform_Addon_Form_Settings_Abstract {
	...
	/**
	 * Powerform_Addon_Zapier_Form_Settings constructor.
	 *
	 * @since 1.0 Zapier Addon
	 *
	...
	public function setup_webhook_url( $submitted_data ) {
		...
		try {
			if ( $is_submit ) {
				$input_exceptions = new Powerform_Addon_Zapier_Form_Settings_Exception();
				if ( empty( $current_data['name'] ) ) {
					$input_exceptions->add_input_exception( __( 'Please create a name for this Zapier integration', Powerform::DOMAIN ), 'name' );
				}
		...
		} catch ( Powerform_Addon_Zapier_Form_Settings_Exception $e ) {
		...
		if ( $this->setup_webhook_url_is_completed( $current_data ) ) {
			$buttons['disconnect']['markup'] = Powerform_Addon_Zapier::get_button_markup( esc_html__( 'DISCONNECT', Powerform::DOMAIN ),
			                                                                               'sui-button-ghost sui-tooltip sui-tooltip-top-center powerform-addon-form-disconnect',
			                                                                               esc_html__( 'Disconnect Zapier from this Form.', Powerform::DOMAIN )
			);
		}

		$buttons['next']['markup'] = '<div class="sui-actions-right">' .
		                             Powerform_Addon_Zapier::get_button_markup( esc_html__( 'Save', Powerform::DOMAIN ), 'sui-button-primary powerform-addon-finish' ) .
		                             '</div>';

		$help_message = esc_html__( 'Put your ZAP Webhook URL below.', Powerform::DOMAIN );

		$notice_message = sprintf(
			__( 'Please go %1$shere%2$s if you do not have any ZAP created. Remember to choose %3$s as Trigger App.', Powerform::DOMAIN ),
		...
		return array(
			'html'         => '<div class="integration-header"><h3 class="sui-box-title" id="dialogTitle2">' . __( 'Setup Webhook', Powerform::DOMAIN ) . '</h3>
							...
							<form enctype="multipart/form-data">
								<div class="sui-form-field ' . ( isset( $input_error_messages['name'] ) ? 'sui-form-field-error' : '' ) . '">
									<label class="sui-label">' . esc_html__( 'Zapier Integration Name', Powerform::DOMAIN ) . '</label>
									<div class="sui-control-with-icon">
										<input type="text"
											name="name"
											placeholder="' . esc_attr__( 'Friendly Name', Powerform::DOMAIN ) . '"
								...
								<div class="sui-form-field ' . ( isset( $input_error_messages['webhook_url'] ) ? 'sui-form-field-error' : '' ) . '">
									<label class="sui-label">' . esc_html__( 'Webhook URL', Powerform::DOMAIN ) . '</label>
									<div class="sui-control-with-icon">
										<input
										type="text"
										name="webhook_url"
										placeholder="' . esc_attr__( 'Webhook URL', Powerform::DOMAIN ) . '"
	 ...
	 *
	 * @param                                                 $submitted_data
	 * @param Powerform_Addon_Zapier_Form_Settings_Exception $current_input_exception
	 *
	 * @throws Powerform_Addon_Zapier_Form_Settings_Exception
	 * @throws Powerform_Addon_Zapier_Wp_Api_Not_Found_Exception
	 * @throws Powerform_Addon_Zapier_Wp_Api_Exception
	 */
	private function validate_and_send_sample( $submitted_data, Powerform_Addon_Zapier_Form_Settings_Exception $current_input_exception ) {
		$form_id = $this->form_id;
		if ( ! isset( $submitted_data['webhook_url'] ) ) {
			$current_input_exception->add_input_exception( __( 'Please put a valid Webhook URL.', Powerform::DOMAIN ), 'webhook_url' );
		...
		if ( stripos( $submitted_data['webhook_url'], 'silent' ) !== false ) {
			$current_input_exception->add_input_exception( __( 'Please disable Silent Mode on Webhook URL.', Powerform::DOMAIN ), 'webhook_url' );
			throw $current_input_exception;
		}
		...
		powerform_addon_maybe_log( __METHOD__, $endpoint );
		...
		$sample_data = apply_filters(
			'powerform_addon_zapier_sample_data',
			$sample_data,
			$form_id,
			$submitted_data
		);
