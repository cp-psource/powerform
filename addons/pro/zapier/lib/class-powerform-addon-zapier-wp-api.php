...
require_once dirname( __FILE__ ) . '/class-powerform-addon-zapier-wp-api-exception.php';
require_once dirname( __FILE__ ) . '/class-powerform-addon-zapier-wp-api-not-found-exception.php';

/**
 * Class Powerform_Addon_Zapier_Wp_Api
 */
class Powerform_Addon_Zapier_Wp_Api {
	...
	/**
	 * Powerform_Addon_Zapier_Wp_Api constructor.
	 *
	 * @since 1.0 Zapier Addon
	 *
	 * @param $_endpoint
	...
	public function __construct( $_endpoint ) {
		...
		//prerequisites
		if ( ! $_endpoint ) {
			throw new Powerform_Addon_Zapier_Wp_Api_Exception( __( 'Missing required Static Webhook URL', Powerform::DOMAIN ) );
		}
	...
	public function filter_user_agent( $user_agent ) {
		$user_agent .= ' PowerformZapier/' . POWERFORM_ADDON_ZAPIER_VERSION;
		 ...
		 */
		$user_agent = apply_filters( 'powerform_addon_zapier_api_user_agent', $user_agent );
	...
	private function request( $verb, $path, $args = array() ) {
		 ...
		 */
		$url = apply_filters( 'powerform_addon_zapier_api_url', $url, $verb, $path, $args );
		 ...
		 * @param array  $args argument sent to this function
		 */
		$headers = apply_filters( 'powerform_addon_zapier_api_request_headers', $headers, $verb, $path, $args );
		 ...
		 * @param string $path         requested path resource
		 */
		$args = apply_filters( 'powerform_addon_zapier_api_request_data', $request_data, $verb, $path );
		...
		if ( is_wp_error( $res ) || ! $res ) {
			...
			throw new Powerform_Addon_Zapier_Wp_Api_Exception(
		...
		if ( isset( $res['response']['code'] ) ) {
			...
			if ( $status_code > 400 ) {
				...
				if ( 404 === $status_code ) {
					/* translators: ... */
					throw new Powerform_Addon_Zapier_Wp_Api_Not_Found_Exception( sprintf( __( 'Failed to processing request : %s', Powerform::DOMAIN ), $msg ) );
				}
				/* translators: ... */
				throw new Powerform_Addon_Zapier_Wp_Api_Exception( sprintf( __( 'Failed to processing request : %s', Powerform::DOMAIN ), $msg ) );
			}
		}
		 ...
		 */
		$res = apply_filters( 'powerform_addon_zapier_api_response', $response, $body, $wp_response );
