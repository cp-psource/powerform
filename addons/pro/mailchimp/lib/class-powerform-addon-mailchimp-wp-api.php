...
require_once dirname( __FILE__ ) . '/class-powerform-addon-mailchimp-wp-api-exception.php';
require_once dirname( __FILE__ ) . '/class-powerform-addon-mailchimp-wp-api-not-found-exception.php';

/**
 * Class Powerform_Addon_Mailchimp_Wp_Api
...
class Powerform_Addon_Mailchimp_Wp_Api {
	...
	/**
	 * Powerform_Addon_Mailchimp_Wp_Api constructor.
	 *
	 * @since 1.0 Mailchimp Addon
	 *
	 * @param $api_key
	...
	public function __construct( $api_key ) {
		//prerequisite
		if ( ! $api_key ) {
			throw new Powerform_Addon_Mailchimp_Wp_Api_Exception( __( 'Missing required API Key', Powerform::DOMAIN ) );
		}
	...
	public function filter_user_agent( $user_agent ) {
		...
		$user_agent .= ' PowerformMailChimp/' . POWERFORM_ADDON_MAILCHIMP_VERSION;
		 ...
		 */
		$user_agent = apply_filters( 'powerform_addon_mailchimp_api_user_agent', $user_agent );
	 ...
	 * @throws Powerform_Addon_Mailchimp_Wp_Api_Exception
	 * @throws Powerform_Addon_Mailchimp_Wp_Api_Not_Found_Exception
	 */
	private function request( $verb, $path, $args = array() ) {
		 ...
		 */
		$url = apply_filters( 'powerform_addon_mailchimp_api_url', $url, $verb, $path, $args );

		$this->_last_url_request = $url;

		$headers = array(
			'Authorization' => 'PowerformMailChimp ' . $this->_api_key,
		);
		 ...
		 * @param array  $args argument sent to this function
		 */
		$headers = apply_filters( 'powerform_addon_mailchimp_api_request_headers', $headers, $verb, $path, $args );
		 ...
		 * @param string $path         requested path resource
		 */
		$args = apply_filters( 'powerform_addon_mailchimp_api_request_data', $request_data, $verb, $path );
		...
		if ( is_wp_error( $res ) || ! $res ) {
			powerform_addon_maybe_log( __METHOD__, $res );
			throw new Powerform_Addon_Mailchimp_Wp_Api_Exception( __( 'Failed to process request, make sure API KEY is correct and your server has internet connection.', Powerform::DOMAIN ) );
		}
		...
		if ( 'DELETE' !== $verb ) {
			...
			if ( empty( $body ) ) {
				powerform_addon_maybe_log( __METHOD__, $res );
				throw new Powerform_Addon_Mailchimp_Wp_Api_Exception(
					__(
		...
		if ( ! empty( $body ) ) {
			...
			if ( isset( $response->status ) ) {
				if ( $response->status >= 400 ) {
					powerform_addon_maybe_log( __METHOD__, $response );
					$msg = '';
					...
					if ( 404 === $response->status ) {
						/* translators: ... */
						throw new Powerform_Addon_Mailchimp_Wp_Api_Not_Found_Exception( sprintf( __( 'Failed to processing request : %s', Powerform::DOMAIN ), $msg ) );
					}
					/* translators: ... */
					throw new Powerform_Addon_Mailchimp_Wp_Api_Exception( sprintf( __( 'Failed to processing request : %s', Powerform::DOMAIN ), $msg ) );
				}
			}
			...
			if ( is_null( $response ) ) {
				...
				powerform_addon_maybe_log( __METHOD__, $res );
				/* translators: ... */
				throw new Powerform_Addon_Mailchimp_Wp_Api_Exception( sprintf( __( 'Failed to processing request : %s', Powerform::DOMAIN ), json_last_error_msg() ) );
			}
		}
		 ...
		 */
		$response = apply_filters( 'powerform_addon_mailchimp_api_response', $response, $body, $wp_response );
	...
	public function add_merge_field_to_list( $list_id, $args ) {
		...
		if ( ! in_array( $args['type'], $available_types, true ) ) {
			throw new Powerform_Addon_Mailchimp_Wp_Api_Exception( __( 'Invalid Field Type', Powerform::DOMAIN ) );
		}
	...
	public function add_member_to_list( $list_id, $args ) {
		...
		if ( ! isset( $args['email_address'] ) ) {
			throw new Powerform_Addon_Mailchimp_Wp_Api_Exception( __( 'email_address are required for mailchimp', Powerform::DOMAIN ) );
		}
	...
	public function add_or_update_member( $list_id, $subscriber_hash, $args ) {
		...
		$args = array_merge( $default_args, $args );
		if ( ! isset( $args['email_address'] ) ) {
			throw new Powerform_Addon_Mailchimp_Wp_Api_Exception( __( 'email_address are required for adding member to mailchimp list', Powerform::DOMAIN ) );
		}
