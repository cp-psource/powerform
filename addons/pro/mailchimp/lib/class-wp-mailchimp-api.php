...
class Powerform_Addon_Mailchimp_Wp_Api {
	...
	private function _request( $verb = 'GET', $path, $args = array() ) {
		...
		if ( 'DELETE' !== $verb ) {
			...
			if ( empty( $body ) ) {
				powerform_addon_maybe_log( __METHOD__, $res );
				throw new Powerform_Addon_Mailchimp_Wp_Api_Exception( __( 'Failed to process request, make sure API KEY is correct and your server has internet connection.',
				Powerform::DOMAIN ) );
			}
		}
