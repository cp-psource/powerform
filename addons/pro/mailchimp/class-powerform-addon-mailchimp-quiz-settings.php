...
require_once dirname( __FILE__ ) . '/class-powerform-addon-mailchimp-quiz-settings-exception.php';

/**
 * Class Powerform_Addon_Mailchimp_Quiz_Settings
 * Form Settings Mailchimp Process
 *
 * @since 1.0 Mailchimp Addon
 */
class Powerform_Addon_Mailchimp_Quiz_Settings extends Powerform_Addon_Quiz_Settings_Abstract {
	...
	public function mail_list_limit( $submitted_data ) {
		...
		if ( $this->addon->is_quiz_connected( $this->quiz_id ) ) {
			$buttons['disconnect']['markup'] = Powerform_Addon_Mailchimp::get_button_markup(
				...
				esc_html__( 'Disconnect Mailchimp from this quiz.', Powerform::DOMAIN )
			);
		}
	...
	public function choose_mail_list( $submitted_data ) {
		...
		try {
			...
			if ( $is_submit ) {
				...
				if ( empty( $mail_list_name ) ) {
					throw new Powerform_Addon_Mailchimp_Quiz_Settings_Exception( __( 'Please select valid Email List', Powerform::DOMAIN ), 'mail_list_id' );
				}
		...
		} catch ( Powerform_Addon_Mailchimp_Quiz_Settings_Exception $e ) {
		...
		if ( $this->addon->is_quiz_connected( $this->quiz_id ) ) {
			$buttons['disconnect']['markup'] = Powerform_Addon_Mailchimp::get_button_markup(
				...
				'sui-button-ghost sui-tooltip sui-tooltip-top-center powerform-addon-form-disconnect',
				esc_html__( 'Deactivate Mailchimp from this quiz.', Powerform::DOMAIN )
			);
		}
		...
		return array(
			'html'       => '<div class="sui-box-content integration-header"><h3 class="sui-box-title" id="dialogTitle2">' . __( 'Choose your list', Powerform::DOMAIN ) . '</h3>
							<span class="sui-description" style="margin-top: 20px;">' . __( 'Choose the list you want to send quiz data to.', Powerform::DOMAIN ) . '</span>
							' . $error_message . '</div>
	...
	public function map_fields( $submitted_data ) {
		...
		return array(
			'html'         => '<div class="sui-box-content integration-header"><h3 class="sui-box-title" id="dialogTitle2">' . __( 'Assign Fields', Powerform::DOMAIN ) . '</h3>
							<span class="sui-description" style="margin-top: 20px;">' . __( 'Lastly, match up your quiz fields with your campaign fields to make sure we\'re sending data to the right place.', Powerform::DOMAIN ) . '</span>
							' . $error_message . '</div>
							<form enctype="multipart/form-data">
								' . $html_input_map_fields . '
							</form>',
	...
	private function get_input_map_fields( $email_fields, $mailchimp_fields_list, $current_data ) {
        ...
        <table class="sui-table">
            <thead>
            <tr>
                <th><?php echo esc_html( __( 'Mailchimp Field', Powerform::DOMAIN ) ); ?></th>
                <th><?php echo esc_html( __( 'Powerform Field', Powerform::DOMAIN ) ); ?></th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td><?php echo esc_html( __( 'Email Address', Powerform::DOMAIN ) ); ?> <span class="integrations-required-field">*</span></td>
                <td>
	...
	public function step_map_fields_validate( $mailchimp_fields_list, $mailchimp_required_fields, $post_data ) {
		...
		if ( ! isset( $post_data['fields_map'] ) ) {
			...
			throw new Powerform_Addon_Mailchimp_Exception( $this->_update_quiz_settings_error_message );
		}
		...
		$input_exceptions = new Powerform_Addon_Mailchimp_Quiz_Settings_Exception();
		// EMAIL : super required**
		if ( ! isset( $post_data['EMAIL'] ) || empty( $post_data['EMAIL'] ) ) {
			$this->_update_quiz_settings_error_message = __( 'Please choose valid Powerform field for email address.', Powerform::DOMAIN );
		...
		foreach ( $this->addon_quiz_settings['fields_map'] as $mailchimp_field_tag => $powerform_field_id ) {
			if ( empty( $powerform_field_id ) ) {
				continue;
			}
			if ( is_array( $powerform_field_id ) ) {
				foreach ( $powerform_field_id as $addr => $field_id ) {
					if ( ! empty( $field_id ) ) {
				...
				foreach ( $powerform_field_id as $addr => $field_id ) {
					...
					if ( ! empty( $address_value ) && ! in_array( $field_id, $powerform_field_element_ids, true ) ) {
			...
			if ( ! is_array( $powerform_field_id ) && ! in_array( $powerform_field_id, $powerform_field_element_ids, true ) ) {
