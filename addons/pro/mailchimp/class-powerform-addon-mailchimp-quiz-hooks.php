...
/**
 * Class Powerform_Addon_Mailchimp_Quiz_Hooks
 *
 * Hooks that used by Mailchimp Addon defined here
 *
 * @since 1.0 Mailchimp Addon
 */
class Powerform_Addon_Mailchimp_Quiz_Hooks extends Powerform_Addon_Quiz_Hooks_Abstract {
	 ...
	 *
	 * @since 1.0 Mailchimp Addon
	 * @var Powerform_Addon_Mailchimp_Quiz_Settings | null
	 *
	 */
	...
	/**
	 * Powerform_Addon_Mailchimp_Form_Hooks constructor.
	 *
	 * @since 1.0 Mailchimp Addon
	 *
	...
	public function __construct( Powerform_Addon_Abstract $addon, $quiz_id ) {
		...
		$this->_submit_quiz_error_message = __( 'Mailchimp failed to process submitted data. Please check your form and try again', Powerform::DOMAIN );
	}
	...
	public function on_after_render_form_fields() {
		 ...
		 * @param Powerform_Addon_Mailchimp_Quiz_Settings $quiz_settings_instance Mailchimp quiz settings Instance
		 */
		do_action(
			'powerform_addon_mailchimp_on_after_render_quiz_fields',
			$quiz_id,
	...
	private function render_gdpr_field( $addon_setting_values ) {
		...
		if ( 'clean' === $design_class ) {
			...
			$html .= sprintf(
				'<input id="%s" type="checkbox" name="%s" value="1" class="powerform-checkbox--input">',
				$input_id,
				$input_name
			);
			$html .= sprintf(
				'<label for="%s" class="powerform-checkbox--design wpdui-icon wpdui-icon-check" aria-hidden="true"></label>',
				$input_id
			);
			$html .= sprintf(
				'<label for="%s" class="powerform-checkbox--label">%s</label>',
				$input_id,
				$addon_setting_values['gdpr_text']
			);
			$html .= '</div>';
	...
	public function on_form_submit( $submitted_data ) {
		 ...
		 * @param int                                      $quiz_id                current Quiz ID
		 * @param Powerform_Addon_Mailchimp_Quiz_Settings $quiz_settings_instance Mailchimp API Form Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_mailchimp_form_submitted_data',
			$submitted_data,
			$quiz_id,
			$quiz_settings_instance
		);
		...
		$is_success = apply_filters(
			'powerform_addon_mailchimp_on_form_submit_result',
			$is_success,
			$quiz_id,
			$submitted_data,
			$quiz_settings_instance
		...
		if ( ! isset( $submitted_data['powerform-addon-mailchimp-gdpr'] ) || '1' !== $submitted_data['powerform-addon-mailchimp-gdpr'] ) {
	...
	public function add_entry_fields( $submitted_data, $form_entry_fields = array() ) {
		 ...
		 * @param array                                    $submitted_data
		 * @param int                                      $quiz_id                current Quiz ID
		 * @param Powerform_Addon_Mailchimp_Quiz_Settings $quiz_settings_instance Mailchimp API Quiz Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_mailchimp_quiz_submitted_data',
			$submitted_data,
			$quiz_id,
			$quiz_settings_instance
		);
		...
		$form_entry_fields   = powerform_lead_form_data( $submitted_data );
		...
		try {
			...
			powerform_addon_maybe_log( __METHOD__, $mailchimp_fields_list_request );
			...
			} catch ( Powerform_Addon_Mailchimp_Wp_Api_Not_Found_Exception $e ) {
			...
			powerform_addon_maybe_log( __METHOD__, $mailchimp_fields_list, $addon_setting_values, $submitted_data, $merge_fields );
			 ...
			 * @param Powerform_Addon_Mailchimp_Quiz_Settings $quiz_settings_instance Mailchimp Form Settings
			 */
			$mail_list_id = apply_filters(
				'powerform_addon_mailchimp_add_update_member_request_mail_list_id',
				$mail_list_id,
				$quiz_id,
				$submitted_data,
				$quiz_settings_instance
			 ...
			 * @param int                                      $quiz_id                current Quiz ID
			 * @param array                                    $submitted_data         Submitted data
			 * @param Powerform_Addon_Mailchimp_Quiz_Settings $quiz_settings_instance Mailchimp Quiz Settings
			 */
			$args = apply_filters(
				'powerform_addon_mailchimp_add_update_member_request_args',
				$args,
				$quiz_id,
				$submitted_data,
				$quiz_settings_instance
			 ...
			 * @param Powerform_Addon_Mailchimp_Quiz_Settings $quiz_settings_instance Mailchimp Quiz Settings
			 */
			do_action( 'powerform_addon_mailchimp_before_add_update_member', $quiz_id, $submitted_data, $quiz_settings_instance );
			...
			powerform_addon_maybe_log( __METHOD__, 'Success Add Member' );
			...
			powerform_addon_maybe_log( __METHOD__, 'Failed to Add Member' );
			...
			$entry_fields = array(
				array(
					...
					'value' => array(
						...
						'data_sent'     => ( ( $mailchimp_api instanceof Powerform_Addon_Mailchimp_Wp_Api ) ? $mailchimp_api->get_last_data_sent() : array() ),
						'data_received' => ( ( $mailchimp_api instanceof Powerform_Addon_Mailchimp_Wp_Api ) ? $mailchimp_api->get_last_data_received() : array() ),
						'url_request'   => ( ( $mailchimp_api instanceof Powerform_Addon_Mailchimp_Wp_Api ) ? $mailchimp_api->get_last_url_request() : '' ),
					),
				),
			);
		}
		...
		$entry_fields = apply_filters(
			'powerform_addon_mailchimp_entry_fields',
			$entry_fields,
			$quiz_id,
			$submitted_data,
			$quiz_settings_instance
	 ...
	 * - Below subentries will be added if full log enabled, @see Powerform_Addon_Mailchimp::is_show_full_log() @see POWERFORM_ADDON_MAILCHIMP_SHOW_FULL_LOG
	...
	public function on_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$addon_meta_data = apply_filters(
			'powerform_addon_mailchimp_metadata',
			$addon_meta_data,
			$quiz_id,
			$entry_model,
			$quiz_settings_instance
		...
		$entry_items = apply_filters(
			'powerform_addon_mailchimp_entry_items',
			$entry_items,
			$quiz_id,
			$entry_model,
			$addon_meta_data,
	 ...
	 * @return array
	 */
	private function format_metadata_for_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$additional_entry_item = array(
			'label' => __( 'Mailchimp Integration', Powerform::DOMAIN ),
			'value' => '',
		);
		...
		if ( Powerform_Addon_Mailchimp::is_show_full_log() ) {
			// too long to be added on entry data enable this with `define('POWERFORM_ADDON_MAILCHIMP_SHOW_FULL_LOG', true)`
	...
	public function on_export_render_title_row() {
		...
		$export_headers = apply_filters(
			'powerform_addon_mailchimp_export_headers',
			$export_headers,
			$quiz_id,
			$quiz_settings_instance
		);
	...
	public function on_export_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$export_columns = apply_filters(
			'powerform_addon_mailchimp_export_columns',
			$export_columns,
			$quiz_id,
			$entry_model,
			$addon_meta_data,
	...
	public function on_before_delete_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		 ...
		 * @param int                                          $quiz_id                current quiz ID
		 * @param Powerform_Form_Entry_Model                  $entry_model            Powerform Entry Model
		 * @param Powerform_Addon_Quiz_Settings_Abstract|null $quiz_settings_instance of Addon quiz Settings
		 */
		$addon_meta_data = apply_filters(
			'powerform_addon_mailchimp_metadata',
			$addon_meta_data,
		 ...
		 */
		do_action(
			'powerform_addon_mailchimp_on_before_delete_submission',
			$quiz_id,
			$entry_model,
			$addon_meta_data,
			$quiz_settings_instance
		...
		if ( ! Powerform_Addon_Mailchimp::is_enable_delete_member() ) {
		...
		try {
			...
			$delete_member_url = apply_filters(
				'powerform_addon_mailchimp_delete_member_url',
				$delete_member_url,
				$quiz_id,
				$addon_meta_data,
				$quiz_settings_instance
			...
			powerform_addon_maybe_log( __METHOD__, $delete_member_url );
			...
			// handle all internal addon exceptions with `Powerform_Addon_Mailchimp_Exception`

			// use wp_error, for future usage it can be returned to page entries
			$wp_error = new WP_Error( 'powerform_addon_mailchimp_delete_member', $e->getMessage() );
