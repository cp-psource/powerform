...
require_once dirname( __FILE__ ) . '/class-powerform-addon-mailchimp-exception.php';
require_once dirname( __FILE__ ) . '/lib/class-powerform-addon-mailchimp-wp-api.php';

/**
 * Class Powerform_Addon_Mailchimp
 * The class that defines mailchimp addon
 *
 * @since 1.0 Mailchimp Addon
 */
class Powerform_Addon_Mailchimp extends Powerform_Addon_Abstract {
	 ...
	 * @var string
	 */
	protected $_version = POWERFORM_ADDON_MAILCHIMP_VERSION;
	 ...
	 * @var string
	 */
	protected $_form_settings = 'Powerform_Addon_Mailchimp_Form_Settings';
	 ...
	 * @var string
	 */
	protected $_form_hooks = 'Powerform_Addon_Mailchimp_Form_Hooks';
	 ...
	 * @var string
	 */
	protected $_quiz_settings = 'Powerform_Addon_Mailchimp_Quiz_Settings';
	 ...
	 * @var string
	 */
	protected $_quiz_hooks = 'Powerform_Addon_Mailchimp_Quiz_Hooks';

	/**
	 * Hold account information that currently connected
	 * Will be saved to @see Powerform_Addon_Mailchimp::save_settings_values()
	 *
	 * @since 1.0 Mailchimp Addon
	 * @var array
	 */
	...
	/**
	 * Powerform_Addon_Mailchimp constructor.
	...
	public function __construct() {
		...
		$this->_description                = __( 'Make form data as Mailchimp List', Powerform::DOMAIN );
		$this->_activation_error_message   = __( 'Sorry but we failed to activate Mailchimp Integration, don\'t hesitate to contact us', Powerform::DOMAIN );
		$this->_deactivation_error_message = __( 'Sorry but we failed to deactivate Mailchimp Integration, plese try again', Powerform::DOMAIN );
		...
		$this->_icon     = powerform_addon_mailchimp_assets_url() . 'icons/mailchimp.png';
		$this->_icon_x2  = powerform_addon_mailchimp_assets_url() . 'icons/mailchimp@2x.png';
		$this->_image    = powerform_addon_mailchimp_assets_url() . 'img/mailchimp.png';
		$this->_image_x2 = powerform_addon_mailchimp_assets_url() . 'img/mailchimp@2x.png';
	}
	 ...
	 * to include @see Powerform_Addon_Mailchimp::$_connected_account
	...
	public function before_save_settings_values( $values ) {
		powerform_addon_maybe_log( __METHOD__, $values );
	...
	public function is_connected() {
		try {
			// check if its active
			if ( ! $this->is_active() ) {
				throw new Powerform_Addon_Mailchimp_Exception( __( 'Mailchimp is not active', Powerform::DOMAIN ) );
			}
		 ...
		 */
		$is_connected = apply_filters( 'powerform_addon_mailchimp_is_connected', $is_connected );
	...
	public function is_form_connected( $form_id ) {
		...
		try {
			...
			if ( ! $this->is_connected() ) {
				throw new Powerform_Addon_Mailchimp_Exception( __( 'Mailchimp addon not connected.', Powerform::DOMAIN ) );
			}

			$form_settings_instance = $this->get_addon_form_settings( $form_id );
			if ( ! $form_settings_instance instanceof Powerform_Addon_Mailchimp_Form_Settings ) {
				throw new Powerform_Addon_Mailchimp_Exception( __( 'Form settings instance is not valid Powerform_Addon_Mailchimp_Form_Settings.', Powerform::DOMAIN ) );
			}
			...
			if ( ! $last_step_is_complete ) {
				throw new Powerform_Addon_Mailchimp_Exception( __( 'Form settings is not yet completed.', Powerform::DOMAIN ) );
			}
		 ...
		 * @param Powerform_Addon_Mailchimp_Form_Settings|null $form_settings_instance Instance of form settings, or null when unavailable
		 *
		 */
		$is_form_connected = apply_filters( 'powerform_addon_mailchimp_is_form_connected', $is_form_connected, $form_id, $form_settings_instance );
	...
	public function is_quiz_connected( $quiz_id ) {
		...
		try {
			...
			if ( ! $quiz_settings_instance instanceof Powerform_Addon_Mailchimp_Quiz_Settings ) {
				throw new Powerform_Addon_Mailchimp_Exception( __( 'Form settings instance is not valid Powerform_Addon_Mailchimp_Quiz_Settings.', Powerform::DOMAIN ) );
			}
		 ...
		 * @param Powerform_Addon_Mailchimp_Quiz_Settings|null $quiz_settings_instance Instance of form settings, or null when unavailable
		 *
		 */
		$is_quiz_connected = apply_filters( 'powerform_addon_mailchimp_is_form_connected', $is_quiz_connected, $quiz_id, $quiz_settings_instance );
	...
	protected function validate_api_key( $api_key ) {
		if ( empty( $api_key ) ) {
			$this->_update_settings_error_message = __( 'Please add valid Mailchimp API Key.', Powerform::DOMAIN );
		...
		try {
			...
			powerform_addon_maybe_log( __METHOD__, $info );
		...
		} catch ( Powerform_Addon_Mailchimp_Wp_Api_Exception $e ) {
	 ...
	 * @param null $api_key
	 *
	 * @return Powerform_Addon_Mailchimp_Wp_Api|null
	 * @throws Powerform_Addon_Mailchimp_Wp_Api_Exception
	 */
	public function get_api( $api_key = null ) {
		if ( is_null( self::$api ) ) {
			...
			$api       = Powerform_Addon_Mailchimp_Wp_Api::get_instance( $api_key );
			self::$api = $api;
		}
	...
	public function settings_help() {
		...
		$help = sprintf( __( 'Please get your Mailchimp API key %1$s', Powerform::DOMAIN ), '<a href="https://admin.mailchimp.com/account/api-key-popup" target="_blank">here</a>' );
	...
	public function settings_description() {
		...
		if (
			...
			// Show currently connected mailchimp account if its already connected
			/* translators:  placeholder is Name and Email of Connected MailChimp Account */
			$description .= '<span class="sui-description">' . esc_html__( 'Please take a note that changing API Key or disconnect will affect to ALL of your connected forms.', Powerform::DOMAIN ) . '</span>';
	...
	public function settings_account() {
		...
		if (
			...
			$myaccount .= sprintf(
				/* translators:  placeholder is Name and Email of Connected MailChimp Account */
				__( 'Your Mailchimp is connected to %1$s: %2$s.', Powerform::DOMAIN ),
	 ...
	 * set @see POWERFORM_ADDON_MAILCHIMP_SHOW_FULL_LOG to `true` on wp-config.php to show it
	 *
	 * @since 1.0 Mailchimp Addon
	 * @return bool
	 */
	public static function is_show_full_log() {
		if ( defined( 'POWERFORM_ADDON_MAILCHIMP_SHOW_FULL_LOG' ) && POWERFORM_ADDON_MAILCHIMP_SHOW_FULL_LOG ) {
			return true;
		}
	 ...
	 *
	 * Default is `true`,
	 * which can be changed via `POWERFORM_ADDON_MAILCHIMP_ENABLE_DELETE_MEMBER` constant
	 *
	 * @return bool
	 */
	public static function is_enable_delete_member() {
		if ( defined( 'POWERFORM_ADDON_MAILCHIMP_ENABLE_DELETE_MEMBER' ) && POWERFORM_ADDON_MAILCHIMP_ENABLE_DELETE_MEMBER ) {
			return true;
		}
	 ...
	 * And disabled by default to enable it set @see POWERFORM_ADDON_MAILCHIMP_ENABLE_GDPR to true in wp-config.php
	...
	public static function is_enable_gdpr() {
		if ( defined( 'POWERFORM_ADDON_MAILCHIMP_ENABLE_GDPR' ) && POWERFORM_ADDON_MAILCHIMP_ENABLE_GDPR ) {
			return true;
		}
	...
	public function configure_api_key( $submitted_data, $form_id = 0 ) {
		...
		if ( isset( $submitted_data['api_key'] ) ) {
			 ...
			 */
			$api_key_validated = apply_filters( 'powerform_addon_mailchimp_validate_api_key', $api_key_validated, $api_key );
			...
			if ( ! $api_key_validated ) {
				...
				if ( ! powerform_addon_is_active( $this->_slug ) ) {
					...
					if ( ! $activated ) {
						$error_message = '<div class="sui-notice sui-notice-error"><p>' . Powerform_Addon_Loader::get_instance()->get_last_error_message() . '</p></div>';
						$show_success  = false;
					} else {
				...
				if ( $show_success ) {
					...
					return array(
						'html'         => '<div class="integration-header"><h3 class="sui-box-title" id="dialogTitle2">' .
										/* translators: ... */
										sprintf( __( '%1$s Added', Powerform::DOMAIN ), 'Mailchimp' ) .
										'</h3></div>
						...
						'buttons'      => array(
							'close' => array(
								'markup' => self::get_button_markup( esc_html__( 'Close', Powerform::DOMAIN ), 'powerform-addon-close' ),
							),
						),
						'redirect'     => false,
						'has_errors'   => false,
		...
		if ( $is_edit ) {
			...
			$buttons['submit'] = array(
				'markup' => self::get_button_markup( esc_html__( 'Connect', Powerform::DOMAIN ), 'powerform-addon-connect' ),
			);
		}
		...
		return array(
			'html'       => '<div class="integration-header">
					<h3 class="sui-box-title" id="dialogTitle2">' .
							/* translators: ... */
							sprintf( __( 'Configure %1$s', Powerform::DOMAIN ), 'Mailchimp' ) .
							'</h3>
					' . $this->settings_help() . '
					' . $error_message . '
				</div>
				<form>
					<div class="sui-form-field ' . ( ! empty( $api_key_error_message ) ? 'sui-form-field-error' : '' ) . '">
						<label class="sui-label">' . __( 'API Key', Powerform::DOMAIN ) . '</label>
						<div class="sui-control-with-icon">
							<input name="api_key"
								placeholder="' .
							/* translators: ... */
							sprintf( __( 'Enter %1$s API Key', Powerform::DOMAIN ), 'Mailchimp' ) .
							'"
	...
	public function is_quiz_lead_connected( $quiz_id ) {
		 ...
		 *
		 */
		$is_quiz_connected = apply_filters( 'powerform_addon_mailchimp_is_quiz_lead_connected', $is_quiz_connected, $quiz_id, $quiz_settings_instance );
