...
require_once dirname( __FILE__ ) . '/class-powerform-addon-googlesheet-form-settings-exception.php';

/**
 * Class Powerform_Addon_Googlesheet_Form_Settings
 * Handle how form settings displayed and saved
 *
 * @since 1.0 Google Sheets Addon
 */
class Powerform_Addon_Googlesheet_Form_Settings extends Powerform_Addon_Form_Settings_Abstract {
	...
	public function pick_name( $submitted_data ) {
		$template = powerform_addon_googlesheet_dir() . 'views/form-settings/pick-name.php';
		...
		if ( $this->pick_name_is_completed( array( 'multi_id' => $multi_id ) ) ) {
			$buttons['disconnect']['markup'] = Powerform_Addon_Abstract::get_button_markup(
				...
				esc_html__( 'Deactivate Google Sheets Integration from this Form.', Powerform::DOMAIN )
			);
		}
	...
	public function setup_sheet( $submitted_data ) {
		$template = powerform_addon_googlesheet_dir() . 'views/form-settings/setup-sheet.php';
		...
		if ( $is_submit ) {
			...
			try {
				$input_exceptions = new Powerform_Addon_Googlesheet_Form_Settings_Exception();
				if ( empty( $file_name ) ) {
			...
			} catch ( Powerform_Addon_Googlesheet_Form_Settings_Exception $e ) {
