...
require_once dirname( __FILE__ ) . '/class-powerform-addon-googlesheet-exception.php';
...
final class Powerform_Addon_Googlesheet extends Powerform_Addon_Abstract {
	...
	/**
	 * Get Powerform_Google_Client Object
	 *
	 * @since 1.0 Google Sheets Addon
	...
	public function get_google_client() {
		...
		$client_secret = $this->get_client_secret();
		$scopes        = array(
			Powerform_Google_Service_Sheets::SPREADSHEETS,
			Powerform_Google_Service_Sheets::DRIVE,
		);

		$config = new Powerform_Google_Config();
		$config->setLoggerClass( 'Powerform_Addon_Wp_Googlesheet_Client_Logger' );
		$google_client = new Powerform_Google_Client( $config );
		$google_client->setApplicationName( __( 'Powerform', Powerform::DOMAIN ) );
		 ...
		 * @since 1.2
		 *
		 * @param Powerform_Google_Client $google_client
		 * @param string        $client_id
		 * @param string        $client_secret
		 * @param array         $scopes
