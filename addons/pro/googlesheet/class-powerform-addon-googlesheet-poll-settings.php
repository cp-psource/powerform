...
require_once dirname( __FILE__ ) . '/class-powerform-addon-googlesheet-poll-settings-exception.php';

/**
 * Class Powerform_Addon_Googlesheet_Poll_Settings
 * Handle how poll settings displayed and saved
 *
 * @since 1.6.1
 */
class Powerform_Addon_Googlesheet_Poll_Settings extends Powerform_Addon_Poll_Settings_Abstract {
	...
	public function pick_name( $submitted_data ) {
		$template = powerform_addon_googlesheet_dir() . 'views/poll-settings/pick-name.php';
		...
		if ( $this->pick_name_is_completed( array( 'multi_id' => $multi_id ) ) ) {
			$buttons['disconnect']['markup'] = Powerform_Addon_Abstract::get_button_markup(
				...
				esc_html__( 'Deactivate this Google Sheets Integration from this Poll.', Powerform::DOMAIN )
			);
		}
	...
	public function setup_sheet( $submitted_data ) {
		$template = powerform_addon_googlesheet_dir() . 'views/poll-settings/setup-sheet.php';
		...
		if ( $is_submit ) {
			...
			try {
				$input_exceptions = new Powerform_Addon_Googlesheet_Poll_Settings_Exception();
				if ( empty( $file_name ) ) {
				...
				if ( ! empty( $folder_id ) ) {
					$drive = new Powerform_Google_Service_Drive( $google_client );
					try {
					...
					} catch ( Powerform_Google_Exception $google_exception ) {
						// catch 404
				...
				$file = new Powerform_Google_Service_Drive_DriveFile();
			...
			} catch ( Powerform_Addon_Googlesheet_Poll_Settings_Exception $e ) {
