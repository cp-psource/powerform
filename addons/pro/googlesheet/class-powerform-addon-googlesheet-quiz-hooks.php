...
/**
 * Class Powerform_Addon_Googlesheet_Quiz_Hooks
 *
 * @since 1.6.2
 *
 */
class Powerform_Addon_Googlesheet_Quiz_Hooks extends Powerform_Addon_Quiz_Hooks_Abstract {
	 ...
	 *
	 * @since 1.6.2
	 * @var Powerform_Addon_Googlesheet_Quiz_Settings | null
	 */
	...
	/**
	 * Powerform_Addon_Googlesheet_Quiz_Hooks constructor.
	 *
	 * @since 1.6.2
	 *
	...
	public function __construct( Powerform_Addon_Abstract $addon, $quiz_id ) {
		...
		$this->_submit_quiz_error_message = __( 'Google Sheets failed to process submitted data. Please check your quiz and try again', Powerform::DOMAIN );
	}
	...
	public function add_entry_fields( $submitted_data, $current_entry_fields = array() ) {
		 ...
		 * @param Powerform_Addon_Googlesheet_Quiz_Settings $quiz_settings_instance Google Sheets Addon quiz Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_googlesheet_quiz_submitted_data',
			$submitted_data,
		...
		$current_entry_fields = apply_filters(
			'powerform_addon_googlesheet_quiz_entry_fields',
			$current_entry_fields,
			$submitted_data,
			$quiz_id,
			$quiz_settings_instance
		 ...
		 * @param Powerform_Addon_Googlesheet_Quiz_Settings $quiz_settings_instance Google Sheets Addon Quiz Settings instance
		 */
		do_action( 'powerform_addon_googlesheet_quiz_before_create_row', $quiz_id, $submitted_data, $quiz_settings_instance );
		 ...
		 */
		$data = apply_filters(
			'powerform_addon_quiz_googlesheet_entry_fields',
			$entry_fields,
			$quiz_id,
			$submitted_data,
			$current_entry_fields,
	...
	public function get_status_on_create_row( $connection_id, $submitted_data, $connection_settings, $quiz_entry_fields ) {
		...
		try {
			 ...
			 * @param Powerform_Addon_Googlesheet_Quiz_Settings $quiz_settings_instance Google Sheets Addon Quiz Settings instance
			 */
			do_action( 'powerform_addon_quiz_googlesheet_before_prepare_sheet_headers', $connection_settings, $quiz_id, $submitted_data, $quiz_entry_fields, $quiz_settings_instance );
			...
			$header_fields = apply_filters(
				'powerform_addon_quiz_googlesheet_sheet_headers',
				$header_fields,
				$connection_settings,
				$quiz_id,
				$submitted_data,
			 ...
			 * @param Powerform_Addon_Googlesheet_Quiz_Settings $quiz_settings_instance Google Sheets Addon Quiz Settings instance
			 */
			do_action( 'powerform_addon_quiz_googlesheet_after_prepare_sheet_headers', $header_fields, $connection_settings, $quiz_id, $submitted_data, $quiz_entry_fields, $quiz_settings_instance );
						...
						if ( 'knowledge' === $this->quiz->quiz_type ) {
							foreach ( $quiz_entry['value'] as $data ) {
								...
								$answers[] = array(
									'question' => $question,
									'answer'   => $answer,
									'result'   => $is_correct ? __( 'Correct', Powerform::DOMAIN ) : __( 'Incorrect', Powerform::DOMAIN ),
								);
							}
			...
			foreach ( $answers as $i => $answer ) {
				...
				foreach ( $header_fields as $column_name => $header_field ) {
					...
					if ( self::GSHEET_DATE_COLUMN_NAME === $column_name ) {
						$value     = new Powerform_Google_Service_Sheets_ExtendedValue();
						$cell_data = new Powerform_Google_Service_Sheets_CellData();
						...
						powerform_addon_maybe_log( __METHOD__, $field_type, $meta_value );
				...
				// Build the RowData
				$row_data = new Powerform_Google_Service_Sheets_RowData();
				$row_data->setValues( $values );

				// Prepare the request
				$append_request = new Powerform_Google_Service_Sheets_AppendCellsRequest();
				$append_request->setSheetId( 0 );
				$append_request->setRows( $row_data );
				$append_request->setFields( 'userEnteredValue' );

				// Set the request
				$request = new Powerform_Google_Service_Sheets_Request();
			...
			if ( empty( $requests ) ) {
				throw new Powerform_Addon_Googlesheet_Exception( __( 'No data to be send to GoogleSheet', Powerform::DOMAIN ) );
			}

			// Prepare the update
			$batch_update_request = new Powerform_Google_Service_Sheets_BatchUpdateSpreadsheetRequest(
				array(
					'requests' => $requests,
				)
			);
			...
			$spreadsheet_service = new Powerform_Google_Service_Sheets( $google_client );
			...
			return array(
				...
				'description'     => __( 'Successfully send data to Google Sheets', Powerform::DOMAIN ),
			);

		} catch ( Powerform_Google_Exception $e ) {
			powerform_addon_maybe_log( __METHOD__, 'Failed to Send to Google Sheets' );
	...
	public function get_sheet_headers( $file_id, $quiz_settings = array() ) {
		...
		foreach ( $required_header_columns as $required_header_column ) {
			...
			if ( ! in_array( $required_header_column, array_keys( $header_fields ), true ) ) {
				...
				$column_number ++;
				$update_body = new Powerform_Google_Service_Sheets_ValueRange();
		...
		if ( $new_column_needed > 0 ) {
			$dimension_range = new Powerform_Google_Service_Sheets_DimensionRange();
			...
			$insert_dimension = new Powerform_Google_Service_Sheets_InsertDimensionRequest();
			...
			$request_body = new Powerform_Google_Service_Sheets_BatchUpdateSpreadsheetRequest();
		...
		if ( ! empty( $update_bodies ) ) {
			$request_body = new Powerform_Google_Service_Sheets_BatchUpdateValuesRequest();
		...
		$grid_properties = new Powerform_Google_Service_Sheets_GridProperties();
		$grid_properties->setFrozenRowCount( 1 );

		$sheet_properties = new Powerform_Google_Service_Sheets_SheetProperties();
		$sheet_properties->setSheetId( 0 );
		$sheet_properties->setGridProperties( $grid_properties );

		$update_properties = new Powerform_Google_Service_Sheets_UpdateSheetPropertiesRequest();
	 ...
	 * its called `Google Sheets Info` which can be translated on powerform lang
	 *
	 * @since 1.6.2
	 * @return array
	 */
	public function on_export_render_title_row() {
		...
		$export_headers = apply_filters(
			'powerform_addon_quiz_googlesheet_export_headers',
			$export_headers,
			$quiz_id,
			$quiz_settings_instance
		);
	...
	public function on_export_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$addon_meta_data = apply_filters(
			'powerform_addon_quiz_googlesheet_metadata',
			$addon_meta_data,
			$quiz_id,
			$quiz_settings_instance
		);
		...
		$export_columns = apply_filters(
			'powerform_addon_quiz_googlesheet_export_columns',
			$export_columns,
			$quiz_id,
			$entry_model,
			$addon_meta_data,
	 ...
	 * sub-entries included are defined in @see Powerform_Addon_Googlesheet_Quiz_Hooks::get_additional_entry_item()
	 *
	 * @since 1.6.2
	 *
	 * @param Powerform_Form_Entry_Model $entry_model
	...
	public function on_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		 ...
		 * @param Powerform_Addon_Googlesheet_Quiz_Settings $quiz_settings_instance Google Sheets Addon Form Settings instance
		 */
		$addon_meta_data = apply_filters(
			'powerform_addon_quiz_googlesheet_metadata',
			$addon_meta_data,
	 ...
	 * - Info : Text that are generated by addon when building and sending data to Google Sheets @see Powerform_Addon_Googlesheet_Quiz_Hooks::add_entry_fields()
	 *
	 * @param $addon_meta_data
	 *
	 * @since 1.6.2
	...
	private function get_additional_entry_item( $addon_meta_data ) {
		...
		$additional_entry_item = array(
			'label' => __( 'Google Sheets Integration', Powerform::DOMAIN ),
			'value' => '',
		);
		...
		if ( isset( $status['is_sent'] ) ) {
			...
			$sub_entries[] = array(
				'label' => __( 'Sent To Google Sheets', Powerform::DOMAIN ),
				'value' => $is_sent,
			);
		}
