 ...
 */
class Powerform_Google_Task_Runner
{
  /**
  ...
  /**
   * @var Powerform_Google_Client $client The current API client.
   */
  private $client;
   ...
   *
   * @param Powerform_Google_Client $client The current API client
   * @param string $name The name of the current task (used for logging)
   * @param callable $action The task to run and possibly retry
   * @param array $arguments The task arguments
   * @throws Powerform_Google_Task_Exception when misconfigured
   */
  public function __construct(
    ...
    $config = (array) $client->getClassConfig('Powerform_Google_Task_Runner');

    if (isset($config['initial_delay'])) {
      if ($config['initial_delay'] < 0) {
        throw new Powerform_Google_Task_Exception(
   ...
   *
   * @return mixed
   * @throws Powerform_Google_Task_Retryable on failure when no retries are available.
   */
  public function run()
  {
    while ($this->attempt()) {
      ...
      } catch (Powerform_Google_Task_Retryable $exception) {
