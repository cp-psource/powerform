 ...
 */
abstract class Powerform_Google_Logger_Abstract
{
  /**
   * Default log format
   */
  ...
  /**
   * @param Powerform_Google_Client $client  The current Google client
   */
  public function __construct(Powerform_Google_Client $client)
  {
    $this->setLevel(
        $client->getClassConfig('Powerform_Google_Logger_Abstract', 'level')
    );

    $format = $client->getClassConfig('Powerform_Google_Logger_Abstract', 'log_format');
    $this->logFormat = $format ? $format : self::DEFAULT_LOG_FORMAT;

    $format = $client->getClassConfig('Powerform_Google_Logger_Abstract', 'date_format');
    $this->dateFormat = $format ? $format : self::DEFAULT_DATE_FORMAT;

    $this->allowNewLines = (bool) $client->getClassConfig(
        'Powerform_Google_Logger_Abstract',
        'allow_newlines'
    );
  }
   ...
   * @throws Powerform_Google_Logger_Exception If $level is invalid
   */
  protected function normalizeLevel($level)
  {
    ...
    throw new Powerform_Google_Logger_Exception(
