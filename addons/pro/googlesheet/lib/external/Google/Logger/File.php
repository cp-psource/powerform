 ...
 */
class Powerform_Google_Logger_File extends Powerform_Google_Logger_Abstract
{
  /**
  ...
  public function __construct(Powerform_Google_Client $client)
  {
    ...
    $file = $client->getClassConfig('Powerform_Google_Logger_File', 'file');
    ...
    $mode = $client->getClassConfig('Powerform_Google_Logger_File', 'mode');
    if (!$mode) {
      $this->mode = $mode;
    }

    $this->lock = (bool) $client->getClassConfig('Powerform_Google_Logger_File', 'lock');
    $this->file = $file;
  }
  ...
  protected function write($message)
  {
    if (is_string($this->file)) {
      ...
      throw new Powerform_Google_Logger_Exception('File pointer is no longer available');
    }
