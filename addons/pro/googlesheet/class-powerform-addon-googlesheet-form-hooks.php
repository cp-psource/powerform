...
/**
 * Class Powerform_Addon_Googlesheet_Form_Hooks
 *
 * @since 1.0 Google Sheets Addon
 *
 */
class Powerform_Addon_Googlesheet_Form_Hooks extends Powerform_Addon_Form_Hooks_Abstract {
	 ...
	 *
	 * @since 1.0 Google Sheets Addon
	 * @var Powerform_Addon_Googlesheet_Form_Settings | null
	 */
	protected $form_settings_instance;

	/**
	 * Powerform_Addon_Googlesheet_Form_Hooks constructor.
	 *
	 * @since 1.0 Google Sheets Addon
	 *
	...
	public function __construct( Powerform_Addon_Abstract $addon, $form_id ) {
		...
		$this->_submit_form_error_message = __( 'Google Sheets failed to process submitted data. Please check your form and try again', Powerform::DOMAIN );
	}
	...
	public function add_entry_fields( $submitted_data, $form_entry_fields = array() ) {
		 ...
		 * @param Powerform_Addon_Googlesheet_Form_Settings $form_settings_instance Google Sheets Addon Form Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_googlesheet_form_submitted_data',
			$submitted_data,
			$form_entry_fields,
			$form_id,
		...
		$form_entry_fields = apply_filters(
			'powerform_addon_googlesheet_form_entry_fields',
			$form_entry_fields,
			$submitted_data,
			$form_id,
			$form_settings_instance
		 ...
		 * @param Powerform_Addon_Googlesheet_Form_Settings $form_settings_instance Google Sheets Addon Form Settings instance
		 */
		do_action( 'powerform_addon_googlesheet_before_create_row', $form_id, $submitted_data, $form_settings_instance );
		 ...
		 */
		$data = apply_filters(
			'powerform_addon_googlesheet_entry_fields',
			$entry_fields,
			$form_id,
			$submitted_data,
			$form_entry_fields,
	...
	public function get_status_on_create_row( $connection_id, $submitted_data, $connection_settings, $form_entry_fields ) {
		...
		try {
			 ...
			 * @param Powerform_Addon_Googlesheet_Form_Settings $form_settings_instance Google Sheets Addon Form Settings instance
			 */
			do_action( 'powerform_addon_googlesheet_before_prepare_sheet_headers', $connection_settings, $form_id, $submitted_data, $form_entry_fields, $form_settings_instance );
			...
			$header_fields = apply_filters(
				'powerform_addon_googlesheet_sheet_headers',
				$header_fields,
				$connection_settings,
				$form_id,
				$submitted_data,
			 ...
			 * @param Powerform_Addon_Googlesheet_Form_Settings $form_settings_instance Google Sheets Addon Form Settings instance
			 */
			do_action( 'powerform_addon_googlesheet_after_prepare_sheet_headers', $header_fields, $connection_settings, $form_id, $submitted_data, $form_entry_fields, $form_settings_instance );
			...
			foreach ( $header_fields as $element_id => $header_field ) {
				...
				$form_value = Powerform_Form_Entry_Model::meta_value_to_string( $field_type, $meta_value, false );
	...
	public function get_sheet_headers( $file_id ) {
		...
		if ( isset( $values[0] ) && is_array( $values[0] ) ) {
			foreach ( $values[0] as $value ) {
				...
				// powerform header field format = 'FIELD-label|field-id'
	 ...
	 * subentries included are defined in @see Powerform_Addon_Googlesheet_Form_Hooks::get_additional_entry_item()
	 *
	 * @since 1.0 Google Sheets Addon
	 *
	...
	public function on_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$addon_meta_data = apply_filters(
			'powerform_addon_googlesheet_metadata',
			$addon_meta_data,
			$form_id,
			$form_settings_instance
		);
	 ...
	 * - Info : Text that are generated by addon when building and sending data to Google Sheets @see Powerform_Addon_Googlesheet_Form_Hooks::add_entry_fields()
	 *
	 * @param $addon_meta_data
	 *
	 * @since 1.0 Google Sheets Addon
	...
	public function on_export_render_title_row() {
		...
		$export_headers = apply_filters(
			'powerform_addon_googlesheet_export_headers',
			$export_headers,
			$form_id,
			$form_settings_instance
		);
	...
	public function on_export_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$export_columns = apply_filters(
			'powerform_addon_googlesheet_export_columns',
			$export_columns,
			$form_id,
			$entry_model,
			$addon_meta_data,
