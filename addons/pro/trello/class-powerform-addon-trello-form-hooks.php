...
/**
 * Class Powerform_Addon_Trello_Form_Hooks
 *
 * @since 1.0 Trello Addon
 *
 */
class Powerform_Addon_Trello_Form_Hooks extends Powerform_Addon_Form_Hooks_Abstract {
	 ...
	 *
	 * @since 1.0 Trello Addon
	 * @var Powerform_Addon_Trello_Form_Settings | null
	 */
	protected $form_settings_instance;

	/**
	 * Powerform_Addon_Trello_Form_Hooks constructor.
	 *
	 * @since 1.0 Trello Addon
	 *
	...
	public function __construct( Powerform_Addon_Abstract $addon, $form_id ) {
		...
		$this->_submit_form_error_message = __( 'Trello failed to process submitted data. Please check your form and try again', Powerform::DOMAIN );
	}
	...
	public function add_entry_fields( $submitted_data, $form_entry_fields = array() ) {
		 ...
		 * @param Powerform_Addon_Trello_Form_Settings $form_settings_instance Trello Addon Form Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_trello_form_submitted_data',
			$submitted_data,
			$form_entry_fields,
			$form_id,
		...
		$form_entry_fields = apply_filters(
			'powerform_addon_trello_form_entry_fields',
			$form_entry_fields,
			$submitted_data,
			$form_id,
			$form_settings_instance
		 ...
		 * @param Powerform_Addon_Trello_Form_Settings $form_settings_instance Trello Addon Form Settings instance
		 */
		do_action( 'powerform_addon_trello_before_create_card', $form_id, $submitted_data, $form_settings_instance );
		 ...
		 */
		$data = apply_filters(
			'powerform_addon_trello_entry_fields',
			$entry_fields,
			$form_id,
			$submitted_data,
			$form_entry_fields,
	...
	private function get_status_on_create_card( $connection_id, $submitted_data, $connection_settings, $form_entry_fields ) {
		...
		try {
			...
			if ( isset( $connection_settings['card_name'] ) ) {
				...
				$card_name = powerform_addon_replace_custom_vars( $card_name, $submitted_data, $this->custom_form, $form_entry_fields, false );
				...
				$card_name    = apply_filters(
					'powerform_addon_trello_card_name',
					$card_name,
					$form_id,
					$connection_id,
					$submitted_data,
			...
			if ( isset( $connection_settings['card_description'] ) ) {
				...
				$card_description       = powerform_addon_replace_custom_vars( $card_description, $submitted_data, $this->custom_form, $form_entry_fields, false );
				...
				$card_description = apply_filters(
					'powerform_addon_trello_card_description',
					$card_description,
					$form_id,
					$connection_id,
					$submitted_data,
			...
			if ( isset( $connection_settings['due_date'] ) && ! empty( $connection_settings['due_date'] ) ) {
				$due_date            = powerform_addon_replace_custom_vars( $connection_settings['due_date'], $submitted_data, $this->custom_form, $form_entry_fields, false );
				$args['due']         = $due_date;
			}
			 ...
			 */
			$args = apply_filters(
				'powerform_addon_trello_create_card_args',
				$args,
				$form_id,
				$connection_id,
				$submitted_data,
	...
	private function all_fields_to_markdown() {
		...
		foreach ( $form_fields as $form_field ) {
			...
			switch ( $field_type ) {
				...
				case 'postdata-post-image':
					$value_format = '[' . __( 'Edit Post', Powerform::DOMAIN ) . ']({' . $post_element_id . '})';
					break;
				case 'url':
		...
		$markdown = apply_filters(
			'powerform_addon_trello_all_fields_markdown',
			$markdown,
			$form_fields
		);
	...
	public static function get_field_markdown( $type, $label, $value ) {
		 ...
		 */
		$markdown = apply_filters(
			'powerform_addon_trello_field_markdown',
			$markdown,
			$type,
			$label,
			$value
	 ...
	 * subentries included are defined in @see Powerform_Addon_Trello_Form_Hooks::get_additional_entry_item()
	 *
	 * @since 1.0 Trello Addon
	 *
	...
	public function on_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$addon_meta_data = apply_filters(
			'powerform_addon_trello_metadata',
			$addon_meta_data,
			$form_id,
			$form_settings_instance
		);
	 ...
	 * - Info : Text that are generated by addon when building and sending data to Trello @see Powerform_Addon_Trello_Form_Hooks::add_entry_fields()
	...
	public function on_export_render_title_row() {
		...
		$export_headers = apply_filters(
			'powerform_addon_trello_export_headers',
			$export_headers,
			$form_id,
			$form_settings_instance
		);
	...
	public function on_export_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$export_columns = apply_filters(
			'powerform_addon_trello_export_columns',
			$export_columns,
			$form_id,
			$entry_model,
			$addon_meta_data,
	...
	public function on_before_delete_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		 ...
		 */
		do_action(
			'powerform_addon_trello_on_before_delete_submission',
			$form_id,
			$entry_model,
			$addon_meta_data,
			$form_settings_instance
		...
		if ( ! Powerform_Addon_Trello::is_enable_delete_card() ) {
		...
		try {
			if ( is_array( $addon_meta_data ) ) {
				$card_delete_mode = Powerform_Addon_Trello::get_card_delete_mode();
			...
			// handle all internal addon exceptions with `Powerform_Addon_Trello_Exception`

			// use wp_error, for future usage it can be returned to page entries
			$wp_error
				= new WP_Error( 'powerform_addon_trello_delete_card', $e->getMessage() );
	 ...
	 * @throws Powerform_Addon_Trello_Wp_Api_Not_Found_Exception
	 */
	public function delete_card( $card_id, $card_delete_mode, $addon_meta_datum ) {
		 ...
		 */
		$args = apply_filters(
			'powerform_addon_trello_delete_card_args',
			$args,
			$card_id,
			$card_delete_mode,
			$addon_meta_datum,
		...
		switch ( $card_delete_mode ) {
			case Powerform_Addon_Trello::CARD_DELETE_MODE_DELETE:
				$api->delete_card( $card_id, $args );
				break;
			case Powerform_Addon_Trello::CARD_DELETE_MODE_CLOSED:
		 ...
		 */
		do_action(
			'powerform_addon_trello_delete_card',
			$args,
			$card_id,
			$card_delete_mode,
			$addon_meta_datum,
