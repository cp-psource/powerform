...
require_once dirname( __FILE__ ) . '/class-powerform-addon-trello-wp-api-exception.php';
require_once dirname( __FILE__ ) . '/class-powerform-addon-trello-wp-api-not-found-exception.php';

/**
 * Class Powerform_Addon_Trello_Wp_Api
 */
class Powerform_Addon_Trello_Wp_Api {
	...
	/**
	 * Powerform_Addon_Trello_Wp_Api constructor.
	 *
	 * @since 1.0 Trello Addon
	 *
	...
	public function __construct( $app_key, $token ) {
		//prerequisites
		if ( ! $app_key ) {
			throw new Powerform_Addon_Trello_Wp_Api_Exception( __( 'Missing required APP Key', Powerform::DOMAIN ) );
		}

		if ( ! $token ) {
			throw new Powerform_Addon_Trello_Wp_Api_Exception( __( 'Missing required Token', Powerform::DOMAIN ) );
		}
	...
	public function filter_user_agent( $user_agent ) {
		$user_agent .= ' PowerformTrello/' . POWERFORM_ADDON_TRELLO_VERSION;
		 ...
		 */
		$user_agent = apply_filters( 'powerform_addon_trello_api_user_agent', $user_agent );
	...
	private function request( $verb, $path, $args = array() ) {
		 ...
		 */
		$url = apply_filters( 'powerform_addon_trello_api_url', $url, $verb, $path, $args );
		 ...
		 * @param array  $args argument sent to this function
		 */
		$headers = apply_filters( 'powerform_addon_trello_api_request_headers', $headers, $verb, $path, $args );
		 ...
		 * @param string $path         requested path resource
		 */
		$args = apply_filters( 'powerform_addon_trello_api_request_data', $request_data, $verb, $path );
		...
		if ( is_wp_error( $res ) || ! $res ) {
			...
			throw new Powerform_Addon_Trello_Wp_Api_Exception(
				__( 'Failed to process request, make sure you authorized Trello and your server has internet connection.', Powerform::DOMAIN )
			);
		}
		...
		if ( isset( $res['response']['code'] ) ) {
			...
			if ( $status_code > 400 ) {
				...
				if ( 404 === $status_code ) {
					/* translators: ... */
					throw new Powerform_Addon_Trello_Wp_Api_Not_Found_Exception( sprintf( __( 'Failed to processing request : %s', Powerform::DOMAIN ), $msg ) );
				}
				/* translators: ... */
				throw new Powerform_Addon_Trello_Wp_Api_Exception( sprintf( __( 'Failed to processing request : %s', Powerform::DOMAIN ), $msg ) );
			}
		}
		 ...
		 */
		$res = apply_filters( 'powerform_addon_trello_api_response', $response, $body, $wp_response );
	...
	public function create_card( $args = array() ) {
		$default_args = array(
			'name' => __( 'Powerform Trello Card', Powerform::DOMAIN ),
			'pos'  => 'bottom',
		);
		$args         = array_merge( $default_args, $args );

		if ( ! isset( $args['idList'] ) ) {
			throw new Powerform_Addon_Trello_Wp_Api_Exception( __( 'idList Required to create a Trello Card', Powerform::DOMAIN ) );
		}
