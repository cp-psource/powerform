...
require_once dirname( __FILE__ ) . '/class-powerform-addon-trello-exception.php';
require_once dirname( __FILE__ ) . '/lib/class-powerform-addon-trello-wp-api.php';

/**
 * Class Powerform_Addon_Trello
 * Trello Addon Main Class
 *
 * @since 1.0 Trello Addon
 */
final class Powerform_Addon_Trello extends Powerform_Addon_Abstract {
	...
	protected $_version                = POWERFORM_ADDON_TRELLO_VERSION;
	...
	protected $_form_settings = 'Powerform_Addon_Trello_Form_Settings';
	protected $_form_hooks    = 'Powerform_Addon_Trello_Form_Hooks';
	...
	protected $_poll_settings = 'Powerform_Addon_Trello_Poll_Settings';
	protected $_poll_hooks    = 'Powerform_Addon_Trello_Poll_Hooks';

	protected $_quiz_settings = 'Powerform_Addon_Trello_Quiz_Settings';
	protected $_quiz_hooks    = 'Powerform_Addon_Trello_Quiz_Hooks';

	/**
	 * Powerform_Addon_Trello constructor.
	 *
	 * @since 1.0 Trello Addon
	 */
	public function __construct() {
		...
		$this->_activation_error_message   = __( 'Sorry but we failed to activate Trello Integration, don\'t hesitate to contact us', Powerform::DOMAIN );
		$this->_deactivation_error_message = __( 'Sorry but we failed to deactivate Trello Integration, please try again', Powerform::DOMAIN );
		...
		$this->_icon     = powerform_addon_trello_assets_url() . 'icons/trello.png';
		$this->_icon_x2  = powerform_addon_trello_assets_url() . 'icons/trello@2x.png';
		$this->_image    = powerform_addon_trello_assets_url() . 'img/trello.png';
		$this->_image_x2 = powerform_addon_trello_assets_url() . 'img/trello@2x.png';
	}
	...
	public function is_connected() {
		try {
			// check if its active
			if ( ! $this->is_active() ) {
				throw new Powerform_Addon_Trello_Exception( __( 'Trello is not active', Powerform::DOMAIN ) );
			}
		...
		} catch ( Powerform_Addon_Trello_Exception $e ) {
			$is_connected = false;
		}
		 ...
		 */
		$is_connected = apply_filters( 'powerform_addon_trello_is_connected', $is_connected );
	...
	public function is_form_connected( $form_id ) {
		try {
			...
			if ( ! $this->is_connected() ) {
				throw new Powerform_Addon_Trello_Exception( __( ' Trello is not connected', Powerform::DOMAIN ) );
			}

			$form_settings_instance = $this->get_addon_form_settings( $form_id );
			if ( ! $form_settings_instance instanceof Powerform_Addon_Trello_Form_Settings ) {
				throw new Powerform_Addon_Trello_Exception( __( 'Invalid Form Settings of Trello', Powerform::DOMAIN ) );
			}

			// Mark as active when there is at least one active connection
			if ( false === $form_settings_instance->find_one_active_connection() ) {
				throw new Powerform_Addon_Trello_Exception( __( 'No active Trello connection found in this form', Powerform::DOMAIN ) );
			}
		 ...
		 * @param Powerform_Addon_Trello_Form_Settings|null $form_settings_instance Instance of form settings, or null when unavailable
		 *
		 */
		$is_form_connected = apply_filters( 'powerform_addon_trello_is_form_connected', $is_form_connected, $form_id, $form_settings_instance );
	...
	public static function is_show_full_log() {
		$show_full_log = ( defined( 'POWERFORM_ADDON_TRELLO_SHOW_FULL_LOG' ) && POWERFORM_ADDON_TRELLO_SHOW_FULL_LOG );
		 ...
		 */
		$show_full_log = apply_filters( 'powerform_addon_trello_show_full_log', $show_full_log );
	...
	public static function is_enable_delete_card() {
		$enable_delete_card = ( defined( 'POWERFORM_ADDON_TRELLO_ENABLE_DELETE_CARD' ) && POWERFORM_ADDON_TRELLO_ENABLE_DELETE_CARD );
		 ...
		 */
		$enable_delete_card = apply_filters( 'powerform_addon_trello_delete_card', $enable_delete_card );
	 ...
	 * default is 'delete'
	 *
	 * @see   Powerform_Addon_Trello::is_enable_delete_card()
	 *
	 * @since 1.0 Trello Addon
	 * @return string
	 */
	public static function get_card_delete_mode() {
		...
		if ( defined( 'POWERFORM_ADDON_TRELLO_CARD_DELETE_MODE' ) ) {
			$card_delete_mode = POWERFORM_ADDON_TRELLO_CARD_DELETE_MODE;
		}
		 ...
		 */
		$card_delete_mode = apply_filters( 'powerform_addon_trello_card_delete_mode', $card_delete_mode );
	...
	public function get_app_key() {
		...
		if ( defined( 'POWERFORM_ADDON_TRELLO_APP_KEY' ) && POWERFORM_ADDON_TRELLO_APP_KEY ) {
			$app_key = POWERFORM_ADDON_TRELLO_APP_KEY;
		}
		 ...
		 */
		$app_key = apply_filters( 'powerform_addon_trello_app_key', $app_key );
	...
	public function authorize_access() {
		$template = powerform_addon_trello_dir() . 'views/settings/authorize.php';
	...
	public function wait_authorize_access() {
		$template         = powerform_addon_trello_dir() . 'views/settings/wait-authorize.php';
		$template_success = powerform_addon_trello_dir() . 'views/settings/success-authorize.php';
	...
	public function authorize_page_callback( $query_args ) {
		$template        = powerform_addon_trello_dir() . 'views/sections/authorize.php';
		$template_params = array(
			'error_message' => '',
			'is_close'      => false,
		);
		...
		if ( isset( $query_args['token'] ) ) {
			try {
				...
				if ( true !== $validated ) {
					throw new Powerform_Addon_Trello_Exception( $validated );
				}
				if ( ! $this->is_active() ) {
					...
					if ( ! $activated ) {
						...
						throw new Powerform_Addon_Trello_Exception( $last_message );
					}
				}
	...
	public function get_auth_url( $return_url = '' ) {
		...
		if ( ! $return_url ) {
			$return_url = powerform_addon_integration_section_admin_url( $this->_slug, 'authorize', true );
		}
		...
		$auth_params = array(
			...
			'expiration'      => 'never',
			'name'            => __( 'Powerform', POWERFORM::DOMAIN ),
		 ...
		 */
		$auth_params = apply_filters( 'powerform_addon_trello_authorize_params', $auth_params );
	...
	public function validate_token( $token ) {
		try {
			...
			if ( ! isset( $me_request->id ) || empty( $me_request->id ) ) {
				throw new Powerform_Addon_Trello_Exception( __( 'Failed to acquire user ID.', Powerform::DOMAIN ) );
			}
	 ...
	 * @param null $token
	 *
	 * @return Powerform_Addon_Trello_Wp_Api
	 * @throws Powerform_Addon_Trello_Wp_Api_Exception
	 */
	public function get_api( $token = null ) {
		if ( is_null( self::$api ) ) {
			...
			$api       = new Powerform_Addon_Trello_Wp_Api( $this->get_app_key(), $token );
			self::$api = $api;
		}
	...
	public function before_get_settings_values( $values ) {
		...
		if ( isset( $values['token'] ) ) {
			...
			powerform_addon_maybe_log( __METHOD__, $this->_token );
		}
	...
	public function deactivate() {
		...
		} catch ( Powerform_Addon_Trello_Wp_Api_Exception $e ) {
	...
	public static function get_card_delete_modes() {
		 ...
		 */
		$card_delete_modes = apply_filters( 'powerform_addon_trello_card_delete_modes', $card_delete_modes );
	...
	public function is_poll_connected( $poll_id ) {
		try {
			...
			if ( ! $this->is_connected() ) {
				throw new Powerform_Addon_Trello_Exception( 'Trello is not connected' );
			}

			$poll_settings_instance = $this->get_addon_poll_settings( $poll_id );
			if ( ! $poll_settings_instance instanceof Powerform_Addon_Trello_Poll_Settings ) {
				throw new Powerform_Addon_Trello_Exception( 'Invalid Poll Settings of Trello' );
			}

			// Mark as active when there is at least one active connection
			if ( false === $poll_settings_instance->find_one_active_connection() ) {
				throw new Powerform_Addon_Trello_Exception( 'No active Poll connection found in this poll' );
			}
		 ...
		 *
		 */
		$is_poll_connected = apply_filters( 'powerform_addon_trello_is_poll_connected', $is_poll_connected, $poll_id, $poll_settings_instance );
	...
	public function is_quiz_connected( $quiz_id ) {
		try {
			...
			if ( ! $quiz_settings_instance instanceof Powerform_Addon_Trello_Quiz_Settings ) {
				throw new Powerform_Addon_Trello_Exception( 'Invalid Quiz Settings of Trello' );
			}

			// Mark as active when there is at least one active connection
			if ( false === $quiz_settings_instance->find_one_active_connection() ) {
				throw new Powerform_Addon_Trello_Exception( 'No active Trello connection found in this quiz' );
			}
		 ...
		 *
		 */
		$is_quiz_connected = apply_filters( 'powerform_addon_trello_is_quiz_connected', $is_quiz_connected, $quiz_id, $quiz_settings_instance );
