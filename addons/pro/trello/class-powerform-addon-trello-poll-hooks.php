...
/**
 * Class Powerform_Addon_Trello_Poll_Hooks
 *
 * @since 1.6.1
 *
 */
class Powerform_Addon_Trello_Poll_Hooks extends Powerform_Addon_Poll_Hooks_Abstract {
	 ...
	 *
	 * @since 1.6.1
	 * @var Powerform_Addon_Trello_Poll_Settings | null
	 */
	protected $poll_settings_instance;

	/**
	 * Powerform_Addon_Trello_Poll_Hooks constructor.
	 *
	 * @since 1.6.1
	 *
	...
	public function __construct( Powerform_Addon_Abstract $addon, $poll_id ) {
		...
		$this->_submit_poll_error_message = __( 'Trello failed to process submitted data. Please check your form and try again', Powerform::DOMAIN );
	}
	...
	public function add_entry_fields( $submitted_data, $current_entry_fields = array() ) {
		 ...
		 * @param Powerform_Addon_Trello_Poll_Settings $poll_settings_instance Trello Addon Poll Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_trello_poll_submitted_data',
			$submitted_data,
		...
		$current_entry_fields = apply_filters(
			'powerform_addon_trello_poll_current_entry_fields',
			$current_entry_fields,
			$submitted_data,
			$poll_id,
			$poll_settings_instance
		 ...
		 * @param Powerform_Addon_Trello_Poll_Settings $poll_settings_instance Trello Addon Poll Settings instance
		 */
		do_action( 'powerform_addon_trello_poll_before_create_card', $poll_id, $submitted_data, $poll_settings_instance );
		 ...
		 */
		$data = apply_filters(
			'powerform_addon_trello_poll_entry_fields',
			$entry_fields,
			$poll_id,
			$submitted_data,
			$current_entry_fields,
	...
	private function get_status_on_create_card( $connection_id, $submitted_data, $connection_settings, $current_entry_fields ) {
		...
		try {
			...
			if ( isset( $connection_settings['card_name'] ) ) {
				...
				$card_name = str_ireplace( '{poll_name}', powerform_get_name_from_model( $this->poll ), $card_name );
				...
				$card_name    = apply_filters(
					'powerform_addon_trello_poll_card_name',
					$card_name,
					$poll_id,
					$connection_id,
					$submitted_data,
			...
			if ( isset( $connection_settings['card_description'] ) ) {
				...
				$card_description = apply_filters(
					'powerform_addon_trello_poll_card_description',
					$card_description,
					$poll_id,
					$connection_id,
					$submitted_data,
			 ...
			 */
			$args = apply_filters(
				'powerform_addon_trello_poll_create_card_args',
				$args,
				$poll_id,
				$connection_id,
				$submitted_data,
	...
	private function poll_name_to_markdown() {
		...
		$poll_name = powerform_get_name_from_model( $this->poll );
		...
		$markdown = apply_filters(
			'powerform_addon_trello_poll_name_markdown',
			$markdown
		);
	...
	private function poll_answer_to_markdown( $submitted_data ) {
		...
		$markdown  = '##' . __( 'Poll Answer', Powerform::DOMAIN ) . "\n";
		$markdown .= '**' . __( 'Vote', Powerform::DOMAIN ) . ':** ' . $answer;
		if ( ! empty( $extra ) ) {
			$markdown .= "\n**" . __( 'Extra', Powerform::DOMAIN ) . ':** ' . $extra;
		}
		 ...
		 */
		$markdown = apply_filters(
			'powerform_addon_trello_poll_answer_markdown',
			$markdown,
			$submitted_data,
			$fields_labels
		);
	...
	private function poll_result_to_markdown( $submitted_data ) {
		...
		$map_entries  = Powerform_Form_Entry_Model::map_polls_entries( $this->poll_id, $fields_array );
		...
		$markdown = '##' . __( 'Poll Results', Powerform::DOMAIN );
		if ( ! is_null( $fields ) ) {
		 ...
		 */
		$markdown = apply_filters(
			'powerform_addon_trello_poll_result_markdown',
			$markdown,
			$fields_array,
			$map_entries
		);
	...
	private function poll_answer_to_plain_text( $submitted_data ) {
		...
		$plain_text = apply_filters(
			'powerform_addon_trello_poll_answer_plain_text',
			$plain_text,
			$submitted_data,
			$fields_labels
		);
	...
	private function poll_result_to_plain_text( $submitted_data ) {
		 ...
		 */
		$plain_text = apply_filters(
			'powerform_addon_trello_poll_result_plain_text',
			$plain_text,
			$fields_array,
			$map_entries
		);
	...
	public function on_export_render_title_row() {
		...
		$export_headers = apply_filters(
			'powerform_addon_trello_poll_export_headers',
			$export_headers,
			$poll_id,
			$poll_settings_instance
		);
	...
	public function on_export_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$addon_meta_data = apply_filters(
			'powerform_addon_trello_poll_metadata',
			$addon_meta_data,
			$poll_id,
			$poll_settings_instance
		);
		...
		$export_columns = apply_filters(
			'powerform_addon_trello_poll_export_columns',
			$export_columns,
			$poll_id,
			$entry_model,
			$addon_meta_data,
