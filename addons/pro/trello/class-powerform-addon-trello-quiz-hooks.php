...
/**
 * Class Powerform_Addon_Trello_Quiz_Hooks
 *
 * @since 1.6.2
 *
 */
class Powerform_Addon_Trello_Quiz_Hooks extends Powerform_Addon_Quiz_Hooks_Abstract {
	 ...
	 *
	 * @since 1.6.2
	 * @var Powerform_Addon_Trello_Quiz_Settings | null
	 */
	protected $quiz_settings_instance;

	/**
	 * Powerform_Addon_Trello_Quiz_Hooks constructor.
	 *
	 * @since 1.6.2
	 *
	...
	public function __construct( Powerform_Addon_Abstract $addon, $quiz_id ) {
		...
		$this->_submit_quiz_error_message = __( 'Trello failed to process submitted data. Please check your form and try again', Powerform::DOMAIN );
	}
	...
	public function add_entry_fields( $submitted_data, $current_entry_fields = array() ) {
		 ...
		 * @param Powerform_Addon_Trello_Quiz_Settings $quiz_settings_instance Trello Addon Quiz Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_trello_quiz_submitted_data',
			$submitted_data,
		...
		$current_entry_fields = apply_filters(
			'powerform_addon_trello_quiz_current_entry_fields',
			$current_entry_fields,
			$submitted_data,
			$quiz_id,
			$quiz_settings_instance
		 ...
		 * @param Powerform_Addon_Trello_Quiz_Settings $quiz_settings_instance Trello Addon Quiz Settings instance
		 */
		do_action( 'powerform_addon_trello_quiz_before_create_card', $quiz_id, $submitted_data, $quiz_settings_instance );
		 ...
		 */
		$data = apply_filters(
			'powerform_addon_trello_quiz_entry_fields',
			$entry_fields,
			$quiz_id,
			$submitted_data,
			$current_entry_fields,
	...
	private function get_status_on_create_card( $connection_id, $submitted_data, $connection_settings, $current_entry_fields ) {
		...
		try {
			...
			$lead_submitted_data = powerform_addons_lead_submitted_data( $addons_fields, $form_entry_fields );

			if ( isset( $connection_settings['card_name'] ) ) {
				$card_name = $connection_settings['card_name'];
				// disable all_fields here
				$card_name = powerform_replace_variables( $card_name );
				// {quizname_replace} //phpcs:ignore
				$card_name = str_ireplace( '{quiz_name}', powerform_get_name_from_model( $this->quiz ), $card_name );

				if ( isset( $quiz_settings['hasLeads'] ) && $quiz_settings['hasLeads'] ) {
					$card_name = powerform_addon_replace_custom_vars( $card_name, $lead_submitted_data, $this->lead_model, $form_entry_fields );
				}
				...
				$card_name    = apply_filters(
					'powerform_addon_trello_quiz_card_name',
					$card_name,
					$quiz_id,
					$connection_id,
					$submitted_data,
			...
			if ( isset( $connection_settings['card_description'] ) ) {
				...
				$card_description         = powerform_replace_variables( $card_description );
				if ( isset( $quiz_settings['hasLeads'] ) && $quiz_settings['hasLeads'] ) {
					$card_description = powerform_addon_replace_custom_vars( $card_description, $lead_submitted_data, $this->lead_model, $form_entry_fields );
				}
				...
				$card_description = apply_filters(
					'powerform_addon_trello_quiz_card_description',
					$card_description,
					$quiz_id,
					$connection_id,
					$submitted_data,
			...
			if ( isset( $connection_settings['due_date'] ) && ! empty( $connection_settings['due_date'] ) ) {
				$due_date    = powerform_addon_replace_custom_vars( $connection_settings['due_date'], $lead_submitted_data, $this->lead_model, $form_entry_fields, false );
				$args['due'] = $due_date;
			}
			 ...
			 */
			$args = apply_filters(
				'powerform_addon_trello_quiz_create_card_args',
				$args,
				$quiz_id,
				$connection_id,
				$submitted_data,
			...
			return array(
				...
				'description'     => __( 'Successfully send data to Trello', Powerform::DOMAIN ),
			...
			powerform_addon_maybe_log( __METHOD__, 'Failed to Send to Trello' );

			return array(
				'is_sent'         => false,
				'description'     => $e->getMessage(),
				'connection_name' => $connection_settings['name'],
				'data_sent'       => ( ( $api instanceof Powerform_Addon_Trello_Wp_Api ) ? $api->get_last_data_sent() : array() ),
				'data_received'   => ( ( $api instanceof Powerform_Addon_Trello_Wp_Api ) ? $api->get_last_data_received() : array() ),
				'url_request'     => ( ( $api instanceof Powerform_Addon_Trello_Wp_Api ) ? $api->get_last_url_request() : '' ),
			);
		}
	}
	...
	private function quiz_answers_to_markdown( $quiz_entry_fields ) {
				...
				if ( is_array( $quiz_entry['value'] ) ) {
					if ( 'knowledge' === $this->quiz->quiz_type ) {
						foreach ( $quiz_entry['value'] as $data ) {
							...
							$markdown .= '###' . $question . "\n";
							$markdown .= $answer . "\n";
							$markdown .= __( 'Correct : ', Powerform::DOMAIN )
							             . '**' . ( $is_correct ? __( 'Yes', Powerform::DOMAIN ) : __( 'No', Powerform::DOMAIN ) ) . '**'
							             . "\n";
						}
		...
		$markdown = apply_filters(
			'powerform_addon_trello_quiz_answer_markdown',
			$markdown,
			$quiz_entry_fields
		);
	...
	private function quiz_result_to_markdown( $quiz_entry_fields ) {
			...
			if ( isset( $quiz_entry['name'] ) && isset( $quiz_entry['value'] ) && 'entry' === $quiz_entry['name'] ) {
				if ( is_array( $quiz_entry['value'] ) ) {
					if ( 'knowledge' === $this->quiz->quiz_type ) {
						...
						$markdown .= '##' . __( 'Quiz Result', Powerform::DOMAIN ) . "\n";
						$markdown .= __( 'Correct Answers : ', Powerform::DOMAIN )
						             . '**' . $total_correct . '**'
						             . "\n";
						$markdown .= __( 'Total Answers : ', Powerform::DOMAIN )
						             . '**' . $total_answers . '**'
						             . "\n";
		 ...
		 */
		$markdown = apply_filters(
			'powerform_addon_trello_quiz_result_markdown',
			$markdown,
			$quiz_entry_fields
		);
	 ...
	 * its called `Trello Info` which can be translated on powerform lang
	 *
	 * @since 1.6.2
	 * @return array
	 */
	public function on_export_render_title_row() {
		...
		$export_headers = array(
			'info' => __( 'Trello Info', Powerform::DOMAIN ),
		);
		...
		$export_headers = apply_filters(
			'powerform_addon_trello_quiz_export_headers',
			$export_headers,
			$quiz_id,
			$quiz_settings_instance
		);
	...
	public function on_export_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$addon_meta_data = apply_filters(
			'powerform_addon_trello_quiz_metadata',
			$addon_meta_data,
			$quiz_id,
			$quiz_settings_instance
		);
		...
		$export_columns = apply_filters(
			'powerform_addon_trello_quiz_export_columns',
			$export_columns,
			$quiz_id,
			$entry_model,
			$addon_meta_data,
	 ...
	 * subentries included are defined in @see Powerform_Addon_Trello_Quiz_Hooks::get_additional_entry_item()
	 *
	 * @since 1.6.1
	 *
	 * @param Powerform_Form_Entry_Model $entry_model
	...
	public function on_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		 ...
		 */
		$addon_meta_data = apply_filters(
			'powerform_addon_quiz_trello_metadata',
			$addon_meta_data,
			$quiz_id,
			$quiz_settings_instance
		);
	 ...
	 * - Info : Text that are generated by addon when building and sending data to Trello @see Powerform_Addon_Trello_Quiz_Hooks::add_entry_fields()
	 * - Below subentries will be added if full log enabled, @see Powerform_Addon_Trello::is_show_full_log() @see POWERFORM_ADDON_TRELLO_SHOW_FULL_LOG
	...
	private function get_additional_entry_item( $addon_meta_data ) {
		...
		$additional_entry_item = array(
			'label' => __( 'Trello Integration', Powerform::DOMAIN ),
			'value' => '',
		);
		...
		if ( isset( $status['is_sent'] ) ) {
			...
			$sub_entries[] = array(
				'label' => __( 'Sent To Trello', Powerform::DOMAIN ),
				'value' => $is_sent,
			);
		}
		...
		if ( Powerform_Addon_Trello::is_show_full_log() ) {
			// too long to be added on entry data enable this with `define('POWERFORM_ADDON_TRELLO_SHOW_FULL_LOG', true)`
			...
			if ( isset( $status['data_sent'] ) ) {
				$sub_entries[] = array(
					'label' => __( 'Data sent to Trello', Powerform::DOMAIN ),
			...
			if ( isset( $status['data_received'] ) ) {
				$sub_entries[] = array(
					'label' => __( 'Data received from Trello', Powerform::DOMAIN ),
