...
require_once dirname( __FILE__ ) . '/class-powerform-addon-trello-quiz-settings-exception.php';

/**
 * Class Powerform_Addon_Trello_Quiz_Settings
 * Handle how quiz settings displayed and saved
 *
 * @since 1.6.2
 */
class Powerform_Addon_Trello_Quiz_Settings extends Powerform_Addon_Quiz_Settings_Abstract {
	...
	/**
	 * Powerform_Addon_Trello_Quiz_Settings constructor.
	 *
	 * @since 1.6.2
	 *
	...
	public function setup_name( $submitted_data ) {
		$template = powerform_addon_trello_dir() . 'views/quiz-settings/setup-name.php';
		...
		if ( $this->setup_name_is_completed( array( 'multi_id' => $multi_id ) ) ) {
			$buttons['disconnect']['markup'] = Powerform_Addon_Abstract::get_button_markup(
				...
				esc_html__( 'Deactivate this Trello Integration from this Quiz.', Powerform::DOMAIN )
			);
		}
	...
	public function setup_board( $submitted_data ) {
		$template = powerform_addon_trello_dir() . 'views/quiz-settings/setup-board.php';
	...
	public function setup_list( $submitted_data ) {
		$template = powerform_addon_trello_dir() . 'views/quiz-settings/setup-list.php';
	...
	public function setup_card( $submitted_data ) {
		$template = powerform_addon_trello_dir() . 'views/quiz-settings/setup-card.php';
		...
		$var_list              = array();
		$var_list['quiz_name'] = __( 'Quiz Name', Powerform::DOMAIN );
		$var_list              = array_merge( $var_list, powerform_get_vars() );
		unset( $var_list['custom_value'] );

		$template_params['name_fields'] = $var_list;
		$template_params['desc_fields'] = array_merge( powerform_get_quiz_vars(), $var_list );
		...
		if ( $is_submit ) {
			...
			try {
				$input_exceptions = new Powerform_Addon_Trello_Quiz_Settings_Exception();
			...
			} catch ( Powerform_Addon_Trello_Quiz_Settings_Exception $e ) {
	...
	public function get_multi_ids() {
		 ...
		 */
		$multi_ids = apply_filters( 'powerform_addon_quiz_trello_multi_id_labels', $multi_ids, $quiz_settings_values );
