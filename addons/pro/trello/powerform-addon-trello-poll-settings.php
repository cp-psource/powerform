...
require_once dirname( __FILE__ ) . '/powerform-addon-trello-poll-settings-exception.php';

/**
 * Class Powerform_Addon_Trello_Poll_Settings
 * Handle how poll settings displayed and saved
 *
 * @since 1.6.1
 */
class Powerform_Addon_Trello_Poll_Settings extends Powerform_Addon_Poll_Settings_Abstract {
	...
	/**
	 * Powerform_Addon_Trello_Poll_Settings constructor.
	 *
	 * @since 1.6.1
	 *
	...
	public function setup_name( $submitted_data ) {
		$template = powerform_addon_trello_dir() . 'views/poll-settings/setup-name.php';
		...
		if ( $is_submit ) {
			...
			try {
				if ( empty( $name ) ) {
					throw new Powerform_Addon_Trello_Exception( __( 'Please pick valid name' ) );
				}
		...
		if ( $this->setup_name_is_completed( array( 'multi_id' => $multi_id ) ) ) {
			$buttons['disconnect']['markup'] = Powerform_Addon_Abstract::get_button_markup( esc_html__( 'DISCONNECT', Powerform::DOMAIN ),
			                                                                                 ...
			                                                                                 esc_html__( 'Disconnect this Trello Integration from this Poll.', Powerform::DOMAIN )
			);
		}
	...
	public function setup_board( $submitted_data ) {
		$template = powerform_addon_trello_dir() . 'views/poll-settings/setup-board.php';
		...
		try {
			...
			if ( empty( $boards ) ) {
				throw new Powerform_Addon_Trello_Exception( __( 'No board found on your Trello account. Please create one.', Powerform::DOMAIN ) );
			}
		...
		if ( $is_submit ) {
			...
			try {
				if ( empty( $board_id ) ) {
					throw new Powerform_Addon_Trello_Exception( __( 'Please pick valid board.' ) );
				}
	...
	public function setup_list( $submitted_data ) {
		$template = powerform_addon_trello_dir() . 'views/poll-settings/setup-list.php';
		...
		try {
			...
			if ( empty( $lists ) ) {
				throw new Powerform_Addon_Trello_Exception( sprintf( __( 'No list found on Trello Board of %1$s. Please create one.', Powerform::DOMAIN ), $board_name ) );
			}
		...
		if ( $is_submit ) {
			...
			try {
				if ( empty( $list_id ) ) {
					throw new Powerform_Addon_Trello_Exception( __( 'Please pick valid list.' ) );
				}
	...
	public function setup_card( $submitted_data ) {
		$template = powerform_addon_trello_dir() . 'views/poll-settings/setup-card.php';
		...
		$positions = array(
			'top'    => __( 'Top', Powerform::DOMAIN ),
			'bottom' => __( 'Bottom', Powerform::DOMAIN ),
		);
		...
		$var_list              = array();
		$var_list['poll_name'] = __( 'Poll Name', Powerform::DOMAIN );
		$var_list              = array_merge( $var_list, powerform_get_vars() );
		unset( $var_list['custom_value'] );

		$template_params['name_fields'] = $var_list;
		$template_params['desc_fields'] = array_merge( powerform_get_poll_vars(), $var_list );
		...
		if ( $is_submit ) {
			...
			try {
				$input_exceptions = new Powerform_Addon_Trello_Poll_Settings_Exception();
			...
			} catch ( Powerform_Addon_Trello_Poll_Settings_Exception $e ) {
	...
	public function get_multi_ids() {
		 ...
		 */
		$multi_ids = apply_filters( 'powerform_addon_poll_trello_multi_id_labels', $multi_ids, $poll_settings_values );
