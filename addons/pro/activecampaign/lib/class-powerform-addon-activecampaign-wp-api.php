...
require_once dirname( __FILE__ ) . '/class-powerform-addon-activecampaign-wp-api-exception.php';
require_once dirname( __FILE__ ) . '/class-powerform-addon-activecampaign-wp-api-not-found-exception.php';

/**
 * Class Powerform_Addon_Activecampaign_Wp_Api
 */
class Powerform_Addon_Activecampaign_Wp_Api {
	...
	/**
	 * Powerform_Addon_Activecampaign_Wp_Api constructor.
	 *
	 * @since 1.0 Activecampaign Addon
	 *
	...
	public function __construct( $_endpoint, $_api_key ) {
		//prerequisites
		if ( ! $_endpoint ) {
			throw new Powerform_Addon_Activecampaign_Wp_Api_Exception( __( 'Missing required API URL', Powerform::DOMAIN ) );
		}

		if ( ! $_api_key ) {
			throw new Powerform_Addon_Activecampaign_Wp_Api_Exception( __( 'Missing required API Key', Powerform::DOMAIN ) );
		}
	...
	public function filter_user_agent( $user_agent ) {
		$user_agent .= ' PowerformActivecampaign/' . POWERFORM_ADDON_ACTIVECAMPAIGN_VERSION;
		 ...
		 */
		$user_agent = apply_filters( 'powerform_addon_activecampaign_api_user_agent', $user_agent );
	...
	private function request( $verb, $path, $args = array() ) {
		 ...
		 */
		$url = apply_filters( 'powerform_addon_activecampaign_api_url', $url, $verb, $path, $args );
		 ...
		 * @param array  $args argument sent to this function
		 */
		$headers = apply_filters( 'powerform_addon_activecampaign_api_request_headers', $headers, $verb, $path, $args );
		 ...
		 * @param string $path         requested path resource
		 */
		$args = apply_filters( 'powerform_addon_activecampaign_api_request_data', $request_data, $verb, $path );
		...
		if ( is_wp_error( $res ) || ! $res ) {
			...
			throw new Powerform_Addon_Activecampaign_Wp_Api_Exception(
				__( 'Failed to process request, make sure your API URL and API KEY are correct and your server has internet connection.', Powerform::DOMAIN )
			);
		}
		...
		if ( isset( $res['response']['code'] ) ) {
			...
			if ( $status_code > 400 ) {
				...
				if ( 404 === $status_code ) {
					/* translators: ... */
					throw new Powerform_Addon_Activecampaign_Wp_Api_Not_Found_Exception( sprintf( __( 'Failed to processing request : %s', Powerform::DOMAIN ), $msg ) );
				}
				/* translators: ... */
				throw new Powerform_Addon_Activecampaign_Wp_Api_Exception( sprintf( __( 'Failed to processing request : %s', Powerform::DOMAIN ), $msg ) );
			}
		}
		...
		if ( ! empty( $body ) ) {
			...
			if ( ! empty( $res ) ) {
				if ( ! isset( $res->result_code ) || 1 !== $res->result_code ) {
					...
					/* translators: ... */
					throw new Powerform_Addon_Activecampaign_Wp_Api_Exception( sprintf( __( 'Failed to get ActiveCampaign data.%1$s', Powerform::DOMAIN ), $message ) );
				}
			}
		}
		 ...
		 */
		$res = apply_filters( 'powerform_addon_activecampaign_api_response', $response, $body, $wp_response );

		$this->_last_data_received = $res;

		powerform_addon_maybe_log( $res );
	...
	public function contact_sync( $args = array() ) {
		...
		if ( empty( $args['email'] ) ) {
			throw new Powerform_Addon_Activecampaign_Wp_Api_Exception( __( 'Required email parameter not set', Powerform::DOMAIN ) );
		}
	...
	public function contact_delete( $args = array() ) {
		...
		if ( empty( $args['id'] ) ) {
			throw new Powerform_Addon_Activecampaign_Wp_Api_Exception( __( 'Required id parameter not set for contact_delete.', Powerform::DOMAIN ) );
		}
