...
/**
 * Class Powerform_Addon_Activecampaign_Form_Hooks
 *
 * @since 1.0 Activecampaign Addon
 *
 */
class Powerform_Addon_Activecampaign_Form_Hooks extends Powerform_Addon_Form_Hooks_Abstract {
	 ...
	 *
	 * @since 1.0 Activecampaign Addon
	 * @var Powerform_Addon_Activecampaign
	 */
	protected $addon;
	 ...
	 *
	 * @since 1.0 Activecampaign Addon
	 * @var Powerform_Addon_Activecampaign_Form_Settings | null
	 */
	protected $form_settings_instance;

	/**
	 * Powerform_Addon_Activecampaign_Form_Hooks constructor.
	 *
	 * @since 1.0 Activecampaign Addon
	 *
	...
	public function __construct( Powerform_Addon_Abstract $addon, $form_id ) {
		...
		$this->_submit_form_error_message = __( 'ActiveCampaign failed to process submitted data. Please check your form and try again', Powerform::DOMAIN );
	}
	...
	public function add_entry_fields( $submitted_data, $form_entry_fields = array() ) {
		 ...
		 * @param Powerform_Addon_Activecampaign_Form_Settings $form_settings_instance ActiveCampaign Addon Form Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_activecampaign_form_submitted_data',
			$submitted_data,
			$form_id,
		...
		if ( ! empty( $data_entry ) ) {
			$submitted_data = powerform_lead_form_data( $data_entry );
		 ...
		 * @param Powerform_Addon_Activecampaign_Form_Settings $form_settings_instance ActiveCampaign Addon Form Settings instance
		 */
		do_action( 'powerform_addon_activecampaign_before_contact_sync', $form_id, $submitted_data, $form_settings_instance );
		 ...
		 */
		$data = apply_filters(
			'powerform_addon_activecampaign_entry_fields',
			$entry_fields,
			$form_id,
			$submitted_data,
			$form_settings_instance
	...
	private function get_status_on_contact_sync( $connection_id, $submitted_data, $connection_settings, $form_settings, $form_entry_fields ) {
		...
		try {
			...
			if ( ! isset( $connection_settings['list_id'] ) ) {
				throw new Powerform_Addon_Activecampaign_Exception( __( 'List ID not properly setup.', Powerform::DOMAIN ) );
			}
			...
			if ( ! isset( $submitted_data[ $email_element_id ] ) || empty( $submitted_data[ $email_element_id ] ) ) {
				throw new Powerform_Addon_Activecampaign_Exception( __( 'Email on element not found or not filled on submitted data.', Powerform::DOMAIN ) );
			}
			 ...
			 */
			$args = apply_filters(
				'powerform_addon_activecampaign_contact_sync_args',
				$args,
				$form_id,
				$connection_id,
				$submitted_data,
			...
			return array(
				...
				'description'     => __( 'Successfully send data to ActiveCampaign', Powerform::DOMAIN ),
			...
			powerform_addon_maybe_log( __METHOD__, 'Failed to Send to ActiveCampaign' );

			return array(
				'is_sent'         => false,
				'description'     => $e->getMessage(),
				'connection_name' => $connection_settings['name'],
				'data_sent'       => ( ( $ac_api instanceof Powerform_Addon_Activecampaign_Wp_Api ) ? $ac_api->get_last_data_sent() : array() ),
				'data_received'   => ( ( $ac_api instanceof Powerform_Addon_Activecampaign_Wp_Api ) ? $ac_api->get_last_data_received() : array() ),
				'url_request'     => ( ( $ac_api instanceof Powerform_Addon_Activecampaign_Wp_Api ) ? $ac_api->get_last_url_request() : '' ),
			);
		}
	}
	 ...
	 * subentries included are defined in @see Powerform_Addon_Activecampaign_Form_Hooks::get_additional_entry_item()
	 *
	 * @since 1.0 Activecampaign Addon
	 *
	...
	public function on_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$addon_meta_data = apply_filters(
			'powerform_addon_activecampaign_metadata',
			$addon_meta_data,
			$form_id,
			$form_settings_instance
		);
	 ...
	 * - Info : Text that are generated by addon when building and sending data to Activecampaign @see Powerform_Addon_Activecampaign_Form_Hooks::add_entry_fields()
	 * - Below subentries will be added if full log enabled, @see Powerform_Addon_Activecampaign::is_show_full_log() @see POWERFORM_ADDON_ACTIVECAMPAIGN_SHOW_FULL_LOG
	...
	private function get_additional_entry_item( $addon_meta_data ) {
		...
		$additional_entry_item = array(
			'label' => __( 'ActiveCampaign Integration', Powerform::DOMAIN ),
			'value' => '',
		);
		...
		if ( isset( $status['is_sent'] ) ) {
			...
			$sub_entries[] = array(
				'label' => __( 'Sent To ActiveCampaign', Powerform::DOMAIN ),
				'value' => $is_sent,
			);
		}
		...
		if ( Powerform_Addon_Activecampaign::is_show_full_log() ) {
			// too long to be added on entry data enable this with `define('POWERFORM_ADDON_ACTIVECAMPAIGN_SHOW_FULL_LOG', true)`
			...
			if ( isset( $status['data_sent'] ) ) {
				$sub_entries[] = array(
					'label' => __( 'Data sent to ActiveCampaign', Powerform::DOMAIN ),
			...
			if ( isset( $status['data_received'] ) ) {
				$sub_entries[] = array(
					'label' => __( 'Data received from ActiveCampaign', Powerform::DOMAIN ),
	 ...
	 * its called `Active Campaign Info` which can be translated on powerform lang
	 *
	 * @since 1.0 Activecampaign Addon
	 * @return array
	 */
	public function on_export_render_title_row() {
		...
		$export_headers = array(
			'info' => __( 'ActiveCampaign Info', Powerform::DOMAIN ),
		);
		 ...
		 * @param int                                           $form_id                current Form ID
		 * @param Powerform_Addon_Activecampaign_Form_Settings $form_settings_instance Activecampaign Form Settings instance
		 */
		$export_headers = apply_filters(
			'powerform_addon_activecampaign_export_headers',
			$export_headers,
			$form_id,
			$form_settings_instance
		);
	...
	public function on_export_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$export_columns = apply_filters(
			'powerform_addon_activecampaign_export_columns',
			$export_columns,
			$form_id,
			$entry_model,
			$addon_meta_data,
	...
	public function on_before_delete_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		 ...
		 */
		do_action(
			'powerform_addon_activecampaign_on_before_delete_submission',
			$form_id,
			$entry_model,
			$addon_meta_data,
			$form_settings_instance
		...
		if ( ! Powerform_Addon_Activecampaign::is_enable_delete_contact() ) {
		...
		try {
			...
			$subscriber_ids_to_delete = apply_filters(
				'powerform_addon_activecampaign_subscriber_ids_to_delete',
			...
			// handle all internal addon exceptions with `Powerform_Addon_Activecampaign_Exception`

			// use wp_error, for future usage it can be returned to page entries
			$wp_error = new WP_Error( 'powerform_addon_activecampaign_delete_contact', $e->getMessage() );
