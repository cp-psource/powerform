...
require_once dirname( __FILE__ ) . '/class-powerform-addon-activecampaign-quiz-settings-exception.php';

/**
 * Class Powerform_Addon_Activecampaign_Quiz_Settings
 * Handle how quiz settings displayed and saved
 *
 * @since 1.0 Activecampaign Addon
 */
class Powerform_Addon_Activecampaign_Quiz_Settings extends Powerform_Addon_Quiz_Settings_Abstract {
	...
	/**
	 * Powerform_Addon_Activecampaign_Quiz_Settings constructor.
	 *
	 * @since 1.0 Activecampaign Addon
	 *
	...
	public function pick_name( $submitted_data ) {
		$template = powerform_addon_activecampaign_dir() . 'views/quiz-settings/pick-name.php';
		...
		if ( $this->pick_name_is_completed( array( 'multi_id' => $multi_id ) ) ) {
			$buttons['disconnect']['markup'] = Powerform_Addon_Abstract::get_button_markup(
				...
				esc_html__( 'Deactivate this ActiveCampaign Integration from this Quiz.', Powerform::DOMAIN )
			);
		}
	...
	public function select_list( $submitted_data ) {
		$template = powerform_addon_activecampaign_dir() . 'views/quiz-settings/select-list.php';
	...
	public function map_fields( $submitted_data ) {
		$template = powerform_addon_activecampaign_dir() . 'views/quiz-settings/map-fields.php';
		...
		if ( $is_submit ) {
			...
			try {
				...
				$input_exceptions = new Powerform_Addon_Activecampaign_Quiz_Settings_Exception();
			...
			} catch ( Powerform_Addon_Activecampaign_Quiz_Settings_Exception $e ) {
	...
	public function setup_options( $submitted_data ) {
		$template = powerform_addon_activecampaign_dir() . 'views/quiz-settings/setup-options.php';
		...
		if ( $is_submit ) {
			...
			try {
				...
				if ( ! empty( $double_opt_form_id ) && ! in_array( $double_opt_form_id, array_keys( $forms ) ) ) {
					$input_exceptions->add_input_exception( __( 'Please pick valid ActiveCampaign Quiz', Powerform::DOMAIN ), 'double_opt_form_id_error' );
				}
	...
	public function get_multi_ids() {
		 ...
		 */
		$multi_ids = apply_filters( 'powerform_addon_activecampaign_multi_id_labels', $multi_ids, $quiz_settings_values );
