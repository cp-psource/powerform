...
require_once dirname( __FILE__ ) . '/powerform-addon-activecampaign-form-settings-exception.php';

/**
 * Class Powerform_Addon_Activecampaign_Form_Settings
 * Handle how form settings displayed and saved
 *
 * @since 1.0 Activecampaign Addon
 */
class Powerform_Addon_Activecampaign_Form_Settings extends Powerform_Addon_Form_Settings_Abstract {
	...
	/**
	 * Powerform_Addon_Activecampaign_Form_Settings constructor.
	 *
	 * @since 1.0 Activecampaign Addon
	 *
	...
	public function pick_name( $submitted_data ) {
		$template = powerform_addon_activecampaign_dir() . 'views/form-settings/pick-name.php';
		...
		if ( $is_submit ) {
			...
			try {
				if ( empty( $name ) ) {
					throw new Powerform_Addon_Activecampaign_Exception( __( 'Please pick valid name' ) );
				}
		...
		if ( $this->pick_name_is_completed( array( 'multi_id' => $multi_id ) ) ) {
			$buttons['disconnect']['markup'] = Powerform_Addon_Abstract::get_button_markup( esc_html__( 'DISCONNECT', Powerform::DOMAIN ),
			                                                                                 'sui-button-ghost sui-tooltip sui-tooltip-top-center powerform-addon-form-disconnect',
			                                                                                 esc_html__( 'Disconnect this ActiveCampaign Integration from this Form.', Powerform::DOMAIN )
			);
		}
	...
	public function select_list( $submitted_data ) {
		$template = powerform_addon_activecampaign_dir() . 'views/form-settings/select-list.php';
		...
		try {
			...
			if ( empty( $lists ) ) {
				throw new Powerform_Addon_Activecampaign_Exception( __( 'No lists found on your ActiveCampaign account. Please create one.', Powerform::DOMAIN ) );
			}
		...
		if ( $is_submit ) {
			...
			try {
				if ( empty( $list_id ) ) {
					throw new Powerform_Addon_Activecampaign_Exception( __( 'Please pick valid list' ) );
				}
	...
	public function map_fields( $submitted_data ) {
		$template = powerform_addon_activecampaign_dir() . 'views/form-settings/map-fields.php';
		...
		$fields = array(
			'email'      => __( 'Email Address', Powerform::DOMAIN ),
			'first_name' => __( 'First Name', Powerform::DOMAIN ),
			'last_name'  => __( 'Last Name', Powerform::DOMAIN ),
			'phone'      => __( 'Phone', Powerform::DOMAIN ),
			'orgname'    => __( 'Organization Name', Powerform::DOMAIN ),
		);
		...
		try {
			...
			if ( ! isset( $list_detail->fields ) && ! is_array( $list_detail->fields ) ) {
				throw new Powerform_Addon_Activecampaign_Exception( __( 'ActiveCampaign list\'s fields could not be found', Powerform::DOMAIN ) );
			}
		...
		if ( $is_submit ) {
			...
			try {
				if ( empty( $fields_map ) ) {
					throw new Powerform_Addon_Activecampaign_Exception( __( 'Please assign fields.', Powerform::DOMAIN ) );
				}

				$input_exceptions = new Powerform_Addon_Activecampaign_Form_Settings_Exception();
			...
			} catch ( Powerform_Addon_Activecampaign_Form_Settings_Exception $e ) {
	...
	public function setup_options( $submitted_data ) {
		$template = powerform_addon_activecampaign_dir() . 'views/form-settings/setup-options.php';
		...
		if ( $is_submit ) {
			...
			try {
				...
				if ( ! empty( $double_opt_form_id ) && ! in_array( $double_opt_form_id, array_keys( $forms ) ) ) {
					$input_exceptions->add_input_exception( __( 'Please pick valid ActiveCampaign Form', Powerform::DOMAIN ), 'double_opt_form_id_error' );
				}
	...
	public function get_multi_ids() {
		 ...
		 */
		$multi_ids = apply_filters( 'powerform_addon_activecampaign_multi_id_labels', $multi_ids, $form_settings_values );
