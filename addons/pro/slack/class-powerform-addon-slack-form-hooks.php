...
/**
 * Class Powerform_Addon_Slack_Form_Hooks
 *
 * @since 1.0 Slack Addon
 *
 */
class Powerform_Addon_Slack_Form_Hooks extends Powerform_Addon_Form_Hooks_Abstract {
	 ...
	 *
	 * @since 1.0 Slack Addon
	 * @var Powerform_Addon_Slack_Form_Settings | null
	 */
	protected $form_settings_instance;

	/**
	 * Powerform_Addon_Slack_Form_Hooks constructor.
	 *
	 * @since 1.0 Slack Addon
	 *
	...
	public function __construct( Powerform_Addon_Abstract $addon, $form_id ) {
		...
		$this->_submit_form_error_message = __( 'Slack failed to process submitted data. Please check your form and try again', Powerform::DOMAIN );
	}
	...
	public function add_entry_fields( $submitted_data, $form_entry_fields = array() ) {
		 ...
		 * @param Powerform_Addon_Slack_Form_Settings $form_settings_instance Slack Addon Form Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_slack_form_submitted_data',
			$submitted_data,
			$form_id,
		 ...
		 * @param Powerform_Addon_Slack_Form_Settings $form_settings_instance Slack Addon Form Settings instance
		 */
		do_action( 'powerform_addon_slack_before_send_message', $form_id, $submitted_data, $form_settings_instance );
		 ...
		 */
		$data = apply_filters(
			'powerform_addon_slack_entry_fields',
			$entry_fields,
			$form_id,
			$submitted_data,
			$form_settings_instance
	...
	private function get_status_on_send_message( $connection_id, $submitted_data, $connection_settings, $form_entry_fields ) {
		...
		try {
			...
			$text_message = powerform_addon_replace_custom_vars( $text_message, $submitted_data, $this->custom_form, $form_entry_fields, false );
			...
			$attachments = apply_filters(
				'powerform_addon_slack_message_attachments',
				$attachments,
				$form_id,
				$connection_id,
				$submitted_data,
			 ...
			 */
			$args = apply_filters(
				'powerform_addon_slack_send_message_args',
				$args,
				$form_id,
				$connection_id,
				$submitted_data,
		...
		foreach ( $form_fields as $form_field ) {
			...
			switch ( $field_type ) {
				...
				case 'postdata-post-image':
					...
					$field_value                     = powerform_addon_replace_custom_vars( $field_value, $submitted_data, $this->custom_form, $form_entry_fields, false );
					$all_fields_attachments_fields[] = array(
						'title' => $field_label,
	 ...
	 * subentries included are defined in @see Powerform_Addon_Slack_Form_Hooks::get_additional_entry_item()
	 *
	 * @since 1.0 Slack Addon
	 *
	...
	public function on_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$addon_meta_data = apply_filters(
			'powerform_addon_slack_metadata',
			$addon_meta_data,
			$form_id,
			$form_settings_instance
		);
	 ...
	 * - Info : Text that are generated by addon when building and sending data to Slack @see Powerform_Addon_Slack_Form_Hooks::add_entry_fields()
	...
	public function on_export_render_title_row() {
		...
		$export_headers = apply_filters(
			'powerform_addon_slack_export_headers',
			$export_headers,
			$form_id,
			$form_settings_instance
		);
	...
	public function on_export_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$export_columns = apply_filters(
			'powerform_addon_slack_export_columns',
			$export_columns,
			$form_id,
			$entry_model,
			$addon_meta_data,
	...
	public function on_before_delete_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		 ...
		 */
		do_action(
			'powerform_addon_slack_on_before_delete_submission',
			$form_id,
			$entry_model,
			$addon_meta_data,
			$form_settings_instance
		...
		if ( ! Powerform_Addon_Slack::enable_delete_chat() ) {
		...
		try {
			...
			// handle all internal addon exceptions with `Powerform_Addon_Slack_Exception`

			// use wp_error, for future usage it can be returned to page entries
			$wp_error
				= new WP_Error( 'powerform_addon_slack_delete_chat', $e->getMessage() );
