...
require_once dirname( __FILE__ ) . '/powerform-addon-slack-poll-settings-exception.php';

/**
 * Class Powerform_Addon_Slack_Poll_Settings
 * Handle how poll settings displayed and saved
 *
 * @since 1.6.1
 */
class Powerform_Addon_Slack_Poll_Settings extends Powerform_Addon_Poll_Settings_Abstract {
	...
	/**
	 * Powerform_Addon_Slack_Poll_Settings constructor.
	 *
	 * @since 1.6.1
	 *
	...
	public function pick_name( $submitted_data ) {
		$template = powerform_addon_slack_dir() . 'views/poll-settings/pick-name.php';
		...
		if ( $this->pick_name_is_completed( array( 'multi_id' => $multi_id ) ) ) {
			$buttons['disconnect']['markup'] = Powerform_Addon_Abstract::get_button_markup( esc_html__( 'DISCONNECT', Powerform::DOMAIN ),
			                                                                                 ...
			                                                                                 esc_html__( 'Disconnect this Slack Integration from this Poll.', Powerform::DOMAIN )
			);
		}
	...
	public function select_type( $submitted_data ) {
		$template = powerform_addon_slack_dir() . 'views/poll-settings/select-type.php';
	...
	public function select_target( $submitted_data ) {
		$template = powerform_addon_slack_dir() . 'views/poll-settings/select-target.php';
		if ( ! isset( $submitted_data['multi_id'] ) ) {
			return $this->get_force_closed_wizard( __( 'Please pick valid connection', Powerform::DOMAIN ) );
		}
	...
	public function setup_message( $submitted_data ) {
		$template = powerform_addon_slack_dir() . 'views/poll-settings/setup-message.php';
		if ( ! isset( $submitted_data['multi_id'] ) ) {
			return $this->get_force_closed_wizard( __( 'Please pick valid connection', Powerform::DOMAIN ) );
		}
		...
		$template_params['tags']['poll_name'] = __( 'Poll Name', Powerform::DOMAIN );
