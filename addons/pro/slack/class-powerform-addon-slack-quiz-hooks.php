...
/**
 * Class Powerform_Addon_Slack_Quiz_Hooks
 *
 * @since 1.6.2
 *
 */
class Powerform_Addon_Slack_Quiz_Hooks extends Powerform_Addon_Quiz_Hooks_Abstract {
	 ...
	 *
	 * @since 1.6.2
	 * @var Powerform_Addon_Slack_Quiz_Settings | null
	 */
	...
	public function add_entry_fields( $submitted_data, $form_entry_fields = array() ) {
		 ...
		 * @param Powerform_Addon_Slack_Quiz_Settings $quiz_settings_instance Slack Addon Quiz Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_slack_quiz_submitted_data',
			$submitted_data,
			$quiz_id,
		 ...
		 * @param Powerform_Addon_Slack_Quiz_Settings $quiz_settings_instance Slack Addon Quiz Settings instance
		 */
		do_action( 'powerform_addon_slack_quiz_before_send_message', $quiz_id, $submitted_data, $quiz_settings_instance );
		 ...
		 */
		$data = apply_filters(
			'powerform_addon_slack_quiz_entry_fields',
			$entry_fields,
			$quiz_id,
			$submitted_data,
			$quiz_settings_instance
	...
	private function get_status_on_send_message( $connection_id, $submitted_data, $connection_settings, $quiz_entry_fields ) {
		...
		try {
			...
			if ( ! isset( $connection_settings['target_id'] ) ) {
				throw new Powerform_Addon_Slack_Exception( __( 'Target ID not properly setup.', Powerform::DOMAIN ) );
			}

			if ( ! isset( $connection_settings['message'] ) ) {
				throw new Powerform_Addon_Slack_Exception( __( 'Message not properly setup.', Powerform::DOMAIN ) );
			}
			$text_message = $connection_settings['message'];
			$text_message = powerform_replace_variables( $text_message );

			// {quiz_name_replace} //phpcs:ignore
			$text_message = str_ireplace( '{quiz_name}', powerform_get_name_from_model( $this->quiz ), $text_message );
			...
			if ( isset( $quiz_settings['hasLeads'] ) && $quiz_settings['hasLeads'] ) {
				...
				$text_message        = powerform_addon_replace_custom_vars( $text_message, $lead_submitted_data, $this->lead_model, $form_entry_fields );
			...
			$attachments = apply_filters(
				'powerform_addon_slack_quiz_message_attachments',
				$attachments,
				$quiz_id,
				$connection_id,
				$submitted_data,
			 ...
			 */
			$args = apply_filters(
				'powerform_addon_slack_quiz_send_message_args',
				$args,
				$quiz_id,
				$connection_id,
				$submitted_data,
			...
			return array(
				...
				'description'     => __( 'Successfully send data to Slack', Powerform::DOMAIN ),
			...
			$addon_entry_fields = array(
				...
				'connection_name' => $connection_settings['name'],
				'data_sent'       => ( ( $api instanceof Powerform_Addon_Slack_Wp_Api ) ? $api->get_last_data_sent() : array() ),
				'data_received'   => ( ( $api instanceof Powerform_Addon_Slack_Wp_Api ) ? $api->get_last_data_received() : array() ),
				'url_request'     => ( ( $api instanceof Powerform_Addon_Slack_Wp_Api ) ? $api->get_last_url_request() : '' ),
	...
	public function get_quiz_data_as_attachments( $submitted_data, $quiz_entry_fields ) {
		...
		if ( 'knowledge' === $this->quiz->quiz_type ) {
			$attachment_fields[] = array(
				'title' => __( 'Correct Answers', Powerform::DOMAIN ),
				'value' => $correct_answers,
				'short' => true,
			);
			$attachment_fields[] = array(
				'title' => __( 'Total Answers', Powerform::DOMAIN ),
				'value' => $total_answers,
				'short' => true,
			);
		} elseif ( 'nowrong' === $this->quiz->quiz_type ) {
		...
		$attachments[] = array(
			'title'  => __( 'Quiz Result', Powerform::DOMAIN ),
		...
		foreach ( $form_fields as $form_field ) {
			...
			switch ( $field_type ) {
				...
				case 'postdata-post-image':
					...
					$field_value                     = powerform_addon_replace_custom_vars( $field_value, $submitted_data, $this->lead_model, $form_entry_fields, false );
					$all_fields_attachments_fields[] = array(
						'title' => $field_label,
	 ...
	 * its called `Slack Info` which can be translated on powerform lang
	 *
	 * @since 1.6.2
	 * @return array
	 */
	public function on_export_render_title_row() {
		...
		$export_headers = array(
			'info' => __( 'Slack Info', Powerform::DOMAIN ),
		);
		...
		$export_headers = apply_filters(
			'powerform_addon_slack_quiz_export_headers',
			$export_headers,
			$quiz_id,
			$quiz_settings_instance
		);
	...
	public function on_export_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$addon_meta_data = apply_filters(
			'powerform_addon_slack_quiz_metadata',
			$addon_meta_data,
			$quiz_id,
			$quiz_settings_instance
		);
		...
		$export_columns = apply_filters(
			'powerform_addon_slack_quiz_export_columns',
			$export_columns,
			$quiz_id,
			$entry_model,
			$addon_meta_data,
	 ...
	 * subentries included are defined in @see Powerform_Addon_Slack_Quiz_Hooks::get_additional_entry_item()
	 *
	 * @since 1.6.1
	 *
	 * @param Powerform_Form_Entry_Model $entry_model
	...
	public function on_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		 ...
		 */
		$addon_meta_data = apply_filters(
			'powerform_addon_quiz_slack_metadata',
			$addon_meta_data,
			$quiz_id,
			$quiz_settings_instance
		);
	 ...
	 * - Info : Text that are generated by addon when building and sending data to Slack @see Powerform_Addon_Slack_Quiz_Hooks::add_entry_fields()
	 * - Below subentries will be added if full log enabled, @see Powerform_Addon_Slack::is_show_full_log() @see POWERFORM_ADDON_SLACK_SHOW_FULL_LOG
	...
	private function get_additional_entry_item( $addon_meta_data ) {
		...
		$additional_entry_item = array(
			'label' => __( 'Slack Integration', Powerform::DOMAIN ),
			'value' => '',
		);
		...
		if ( isset( $status['is_sent'] ) ) {
			...
			$sub_entries[] = array(
				'label' => __( 'Sent To Slack', Powerform::DOMAIN ),
				'value' => $is_sent,
			);
		}
		...
		if ( Powerform_Addon_Slack::is_show_full_log() ) {
			// too long to be added on entry data enable this with `define('POWERFORM_ADDON_SLACK_SHOW_FULL_LOG', true)`
			...
			if ( isset( $status['data_sent'] ) ) {
				$sub_entries[] = array(
					'label' => __( 'Data sent to Slack', Powerform::DOMAIN ),
			...
			if ( isset( $status['data_received'] ) ) {
				$sub_entries[] = array(
					'label' => __( 'Data received from Slack', Powerform::DOMAIN ),
