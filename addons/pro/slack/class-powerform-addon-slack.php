...
require_once dirname( __FILE__ ) . '/class-powerform-addon-slack-exception.php';
require_once dirname( __FILE__ ) . '/lib/class-powerform-addon-slack-wp-api.php';

/**
 * Class Powerform_Addon_Slack
 * Slack Addon Main Class
 *
 * @since 1.0 Slack Addon
 */
final class Powerform_Addon_Slack extends Powerform_Addon_Abstract {
	...
	protected $_version                = POWERFORM_ADDON_SLACK_VERSION;
	...
	protected $_form_settings = 'Powerform_Addon_Slack_Form_Settings';
	protected $_form_hooks    = 'Powerform_Addon_Slack_Form_Hooks';
	...
	/**
	 * @var null|Powerform_Addon_Slack_Wp_Api
	 */
	private static $_api = null;

	protected $_poll_settings = 'Powerform_Addon_Slack_Poll_Settings';
	protected $_poll_hooks    = 'Powerform_Addon_Slack_Poll_Hooks';

	protected $_quiz_settings = 'Powerform_Addon_Slack_Quiz_Settings';
	protected $_quiz_hooks    = 'Powerform_Addon_Slack_Quiz_Hooks';

	protected $_position = 4;

	/**
	 * Powerform_Addon_Slack constructor.
	 *
	 * @since 1.0 Slack Addon
	 */
	public function __construct() {
		...
		$this->_activation_error_message   = __( 'Sorry but we failed to activate Slack Integration, don\'t hesitate to contact us', Powerform::DOMAIN );
		$this->_deactivation_error_message = __( 'Sorry but we failed to deactivate Slack Integration, please try again', Powerform::DOMAIN );
		...
		$this->_icon     = powerform_addon_slack_assets_url() . 'icons/slack.png';
		$this->_icon_x2  = powerform_addon_slack_assets_url() . 'icons/slack@2x.png';
		$this->_image    = powerform_addon_slack_assets_url() . 'img/slack.png';
		$this->_image_x2 = powerform_addon_slack_assets_url() . 'img/slack@2x.png';

		add_filter( 'powerform_addon_slack_api_request_headers', array( $this, 'default_filter_api_headers' ), 1, 4 );
	}
	...
	public function is_connected() {
		try {
			// check if its active
			if ( ! $this->is_active() ) {
				throw new Powerform_Addon_Slack_Exception( __( 'Slack is not active', Powerform::DOMAIN ) );
			}
		 ...
		 */
		$is_connected = apply_filters( 'powerform_addon_slack_is_connected', $is_connected );
	...
	public function is_form_connected( $form_id ) {
		try {
			...
			if ( ! $this->is_connected() ) {
				throw new Powerform_Addon_Slack_Exception( __( ' Slack is not connected', Powerform::DOMAIN ) );
			}

			$form_settings_instance = $this->get_addon_form_settings( $form_id );
			if ( ! $form_settings_instance instanceof Powerform_Addon_Slack_Form_Settings ) {
				throw new Powerform_Addon_Slack_Exception( __( 'Invalid Form Settings of Slack', Powerform::DOMAIN ) );
			}

			// Mark as active when there is at least one active connection
			if ( false === $form_settings_instance->find_one_active_connection() ) {
				throw new Powerform_Addon_Slack_Exception( __( 'No active Slack connection found in this form', Powerform::DOMAIN ) );
			}
		 ...
		 * @param Powerform_Addon_Slack_Form_Settings|null $form_settings_instance Instance of form settings, or null when unavailable
		 *
		 */
		$is_form_connected = apply_filters( 'powerform_addon_slack_is_form_connected', $is_form_connected, $form_id, $form_settings_instance );
	...
	public static function is_show_full_log() {
		$show_full_log = false;
		if ( defined( 'POWERFORM_ADDON_SLACK_SHOW_FULL_LOG' ) && POWERFORM_ADDON_SLACK_SHOW_FULL_LOG ) {
			$show_full_log = true;
		}
		 ...
		 */
		$show_full_log = apply_filters( 'powerform_addon_slack_show_full_log', $show_full_log );
	...
	public static function enable_delete_chat() {
		...
		if ( defined( 'POWERFORM_ADDON_SLACK_ENABLE_DELETE_CHAT' ) && POWERFORM_ADDON_SLACK_ENABLE_DELETE_CHAT ) {
		 ...
		 */
		$enable_delete_chat = apply_filters( 'powerform_addon_slack_enable_delete_chat', $enable_delete_chat );
	...
	public function setup_client_id( $submitted_data ) {
		...
		$template        = powerform_addon_slack_dir() . 'views/settings/setup-client.php';
		...
		if ( $this->is_connected() ) {
			...
			$buttons['next']['markup'] = '<div class="sui-actions-right">' .
										self::get_button_markup( esc_html__( 'RE-AUTHORIZE', Powerform::DOMAIN ), 'powerform-addon-next' ) .
										'</div>';
		} else {
			$buttons['next']['markup'] = '<div class="sui-actions-right">' .
										self::get_button_markup( esc_html__( 'Next', Powerform::DOMAIN ), 'powerform-addon-next' ) .
										'</div>';
		}
	...
	public function authorize_access() {
		...
		$template = powerform_addon_slack_dir() . 'views/settings/authorize.php';
	...
	public function wait_authorize_access() {
		$template         = powerform_addon_slack_dir() . 'views/settings/wait-authorize.php';
		$template_success = powerform_addon_slack_dir() . 'views/settings/success-authorize.php';
		$template_error   = powerform_addon_slack_dir() . 'views/settings/error-authorize.php';
	...
	public function get_client_id() {
		 ...
		 */
		$client_id = apply_filters( 'powerform_addon_slack_client_id', $client_id );
	...
	public function get_client_secret() {
		 ...
		 */
		$client_secret = apply_filters( 'powerform_addon_slack_client_secret', $client_secret );
	...
	public function get_client_access_token() {
		 ...
		 * @param string $token
		 */
		$token = apply_filters( 'powerform_addon_slack_client_access_token', $token );
	...
	public function get_auth_url() {
		$base_authorize_url = Powerform_Addon_Slack_Wp_Api::AUTHORIZE_URL;
		$client_id          = $this->get_client_id();
		$redirect_url       = rawurlencode( powerform_addon_integration_section_admin_url( $this->_slug, 'authorize', false ) );
		$scopes             = Powerform_Addon_Slack_Wp_Api::$oauth_scopes;
		 ...
		 * @param array $scopes
		 */
		$scopes = apply_filters( 'powerform_addon_slack_oauth_scopes', $scopes );
		 ...
		 */
		$auth_url = apply_filters( 'powerform_addon_slack_auth_url', $auth_url, $base_authorize_url, $client_id, $scopes, $redirect_url );
	...
	public function authorize_page_callback( $query_args ) {
		...
		$template        = powerform_addon_slack_dir() . 'views/sections/authorize.php';
		$template_params = array(
			'error_message' => '',
			'is_close'      => false,
		);
		...
		if ( isset( $query_args['code'] ) ) {
			try {
				...
				// prefer new instance
				$api           = Powerform_Addon_Slack_Wp_Api::get_instance( uniqid() );
				...
				if ( empty( $token ) ) {
					throw new Powerform_Addon_Slack_Exception( __( 'Failed to get token', Powerform::DOMAIN ) );
				}
				...
				if ( ! $this->is_active() ) {
					...
					if ( ! $activated ) {
						...
						throw new Powerform_Addon_Slack_Exception( $last_message );
					}
				}
	 ...
	 *
	 * @return Powerform_Addon_Slack_Wp_Api|null
	 * @throws Powerform_Addon_Slack_Wp_Api_Exception
	 */
	public function get_api( $access_token = null ) {
		if ( is_null( self::$_api ) ) {
			...
			$api        = Powerform_Addon_Slack_Wp_Api::get_instance( $access_token );
			self::$_api = $api;
		}
	 ...
	 * - on web API request it uses Bearer realm of access token which default of @see Powerform_Addon_Slack_Wp_Api
	 *
	 * @since 1.0 Slack Addon
	 *
	 * @param $headers
	...
	public function is_poll_connected( $poll_id ) {
		try {
			...
			if ( ! $this->is_connected() ) {
				throw new Powerform_Addon_Slack_Exception( 'Slack is not connected' );
			}

			$poll_settings_instance = $this->get_addon_poll_settings( $poll_id );
			if ( ! $poll_settings_instance instanceof Powerform_Addon_Slack_Poll_Settings ) {
				throw new Powerform_Addon_Slack_Exception( 'Invalid Poll Settings of Slack' );
			}

			// Mark as active when there is at least one active connection
			if ( false === $poll_settings_instance->find_one_active_connection() ) {
				throw new Powerform_Addon_Slack_Exception( 'No active Slack connection found in this poll' );
			}
		 ...
		 * @param Powerform_Addon_Slack_Poll_Settings|null $poll_settings_instance Instance of poll settings, or null when unavailable
		 *
		 */
		$is_poll_connected = apply_filters( 'powerform_addon_slack_is_poll_connected', $is_poll_connected, $poll_id, $poll_settings_instance );
	...
	public function is_quiz_connected( $quiz_id ) {
		try {
			...
			if ( ! $quiz_settings_instance instanceof Powerform_Addon_Slack_Quiz_Settings ) {
				throw new Powerform_Addon_Slack_Exception( 'Invalid Quiz Settings of Slack' );
			}

			// Mark as active when there is at least one active connection
			if ( false === $quiz_settings_instance->find_one_active_connection() ) {
				throw new Powerform_Addon_Slack_Exception( 'No active Slack connection found in this quiz' );
			}
		 ...
		 * @param Powerform_Addon_Slack_Quiz_Settings|null $quiz_settings_instance Instance of Quiz settings, or null when unavailable
		 *
		 */
		$is_quiz_connected = apply_filters( 'powerform_addon_slack_is_quiz_connected', $is_quiz_connected, $quiz_id, $quiz_settings_instance );
