...
require_once dirname( __FILE__ ) . '/powerform-addon-slack-form-settings-exception.php';

/**
 * Class Powerform_Addon_Slack_Form_Settings
 * Handle how form settings displayed and saved
 *
 * @since 1.0 Slack Addon
 */
class Powerform_Addon_Slack_Form_Settings extends Powerform_Addon_Form_Settings_Abstract {
	...
	/**
	 * Powerform_Addon_Slack_Form_Settings constructor.
	 *
	 * @since 1.0 Slack Addon
	 *
	...
	public function pick_name( $submitted_data ) {
		$template = powerform_addon_slack_dir() . 'views/form-settings/pick-name.php';
		...
		if ( $this->pick_name_is_completed( array( 'multi_id' => $multi_id ) ) ) {
			$buttons['disconnect']['markup'] = Powerform_Addon_Abstract::get_button_markup( esc_html__( 'DISCONNECT', Powerform::DOMAIN ),
			                                                                                 ...
			                                                                                 esc_html__( 'Disconnect this Slack Integration from this Form.', Powerform::DOMAIN )
			);
		}
	...
	public function select_type( $submitted_data ) {
		$template = powerform_addon_slack_dir() . 'views/form-settings/select-type.php';
	...
	public function select_target( $submitted_data ) {
		$template = powerform_addon_slack_dir() . 'views/form-settings/select-target.php';
		if ( ! isset( $submitted_data['multi_id'] ) ) {
			return $this->get_force_closed_wizard( __( 'Please pick valid connection', Powerform::DOMAIN ) );
		}
	...
	public function setup_message( $submitted_data ) {
		$template = powerform_addon_slack_dir() . 'views/form-settings/setup-message.php';
		if ( ! isset( $submitted_data['multi_id'] ) ) {
			return $this->get_force_closed_wizard( __( 'Please pick valid connection', Powerform::DOMAIN ) );
		}
		...
		if ( $is_submit ) {
			...
			try {
				...
				if ( empty( $message ) ) {
					throw new Powerform_Addon_Slack_Exception( __( 'Please add a message' ) );
				}
