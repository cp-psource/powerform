...
/**
 * Class Powerform_Addon_Slack_Poll_Hooks
 *
 * @since 1.6.1
 *
 */
class Powerform_Addon_Slack_Poll_Hooks extends Powerform_Addon_Poll_Hooks_Abstract {
	 ...
	 *
	 * @since 1.6.1
	 * @var Powerform_Addon_Slack_Poll_Settings | null
	 */
	...
	public function add_entry_fields( $submitted_data, $form_entry_fields = array() ) {
		 ...
		 * @param Powerform_Addon_Slack_Poll_Settings $poll_settings_instance Slack Addon Poll Settings instance
		 */
		$submitted_data = apply_filters(
			'powerform_addon_slack_poll_submitted_data',
			$submitted_data,
			$pol_id,
		 ...
		 * @param Powerform_Addon_Slack_Poll_Settings $poll_settings_instance Slack Addon Poll Settings instance
		 */
		do_action( 'powerform_addon_slack_poll_before_send_message', $pol_id, $submitted_data, $poll_settings_instance );
		 ...
		 */
		$data = apply_filters(
			'powerform_addon_slack_poll_entry_fields',
			$entry_fields,
			$pol_id,
			$submitted_data,
			$poll_settings_instance
	...
	private function get_status_on_send_message( $connection_id, $submitted_data, $connection_settings, $poll_entry_fields ) {
		...
		try {
			...
			$text_message = str_ireplace( '{poll_name}', powerform_get_name_from_model( $this->poll ), $text_message );
			...
			$attachments = apply_filters(
				'powerform_addon_slack_poll_message_attachments',
				$attachments,
				$poll_id,
				$connection_id,
				$submitted_data,
			 ...
			 */
			$args = apply_filters(
				'powerform_addon_slack_poll_send_message_args',
				$args,
				$poll_id,
				$connection_id,
				$submitted_data,
	...
	public function get_poll_data_as_attachments( $submitted_data, $poll_entry_fields ) {
		...
		$attachment_fields[] = array(
			'title' => __( 'Vote', Powerform::DOMAIN ),
		...
		if ( ! empty( $extra ) ) {
			$attachment_fields[] = array(
				'title' => __( 'Extra', Powerform::DOMAIN ),
		...
		$attachments[] = array(
			'title'  => __( 'Submitted Vote', Powerform::DOMAIN ),
		...
		$attachments[] = array(
			'title'  => __( 'Current Poll Result', Powerform::DOMAIN ),
			'fields' => $attachment_fields,
		);
	...
	public function on_export_render_title_row() {
		...
		$export_headers = apply_filters(
			'powerform_addon_slack_poll_export_headers',
			$export_headers,
			$poll_id,
			$poll_settings_instance
		);
	...
	public function on_export_render_entry( Powerform_Form_Entry_Model $entry_model, $addon_meta_data ) {
		...
		$addon_meta_data = apply_filters(
			'powerform_addon_slack_poll_metadata',
			$addon_meta_data,
			$poll_id,
			$poll_settings_instance
		);
		...
		$export_columns = apply_filters(
			'powerform_addon_slack_poll_export_columns',
			$export_columns,
			$poll_id,
			$entry_model,
			$addon_meta_data,
