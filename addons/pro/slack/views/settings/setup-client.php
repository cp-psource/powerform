...
<div class="integration-header">
	<h3 class="sui-box-title" id="dialogTitle2">
		<?php
		/* translators: ... */
		echo esc_html( sprintf( __( 'Setup %1$s Client', Powerform::DOMAIN ), 'Slack' ) );
		?>
	</h3>
	...
	<?php else : ?>
		<p><?php esc_html_e( 'Setup Slack to be used by Powerform to communicating with Slack server.', Powerform::DOMAIN ); ?></p>
			...
			<ol class="instructions" id="clientid-instructions">
					<li>
						...
						echo sprintf(/* translators: ... */
							esc_html__( 'Go %1$shere%2$s to create new Slack App.', Powerform::DOMAIN ),
					...
					<li>
						<?php
						echo sprintf(/* translators: ... */
							esc_html__( 'Once the Project creation is completed go to the %1$sBasic Information%2$s. Then scroll through %3$sApp Credentials%4$s, to take a note of %5$sClient ID%6$s and %7$sClient Secret%8$s.', Powerform::DOMAIN ),
							'<strong>',
							'</strong>',
							'<strong>',
							'</strong>',
					...
					<li>
						<?php
						echo sprintf(/* translators: ... */
							esc_html__( 'Next, go to the %1$sFeatures%2$s &gt; %3$sOAuth & Permissions%4$s &gt; %5$sRedirect URLs%6$s section.', Powerform::DOMAIN ),
							'<strong>',
							'</strong>',
							'<strong>',
							'</strong>',
						...
						<ol>
							<li>
								<?php
								echo sprintf(/* translators: ... */
									esc_html__( 'Click %1$sAdd a new Redirect URL%2$s.', Powerform::DOMAIN ),
									'<strong>',
									'</strong>'
								); //phpcs:ignore Standard.Category.SniffName.ErrorCode
								?>
							...
							<li>
								<?php
								echo sprintf(/* translators: ... */
									esc_html__( 'Then click the %1$sAdd%2$s button.', Powerform::DOMAIN ),
									'<strong>',
									'</strong>'
								); //phpcs:ignore Standard.Category.SniffName.ErrorCode
								?>
							...
							<li>
								<?php
								echo sprintf(/* translators: ... */
									esc_html__( 'Then click the %1$sSave URLs%2$s button.', Powerform::DOMAIN ),
									'<strong>',
									'</strong>'
								); //phpcs:ignore Standard.Category.SniffName.ErrorCode
								?>
