...
/**
 * Class Powerform_Admin_Page
 *
 * @since 1.0
 */
abstract class Powerform_Admin_Page {
	...
	public function trigger_before_render_action() {
		do_action( 'powerform_loaded_admin_page_' . $this->get_the_slug() );
	}
	...
	public function enqueue_scripts( $hook ) {
		...
		// Load admin scripts
		powerform_admin_enqueue_scripts(
			POWERFORM_VERSION,
	...
	protected function render_footer() {
		...
		$hide_footer = false;
		$footer_text = sprintf( __( 'Gecodet mit %s vom WMS N@W', 'wpmudev' ), ' <i class="sui-icon-heart"></i>' );
	...
	public function render() {
		...
		?>
		<main class="sui-wrap <?php echo $accessibility_enabled ? 'sui-color-accessible' : ''; ?> <?php echo esc_attr( 'wpmudev-powerform-' . $this->page_slug ); ?>">
			<?php
			$this->render_header();
	...
	protected function do_content_box( $box_id ) {
		...
		do_action_ref_array( 'powerform_admin_print_content_box' . $this->page_slug, array( &$this ) );
	...
	protected function do_popup_box( $box_id ) {
		...
		do_action_ref_array( 'powerform_admin_print_popup_box' . $this->page_slug, array( &$this ) );
