...
/**
 * Class Powerform_Admin_Import_CF7
 *
 * @since 1.11
 */
class Powerform_Admin_Import_CF7 extends Powerform_Import_Mediator {
	...
	public function import_form( $id, $post_data = array() ) {
		...
		$settings          = array(
			...
			'indicator-label'      => __( 'Submitting...', Powerform::DOMAIN ),
		...
		foreach ( $form_fields as $mkey => $field ) {
			...
			if ( 'submit' === $field['type'] ) {
				...
				if ( 'captcha' === $type ) {
					...
					$condition           = array();
					$field_name          = esc_html__( 'reCaptcha', Powerform::DOMAIN );
				} else {
		...
		$data['version']          = POWERFORM_VERSION;
		$data['type']             = 'form';
		$data['data']['fields']   = $new_fields;
		$data['data']['settings'] = $settings;

		$data = apply_filters( 'powerform_cf7_form_import_data', $data );
	 ...
	 * @param $meta
	 */
	public function import_flamingo( $id, Powerform_Form_Entry_Model $entry, $import, $meta ) {
	 ...
	 * @param $meta
	 */
	public function import_cfdb7( $id, Powerform_Form_Entry_Model $entry, $import, $meta ) {
		global $wpdb;
		$field_data_array = array();
	 ...
	 * @param $meta
	 */
	public function import_submissions( $id, Powerform_Form_Entry_Model $entry, $import, $meta ) {
		$field_data_array = array();
			...
			if ( isset( $setting['captchaV2'] ) ) {
				if ( class_exists( 'WPCF7_RECAPTCHA' ) ) {
					...
					if ( $cf7_captcha_key && $cf7_captcha_secret ) {
						update_option( 'powerform_captcha_key', $cf7_captcha_key );
						update_option( 'powerform_captcha_secret', $cf7_captcha_secret );
					}
				}
			}
		}
	 ...
	 * @param $meta
	 */
	public function import_advanced_cf7( $id, Powerform_Form_Entry_Model $entry, $import, $meta ) {
		global $wpdb;
		$field_data_array = array();
