					...
					foreach ( $this->entries_iterator() as $entries ) {
						...
						<tr class="sui-accordion-item" data-entry-id="<?php echo esc_attr( $db_entry_id ); ?>">
							...
							<?php foreach ( $summary_items as $key => $summary_item ) { ?>
								...
								<?php if ( 1 === $summary_item['colspan'] ) : ?>
									<td class="sui-accordion-item-title"><label class="sui-checkbox">
										...
										<span></span>
										<span class="sui-screen-reader-text"><?php printf( esc_html__( "Wähle die Eintragsnummer %s", Powerform::DOMAIN ), esc_html( $db_entry_id ) ); ?></span>
									</label>
							...
							<?php if ( $summary['num_fields_left'] ) { ?>
								...
								<td><?php printf( esc_html__( "+ %s andere Felder", Powerform::DOMAIN ), esc_html( $summary['num_fields_left'] ) ); ?>
							...
							<td colspan="<?php echo esc_attr( $detail['colspan'] ); ?>">
								...
								<div class="sui-box">
									...
									<div class="sui-box-body">
										...
										<h2><?php printf( esc_html__( "Übermittlung #%s", Powerform::DOMAIN ), $db_entry_id ); // WPCS: XSS ok. ?></h2>
									...
									<div class="sui-box-footer">
										...
										<button type="button" class="sui-button sui-button-ghost sui-button-red wpmudev-open-modal"
												...
												data-nonce="<?php echo wp_create_nonce( 'powerformCustomFormEntries' ); // WPCS: XSS ok. ?>"><i class="sui-icon-trash" aria-hidden="true"></i> <?php esc_html_e( "Löschen", Powerform::DOMAIN ); ?></button>
...
<?php else : ?>
	...
	<div class="sui-box sui-message">
		...
		<div class="sui-message-content">
			...
			<h2><?php echo powerform_get_form_name( $this->form_id, 'custom_form' ); // WPCS: XSS ok. ?></h2>

			<p><?php esc_html_e( "Du hast noch keine Einreichungen für dieses Formular erhalten. Wenn Du dies tust, kannat Du hier alle Daten anzeigen.", Powerform::DOMAIN ); ?></p>
